...
static void gr_linear_rep_movsdm_faded(
    uint8_t *src,
    uint8_t *dest,
    uint32_t num_pixels,
    uint8_t fadeValue,
    CPalette *srcPalette,
    CPalette *destPalette);
...
static void gr_linear_rep_movsdm_faded(
    ...
    for (i = num_pixels; i != 0; i--) {
        ...
        if ((uint8_t)c != (uint8_t)TRANSPARENCY_COLOR) {
            c *= 3;
            c = destPalette->ClosestColor(
                (uint8_t)FRound(srcPalette->Raw()[c] * fade),
...
void gr_ubitmap00m(int32_t x, int32_t y, CBitmap *pBm) {
    ...
    if (grBitBltFadeTable == NULL) {
        ...
        for (y1 = 0; y1 < pBm->Height(); y1++) {
            gr_linear_rep_movsdm_faded(
                src,
                dest,
                pBm->Width(),
                grBitBltFadeTable[y1 + y],
                pBm->Palette(),
                CCanvas::Current()->Palette());
            src += pBm->RowSize();
            dest += (int32_t)(destRowSize);
        }
    }
...
extern void
gr_rle_expand_scanline_generic_masked(CBitmap *dest, int32_t dx, int32_t dy, uint8_t *src, int32_t x1, int32_t x2);
extern void
gr_rle_expand_scanline_svga_masked(CBitmap *dest, int32_t dx, int32_t dy, uint8_t *src, int32_t x1, int32_t x2);

void StretchToBitmapRLE(
    int32_t w,
    int32_t h,
    int32_t dx,
    int32_t dy,
