...
uint8_t *CBitmap::CreateBuffer(void) {
    if (m_info.props.rowSize * m_info.props.h) {
        ...
        CArray<uint8_t>::Resize(
            (m_info.nBPP > 1) ? m_info.props.h * m_info.props.rowSize : MAX_BMP_SIZE(m_info.props.w, m_info.props.h));
#if DBG
        Clear();
#endif
    }
...
void CBitmap::DestroyFrames(void) {
    if (m_info.frames.pBm) {
        ...
        m_info.frames.pBm = m_info.frames.pCurrent = NULL;
...
bool CBitmap::InitChild(CBitmap *parent, int32_t x, int32_t y, int32_t w, int32_t h) {
    ...
    m_info.pParent = m_info.pOverride = m_info.pMask = NULL;
...
void CBitmap::SetPalette(
    CPalette *palette,
    int32_t transparentColor,
    int32_t superTranspColor,
    uint8_t *pBuffer,
    int32_t bufLen) {
    if (!palette) {
        m_info.palette = NULL;
    ...
    int32_t colorFrequencies[256] = {0};
...
int32_t CBitmap::AvgColor(CFloatVector3 *pColor, bool bForce) {
    ...
    if (!(pBuffer = Buffer())) {
        m_info.avgColor.Red() = m_info.avgColor.Green() = m_info.avgColor.Blue() = 0;
        return -1;
    }
