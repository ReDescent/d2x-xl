...
void CCanvas::Setup(CCanvas *pParent, int32_t x, int32_t y, int32_t w, int32_t h, bool bUnscale) {
    if (pParent)
        Setup(pParent);
    *((CViewport *)this) =
        bUnscale ? CViewport(Unscaled(x), Unscaled(y), Unscaled(w), Unscaled(h)) : CViewport(x, y, w, h);
    SetWidth();
    SetHeight();
}
...
void SetupCanvasses(float scale) {
    ...
    if (!ogl.IsSideBySideDevice())
        ...
        if (ogl.StereoSeparation() < 0)
            gameData.renderData.frame.Setup(
                &gameData.renderData.screen,
                0,
                0,
                gameData.renderData.screen.Width(false) / 2,
                gameData.renderData.screen.Height(false));
        else
            gameData.renderData.frame.Setup(
                &gameData.renderData.screen,
                gameData.renderData.screen.Width(false) / 2,
