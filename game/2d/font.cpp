...
int16_t CFont::Width(void) { return int16_t(m_info.width * fontManager.Scale()); }

int16_t CFont::Height(void) { return int16_t(m_info.height * fontManager.Scale()); }
...
int32_t CFont::StringSizeTabbed(
    const char *s,
    int32_t &stringWidth,
    int32_t &stringHeight,
    int32_t &averageWidth,
    int32_t *nTabs,
    int32_t nMaxWidth) {
    float fScale = fontManager.Scale();
...
float CFontManager::Scale(bool bScaled) { return bScaled ? m_scale : m_scale / float(gameStates.app.bDemoData + 1); }
