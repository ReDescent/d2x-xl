...
bool CPalette::Read(CFile &cf) { return cf.File() && (cf.Read(&m_data, sizeof(m_data), 1) == 1); }
...
void CPaletteManager::StartEffect(int32_t red, int32_t green, int32_t blue, bool bForce) {
    StartEffect(
        Clamp(float(red) / 64.0f, 0.0f, 1.0f),
...
void CPaletteManager::StopEffect(void) { StartEffect(0, 0, 0); }
...
void CPaletteManager::BumpEffect(float red, float green, float blue) {
    ...
    StartEffect(
        Max(red, m_data.effect.Red() * fFade),
...
void CPaletteManager::FadeEffect(void) {
    ...
    if (m_data.xFadeDelay) {
        ...
        if ((m_data.xLastEffectTime + I2X(1) / 8 < gameData.timeData.xGame) ||
        ...
        if (bForce || (RandShort() > 4096)) {
            if ((gameData.demoData.nState == ND_STATE_RECORDING) &&
                (m_data.effect.Red() || m_data.effect.Green() || m_data.effect.Blue()))
                NDRecordPaletteEffect(
                    int16_t(m_data.effect.Red() * 64.0f),
                    int16_t(m_data.effect.Green() * 64.0f),
                    int16_t(m_data.effect.Blue() * 64.0f));
        }
    } else {
