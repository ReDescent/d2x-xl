...
#pragma aux rep_stosb = "               rep stosb" parm[EDI][ECX][EAX] modify[];
...
#pragma aux scale_row_asm_transparent parm[esi][edi][ecx][ebx][edx] modify exact \
    [edi eax ebx ecx] = "newpixel:  mov eax, ebx            " \
                        "               shr eax, 16         " \
                        "               mov al, [esi+eax]   " \
                        "               cmp al, 255         " \
                        "               je      skip_it         " \
...
#pragma aux scale_row_asm parm[esi][edi][ecx][ebx][edx] modify exact \
    [edi eax ebx ecx] = "newpixel1: mov eax, ebx            " \
                        "               shr eax, 16         " \
                        "               mov al, [esi+eax]   " \
                        "               add ebx, edx            " \
                        "               mov [edi], al       " \
...
#pragma aux rep_movsb parm[esi][edi][ecx] modify exact[esi edi ecx] = "rep movsb"
...
static inline void rep_movsb(ubyte *sbits, ubyte *dbits, int width) {
    __asm__ __volatile__("cld; rep; movsb" : : "S"(sbits), "D"(dbits), "c"(width) : "%ecx", "%esi", "%edi");
}
#endif
