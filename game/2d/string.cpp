...
// note we subtract one from color, since 255 is "transparent" so it'll never be used, and 0 would otherwise end the
// string. function must already have origColor var set (or they could be passed as args...) perhaps some sort of
// recursive origColor nType thing would be better, but that would be way too much trouble for little gain
    ...
    if ((c >= 1) && (c <= 3)) {
        if (pText[1]) {
            if (grMsgColorLevel >= c) {
                ...
                CCanvas::Current()->FontColor(0).Set(
                    (pText[1] - nOffset) * nScale,
                    (pText[2] - nOffset) * nScale,
                    (pText[3] - nOffset) * nScale,
                    255);
            }
            return pText + 4;
        }
...
#include "args.h"
// font handling routines for OpenGL - Added 9/25/99 Matthew Mueller - they are here instead of in arch/ogl because they
// use all these defines
        ...
        while ((c = *pText)) {
            ...
            if (fontManager.Current()->InFont(letter)) {
                ...
                bmf->RenderScaled(
                    x,
                    y,
                    int32_t(bmf->Width() * fScale),
                    int32_t(bmf->Height() * fScale),
                    I2X(1),
                    0,
                    pColor,
                    !gameStates.app.bDemoData);
            }
            x += spacing;
            pText++;
        }
    ...
    while (nextRowP) {
        ...
        while ((c = *pText)) {
            ...
            if ((bHotKey = ((nKey < 0) && isalnum(c)) || (nKey && ((int32_t)c == nKey))))
                nKey = 0;
            pBmf = (bHotKey && (fontManager.Current() != SMALL_FONT)) ? SELECTED_FONT->Bitmaps() + letter
                                                                      : pFont->Bitmaps() + letter;
...
int32_t GrString(int32_t x, int32_t y, const char *s, int32_t *idP) {
    ...
    if (/*(CCanvas::Current ()->Mode () == BM_OGL) &&*/ (ps = GetPoolString(s, idP))) {
        ...
        ps->pBm->RenderScaled(
            ...
            y,
            gameData.X(x + w) - xs,
            int32_t(pBm->Height() * fScale),
            I2X(1),
            0,
            &CCanvas::Current()->FontColor(0),
            !gameStates.app.bDemoData);
...
int32_t CenteredStringPos(char *s) { return (CCanvas::Current()->Width() - StringWidth(s)) / 2; }
