...
int32_t CContourInfo::IsFacingViewer(void) { return CFloatVector3::Dot(m_vCenterf[1], m_vNormalf[1]) >= 0; }

//------------------------------------------------------------------------------

void CContourInfo::RotateNormal(void) { transformation.Rotate(m_vNormalf[1], m_vNormalf[0]); }

//------------------------------------------------------------------------------

void CContourInfo::TransformCenter(void) { transformation.Transform(m_vCenterf[1], m_vCenterf[0]); }

//------------------------------------------------------------------------------

int32_t CContourInfo::IsLit(CFloatVector3 &vLight) { return m_bFacingLight = IsFacingPoint(vLight); }

//------------------------------------------------------------------------------

int32_t CContourInfo::IsFront(void) { return m_bFrontFace = IsFacingViewer(); }
...
int32_t CModelEdge::Visibility(void) { return IsFacingViewer(0) | ((m_nFaces == 1) ? 0 : (IsFacingViewer(1) << 1)); }

//------------------------------------------------------------------------------

int32_t CModelEdge::IsContour(void) { return IsFacingViewer(0) != IsFacingViewer(1); }

//------------------------------------------------------------------------------

float CModelEdge::PlanarAngle(void) { return 0.97f; }

//------------------------------------------------------------------------------

float CModelEdge::PartialAngle(void) { return 0.3f; }
...
CFloatVector &CModelEdge::Normal(int32_t nFace) { return m_faces[nFace].m_vNormal[0]; }
...
bool CModel::Create(void) {
    ...
    if (ogl.m_features.bVertexBufferObjects) {
        ...
        m_vertBuf[1].SetBuffer(
            reinterpret_cast<CRenderVertex *>(glMapBufferARB(GL_ARRAY_BUFFER_ARB, GL_WRITE_ONLY_ARB)),
            1,
            m_nFaceVerts);
#endif
        m_vboIndexHandle = 0;
        ...
        if (m_vboIndexHandle) {
            ...
            m_index[1].SetBuffer(
                reinterpret_cast<int16_t *>(glMapBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, GL_WRITE_ONLY_ARB)),
                1,
                m_nFaceVerts);
#endif
        }
...
void CFace::SetTexture(CBitmap *pTexture) { m_pTexture = (pTexture && (m_nBitmap >= 0)) ? pTexture + m_nBitmap : NULL; }
...
bool CSubModel::BuildEdgeList(CModel *pModel) {
    ...
    for (uint16_t i = 0; i < m_nFaces; i++, pFace++) {
        ...
        pFace->m_faceWinding = FaceWinding(
        ...
        for (uint16_t j = 0; j < pFace->m_nVerts; j++)
            if (AddEdge(
                    pModel,
                    pFace,
                    pModel->m_faceVerts[pFace->m_nIndex + j].m_nIndex,
...
void CModel::Setup(int32_t bHires, int32_t bSort) {
    ...
    m_vbColor.SetBuffer(
        reinterpret_cast<CFloatVector *>(m_vbNormals.Buffer() + m_nFaceVerts),
        1,
        m_vertBuf[0].Length());
    m_vbTexCoord.SetBuffer(
        reinterpret_cast<tTexCoord2f *>(m_vbColor.Buffer() + m_nFaceVerts),
        1,
        m_vertBuf[0].Length());
    ...
    if (m_vertBuf[0].Buffer()) { // points to graphics driver buffer for VBO based rendering
        ...
        glBufferDataARB(
            GL_ARRAY_BUFFER,
            m_nFaceVerts * sizeof(CRenderVertex),
            reinterpret_cast<void *>(m_vertBuf[0].Buffer()),
            GL_STATIC_DRAW_ARB);
#else
    ...
    if (m_index[0].Buffer()) { // points to graphics driver buffer for VBO based rendering
        ...
        glBufferDataARB(
            ...
            m_nFaceVerts * sizeof(int16_t),
            reinterpret_cast<void *>(m_index[0].Buffer()),
            GL_STATIC_DRAW_ARB);
#else
...
fix CModel::Radius(CObject *pObj) {
    ...
    if (m_nType >= 0) {
        if ((m_nSubModels == pSphere->nSubModels) && (m_nFaces == pSphere->nFaces) &&
            (m_nFaceVerts == pSphere->nFaceVerts)) {
    ...
    if (vertices.Create(m_nFaceVerts, "CModel::Radius::vertices")) {
        ...
        for (i = 0, h = m_nSubModels, pSubModel = m_subModels.Buffer(), pv = vertices.Buffer(); i < h;
             i++, pSubModel++) {
            if (pSubModel->m_nHitbox > 0) {
                ...
                for (j = pSubModel->m_nFaces, pModelFace = pSubModel->m_faces; j; j--, pModelFace++) {
                    for (k = pModelFace->m_nVerts, pModelVertex = m_faceVerts + pModelFace->m_nIndex; k;
                         k--, pModelVertex++, pv++)
...
fix CModel::Size(CObject *pObj, int32_t bHires) {
    ...
    if (m_nSubModels == 1) {
        ...
        for (i = m_nSubModels; i; i--, pSubModel++)
            pSubModel->m_nHitbox =
                (pSubModel->m_bGlow || pSubModel->m_bThruster || pSubModel->m_bWeapon || (pSubModel->m_nGunPoint >= 0))
                    ? -1
                    : j++;
    ...
    do {
        ...
        if (IsMultiGame)
            gameData.modelData.offsets[m_nModel].v.coord.x = gameData.modelData.offsets[m_nModel].v.coord.y =
                gameData.modelData.offsets[m_nModel].v.coord.z = 0;
        else {
            gameData.modelData.offsets[m_nModel].v.coord.x =
                (pHitbox[0].vMin.v.coord.x + pHitbox[0].vMax.v.coord.x) / -2;
            gameData.modelData.offsets[m_nModel].v.coord.y =
                (pHitbox[0].vMin.v.coord.y + pHitbox[0].vMax.v.coord.y) / -2;
            gameData.modelData.offsets[m_nModel].v.coord.z =
                (pHitbox[0].vMin.v.coord.z + pHitbox[0].vMax.v.coord.z) / -2;
        }
...
void G3SetShipGunPoints(OOF::CModel *po, CModel *pm) { pm->SetShipGunPoints(po); }

//------------------------------------------------------------------------------

void G3SetRobotGunPoints(OOF::CModel *po, CModel *pm) { pm->SetRobotGunPoints(po); }
...
int32_t G3BuildModel(
    CObject *pObj,
    int32_t nModel,
    CPolyModel *pp,
    CArray<CBitmap *> &modelBitmaps,
    CFloatVector *pObjColor,
    int32_t bHires) {
...
#endif
    pm->Init();
    return bHires ? GetASEModel(nModel) ? pm->BuildFromASE(pObj, nModel) : pm->BuildFromOOF(pObj, nModel)
...
int32_t G3ModelMinMax(int32_t m_nModel, tHitbox *pHitbox) {
    ...
    if (!((pm = gameData.modelData.renderModels[1] + m_nModel) || (pm = gameData.modelData.renderModels[0] + m_nModel)))
        return 0;
