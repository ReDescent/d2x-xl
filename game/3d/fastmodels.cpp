...
void G3DynLightModel(
    ...
    if (!gameStates.render.bBrightObject) {
        ...
        for (i = iVerts,
            pv = pModel->m_vertices + iVerts,
            pn = pModel->m_vertNorms + iVerts,
            pColor = pModel->m_color + iVerts;
             i < nVerts;
             i++, pv++, pn++, pColor++) {
            pColor->index = 0;
    ...
    if (SHOW_DYN_LIGHT &&
        (gameOpts->ogl.bObjLighting ||
         (gameOpts->ogl.bLightObjects &&
          (gameOpts->ogl.bLightPowerups || (pObj->info.nType == OBJ_PLAYER) || (pObj->info.nType == OBJ_ROBOT))))) {
        tiRender.pObj = pObj;
        ...
        if (EGI_FLAG(bShowWeapons, 0, 1, 0)) {
            if (pSubModel->m_nGun == nGunId + 1) {
                if (pSubModel->m_nGun == FUSION_INDEX + 1) {
                    if ((pSubModel->m_nWeaponPos == 3) &&
                        !gameData.multiplayer.weaponStates[N_LOCALPLAYER].bTripleFusion)
                        RETVAL(1)
                } else if (bLasers) {
                ...
                if (pSubModel->m_nWeaponPos > nMissiles)
                    ...
                    RETVAL(
                        (nId == N_LOCALPLAYER) && !AllowedToFireMissile(nId, 0) &&
...
void G3TransformSubModel(
    CObject *pObj,
    int16_t nModel,
    int16_t nSubModel,
    CAngleVector *animAnglesP,
    CFixVector *pvOffset,
    int32_t bHires,
    int32_t nGunId,
    int32_t nBombId,
...
void G3DrawSubModel(
    ...
    CAngleVector va = animAnglesP ? animAnglesP[pSubModel->m_nAngles] : CAngleVector::ZERO;
    CFixVector vo;
    int32_t h, i, j, bTranspState, bRestoreMatrix = 0,
                                   bTextured = gameOpts->render.debug.bTextures /*&& !gameStates.render.bCloaked*/ &&
                                               !(gameOpts->render.debug.bWireFrame & 2),
    ...
    if (pObj->info.nType == OBJ_PLAYER)
        nTeamColor = IsMultiGame ? IsTeamGame ? GetTeam(pObj->info.nId) + 1 : PlayerColor(pObj->Index())
                                 : gameOpts->render.ship.nColor;
    else
        nTeamColor = 0;
    ...
    if (pSubModel->m_bThruster) {
        if (!nPass) {
            ...
            G3GetThrusterPos(
                ...
                NULL,
                &vo,
                &pSubModel->m_faces->m_vNormal,
                pSubModel->m_nRad,
                bHires,
                pSubModel->m_bThruster);
        }
        if (!pSubModel->m_nFrames)
            RETURN
    }
    ...
    if ((nExclusive < 0) || (nSubModel == nExclusive)) {
        ...
        if (bEdges) {
            if (!pSubModel->m_bThruster) {
                ...
                RenderModel::CModelEdge *pEdge = pSubModel->m_edges.Buffer();
                int32_t nScale = 0xFFFF,
                        nEdgeFilter = pObj->IsWeapon() ? 0 : bHires || (fSize > 9.0f); // 0 : 2; //bHires;
            ...
            for (i = pSubModel->m_nFaces; i;) {
                ...
                if (bHires) {
                    if (!(bTranspState = (pModel->m_bHasTransparency >> 2)))
                        bTranspState = !pBm ? 0
                                            : gameStates.render.bCloaked
                                                  ? 0
                                                  : pSubModel->m_bThruster
                                                        ? 0
                                                        : (pSubModel->m_bGlow || pSubModel->m_bFlare)
                                                              ? 2
                                                              : ((pBm->Flags() & BM_FLAG_TRANSPARENT) != 0) ? 1 : 0;
...
void G3DrawModel(
    ...
    int32_t bLighting = !bEdges && SHOW_DYN_LIGHT && gameOpts->ogl.bObjLighting && (bTranspFilter < 2) &&
                        !(gameStates.render.bCloaked || bEmissive || bBright) &&
                        !(gameOpts->render.debug.bWireFrame & 2);
    GLenum hLight;
...
#if DBG
                    if ((nDbgSeg >= 0) && (pLight->info.nObject >= 0) &&
                        (OBJECT(pLight->info.nObject)->info.nSegment == nDbgSeg))
                        BRP;
#endif
                    hLight = GL_LIGHT0 + iLight++;
                    glEnable(hLight);
                ...
                for (int32_t i = 0; i < pModel->m_nSubModels; i++)
                    if (pModel->m_subModels[i].m_nParent == -1)
                        G3TransformSubModel(
                            ...
                            i,
                            animAnglesP,
                            (nSubModel < 0) ? &pModel->m_subModels[0].m_vOffset : pvOffset,
                            bHires,
                            nGunId,
                            nBombId,
                            nMissileId,
...
int32_t G3RenderModel(
    ...
    int32_t i = 0, bHires = (nModel > 0),
            bUseVBO = ogl.m_features.bVertexBufferObjects &&
                      ((gameStates.render.bPerPixelLighting == 2) || gameOpts->ogl.bObjLighting),
            bEmissive = (pObj != NULL) && pObj->IsProjectile(), bBlur = 0, nGunId, nBombId, nMissileId, nMissiles;
    ...
    int32_t bRenderTransparency = bHires && !gameStates.render.bCloaked &&
                                  (gameStates.render.nType == RENDER_TYPE_TRANSPARENCY) && pModel->m_bHasTransparency;
    ...
    //#pragma omp critical (fastModelRender)
    {
        ...
        if (bUseVBO)
            ...
            pModel->m_vbVerts.SetBuffer(
                reinterpret_cast<CFloatVector3 *>(pModel->m_vertBuf[0].Buffer()),
                1,
                pModel->m_nFaceVerts);
            pModel->m_vbNormals.SetBuffer(pModel->m_vbVerts.Buffer() + pModel->m_nFaceVerts, 1, pModel->m_nFaceVerts);
            pModel->m_vbColor.SetBuffer(
                reinterpret_cast<CFloatVector *>(pModel->m_vbNormals.Buffer() + pModel->m_nFaceVerts),
                1,
                pModel->m_nFaceVerts);
            pModel->m_vbTexCoord.SetBuffer(
                reinterpret_cast<tTexCoord2f *>(pModel->m_vbColor.Buffer() + pModel->m_nFaceVerts),
                1,
                pModel->m_nFaceVerts);
        }
#if G3_SW_SCALING
        ...
        if (bUseVBO) {
            if (!(gameOpts->render.debug.bWireFrame & 2)) {
                ...
                OglTexCoordPointer(
                    ...
                    GL_FLOAT,
                    0,
                    G3_BUFFER_OFFSET(pModel->m_nFaceVerts * ((2 * sizeof(CFloatVector3) + sizeof(CFloatVector)))));
            }
