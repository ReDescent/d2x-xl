...
void G3SetModelPoints(CRenderPoint *pointlist) { modelPointList = pointlist; }
...
void GetThrusterPos(int32_t nModel, CFixVector *vNormal, CFixVector *vOffset, CBitmap *pBm, int32_t nPoints) {
    ...
    if (vOffset)
        v += *vOffset;
    if (pThruster->nCount && (v.v.coord.x == pThruster->vPos[0].v.coord.x) &&
        (v.v.coord.y == pThruster->vPos[0].v.coord.y) && (v.v.coord.z == pThruster->vPos[0].v.coord.z))
        RETURN
...
int32_t G3DrawPolyModel(
    ...
    if (gameStates.app.bNostalgia)
        bGetThrusterPos = bLightning = 0;
    else {
        bGetThrusterPos =
            !pObj || (pObj->info.nType == OBJ_PLAYER) || (pObj->info.nType == OBJ_ROBOT) || (pObj->IsProjectile());
    ...
    if (SHOW_DYN_LIGHT && !nDepth && !po && pObj &&
        ((pObj->info.nType == OBJ_ROBOT) || (pObj->info.nType == OBJ_PLAYER))) {
...
int32_t G3DrawMorphingModel(
    void *pModel,
    CArray<CBitmap *> &modelBitmaps,
    CAngleVector *pAnimAngles,
    CFixVector *vOffset,
    fix xModelLight,
    CFixVector *new_points,
    int32_t nModel) {
