...
void SetupTransformation(
    ...
    bool bSetupRenderer) {
    transformation.m_info.zoom = (ogl.IsOculusRift() && !(gameStates.render.nWindow[0] || gameStates.render.bBriefing))
                                     ? F2X(X2F(xZoom) / X2F(DEFAULT_ZOOM) * float(RIFT_DEFAULT_ZOOM))
                                     : xZoom;
...
void ScaleTransformation(CTransformation &transformation, int32_t bOglScale) {
    ...
    if (transformation.m_info.zoom < I2X(1)) // xZoom in by scaling z
        transformation.m_info.scale.v.coord.z =
            FixMul(transformation.m_info.scale.v.coord.z, transformation.m_info.zoom);
    ...
    // transformation.m_info.scale.v.c.x = transformation.m_info.scale.v.c.y = transformation.m_info.scale.v.c.z = I2X
    // (1); now scale matrix elements
    if (bOglScale > 0) {
#if 0
