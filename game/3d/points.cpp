...
#endif
    int32_t
    CheckMulDiv(fix *r, fix a, fix b, fix c) {
#ifdef _WIN32
    int64_t q;
    if (!c)
        return 0;
...
inline int32_t ScreenWidth(void) { return gameData.renderData.screen.Width() /*/ ScreenScale ()*/; }

inline int32_t ScreenHeight(void) { return gameData.renderData.screen.Height() /*/ ScreenScale ()*/; }
...
uint8_t OglProjectPoint(CFloatVector3 &p, tScreenPos &s, uint8_t flags, uint8_t codes) {
    ...
    gluProject(
        ...
        (double)p.v.coord.y,
        (double)p.v.coord.z,
        &transformation.m_info.oglModelview[0],
        &transformation.m_info.oglProjection[0][0],
...
fix G3CalcPointDepth(const CFixVector &v) {
#ifdef _WIN32
    int64_t q =
        mul64(v.v.coord.x - transformation.m_info.pos.v.coord.x, transformation.m_info.view[0].m.dir.f.v.coord.x);
    ...
    FixMulAccum(
        &q,
        (v.v.coord.x - transformation.m_info.pos.v.coord.x),
        transformation.m_info.view[0].m.dir.f.v.coord.x);
    FixMulAccum(
        &q,
        (v.v.coord.y - transformation.m_info.pos.v.coord.y),
        transformation.m_info.view[0].m.dir.f.v.coord.y);
    FixMulAccum(
        &q,
        (v.v.coord.z - transformation.m_info.pos.v.coord.z),
        transformation.m_info.view[0].m.dir.f.v.coord.z);
