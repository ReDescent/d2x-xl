...
inline int32_t CFace::IsFacingLight(void) { return G3CheckPointFacing(&m_vCenterf, &m_vNormf, &vLightPosf); }

//------------------------------------------------------------------------------

inline int32_t CFace::IsFacingViewer(void) { return CFloatVector::Dot(m_vCenterf, m_vNormf) >= 0; }
...
inline int32_t CFace::IsLit(void) { return m_bFacingLight = IsFacingLight(); }

//------------------------------------------------------------------------------

inline int32_t CFace::IsFront(void) { return m_bFrontFace = IsFacingViewer(); }
...
int32_t CModel::GatherItems(
    void *modelDataP,
    CAngleVector *animAngleP,
    int32_t bInitModel,
    int32_t bShadowData,
    int32_t nThis,
    int32_t nParent) {
...
int32_t CSubModel::RenderShadowVolume(CModel *po, int32_t bCullFront) {
    ...
    // if (!bCullFront)
    {
        ...
        for (i = m_nLitFaces, ppf = m_litFaces; i; i--, ppf++) {
            pf = *ppf;
            paf = po->m_adjFaces + pf->m_nAdjFaces;
            // walk through all edges of the current lit face and check whether the adjacent face does not face the
            // light source if so, that edge is a contour edge: Use it to render a shadow volume face ("side wall" of
            // the shadow volume)
            for (j = 0, n = pf->m_nVerts, pfv = pf->m_vertices; j < n; j++) {
                h = *paf++;
                if ((h >= m_nFaces) || !m_faces[h].m_bFacingLight) {
...
int32_t _CDECL_ ClipDistThread(void *pThreadId) {
    ...
    while (!gameStates.app.bExit) {
        while (!gameData.threadData.clipDist.info[nId].bExec)
            G3_SLEEP(0);
        gameData.threadData.clipDist.data.fClipDist[nId] = G3ClipDistByFaceCenters(
            gameData.threadData.clipDist.data.pObj,
            gameData.threadData.clipDist.data.po,
            gameData.threadData.clipDist.data.pso,
            nId,
            2);
        if (gameOpts->render.shadows.nClip == 3)
            gameData.threadData.clipDist.data.fClipDist[nId] = G3ClipDistByLitVerts(
                gameData.threadData.clipDist.data.pObj,
                gameData.threadData.clipDist.data.po,
...
m_bCalcClipDist = 0;
    ...
    if (gameStates.app.bMultiThreaded) {
        ...
        fMaxDist = (gameData.threadData.clipDist.data.fClipDist[0] > gameData.threadData.clipDist.data.fClipDist[1])
                       ? gameData.threadData.clipDist.data.fClipDist[0]
                       : gameData.threadData.clipDist.data.fClipDist[1];
    } else
#endif
    {
...
if (pso - po->m_subModels == 8)
...
#endif
    {
        ...
        h = RenderShadowCaps(pObj, po, 0) && RenderShadowCaps(pObj, po, 1) && RenderShadowVolume(po, 0) &&
...
int32_t
POFGatherPolyModelItems(CObject *pObj, void *modelDataP, CAngleVector *animAngleP, CModel *po, int32_t bShadowData) {
    int32_t j;
    CFixVector *pv;
    CFloatVector *pvf;
...
int32_t G3DrawPolyModelShadow(CObject *pObj, void *modelDataP, CAngleVector *animAngleP, int32_t nModel) {
    ...
    if (FAST_SHADOWS) {
        int16_t nLights =
            lightManager.SetNearestToSegment(pObj->Segment(), -1, 0, 0, 0); // only get light emitting objects here
        if (nLights > 0) {
                ...
                if ((gameData.renderData.shadows.pLight = pActiveLights->pLight)) {
                    ...
                    if (gameData.renderData.shadows.pLight->info.nType == 3) {
                        ...
                        if (gameData.renderData.shadows.pLight->info.nObject >= 0) {
                            ...
                            if ((nType != OBJ_FIREBALL) && (nType != OBJ_FLARE) && (nType != OBJ_LIGHT) &&
                                !pLightObj->IsEnergyProjectile())
                                continue;
                    ...
                    if (!CanSeePoint(
                            ...
                            &gameData.renderData.shadows.pLight->info.vPos,
                            pObj->info.nSegment,
                            0,
                            -1.0f,
                            0))
                    ...
                    vLightDir = pObj->info.position.vPos - gameData.renderData.shadows.pLight->info.vPos;
                    CFixVector::Normalize(vLightDir);
                    if ((gameData.renderData.shadows.pLight->info.bSpot) &&
                        (!pLightObj || (CFixVector::Dot(vLightDir, pLightObj->Orientation().m.dir.f) < 0.6666667f)))
                        continue;
                    if (gameData.renderData.shadows.nLight) {
                        for (j = 0; j < gameData.renderData.shadows.nLight; j++)
                            if (abs(CFixVector::Dot(vLightDir, gameData.renderData.shadows.vLightDir[j])) >
                                I2X(2) / 3) // 60 deg
                                break;
                        if (j < gameData.renderData.shadows.nLight)
                            continue;
