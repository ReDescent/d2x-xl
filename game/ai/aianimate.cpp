    ...
    if (!IsMultiGame && gameOpts->gameplay.bIdleAnims && (pLocalInfo->mode == AIM_IDLING)) {
        ...
        for (i = 0; i < 8; i++) {
            if (pSeg->m_vertices[i] != 0xFFFF) {
                ...
                if ((vGoal.v.coord.x == (*vVertex).v.coord.x) && (vGoal.v.coord.y == (*vVertex).v.coord.y) &&
                    (vGoal.v.coord.z == (*vVertex).v.coord.z))
                    break;
            }
        }
        ...
        if (i == 8)
            ...
            (AITurnTowardsVector(&vVecToGoal, pObj, pRobotInfo->turnTime[2]) < I2X(1) - I2X(1) / 5)) {
        ...
        if (!*bDyingSoundPlaying) {
            ...
            *bDyingSoundPlaying = 1;
            ...
            CreateSmallFireballOnObject(
                pObj,
                (I2X(1) + RandShort()) * (16 * xExplScale / I2X(1)) / 8,
                RandShort() < gameData.timeData.xFrame * 2);
        }
    } else if (RandShort() < gameData.timeData.xFrame * 8) {
        CreateSmallFireballOnObject(
            pObj,
            (I2X(1) / 2 + RandShort()) * (16 * xExplScale / I2X(1)) / 8,
            RandShort() < gameData.timeData.xFrame);
    }
...
int32_t DoAnyRobotDyingFrame(CObject *pObj) {
    ...
    if (pObj && pObj->cType.aiInfo.xDyingStartTime) {
        ...
        int32_t rval = DoRobotDyingFrame(
            pObj,
            pObj->cType.aiInfo.xDyingStartTime,
            I2X(Min(bDeathRoll / 2 + 1, 6)),
            &pObj->cType.aiInfo.bDyingSoundPlaying,
            pRobotInfo ? pRobotInfo->deathrollSound : 0,
            I2X(bDeathRoll) / 8,
            I2X(bDeathRoll) / 2);
        if (rval) {
