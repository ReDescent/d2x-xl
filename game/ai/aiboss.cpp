...
int32_t spewBots[2][NUM_D2_BOSSES][MAX_SPEW_BOT] = {
    {{38, 40, -1}, {37, -1, -1}, {43, 57, -1}, {26, 27, 58}, {59, 58, 54}, {60, 61, 54}, {69, 29, 24}, {72, 60, 73}},
    {{3, -1, -1}, {9, 0, -1}, {255, 255, -1}, {26, 27, 58}, {59, 58, 54}, {60, 61, 54}, {69, 29, 24}, {72, 60, 73}}};
...
int32_t CObject::CreateGatedRobot(int16_t nSegment, uint8_t nObjId, CFixVector *vPos) {
    ...
    if (count > 2 * gameStates.app.nDifficultyLevel + 6) {
        gameData.bossData[nBoss].m_nLastGateTime =
            gameData.timeData.xGame - 3 * gameData.bossData[nBoss].m_nGateInterval / 4;
        RETVAL(-1)
    }
    vObjPos = pSeg->Center();
    for (;;) {
...
int32_t CObject::BossSpewRobot(CFixVector *vPos, int16_t objType, int32_t bObjTrigger) {
    ...
    if (!bObjTrigger && FindObjTrigger(
                            OBJ_IDX(this),
                            TT_SPAWN_BOT,
                            -1)) // not caused by an object trigger, but object has an object trigger
        RETVAL(-1)
...
void StartBossDeathSequence(CObject *pObj) {
    ENTER(0, 0);
    if (pObj && pObj->IsBoss()) {
        int32_t nObject = pObj->Index(), i = gameData.bossData.Find(nObject);
...
void DoBossDyingFrame(CObject *pObj) {
    ...
    int32_t rval = DoRobotDyingFrame(
        pObj,
        gameData.bossData[i].m_nDyingStartTime,
        BOSS_DEATH_DURATION,
        &gameData.bossData[i].m_bDyingSoundPlaying,
        pRobotInfo->deathrollSound,
        I2X(4),
        I2X(4));
    if (rval) {
        ...
        if (pObj->cType.aiInfo.CLOAKED == 1) {
            ...
            if ((gameData.timeData.xGame - gameData.bossData[i].m_nCloakStartTime > BOSS_CLOAK_DURATION / 3) &&
                ...
                (gameData.timeData.xGame - gameData.bossData[i].m_nLastTeleportTime >
                 gameData.bossData[i].m_nTeleportInterval)) {
            ...
            if ((gameData.timeData.xGame > gameData.bossData[i].m_nCloakEndTime) ||
                (gameData.timeData.xGame < gameData.bossData[i].m_nCloakStartTime) || (pObj->Damage() < 0.01) ||
                (RandShort() > pObj->DriveDamage()))
                pObj->cType.aiInfo.CLOAKED = 0;
        } else if (
            ((gameData.timeData.xGame - gameData.bossData[i].m_nCloakEndTime > gameData.bossData[i].m_nCloakInterval) ||
             (gameData.timeData.xGame - gameData.bossData[i].m_nCloakEndTime <
              -gameData.bossData[i].m_nCloakDuration)) &&
