...
int32_t ReadyToFire(tRobotInfo *pRobotInfo, tAILocalInfo *pLocalInfo) {
    return (pLocalInfo->pNextrimaryFire <= 0) ||
           ((pRobotInfo->nSecWeaponType != -1) && (pLocalInfo->nextSecondaryFire <= 0));
}
...
void SetNextFireTime(CObject *pObj, tAILocalInfo *pLocalInfo, tRobotInfo *pRobotInfo, int32_t nGun) {
    ...
    if (((nGun != 0) || (pRobotInfo->nSecWeaponType == -1)) &&
        (pLocalInfo->nRapidFireCount < pRobotInfo->nRapidFireCount[gameStates.app.nDifficultyLevel])) {
...
void DoAIRobotHitAttack(CObject *pRobot, CObject *pTarget, CFixVector *vCollision) {
    ...
    if (!TARGETOBJ->Cloaked()) {
        if (CFixVector::Dist(OBJPOS(TARGETOBJ)->vPos, pRobot->info.position.vPos) <
            pRobot->info.xSize + TARGETOBJ->info.xSize + I2X(2)) {
...
LeadTarget(CObject *pObj, CFixVector *vFirePoint, CFixVector *vBelievedTargetPos, int32_t nGuns, CFixVector *vFire) {
    ...
    (*vFire).v.coord.x = ComputeLeadComponent(
        (*vBelievedTargetPos).v.coord.x,
        (*vFirePoint).v.coord.x,
        TARGETOBJ->mType.physInfo.velocity.v.coord.x,
        xProjectedTime);
    (*vFire).v.coord.y = ComputeLeadComponent(
        (*vBelievedTargetPos).v.coord.y,
        (*vFirePoint).v.coord.y,
        TARGETOBJ->mType.physInfo.velocity.v.coord.y,
        xProjectedTime);
    (*vFire).v.coord.z = ComputeLeadComponent(
        (*vBelievedTargetPos).v.coord.z,
        (*vFirePoint).v.coord.z,
        TARGETOBJ->mType.physInfo.velocity.v.coord.z,
        xProjectedTime);
...
targetLed:
    ...
    if ((nWeaponType == FUSION_ID) && (gameStates.app.nSDLTicks[0] - pObj->TimeLastEffect() > 1000)) {
        ...
        postProcessManager.Add(new CPostEffectShockwave(
            ...
            pObj->info.xSize,
            1,
            OBJPOS(pObj)->vPos + OBJPOS(pObj)->mOrient.m.dir.f * pObj->info.xSize,
            pObj->Index()));
    }
            ...
            if ((!gameData.aiData.bObjAnimates || ReadyToFire(pRobotInfo, pLocalInfo)) &&
                ...
                if (dot >= I2X(7) / 8) {
                    if (pStaticInfo->CURRENT_GUN < pRobotInfo->nGuns) {
                        if (pRobotInfo->attackType == 1) {
                            if (!LOCALPLAYER.m_bExploded &&
                                (gameData.aiData.target.xDist < pObj->info.xSize + TARGETOBJ->info.xSize + I2X(2))) {
                                if (!AILocalPlayerControlsRobot(pObj, ROBOT_FIRE_AGITATION - 2))
                                    RETURN
                                DoAIRobotHitAttack(pObj, TARGETOBJ, &pObj->info.position.vPos);
                            } else
