...
void InitAIObject(int16_t nObject, int16_t behavior, int16_t nHideSegment) {
    ...
    if ((behavior == AIB_SNIPE) || (behavior == AIB_STATION) || (behavior == AIB_RUN_FROM) ||
        (behavior == AIB_FOLLOW)) {
...
void DoLunacyOn(void) {
    ...
    for (int32_t i = 0; i < MAX_ROBOT_TYPES; i++) {
        ...
        gameData.botData.pInfo[i].primaryFiringWait[DIFFICULTY_LEVEL_COUNT - 1] =
            gameData.botData.pInfo[i].primaryFiringWait[1];
        gameData.botData.pInfo[i].secondaryFiringWait[DIFFICULTY_LEVEL_COUNT - 1] =
            gameData.botData.pInfo[i].secondaryFiringWait[1];
        gameData.botData.pInfo[i].nRapidFireCount[DIFFICULTY_LEVEL_COUNT - 1] =
            gameData.botData.pInfo[i].nRapidFireCount[1];
    }
    RETURN
}
