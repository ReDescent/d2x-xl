...
int32_t AICanSeeTarget(CObject *pObj, CFixVector *vPos, fix fieldOfView, CFixVector *vVecToTarget) {
    ENTER(1, 0);
    CHitQuery hitQuery(
        FQ_TRANSWALL | FQ_CHECK_OBJS | FQ_VISIBILITY | (pObj->AttacksRobots() ? FQ_ANY_OBJECT : FQ_CHECK_PLAYER),
        vPos,
    ...
    if (gameOpts->gameplay.nAIAggressivity) { // player visible at raised AI aggressivity when having headlight on
        if ((TARGETOBJ->info.nType == OBJ_PLAYER) && (TARGETOBJ->info.nId == N_LOCALPLAYER) && HeadlightIsOn(-1))
            RETVAL(2)
...
int32_t AICanFireAtTarget(CObject *pObj, CFixVector *vGun, CFixVector *vTarget) {
    ...
    CHitQuery hitQuery(
        FQ_CHECK_OBJS | FQ_ANY_OBJECT | FQ_IGNORE_POWERUPS | FQ_TRANSPOINT | FQ_VISIBILITY,
        vGun,
        vTarget,
        -1,
        pObj->Index(),
    ...
    pObj->SetSize(nSize);
    RETVAL(
        (gameData.aiData.nHitType == HIT_NONE) || ((gameData.aiData.nHitType == HIT_OBJECT) && pObj->Target() &&
                                                   (pObj->Target()->Index() == gameData.aiData.hitResult.nObject)))
}
...
// Updates pLocalInfo->nPrevVisibility if player is not cloaked, in which case the previous visibility is left
// unchanged 	and is copied to gameData.aiData.nTargetVisibility
void ComputeVisAndVec(
    ...
    if (*flag)
        ...
        if (TARGETOBJ->Cloaked()) {
            ...
            dist = CFixVector::NormalizedDir(
                gameData.aiData.target.vDir,
                gameData.aiData.cloakInfo[nCloakIndex].vLastPos,
                *pos);
            gameData.aiData.nTargetVisibility = AICanSeeTarget(
                pObj,
                pos,
...
#endif
            if ((pLocalInfo->nextMiscSoundTime < gameData.timeData.xGame) &&
                ((pLocalInfo->pNextrimaryFire < I2X(1)) || (pLocalInfo->nextSecondaryFire < I2X(1))) &&
                (dist < I2X(20))) {
            ...
            if ((pLocalInfo->nPrevVisibility != gameData.aiData.nTargetVisibility) &&
                ...
                if (pLocalInfo->nPrevVisibility == 0) {
                    if (pLocalInfo->timeTargetSeen + I2X(1) / 2 < gameData.timeData.xGame) {
                        ...
                        // -- 	audio.CreateSegmentSound (pRobotInfo->tauntSound, pObj->info.nSegment, 0, pos, 0 ,
                        // nRobotSoundVolume);
                        // -- else
...
int32_t AIDoorIsOpenable(CObject *pObj, CSegment *pSeg, int16_t nSide) {
    ...
    if ((pObj == NULL) || pObj->IsGuideBot()) {
        ...
        (pObj->info.nId == ROBOT_BRAIN) || (pObj->cType.aiInfo.behavior == AIB_RUN_FROM) ||
...
// --------------------------------------------------------------------------------------------------------------------
// Return true if placing an CObject of size size at pos *pos intersects a (player or robot or control center) in
// CSegment *pSeg.
...
// player. 	OBJECTS are moved by physics, so they can move even if not interested in a player.  However, if their
// velocity
// or 	orientation is changing, this routine will be called. 	Return value: 		0	this player IS NOT allowed to move
// this robot. 		1	this player IS allowed to move this robot.
