...
int32_t CSaveGameManager::LoadAIBinFormat(void) {
    ...
    gameData.aiData.localInfo.Read(
        m_cf,
        (m_nVersion > 39) ? LEVEL_OBJECTS : (m_nVersion > 22) ? MAX_OBJECTS : MAX_OBJECTS_D2);
    ...
    if (m_nVersion >= 21) {
        ...
        for (i = 0; i < j; i++) {
            if (gameData.bossData[i].m_nGateSegs &&
                gameData.bossData[i].m_gateSegs.Create(gameData.bossData[i].m_nGateSegs))
                gameData.bossData[i].m_gateSegs.Read(m_cf);
            else
                return 0;
            if (gameData.bossData[i].m_nTeleportSegs &&
                gameData.bossData[i].m_teleportSegs.Create(gameData.bossData[i].m_nTeleportSegs))
