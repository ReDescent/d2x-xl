...
void DoSnipeWait(CObject *pObj, tAILocalInfo *pLocalInfo) {
    ...
    fix xConnectedDist = simpleRouter[0].PathLength(
...
void DoSnipeFire(CObject *pObj, tAILocalInfo *pLocalInfo) {
    ...
    if (pLocalInfo->nextActionTime < 0) {
        ...
        pStaticInfo->nPathLength =
            (pStaticInfo->nHideIndex < 0)
                ? 0
                : SmoothPath(pObj, &gameData.aiData.routeSegs[pStaticInfo->nHideIndex], pStaticInfo->nPathLength);
        if (RandShort() < 8192)
