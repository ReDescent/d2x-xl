...
static const uint8_t mix8[] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04,
    ...
    0x8A, 0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9A, 0x9B, 0x9C,
    ...
    0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5,
    0xD6, 0xD7, 0xD8, 0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3, 0xE4, 0xE5, 0xE6, 0xE7, 0xE8,
    0xE9, 0xEA, 0xEB, 0xEC, 0xED, 0xEE, 0xEF, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB,
    0xFC, 0xFD, 0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
template <typename _T> int32_t ResampleChannels(_T *pDest, _T *srcP, int32_t nSrcLen) {
    ENTER(0, 0);
    _T nSound;
    float fFade;
...
fix CAudioChannel::Duration(void) {
    ...
    if (!m_info.pMixChunk)
        RETVAL(0)
    RETVAL(
        F2X(float(m_info.pMixChunk->alen) /
                float(gameOpts->sound.audioSampleRate * ((audio.Format() == AUDIO_U8) ? 2 : 4)) +
            0.5f))
}
...
int32_t CAudioChannel::Start(
    ...
    if (!(pszWAV && *pszWAV)) {
        ...
        if (!gameData.pigData.sound.nSoundFiles[gameStates.sound.bD1Sound])
            RETVAL(-1)
        pSound = gameData.pigData.sound.sounds[gameStates.sound.bD1Sound] +
                 nSound % gameData.pigData.sound.nSoundFiles[gameStates.sound.bD1Sound];
...

    {
        ...
        if (pszWAV && *pszWAV) {
            ...
            if (!gameOpts->sound.bHires[0])
                m_info.pMixChunk = Mix_QuickLoad_RAW(
                    reinterpret_cast<Uint8 *>(pSound->data[pSound->bCustom].Buffer()),
                    pSound->nLength[pSound->bCustom]);
                ...
                int32_t l = Resample(
                    pSound,
                    (gameStates.sound.bD1Sound || gameStates.app.bDemoData) &&
...
int32_t CAudio::InternalSetup(float fSlowDown, int32_t nFormat, const char *driver) {
    ...
    if (gameOpts->UseHiresSound())
        h = Mix_OpenAudio(
            int32_t((gameOpts->sound.audioSampleRate = SAMPLE_RATE_44K) / fSlowDown),
            m_info.nFormat = AUDIO_S16SYS,
            2,
            SOUND_BUFFER_SIZE);
    else
        h = Mix_OpenAudio(
            int32_t((gameOpts->sound.audioSampleRate = SAMPLE_RATE_22K) / fSlowDown),
            m_info.nFormat = AUDIO_U8,
            1,
            SOUND_BUFFER_SIZE);
    if (h < 0)
...
void CAudio::Close(void) {
    ...
    if (!(gameStates.sound.bDynamic && m_info.bAvailable))
        RETURN;
    m_info.bInitialized = m_info.bAvailable = 0;
...
void CAudio::UnregisterChannel(int32_t nIndex) {
    ...
    if (nIndex <
        --nToS) { // move the top most entry in m_usedChannels to the position used for the unregistered channel
...
CAudioChannel *CAudio::FindFreeChannel(int32_t nSoundClass) {
    ...
    do {
        ...
        if ((!bUseClass || (pChannel->SoundClass() == nSoundClass)) &&
            (!channelMinVolP[pChannel->Persistent()] ||
             (channelMinVolP[pChannel->Persistent()]->Volume() > pChannel->Volume())))
...
int32_t CAudio::GetMaxChannels(void) { return m_info.nMaxChannels; }
// end edit by adb
...
int32_t CAudio::ChannelIsPlaying(int32_t nChannel) {
    ...
    if ((nChannel < 0) || (nChannel >= m_info.nMaxChannels))
        return 0;
    return audio.m_channels[nChannel].Playing() != 0;
}
