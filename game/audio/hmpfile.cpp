...
static int32_t get_event(hmp_file *hmp, event *ev) {
    ...
    for (trk = hmp->trks, i = hmp->num_trks; (i--) > 0; trk++) {
        ...
        if (trk->left > got + 2 && *(trk->cur + got) == 0xff && *(trk->cur + got + 1) == 0x2f) { /* end of track */
            trk->left = 0;
            continue;
        }
            ...
            *(p++) = 0;
            ...
            if ((i = get_event(hmp, &ev))) {
                mhdr->dwBytesRecorded =
                    (int32_t)(reinterpret_cast<uint8_t *>(p) - reinterpret_cast<uint8_t *>(mhdr->lpData));
                return i;
            }
            if (ev.datalen) {
                ...
                *(p++) = ev.delta;
                *(p++) = 0;
                *(p++) = (((DWORD)MEVT_SHORTMSG) << 24) | ((DWORD)ev.msg[0]) | (((DWORD)ev.msg[1]) << 8) |
                         (((DWORD)ev.msg[2]) << 16);
            }
        }
    }
...
static void _stdcall midi_callback(HMIDISTRM hms, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2) {
    ...
    if (!hmp->stop) {
        ...
        if ((rc = midiStreamOut(hmp->hmidi, hmp->evbuf, sizeof(MIDIHDR))) != MMSYSERR_NOERROR) {
            /* ??? */
        } else {
...
int32_t hmp_play(hmp_file *hmp, int32_t bLoop) {
    ...
    if ((rc = setup_buffers(hmp)))
        return rc;
    if ((midiStreamOpen(&hmp->hmidi, &hmp->devid, 1, (DWORD_PTR)midi_callback, 0, CALLBACK_FUNCTION)) !=
        MMSYSERR_NOERROR) {
        hmp->hmidi = NULL;
        return HMP_MM_ERR;
    }
...
static uint8_t midiSetTempo[19] =
    {'M', 'T', 'r', 'k', 0, 0, 0, 11, 0, 0xFF, 0x51, 0x03, 0x18, 0x80, 0x00, 0, 0xFF, 0x2F, 0};
