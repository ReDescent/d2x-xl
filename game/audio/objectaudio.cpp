...
bool CSoundObject::Start(void) {
    ...
    if ((m_flags & SOF_PERMANENT) && (audio.ActiveObjects() >= Max(1, 33 * audio.GetMaxChannels() / 100)) &&
        !audio.SuspendObjectSound(m_volume))
        RETVAL(false)
    // start the sample playing
    m_channel = audio.StartSound(
        m_nSound,
        m_soundClass,
        m_volume,
        m_pan,
        (m_flags & SOF_PLAY_FOREVER) != 0,
        m_nLoopStart,
        m_nLoopEnd,
        int32_t(this - audio.Objects().Buffer()),
        I2X(1),
        m_szSound,
...
int32_t CAudio::Distance(
    CFixVector &vListenerPos,
    int16_t nListenerSeg,
    CFixVector &vSoundPos,
    int16_t nSoundSeg,
    fix maxDistance,
    int32_t nDecay,
    ...
    if ((m_nListenerSeg != m_router[nThread].StartSeg()) ||
        (m_router[nThread].DestSeg() >
         -1)) { // either we had a different start last time, or the last calculation was a 1:1 routing
        m_router[nThread].PathLength(
            CFixVector::ZERO,
            nListenerSeg,
            CFixVector::ZERO,
            -1,
...
void CAudio::GetVolPan(
    CFixMatrix &mListener,
    CFixVector &vListenerPos,
    int16_t nListenerSeg,
    CFixVector &vSoundPos,
    int16_t nSoundSeg,
...
int32_t CAudio::PlaySound(
    ...
    // start the sample playing
    RETVAL(
        StartSound(nSound, nSoundClass, nVolume, nPan, 0, (nLoops > 0) ? nLoops - 1 : -1, -1, -1, I2X(1), pszWAV, vPos))
}
...
void CAudio::StartLoopingSound(void) {
    ...
    if (m_info.nLoopingSound > -1)
        m_info.nLoopingChannel = StartSound(
            m_info.nLoopingSound,
            SOUNDCLASS_GENERIC,
            m_info.nLoopingVolume,
            DEFAULT_PAN,
            1,
            m_info.nLoopingStart,
            m_info.nLoopingEnd);
    RETURN
}
...
void CAudio::ResumeLoopingSound(void) { StartLoopingSound(); }
...
int32_t CAudio::CreateObjectSound(
    ...
    if (!bForever) { // Hack to keep sounds from building up...
        ...
        GetVolPan(
            gameData.objData.pViewer->info.position.mOrient,
    ...
    if (gameStates.sound.bDontStartObjects) { // started at level start
        ...
        GetVolPan(
            ...
            pObj->info.position.vPos,
            pObj->info.nSegment,
            pSoundObj->m_maxVolume,
            &pSoundObj->m_volume,
            &pSoundObj->m_pan,
            pSoundObj->m_maxDistance,
            pSoundObj->m_nDecay,
            nThread);
        pSoundObj->Start();
...
int32_t CAudio::CreateSegmentSound(
    ...
    if (!SEGMENT(nSegment))
        RETVAL(-1)
    if ((pSoundObj = FindSegmentSound(nSound, nSegment, nSide, vPos))) // if the same sound plays in this segment
                                                                       // nearby, don't start another one
        RETVAL(pSoundObj->m_nSignature)
    ...
    if (gameStates.sound.bDontStartObjects) { // started at level start
        ...
        GetVolPan(
            ...
            gameData.objData.pViewer->info.position.vPos,
            gameData.objData.pViewer->info.nSegment,
            pSoundObj->m_linkType.pos.position,
            pSoundObj->m_linkType.pos.nSegment,
            pSoundObj->m_maxVolume,
            &pSoundObj->m_volume,
            &pSoundObj->m_pan,
            pSoundObj->m_maxDistance,
...
void CAudio::RecordSoundObjects(void) {
    ENTER(0, 0);
    for (uint32_t i = 0; i < m_objects.ToS(); i++) {
        if ((m_objects[i].m_flags & (SOF_USED | SOF_LINK_TO_OBJ | SOF_PLAY_FOREVER)) ==
            (SOF_USED | SOF_LINK_TO_OBJ | SOF_PLAY_FOREVER)) {
            NDRecordCreateObjectSound(
                UnXlatSound(m_objects[i].m_nSound),
                m_objects[i].m_linkType.obj.nObject,
                m_objects[i].m_maxVolume,
                m_objects[i].m_maxDistance,
                m_objects[i].m_nLoopStart,
...
void CAudio::SyncSounds(void) {
    ...
    int32_t nOldVolume, nNewVolume, nOldPan, nAudioVolume[2] = {audio.Volume(0), audio.Volume(1)};
    CObject *pObj = NULL;
    ...
    while (i) {
        ...
        if (soundObj.m_flags & SOF_USED) {
            ...
            if (!(soundObj.m_flags & SOF_PLAY_FOREVER) &&
                ((soundObj.m_channel < 0) && !ChannelIsPlaying(soundObj.m_channel))) {
                DeleteSoundObject(i);
            ...
            if (soundObj.m_flags & SOF_LINK_TO_POS) {
                ...
                GetVolPan(
                    ...
                    vListenerPos,
                    nListenerSeg,
                    soundObj.m_linkType.pos.position,
                    soundObj.m_linkType.pos.nSegment,
                    soundObj.m_maxVolume,
                    &soundObj.m_volume,
                    &soundObj.m_pan,
                    soundObj.m_maxDistance,
                    soundObj.m_nDecay);
                ...
                if (!pObj || (pObj->info.nType == OBJ_NONE) ||
                    (pObj->info.nSignature != soundObj.m_linkType.obj.nObjSig)) {
                    DeleteSoundObject(
                        i); // The object that this is linked to is dead, so just end this sound if it is looping.
                    continue;
                } else if (
                    (pObj->info.nType == OBJ_EFFECT) &&
                    (((pObj->info.nId == SOUND_ID) && !pObj->rType.soundInfo.bEnabled) ||
                     ((pObj->info.nId == LIGHTNING_ID) &&
                      !(SHOW_LIGHTNING(1) && pObj->rType.lightningInfo.bEnabled)))) {
                    soundObj.Stop();
                    continue;
                }
                GetVolPan(
...
void CSoundQueue::Init(void) {
    m_data.nHead = m_data.nTail = m_data.nSounds = 0;
    m_data.nChannel = -1;
...
void CSoundQueue::Destroy(void) { m_data.queue.Destroy(); }

//------------------------------------------------------------------------------

void CSoundQueue::Pause(void) { soundQueue.m_data.nChannel = -1; }
...
void SetD1Sound(void) {
    gameStates.sound.bD1Sound = gameStates.app.bD1Mission && gameOpts->sound.bUseD1Sounds &&
                                (gameStates.app.bHaveD1Data || gameOpts->UseHiresSound());
}
        ...
        for (nSide = 0; nSide < SEGMENT_SIDE_COUNT; nSide++)
            if (nSideSounds[nSide] >= 0)
                audio.CreateSegmentSound(
                    ...
                    nSegment,
                    nSide,
                    pSeg->SideCenter(nSide),
                    1,
                    I2X(1) / (2 * nSegSoundSources));
    }
