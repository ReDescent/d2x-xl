...
void CCockpit::DrawHomingWarning(void) {
    ...
    BitBlt(
        m_info.bLastHomingWarningDrawn[0] ? GAUGE_HOMING_WARNING_ON : GAUGE_HOMING_WARNING_OFF,
        HOMING_WARNING_X,
        HOMING_WARNING_Y);
}
...
int32_t CCockpit::DrawBombCount(int32_t &nIdBombCount, int32_t x, int32_t y, int32_t nColor, char *pszBombCount) {
    ...
    return PrintF(
        &nIdBombCount,
        -(ScaleX(x) + WidthPad(pszBombCount)),
        -(ScaleY(y) + HeightPad()),
        pszBombCount,
        nIdBombCount);
}
...
CBitmap* pBm = BitBlt (GAUGE_NUMERICAL, NUMERICAL_GAUGE_X, NUMERICAL_GAUGE_Y);
    ...
    nIdShield = PrintF(
        &nIdShield,
        -(ScaleX(NUMERICAL_GAUGE_X + pBm->Width() / 2) - w / 2),
        NUMERICAL_GAUGE_Y + (gameStates.video.nDisplayMode ? 36 : 16) + HeightPad(),
        szShield);
...
CBitmap* pBm = BitBlt (GAUGE_NUMERICAL, NUMERICAL_GAUGE_X, NUMERICAL_GAUGE_Y);
    ...
    nIdEnergy = PrintF(
        &nIdEnergy,
        -(ScaleX(NUMERICAL_GAUGE_X + pBm->Width() / 2) - w / 2),
        NUMERICAL_GAUGE_Y + (gameStates.video.nDisplayMode ? 5 : 2),
        szEnergy);
    fontManager.SetScale(1.0f);
}
...
#define ENERGY_GAUGE_BOT_WIDTH 126
    ...
    if (m_info.nEnergy) {
        ...
        if (m_info.nEnergy < 100) { // erase part of gauge corresponding to energy loss
            ...
            {
                int32_t x[4] = {
                    ENERGY_GAUGE_TOP_LEFT,
                    LEFT_ENERGY_GAUGE_W,
                    ENERGY_GAUGE_BOT_LEFT + ENERGY_GAUGE_BOT_WIDTH,
                    ENERGY_GAUGE_BOT_LEFT};
...

            {
                int32_t x[4] = {
                    0,
                    LEFT_ENERGY_GAUGE_W - ENERGY_GAUGE_TOP_LEFT,
                    LEFT_ENERGY_GAUGE_W - ENERGY_GAUGE_BOT_LEFT,
                    LEFT_ENERGY_GAUGE_W - ENERGY_GAUGE_BOT_WIDTH};
                int32_t y[4] = {0, 0, LEFT_ENERGY_GAUGE_H, LEFT_ENERGY_GAUGE_H};
...
uint8_t afterburnerBarTable[AFTERBURNER_GAUGE_H_L * 2] = {
    ...
    1, 9,  1, 9,  1, 9,  1, 9,  0, 9,  0, 9,  0, 8,  0, 8,  0, 8,  0, 8,  1, 8,  2, 8,  3, 8,  4, 8,  5, 8,  6, 7,
};
...
static tKeyGaugeInfo keyGaugeInfo[] = {
    {PLAYER_FLAGS_BLUE_KEY,
     GAUGE_BLUE_KEY,
     GAUGE_BLUE_KEY_OFF,
     {GAUGE_BLUE_KEY_X_L, GAUGE_BLUE_KEY_X_H},
     {GAUGE_BLUE_KEY_Y_L, GAUGE_BLUE_KEY_Y_H}},
    {PLAYER_FLAGS_GOLD_KEY,
     GAUGE_GOLD_KEY,
     GAUGE_GOLD_KEY_OFF,
     {GAUGE_GOLD_KEY_X_L, GAUGE_GOLD_KEY_X_H},
     {GAUGE_GOLD_KEY_Y_L, GAUGE_GOLD_KEY_Y_H}},
    {PLAYER_FLAGS_RED_KEY,
     GAUGE_RED_KEY,
     GAUGE_RED_KEY_OFF,
     {GAUGE_RED_KEY_X_L, GAUGE_RED_KEY_X_H},
     {GAUGE_RED_KEY_Y_L, GAUGE_RED_KEY_Y_H}},
};
