...
void CGenericCockpit::DrawReticle(int32_t bForceBig, fix xStereoSeparation) {
    if (cockpit->Hide())
        return;
    if (!gameOpts->render.cockpit.bReticle || !gameStates.app.bGameRunning || gameStates.menus.nInMenu ||
        gameStates.render.bRearView || gameStates.app.bPlayerIsDead || gameStates.render.bChaseCam ||
        (gameStates.render.bFreeCam > 0) || automap.Active())
        return;
    ...
    if (ogl.IsOculusRift()) {
        ...
        //  (fabs (X2F (transformation.m_info.playerHeadAngles.v.coord.p)) > 0.1f))
        {
            ...
            if (gameOpts->input.oculusRift.nDeadzone) { // display a reference image of the reticle
                ...
                BitBlt(
                    (bSmallReticle ? SML_RETICLE_CROSS : RETICLE_CROSS) + nCrossBm,
                    x + ScaleX(crossOffsets[ofs].x - 1),
                    (y + ScaleY(crossOffsets[ofs].y - 1)),
                    false,
                    true,
                    I2X(1),
                    0,
                    NULL,
                    BM_ADDON(nBmReticle + nCrossBm));
                BitBlt(
                    (bSmallReticle ? SML_RETICLE_PRIMARY : RETICLE_PRIMARY) + nPrimaryBm,
                    x + ScaleX(primaryOffsets[ofs].x - 1),
                    (y + ScaleY(primaryOffsets[ofs].y - 1)),
                    false,
                    true,
                    I2X(1),
                    0,
                    NULL,
                    BM_ADDON(nBmReticle + 2 + nPrimaryBm));
                BitBlt(
                    (bSmallReticle ? SML_RETICLE_SECONDARY : RETICLE_SECONDARY) + nSecondaryBm,
                    x + ScaleX(secondaryOffsets[ofs].x - 1),
                    (y + ScaleY(secondaryOffsets[ofs].y - 1)),
                    false,
                    true,
                    I2X(1),
                    0,
                    NULL,
                    BM_ADDON(nBmReticle + 5 + nSecondaryBm));
