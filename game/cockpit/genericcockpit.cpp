...
void CCockpitInfo::Init(void) {
    nCloakFadeState = 0;
    bLastHomingWarningDrawn[0] = bLastHomingWarningDrawn[1] = -1;
    addedScore[0] = addedScore[1] = 0;
    scoreTime = 0;
    lastWarningBeepTime[0] = lastWarningBeepTime[1] = 0;
    bHaveGaugeCanvases = 0;
    nInvulnerableFrame = 0;
    weaponBoxStates[0] = weaponBoxStates[1] = 0;
    weaponBoxFadeValues[0] = weaponBoxFadeValues[1] = 0;
...
void CGenericCockpit::RenderWindow(
    int32_t nWindow,
    CObject *pViewer,
    int32_t bRearView,
    int32_t nUser,
    ...
    if (gameData.demoData.nState == ND_STATE_PLAYBACK) {
        if (nDemoDoLeft) {
            if (nDemoDoLeft == 3)
                ...
                cockpit->RenderWindow(
                    0,
                    &demoLeftExtra,
                    bDemoRearCheck[nDemoDoLeft],
                    nDemoWBUType[nDemoDoLeft],
                    szDemoExtraMessage[nDemoDoLeft]);
        } else
        ...
        if (nDemoDoRight) {
            if (nDemoDoRight == 3)
                ...
                cockpit->RenderWindow(
                    1,
                    &demoRightExtra,
                    bDemoRearCheck[nDemoDoRight],
                    nDemoWBUType[nDemoDoRight],
                    szDemoExtraMessage[nDemoDoRight]);
        } else
    ...
    for (w = 0; w < 2 - bDidMissileView; w++) {
        ...
        case CV_COOP: {
            ...
            if ((nPlayer != -1) && PLAYER(nPlayer).IsConnected() && SameTeam(nPlayer, N_LOCALPLAYER))
                cockpit->RenderWindow(
                    w,
                    PLAYEROBJECT(gameStates.render.cockpit.nCoopPlayerView[w]),
                    0,
                    WBU_COOP,
                    PLAYER(gameStates.render.cockpit.nCoopPlayerView[w]).callsign);
            else {
                cockpit->RenderWindow(w, NULL, 0, WBU_WEAPON, NULL);
                gameStates.render.cockpit.n3DView[w] = CV_NONE;
            }
        ...
        case CV_RADAR_HEADSUP:
            if (!(gameStates.app.bNostalgia || COMPETITION) && EGI_FLAG(bRadarEnabled, 0, 1, 0))
                cockpit->RenderWindow(
                    ...
                    0,
                    (gameStates.render.cockpit.n3DView[w] == CV_RADAR_TOPDOWN) ? WBU_RADAR_TOPDOWN : WBU_RADAR_HEADSUP,
                    "MINI MAP");
            else
                gameStates.render.cockpit.n3DView[w] = CV_NONE;
            break;
...
void CGenericCockpit::SetupSceneCenter(CCanvas *refCanv, int32_t &w, int32_t &h) {
    ...
    if (ogl.IsOculusRift()) {
        ...
        gameData.renderData.window.Setup(
            refCanv,
            w / 2 - CScreen::Unscaled(gameData.StereoOffset2D()),
            (gameData.renderData.screen.Height(false) - h) / 2,
            w,
            h);
...
m_info.yScale = gameData.renderData.scene.YScale ();
    ...
    m_info.nCockpit =
        (gameStates.video.nDisplayMode && !gameStates.app.bDemoData) ? gameData.modelData.nCockpits / 2 : 0;
    ...
    if (m_info.nShield < 0)
        m_info.nShield = 0;
    m_info.bCloak = ((LOCALPLAYER.flags & PLAYER_FLAGS_CLOAKED) != 0);
    m_info.tInvul = (LOCALPLAYER.invulnerableTime == 0x7fffffff)
                        ? LOCALPLAYER.invulnerableTime
                        : LOCALPLAYER.invulnerableTime + INVULNERABLE_TIME_MAX - gameData.timeData.xGame;
    ...
    if (gameOpts->render.cockpit.bScaleGauges) {
        ...
        m_info.xGaugeScale = m_info.yGaugeScale = 1;
    ...
    if (!GuidedMissileActive() &&
        ((gameOpts->render.cockpit.bHUD > 1) || (gameStates.render.cockpit.nType < CM_FULL_SCREEN)) &&
        ...
        if (ogl.IsOculusRift() && !transformation.HaveHeadAngles()) {
            ...
            int32_t bStereoOffset = ogl.IsSideBySideDevice() && ((gameStates.render.cockpit.nType == CM_FULL_SCREEN) ||
            ...
            bool bLimited =
                (gameStates.render.bRearView || gameStates.render.bChaseCam || (gameStates.render.bFreeCam > 0));
