...
void CHUD::DrawShieldBar(void) {
    ...
    if (!ShowTextGauges()) {
        ...
        int32_t w = int32_t(9 * m_info.xGaugeScale);
        int32_t y =
            CCanvas::Current()->Height() - (int32_t)(((IsMultiGame ? 6 : 2) * nLineSpacing - 1) * m_info.yGaugeScale);
        ...
        if (gameOpts->gameplay.bShieldWarning) {
            if ((nBeep < 0) || (bLastFlash != gameStates.render.cockpit.nShieldFlash)) {
                ...
                nBeep = audio.StartSound(
                    ...
                    I2X(1),
                    AddonSoundName(
                        (gameStates.render.cockpit.nShieldFlash == 1) ? SND_ADDON_LOW_SHIELDS1
                                                                      : SND_ADDON_LOW_SHIELDS2));
            }
        } else if (nBeep >= 0) {
            audio.StopSound((int32_t)nBeep);
            nBeep = -1;
...
void CHUD::DrawEnergyBar(void) {
    ...
    if (!ShowTextGauges()) {
        ...
        int32_t h = int32_t(9 * m_info.yGaugeScale), w = int32_t(9 * m_info.xGaugeScale),
                y = CCanvas::Current()->Height() -
                    (int32_t)(((IsMultiGame ? 5 : 1) * nLineSpacing - 1) * m_info.yGaugeScale);
        if (hudIcons.LoadGaugeIcons() > 0)
...
void CHUD::DrawAfterburnerBar(void) {
    ...
    if (!ShowTextGauges()) {
        int32_t h = int32_t(9 * m_info.yGaugeScale), w = int32_t(9 * m_info.xGaugeScale),
                y = CCanvas::Current()->Height() -
                    (int32_t)(((IsMultiGame ? 8 : 3) * nLineSpacing - 1) * m_info.yGaugeScale);
        if (hudIcons.LoadGaugeIcons() > 0)
            hudIcons.GaugeIcon(2)
                .RenderScaled(6, y, w = int32_t(9 * m_info.xGaugeScale), h = int32_t(9 * m_info.yGaugeScale));
        CCanvas::Current()->SetColorRGB(255, 0, 0, 255);
        glLineWidth(1);
        int32_t x = 6 + int32_t(10 * m_info.xGaugeScale);
...
void CHUD::DrawEnergyLevelsCombined(void) {
    ...
    int32_t nEnergy[3] = {
        LOCALPLAYER.Energy() ? X2IR(LOCALPLAYER.Energy()) : 0,
...
int32_t CHUD::StrWeaponStateColor(char *pszList, int32_t l, int32_t bAvailable, int32_t bActive) {
    memcpy(
        pszList + l,
        bAvailable ? bActive ? CC_EQUIPPED[gameOpts->render.cockpit.nColorScheme] : CC_AVAILABLE : CC_EMPTY,
        4);
    return l + 4;
}
...
void CHUD::SetWeaponStateColor(int32_t bAvailable, int32_t bActive) {
    ...
    fontManager.SetColorRGBi(
        bAvailable
            ? bActive ? equippedColors[gameOpts->render.cockpit.nColorScheme] : /*GREEN_RGBA*/ RGBA(0, 224, 0, 255)
...
char *CHUD::StrPrimaryWeaponList(char *pszList, char *pszAmmo) {
    ...
    const static char *szWeaponIds = "LVSPFSGHXO";
...
static int32_t WeaponListTop(void) {
    ...
    return gameData.renderData.scene.Height() / 2 - (gameStates.app.bD1Mission ? 1 : 2) * nLineSpacing +
...
void CHUD::DrawPrimaryWeaponList(void) {
    ...
    int32_t x = gameData.renderData.scene.Width() / 2 -
                cockpit->ScaleX(X_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactWidth) * 3);
...
void CHUD::DrawSecondaryWeaponList(void) {
    // CHSM MGFS - Concussion, Homer, Smart, Mega, Mercury, Guided, Flash, Shaker
    const static char *szWeaponIds = "CHBSMFGSYE";
...
void CHUD::DrawSecondaryAmmoList(char *pszList) {
    ...
    int32_t nLineSpacing = cockpit->LineSpacing();
    int32_t x =
        gameData.renderData.scene.Width() / 2 + cockpit->ScaleX(X_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactWidth));
    int32_t y = gameData.renderData.scene.Height() / 2 +
                ScaleY(Y_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactHeight)) + LineSpacing() * 2;
...
char *CHUD::StrPrimaryWeapon(char *szLabel) {
    ...
    case GAUSS_INDEX:
        sprintf(
            ...
            szWeapon,
            nLayout ? " " : ": ",
            X2I((uint32_t)LOCALPLAYER.primaryAmmo[VULCAN_INDEX] * (uint32_t)VULCAN_AMMO_SCALE));
        Convert1s(szLabel);
        break;
    ...
    case OMEGA_INDEX:
        sprintf(
            szLabel,
            "%s%s%03i",
            nLayout ? "OMG" : szWeapon,
            nLayout ? " " : ": ",
            gameData.omegaData.xCharge[IsMultiGame] * 100 / MAX_OMEGA_CHARGE);
        Convert1s(szLabel);
        break;
...
char *CHUD::StrSecondaryWeapon(char *szLabel) {
    sprintf(
        szLabel,
        "%s %d",
        SECONDARY_WEAPON_NAMES_VERY_SHORT(gameData.weaponData.nSecondary),
        LOCALPLAYER.secondaryAmmo[gameData.weaponData.nSecondary]);
    return szLabel;
}
...
void CHUD::DrawWeapons(void) {
    ...
    if (!nLayout) {
        ...
        if (nLayout == 2)
            ...
            DrawHUDText(
                NULL,
                gameData.renderData.scene.Width() / 2 - ScaleX(X_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactWidth)) -
                    StringWidth(szLabel + 1),
                gameData.renderData.scene.Height() / 2 +
                    ScaleY(Y_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactHeight)) +
                    LineSpacing() * ((nLayout == 2) ? 3 : 1),
                StrPrimaryWeaponList(szLabel, szAmmo));
            DrawHUDText(
                NULL,
                gameData.renderData.scene.Width() / 2 - ScaleX(X_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactWidth)) -
                    StringWidth(szAmmo),
    ...
    if (!nLayout) {
        ...
        if (nLayout == 2)
            ...
            DrawHUDText(
                ...
                gameData.renderData.scene.Width() / 2 + ScaleX(X_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactWidth)),
                gameData.renderData.scene.Height() / 2 +
                    ScaleY(Y_GAUGE_OFFSET(gameOpts->render.cockpit.nCompactHeight)) + LineSpacing(),
                StrSecondaryWeaponList(szLabel));
...
void CHUD::DrawInvul(void) {
    ...
    if ((LOCALPLAYER.flags & PLAYER_FLAGS_INVULNERABLE) &&
        (((LOCALPLAYER.invulnerableTime + INVULNERABLE_TIME_MAX - gameData.timeData.xGame) > I2X(4)) ||
         (gameData.timeData.xGame & 0x8000))) {
        ...
        if (nLayout) {
            ...
            nIdInvul = DrawHUDText(
                &nIdInvul,
                gameData.renderData.scene.Width() / 2 + ScaleX(X_GAUGE_OFFSET(0)),
                gameData.renderData.scene.Height() / 2 + ScaleY(Y_GAUGE_OFFSET(0)) +
                    ((nLayout == 2) ? 1 : 3) * LineSpacing(),
                "%s",
                "INV");
            ScaleDown();
        } else {
...
void CHUD::DrawCloak(void) {
    ...
    if ((LOCALPLAYER.flags & PLAYER_FLAGS_CLOAKED) &&
        ((LOCALPLAYER.cloakTime + CLOAK_TIME_MAX - gameData.timeData.xGame > I2X(3)) ||
         (gameData.timeData.xGame & 0x8000))) {
        int32_t nLayout = gameStates.menus.nInMenu ? 0 : gameOpts->render.cockpit.nShipStateLayout;
        ...
        if (nLayout) {
            ScaleUp();
            SetFontColor(RGBA(192, 192, 192, 255));
            nIdCloak = DrawHUDText(
                &nIdCloak,
                gameData.renderData.scene.Width() / 2 - ScaleX(X_GAUGE_OFFSET(0)) - StringWidth("CLK", 0),
                gameData.renderData.scene.Height() / 2 + ScaleY(Y_GAUGE_OFFSET(0)) +
                    ((nLayout == 2) ? 1 : 3) * LineSpacing(),
                "%s",
                "CLK");
            ...
            if (ogl.IsOculusRift())
                nIdCloak = DrawHUDText(
                    &nIdCloak,
                    CCanvas::Current()->Width() - StringWidth("CLOAK "),
                    -2 * LineSpacing(),
                    "%s",
                    "CLOAK");
            else {
...
void CHUD::DrawSlowMotion(void) {
    ...
    sprintf(szSlowMotion, "M%1.1f  S%1.1f", gameStates.gameplay.slowmo[0].fSpeed, gameStates.gameplay.slowmo[1].fSpeed);
...
void CHUD::DrawCockpit(bool bAlphaTest) {
    ...
    // 2); gameData.renderData.frame.SetTop ((gameData.renderData.screen.Height () - gameData.renderData.frame.Height
    // ()) / 2);
}
...
void CHUD::SetupWindow(int32_t nWindow) {
    ...
    int32_t w = (int32_t)(
        gameData.renderData.frame.Width() / cockpitWindowScale[gameOpts->render.cockpit.nWindowSize] * HUD_ASPECT);
    int32_t h = I2X(w) / gameData.renderData.screen.Aspect();
    if (!gameStates.app.bNostalgia) //(gameOpts->render.cockpit.bWideDisplays)
        w = int32_t(
            float(w) * float(gameData.renderData.screen.Width()) / float(gameData.renderData.screen.Height()) * 0.75f);
    ...
    if (nWindowPos == 0) // near corners
        x = nWindow ? Canvas()->Width() - w - h / 10 : h / 10;
    else if (nWindowPos == 1) // near middle
        x = nWindow ? Canvas()->Width() / 3 * 2 - w / 3 : Canvas()->Width() / 3 - 2 * w / 3;
    else // middle
    ...
    if (gameOpts->render.cockpit.nWindowPos > 2)
        ...
        if (extraGameInfo[0].nWeaponIcons &&
            ...
            y -= (int32_t)(
                (gameOpts->render.weaponIcons.bSmall ? 20.0 : 30.0) * (double)CCanvas::Current()->Height() / 480.0);
    }
...
bool CWideHUD::Setup(bool bScene, bool bRebuild) {
    ...
    int32_t h = gameData.renderData.screen.Height() -
                21 * gameData.renderData.screen.Width() /
                    40; //(int32_t) FRound ((float) gameData.renderData.frame.Width () * (480.0f / 640.0f * 0.7f));
