...
int32_t
CHUDIcons::GetWeaponState(int32_t &bHave, int32_t &bAvailable, int32_t &bActive, int32_t i, int32_t j, int32_t l) {
    ...
    if (ammoType[i][l]) {
        ...
        if (l == 0) { // Lasers
            sprintf(
                szAmmo,
                "%d",
                (LOCALPLAYER.LaserLevel() > MAX_LASER_LEVEL) ? MAX_LASER_LEVEL + 1 : LOCALPLAYER.LaserLevel() + 1);
...
void CHUDIcons::DrawWeapons(void) {
    ...
    int32_t nIconScale =
        (gameOpts->render.weaponIcons.bSmall || (gameStates.render.cockpit.nType != CM_FULL_SCREEN)) ? 4 : 3;
    ...
    static int32_t wIcon = 0, hIcon = 0;
    ...
    for (int32_t i = 0; i < 2; i++) {
        ...
        for (int32_t j = 0; j < n; j++) {
            ...
            if (!ogl.IsOculusRift()) {
                ...
                glLineWidth(
                    (bActive && bAvailable && gameOpts->render.weaponIcons.bBoldHighlight) ? fLineWidth + 2
                                                                                           : fLineWidth);
            ...
            cockpit->BitBlt(
                -1,
                nIconScale * (x + (w - pBm->Width()) / (2 * nIconScale)),
                nIconScale * (y - hIcon),
                false,
                true,
                I2X(nIconScale),
...
int32_t CHUDIcons::LoadInventoryIcons(void) {
    ...
    if (!((bmpInventory = PiggyLoadBitmap("inventry.bmp")) || (bmpInventory = PiggyLoadBitmap("inventory.bmp"))))
...
void CHUDIcons::DrawInventory(void) {
    ...
    char szCount[20];
    int32_t nIconScale =
        (gameOpts->render.weaponIcons.bSmall || (gameStates.render.cockpit.nType != CM_FULL_SCREEN)) ? 3 : 2;
    int32_t nIconPos = extraGameInfo[0].nWeaponIcons & 1;
    int32_t nHiliteColor = gameOpts->app.bColorblindFriendly;
    int32_t fw, fh, faw;
    int32_t j, n, firstItem, oy = 6, ox = 6, x, y, dy;
    int32_t w = bmpInventory->Width(), h = bmpInventory->Width();
