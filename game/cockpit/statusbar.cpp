    ...
    if (nGauge >= 0) {
        ...
        if (pBm)
            pBm->RenderScaled(
                ScaleX(x),
                ScaleY(y),
                ScaleX((int32_t)DRound(pBm->Width() * xScale)),
                ScaleY((int32_t)DRound(pBm->Height() * yScale)),
                scale,
                orient,
                NULL);
    }
...
void CStatusBar::DrawLives(void) {
    ...
    if (IsMultiGame) {
        ...
        char szKilled[20];
        int32_t x = SB_LIVES_X, y = (ScaleY(SB_LIVES_Y + 1) + HeightPad());
        ...
        nIdLives[1] = DrawHUDText(
            &nIdLives[1],
            ScaleX(SB_LIVES_X + pBm->Width() + m_info.fontWidth),
            y,
            " x %d",
            LOCALPLAYER.lives - 1);
    }
...
void CStatusBar::DrawShieldText(void) {
    ...
    SetFontScale((float)floor(float(CCanvas::Current()->Width()) / 640.0f));
    // LoadTexture (gameData.pigData.tex.cockpitBmIndex [gameStates.render.cockpit.nType +
    // (gameStates.video.nDisplayMode ? gameData.modelData.nCockpits / 2 : 0)].index, 0);
    SetFontColor(BLACK_RGBA);
    Rect(
        SB_SHIELD_NUM_X,
        SB_SHIELD_NUM_Y,
        SB_SHIELD_NUM_X + (gameStates.video.nDisplayMode ? 27 : 13),
        SB_SHIELD_NUM_Y + m_info.fontHeight);
...
void CStatusBar::DrawShieldBar(void) {
    if (!(LOCALPLAYER.flags & PLAYER_FLAGS_INVULNERABLE) || (m_info.tInvul <= 0)) {
        BitBlt(
            GAUGE_SHIELDS + 9 - ((m_info.nShield >= 100) ? 9 : (m_info.nShield / 10)),
            SB_SHIELD_GAUGE_X,
            SB_SHIELD_GAUGE_Y);
    }
}
...
static tKeyGaugeInfo keyGaugeInfo[] = {
    {PLAYER_FLAGS_BLUE_KEY,
     SB_GAUGE_BLUE_KEY,
     SB_GAUGE_BLUE_KEY_OFF,
     {SB_GAUGE_KEYS_X_L, SB_GAUGE_KEYS_X_H},
     {SB_GAUGE_BLUE_KEY_Y_L, SB_GAUGE_BLUE_KEY_Y_H}},
    {PLAYER_FLAGS_GOLD_KEY,
     SB_GAUGE_GOLD_KEY,
     SB_GAUGE_GOLD_KEY_OFF,
     {SB_GAUGE_KEYS_X_L, SB_GAUGE_KEYS_X_H},
     {SB_GAUGE_GOLD_KEY_Y_L, SB_GAUGE_GOLD_KEY_Y_H}},
    {PLAYER_FLAGS_RED_KEY,
     SB_GAUGE_RED_KEY,
     SB_GAUGE_RED_KEY_OFF,
     {SB_GAUGE_KEYS_X_L, SB_GAUGE_KEYS_X_H},
     {SB_GAUGE_RED_KEY_Y_L, SB_GAUGE_RED_KEY_Y_H}},
};
...
void CStatusBar::DrawKeys(void) {
    ...
    for (int32_t i = 0; i < 3; i++)
        BitBlt(
            (LOCALPLAYER.flags & keyGaugeInfo[i].nFlag) ? keyGaugeInfo[i].nGaugeOn : keyGaugeInfo[i].nGaugeOff,
            keyGaugeInfo[i].x[bHires],
...
void CStatusBar::DrawWeaponInfo(int32_t nWeaponType, int32_t nWeaponId, int32_t laserLevel) {
    ...
    if (nWeaponType == 0) {
        ...
        CGenericCockpit::DrawWeaponInfo(
            nWeaponType,
            nIndex,
    ...
    int32_t h =
        gameData.pigData.tex
            .bitmaps[0]
                    [gameData.pigData.tex
                         .cockpitBmIndex
                             [CM_STATUS_BAR + (gameStates.video.nDisplayMode ? (gameData.modelData.nCockpits / 2) : 0)]
                         .index]
            .Height();
...
void CStatusBar::SetupWindow(int32_t nWindow) {
    ...
    gameData.renderData.window.Setup(
        ...
        ScaleX(pHudArea->right - pHudArea->left + 1),
        ScaleY(pHudArea->bot - pHudArea->top + 1));
...
void CStatusBar::Toggle(void) {
    CGenericCockpit::Activate(
        (gameStates.render.cockpit.nNextType < 0) ? CM_FULL_SCREEN : gameStates.render.cockpit.nNextType,
        true);
}
