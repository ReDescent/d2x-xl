...
#define IS_TRACK_GOAL(_objP) \
    (((_objP) == gameData.objData.trackGoals[0]) || ((_objP) == gameData.objData.trackGoals[1]))
...
#define INDICATOR_POSITIONS 60
    ...
    if (bMarker || gameOpts->render.cockpit.bRotateMslLockInd) {
        ...
        mRot.m.dir.r.v.coord.x = mRot.m.dir.u.v.coord.y = sinCosInd[nMslLockIndPos[bMarker]].fCos;
        mRot.m.dir.u.v.coord.x = sinCosInd[nMslLockIndPos[bMarker]].fSin;
        mRot.m.dir.r.v.coord.y = -mRot.m.dir.u.v.coord.x;
        mRot.m.dir.r.v.coord.z = mRot.m.dir.u.v.coord.z = mRot.m.dir.f.v.coord.x = mRot.m.dir.f.v.coord.y = 0;
        mRot.m.dir.f.v.coord.z = 1;

        fVerts[0].v.coord.z = fVerts[1].v.coord.z = fVerts[2].v.coord.z = 0;
        rotVerts[0].v.coord.w = rotVerts[1].v.coord.w = rotVerts[2].v.coord.w = 0;
        fVerts[0].v.coord.x = -r2;
        fVerts[1].v.coord.x = +r2;
        fVerts[2].v.coord.x = 0;
        fVerts[0].v.coord.y = fVerts[1].v.coord.y = +r;
        ...
        for (j = 0; j < 4; j++) {
            ...
            if (!j) { // now rotate by 90 degrees
                mRot.m.dir.r.v.coord.x = mRot.m.dir.u.v.coord.y = 0;
                mRot.m.dir.u.v.coord.x = 1;
                mRot.m.dir.r.v.coord.y = -1;
            }
        }
    } else {
        fVerts[0].v.coord.z = fVerts[1].v.coord.z = fVerts[2].v.coord.z = fPos.v.coord.z;
        fVerts[0].v.coord.w = fVerts[1].v.coord.w = fVerts[2].v.coord.w = 1;
        ...
        if (!nTgtInd || ((nTgtInd == 1) && (!EGI_FLAG(bDamageIndicators, 0, 1, 0) || !bHasDmg)) ||
            ((nTgtInd == 2) && !bHasDmg)) {
        ...
        fVerts[2].v.coord.y = fPos.v.coord.y - r + r2;
        OglDrawArrays(GL_TRIANGLES, 0, 3);
        fVerts[0].v.coord.x = fVerts[1].v.coord.x = fPos.v.coord.x + r;
    ...
    if (EGI_FLAG(bTargetIndicators, 0, 1, 0)) {
        ...
        pc = (EGI_FLAG(bMslLockIndicators, 0, 1, 0) && IS_TRACK_GOAL(pObj) &&
              !gameOpts->render.cockpit.bRotateMslLockInd && (extraGameInfo[IsMultiGame].bTargetIndicators != 1))
                 ? reinterpret_cast<CFloatVector3 *>(&trackGoalColor[0])
                 : ObjectFrameColor(pObj, pc);
        PolyObjPos(pObj, &vPos);
        fPos.Assign(vPos);
        transformation.Transform(fPos, fPos, 0);
        r = X2F(pObj->info.xSize);
        ...
        if (extraGameInfo[IsMultiGame].bTargetIndicators == 1) { // square brackets
            ...
            fVerts[0].v.coord.z = fVerts[1].v.coord.z = fVerts[2].v.coord.z = fVerts[3].v.coord.z = fPos.v.coord.z;
            ogl.EnableClientState(GL_VERTEX_ARRAY, GL_TEXTURE0);
