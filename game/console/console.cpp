    ...
    for (loop = 0; loop < Screenlines - 1 && loop < m_LineBuffer - m_ConsoleScrollBack; loop++) {
        if (m_ConsoleScrollBack != 0 && loop == 0)
            for (loop2 = 0; loop2 < (m_VChars / 5) + 1; loop2++) {
                GrString(
                    CON_CHAR_BORDER + (loop2 * 5 * m_canvas.Font()->Width()),
...
CConsole *CConsole::Create(void) { return new CConsole; }
...
void CConsole::Hide(void) { m_Visible = CON_CLOSING; }

//------------------------------------------------------------------------------
/* tells wether the console is visible or not */
int32_t CConsole::IsVisible(void) { return (m_Visible == CON_OPEN) || (m_Visible == CON_OPENING); }
...
void CConsole::SetHideKey(int32_t key) { m_HideKey = key; }

//------------------------------------------------------------------------------
/* Executes the command entered */
void CConsole::Execute(char *command) { m_CmdFunction(command); }

//------------------------------------------------------------------------------

void CConsole::SetExecuteFunction(void(_CDECL_ *CmdFunction)(char *command)) { m_CmdFunction = CmdFunction; }

//------------------------------------------------------------------------------

void CConsole::DefaultCmdFunction(char *command) { cmd_parse(command); }

//------------------------------------------------------------------------------

void CConsole::SetTabCompletion(char *(_CDECL_ *TabFunction)(char *command)) { m_TabFunction = TabFunction; }
...
char *CConsole::DefaultTabFunction(char *command) { return NULL; }
