...
#define TIR_DELTA_X \
    1024 // +/- 2560 (actual value is multiplied x 10 to pass two decimal places of precision)  [data = (input / 10) -
         // 256]
#define TIR_DELTA_Y \
    2048 // +/- 2560 (actual value is multiplied x 10 to pass two decimal places of precision)  [data = (input / 10) -
         // 256]
#define TIR_DELTA_Z \
    4096 // +/- 2560 (actual value is multiplied x 10 to pass two decimal places of precision)  [data = (input / 10) -
         // 256]
// raw object position from imager
#define TIR_SMOOTH_X \
...
static int TIRLoaded(void) { return (size_t)hTIRDll >= HINSTANCE_ERROR; }
...
int WINAPI TIRStop(void) { return TIRLoaded() && StopDataTransmission && (StopDataTransmission() == TIR_OK); }

// ------------------------------------------------------------------------------------------

int WINAPI TIRStart(void) { return TIRLoaded() && StartDataTransmission && (StartDataTransmission() == TIR_OK); }

// ------------------------------------------------------------------------------------------

int WINAPI TIRCenter(void) { return TIRStop() && ReCenter && (ReCenter() == TIR_OK) && TIRStart(); }
