...
CObject *CreateExplosion(
    ...
    if ((nVClip != ANIM_MORPHING_ROBOT) &&
        ...
        (nVClip != ANIM_POWERUP_DISAPPEARANCE) && (nVClip != ANIM_AFTERBURNER_BLOB)) {
    ...
    if ((pParent && (nVClip == ANIM_POWERUP_DISAPPEARANCE)) || (nVClip == ANIM_MORPHING_ROBOT))
        postProcessManager.Add(new CPostEffectShockwave(
            SDL_GetTicks(),
            pExplObj->LifeLeft() / 4,
            pExplObj->info.xSize,
            1,
...
CObject *CreateSplashDamageExplosion(
    CObject *pObj,
    int16_t nSegment,
    CFixVector &position,
    CFixVector &impact,
    ...
    ENTER(0, 0);
    CObject *pExplObj =
        CreateExplosion(pObj, nSegment, position, impact, size, nVClip, maxDamage, maxDistance, maxForce, nParent);
    if (pExplObj) {
        if (!pObj || ((pObj->info.nType == OBJ_PLAYER) && !SPECTATOR(pObj)) || (pObj->info.nType == OBJ_ROBOT) ||
            pObj->IsSplashDamageWeapon())
            postProcessManager.Add(
                new CPostEffectShockwave(SDL_GetTicks(), BLAST_LIFE, pExplObj->info.xSize, 1, pExplObj->Position()));
...
// adjust the impact location in case it is inside the object
    ...
    audio.CreateObjectSound(
        gameStates.app.bD1Mission ? SOUND_STANDARD_EXPLOSION
    ...
    RETVAL(CreateSplashDamageExplosion(
        this,
        info.nSegment,
        vExplPos,
        vImpact,
        ...
        WI_Strength(info.nId, gameStates.app.nDifficultyLevel),
...
CObject *CObject::ExplodeSplashDamage(fix damage, fix distance, fix force) {
    ...
    ENTER(0, 0);
    CObject *pExplObj = CreateSplashDamageExplosion(
        this,
        info.nSegment,
        info.position.vPos,
        info.position.vPos,
        info.xSize,
        (uint8_t)GetExplosionVClip(this, 0),
        damage,
        distance,
        force,
        OBJ_IDX(this));
    if (pExplObj)
        audio.CreateObjectSound(
            gameStates.app.bD1Mission ? SOUND_STANDARD_EXPLOSION : SOUND_BADASS_EXPLOSION_ACTOR,
            SOUNDCLASS_EXPLOSION,
            OBJ_IDX(pExplObj));
    RETVAL(pExplObj)
}
...
inline double VectorVolume(const CFixVector &vMin, const CFixVector &vMax) {
    return fabs(X2F(vMax.v.coord.x - vMin.v.coord.x)) * fabs(X2F(vMax.v.coord.y - vMin.v.coord.y)) *
...
void DrawFireball(CObject *pObj) {
    ...
    if (pObj->info.xLifeLeft > 0)
        DrawVClipObject(
            ...
            0,
            pObj->info.nId,
            (pObj->info.nType == OBJ_WEAPON) ? gameData.weaponData.color + pObj->info.nId : NULL);
    RETURN
}
...
void CObject::DoExplosionSequence(void) {
    ...
    if ((info.xLifeLeft <= cType.explInfo.nSpawnTime) && (cType.explInfo.nDestroyedObj >= 0)) {
        ...
        if ((cType.explInfo.nDestroyedObj < 0) || (cType.explInfo.nDestroyedObj > gameData.objData.nLastObject[0])) {
#if TRACE
        ...
        if ((pDelObj->info.nType == OBJ_ROBOT) && xSplashDamage)
            pExplObj = CreateSplashDamageExplosion(
                NULL,
                pDelObj->info.nSegment,
                *vSpawnPos,
                *vSpawnPos,
                FixMul(pDelObj->info.xSize, EXPLOSION_SCALE),
                nVClip,
                I2X(xSplashDamage),
                I2X(4) * xSplashDamage,
                I2X(35) * xSplashDamage,
                -1);
        else
        ...
        if (!IsMultiGame) { // Multiplayer handled outside of this code!!
            ...
            if ((pDelObj->info.nType == OBJ_ROBOT) && (!pDelObj->info.contains.nCount || gameStates.app.bD2XLevel)) {
                if (pRobotInfo->containsCount &&
                    ((pRobotInfo->containsType != OBJ_ROBOT) || !(pDelObj->info.nFlags & OF_ARMAGEDDON))) {
#if DBG
