...
int32_t CGlareRenderer::Style(void) {
    return (ogl.m_features.bDepthBlending > 0) && gameOpts->render.coronas.bUse && gameOpts->render.coronas.nStyle &&
...
int32_t CGlareRenderer::CalcFaceDimensions(int16_t nSegment, int16_t nSide, fix *w, fix *h, uint16_t *corners) {
    ...
    j = i + 1;
    d1 = VmLinePointDist(
        gameData.segData.vertices[corners[i]],
        gameData.segData.vertices[corners[j]],
        gameData.segData.vertices[corners[(j + 1) % m_nVertices]]);
    if (m_nVertices == 4)
        d2 = VmLinePointDist(
...
void CGlareRenderer::Render(int16_t nSegment, int16_t nSide, float fIntensity, float fSize) {
    ...
    RenderSoftGlare(
        ...
        fIntensity,
        bAdditive,
        !automap.Active() || automap.m_visited[nSegment] || !gameOpts->render.automap.bGrayOut);
#if DBG
    if ((nSegment == nDbgSeg) && ((nDbgSide < 0) || (nSide == nDbgSide)))
        BRP;
#endif
...
float CGlareRenderer::Visibility(int32_t nQuery) {
    ...
    for (;;) {
        glGetQueryObjectiv(
            gameData.renderData.lights.coronaQueries[nQuery - 1],
...
bool CGlareRenderer::ShaderActive(void) { return (hGlareShader >= 0) && (shaderManager.Current() == hGlareShader); }
...
const char *glareVS = "void main (void){\r\n"
