...
class CGlowRenderer {
    ...
    bool
    Begin(int32_t const nType, int32_t const nStrength = 1, bool const bReplace = true, float const brightness = 1.0f);
    int32_t SetViewport(int32_t const nType, CFloatVector3 *pVertex, int32_t nVerts);
    int32_t SetViewport(int32_t const nType, CFloatVector *pVertex, int32_t nVerts);
    int32_t SetViewport(int32_t const nType, CFixVector pos, float radius);
    int32_t SetViewport(int32_t const nType, CFloatVector3 pos, float width, float height, bool bTransformed = false);
    bool Visible(void);
    ...
    void
    Render(int32_t const source, int32_t const direction = -1, float const radius = 1.0f, bool const bClear = false);
    ...
    bool Compatible(
        int32_t const nType,
        int32_t const nStrength = 1,
        bool const bReplace = true,
...
#define GLOW_FLAGS \
    (GLOW_LIGHTNING | GLOW_SHIELDS | GLOW_SPRITES | GLOW_THRUSTERS | GLOW_HEADLIGHT | BLUR_OUTLINE | BLUR_SHADOW)
