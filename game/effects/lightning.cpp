...
#define STYLE \
    (((m_nStyle < 0) || (gameOpts->render.lightning.nStyle < m_nStyle)) ? gameOpts->render.lightning.nStyle : m_nStyle)
...
void RenderTestImage(void) {
    if (gameStates.render.cameras.bActive) {
        ...
        float colors[4][4] = {{1.0, 0.5, 0.0, 0.5}, {0.0, 0.5, 1.0, 0.5}, {1.0, 0.0, 0.5, 0.5}, {0.0, 1.0, 0.5, 0.5}};
...
int32_t CLightning::ComputeChildEnd(
    CFixVector *vPos,
    CFixVector *vEnd,
    CFixVector *vDir,
    CFixVector *vParentDir,
    int32_t nLength) {
...
bool CLightning::Create(char nDepth, int32_t nThread) {
    ...
    if ((extraGameInfo[0].bUseLightning > 1) && !nDepth && m_nChildren) {
        ...
        for (int32_t nNode = 1 + (nOffset ? Rand(nOffset) : 0);
             (nNode < m_nNodes - nOffset - m_bRandom) && (nBranches < m_nChildren);
             nNode++) {
            if (RandDouble() <= nProb) {
                nBranches++;
                ...
                if (nChildNodes > 1) {
                    ...
                    if (n == 0)
                        n = (m_nFrames < 0) ? -1 : 1;
                    if (!m_nodes[nNode].CreateChild(
                            &m_vEnd,
                            &m_vDelta,
                            m_nLife,
                            l,
                            (int32_t)DRound(m_nAmplitude * scale),
                            m_nAngle,
                            nChildNodes,
                            m_nChildren / 5,
                            nDepth + 1,
...
void CLightning::Smoothe(void) {
    ...
    for (i = m_nNodes - 1, j = 0, pfi = m_nodes.Buffer(), plh = NULL; j < i; j++) {
        ...
        plh = pfi++;
        if (j) {
            plh->m_vNewPos.v.coord.x =
                pfj->m_vNewPos.v.coord.x / 4 + plh->m_vNewPos.v.coord.x / 2 + pfi->m_vNewPos.v.coord.x / 4;
            plh->m_vNewPos.v.coord.y =
                pfj->m_vNewPos.v.coord.y / 4 + plh->m_vNewPos.v.coord.y / 2 + pfi->m_vNewPos.v.coord.y / 4;
            plh->m_vNewPos.v.coord.z =
                pfj->m_vNewPos.v.coord.z / 4 + plh->m_vNewPos.v.coord.z / 2 + pfi->m_vNewPos.v.coord.z / 4;
        }
    }
}
    ...
    if ((nDepth > 1) || m_bRandom) {
        if (nStyle >= 2) {
            ...
            for (i = m_nNodes - 1, plh = m_nodes + 1; i > 0; i--, plh++, bPrevOffs[0] = 1)
                *vPrevOffs = plh->CreateJaggy(
                    vPos,
                    vPos + 1,
                    vBase,
                    bPrevOffs[0] ? vPrevOffs : NULL,
                    nFrames,
                    nAmplitude,
                    nMinDist,
                    i,
        ...
        if (nStyle >= 2) {
            ...
            for (i = m_nNodes - 1, j = 0, pNode[0] = m_nodes + 1, pNode[1] = &m_nodes[i - 1]; i > 0; i--, j = !j) {
                plh = pNode[j];
                vPrevOffs[j] = plh->CreateJaggy(
                    vPos + j,
                    vPos + !j,
                    vBase,
                    bPrevOffs[j] ? vPrevOffs + j : NULL,
                    nFrames,
                    nAmplitude,
                    0,
                    i,
...
void CLightning::Move(CFixVector vNewPos, CFixVector vNewEnd, int16_t nSegment) {
    ...
    m_nodes[0].m_vPos = m_vPos = m_vBase = vNewPos;
    m_nodes[m_nNodes - 1].m_vPos = m_vEnd = vNewEnd;
...
void CLightning::ComputeGlow(int32_t nDepth, int32_t nThread) {
    ...
    CFloatVector *pSrc, *pDest, vEye, vn, vd, vPos[2] = {CFloatVector::ZERO, CFloatVector::ZERO};
...
RETURN
    ...
    if (ogl.EnableClientStates(0, 0, 0, GL_TEXTURE0)) {
        ...
        if (m_bGlow) {
            ...
            float fDist =
                Min(CFloatVector3::Dist(m_coreVerts[0], vViewer),
                    CFloatVector3::Dist(m_coreVerts[m_nNodes - m_bRandom - 1], vViewer));
