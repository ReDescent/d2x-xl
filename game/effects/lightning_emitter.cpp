...
void CLightningEmitter::CreateSound(int32_t bSound, int32_t nThread) {
    ...
    if ((m_bSound = bSound)) {
        audio.CreateObjectSound(
            ...
            -1,
            -1,
            AddonSoundName(SND_ADDON_LIGHTNING),
            0,
            0,
            nThread);
        if (m_bForcefield) {
...
void CLightningEmitter::Animate(int32_t nStart, int32_t nBolts, int32_t nThread) {
    ...
    if (pObj && ((pObj->Type() == OBJ_ROBOT) || (pObj->Type() == OBJ_POWERUP)) &&
        (pObj->Frame() != gameData.appData.nFrameCount - 1))
        RETURN
    if (nBolts < 0)
        nBolts = m_nBolts;
...
void CLightningEmitter::Render(int32_t nStart, int32_t nBolts, int32_t nThread) {
    ...
    CObject *pObj = OBJECT(m_nObject);
    if (pObj && ((pObj->Type() == OBJ_ROBOT) || (pObj->Type() == OBJ_POWERUP)) &&
        (pObj->Frame() != gameData.appData.nFrameCount))
        RETURN
