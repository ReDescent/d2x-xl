...
CLightningManager::CLightningManager() {
    ...
    m_lights = NULL;
    m_nFirstLight = -1;
    robotLightningInfo.color
        .Set((uint8_t)(255 * 0.9f), (uint8_t)(255 * 0.6f), (uint8_t)(255 * 0.6f), (uint8_t)(255 * 0.3f)); // color;
}

//------------------------------------------------------------------------------

CLightningManager::~CLightningManager() { Shutdown(true); }
...
int32_t CLightningManager::Create(
    int32_t nBolts,
    CFixVector *vPos,
    CFixVector *vEnd,
    CFixVector *vDelta,
    ...
    if (!pEmitter)
        RETVAL(-1)
    // for random lightning, add a few extra nodes. These won't get rendered, but will cause the end of the lightning
    // bolts to dance around a bit, making them look more natural
    if (!(pEmitter->Create(
            nBolts,
            vPos,
            vEnd,
...
int32_t
CLightningManager::Create(tLightningInfo &li, CFixVector *vPos, CFixVector *vEnd, CFixVector *vDelta, int16_t nObject) {
    ENTER(0, 0);
    CFloatVector color;
...
#pragma omp for private(pEmitter)
...
#endif
        {
            for (CLightningEmitter *pEmitter = m_emitters.GetFirst(nCurrent), *pNext = NULL; pEmitter;
                 pEmitter = pNext) {
                pNext = m_emitters.GetNext(nCurrent);
        ...
        FORALL_OBJS(pObj) {
            ...
            gameData.objData.bWantEffect[i] &=
                ~(PLAYER_LIGHTNING | ROBOT_LIGHTNING | MISSILE_LIGHTNING | EXPL_LIGHTNING | MOVE_LIGHTNING);
        }
    }
    RETURN
}
...
#endif
        {
            for (CLightningEmitter *pEmitter = m_emitters.GetFirst(nCurrent); pEmitter;
                 pEmitter = m_emitters.GetNext(nCurrent))
                if (!(pEmitter->m_nKey[0] | pEmitter->m_nKey[1]))
...
void CLightningManager::ResetLights(int32_t bForce) {
    ...
    if ((SHOW_LIGHTNING(1) || bForce) && m_lights.Buffer()) {
        ...
        for (i = m_nFirstLight; i >= 0;) {
            ...
            pLight->nNext = -1;
            pLight->vPos.SetZero();
            pLight->color.Red() = pLight->color.Green() = pLight->color.Blue() = 0;
...
#pragma omp parallel for private(pEmitter) reduction(+ : nLights)
        ...
        for (i = m_nFirstLight; i >= 0; i = pLight->nNext) {
            ...
            if (bDynLighting) {
                pLight->nDynLight =
                    lightManager
                        .Add(NULL, &pLight->color, pLight->nBrightness, pLight->nSegment, -1, -1, -1, &pLight->vPos);
                nLights++;
            }
        }
    }
...
void CLightningManager::CreateForExplosion(
    CObject *pObj,
    CFloatVector *pColor,
    int32_t nRods,
    int32_t nRad,
...
void CLightningManager::CreateForTeleport(CObject *pObj, CFloatVector *pColor, float fRodScale) {
    ...
    if (SHOW_LIGHTNING(0) /*&& gameOpts->render.lightning.bExplosions*/) {
        ...
        Create(
            (int32_t)FRound((h + Rand(h)) * fRodScale),
...
void CLightningManager::CreateForRobot(CObject *pObj, CFloatVector *pColor) {
    ...
    if (SHOW_LIGHTNING(1) && gameOpts->render.lightning.bRobots && OBJECT_EXISTS(pObj)) {
        ...
        if (0 <= m_objects[nObject])
            ...
            robotLightningInfo.color.Set(
                uint8_t(255 * pColor->v.color.r),
                uint8_t(255 * pColor->v.color.g),
                uint8_t(255 * pColor->v.color.b));
            int32_t h = Create(
                robotLightningInfo,
                &pObj->Position(),
                &OBJPOS(OBJECT(LOCALPLAYER.nObject))->vPos,
                NULL,
                nObject);
            if (h >= 0)
                m_objects[nObject] = h;
...
int32_t CLightningManager::RenderForDamage(
    CObject *pObj,
    CRenderPoint **pointList,
    RenderModel::CVertex *pVertex,
    int32_t nVertices) {
    ENTER(0, 0);
    CLightningEmitter *pEmitter;
