...
bool CLightningNode::CreateChild(
    CFixVector *vEnd,
    CFixVector *vDelta,
    int32_t nLife,
    int32_t nLength,
...
int32_t CLightningNode::ComputeAttractor(
    CFixVector *vAttract,
    CFixVector *vDest,
    CFixVector *vPos,
    int32_t nMinDist,
...
CFixVector *CLightningNode::Smoothe(CFixVector *vOffs, CFixVector *vPrevOffs, int32_t nDist, int32_t nSmoothe) {
    if (nSmoothe) {
        ...
        if (nSmoothe > 0)
            *vOffs *= FixDiv(
                nSmoothe * nDist,
                nMag); // scale offset vector with distance to attractor (the closer, the smaller)
        else
            *vOffs *= FixDiv(
                nDist,
                nSmoothe * nMag); // scale offset vector with distance to attractor (the closer, the smaller)
        nMag = vPrevOffs->Mag();
        *vOffs += *vPrevOffs;
        nMag = vOffs->Mag();
...
CFixVector *CLightningNode::Attract(
    CFixVector *vOffs,
    CFixVector *vAttract,
    CFixVector *vPos,
    int32_t nDist,
...
CFixVector CLightningNode::CreateJaggy(
    CFixVector *vPos,
    CFixVector *vDest,
    CFixVector *vBase,
...
CFixVector CLightningNode::CreateErratic(
    CFixVector *vPos,
    CFixVector *vBase,
    int32_t nSteps,
    int32_t nAmplitude,
