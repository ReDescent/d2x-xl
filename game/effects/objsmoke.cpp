...
static CFloatVector smokeColors[] = {
    {{{0.5f, 0.5f, 0.5f, 2.0f}}}, // alpha == 2.0 means that the particles simulate a flame in the beginning (additive
                                  // orange color effect)
...
void KillPlayerSmoke(int32_t i) { KillObjectSmoke(PLAYER(i).nObject); }
...
void InitObjectSmoke(void) { particleManager.InitObjects(); }
        ...
        if (bHires >= 0) {
            ...
            if (pm->m_bBullets) {
                ...
                if (bDoEffect) {
                    ...
                    vDir.Neg();
                    if (i < 0) {
                        gameData.multiplayer.bulletEmitters[nPlayer] = particleManager.Create(
                            &vEmitter,
                            &vDir,
                            &pPos->mOrient,
                            pObj->info.nSegment,
        ...
        if (pm->m_bBullets) {
            ...
            if (bDoEffect) {
                ...
                if (gameData.multiplayer.gatlingSmoke[nPlayer] < 0) {
                    gameData.multiplayer.gatlingSmoke[nPlayer] = particleManager.Create(
                        &vEmitter,
                        &vDir,
                        &pPos->mOrient,
                        pObj->info.nSegment,
...
void DoPlayerSmoke(CObject *pObj, int32_t nPlayer) {
    ...
    if ((PLAYER(nPlayer).flags & PLAYER_FLAGS_CLOAKED) || (automap.Active() && IsMultiGame && !AM_SHOW_PLAYERS)) {
    ...
    if (IsNetworkGame && !PLAYER(nPlayer).connected)
        ...
        if (nParts && ((nType < 2) || pvDir)) {
            ...
            if (0 > (nSmoke = particleManager.GetObjectSystem(nObject))) {
                ...
                particleManager.SetDensity(
                    nSmoke,
                    nParts,
                    1 /*, gameOpts->render.particles.bSyncSizes ? -1 : gameOpts->render.particles.nSize [1]*/);
                particleManager.SetSpeed(
                    particleManager.GetObjectSystem(nObject),
    ...
    if (nParts > 0) {
        ...
        if (0 > (nSmoke = particleManager.GetObjectSystem(nObject))) {
            nSmoke = particleManager.Create(
                ...
                gameOpts->render.particles.nSize[3] * BOT_PART_LIFE / 2,
                BOT_PART_SPEED,
                SMOKE_PARTICLES,
                nObject,
                smokeColors,
            ...
            particleManager.SetDensity(
                nSmoke,
                nParts,
                2 /*, gameOpts->render.particles.bSyncSizes ? -1 : gameOpts->render.particles.nSize [2]*/);
            particleManager.SetSpeed(
                nSmoke,
                !pObj->mType.physInfo.velocity.IsZero() ? BOT_PART_SPEED : BOT_PART_SPEED * 2 / 3);
        }
    ...
    if (nParts > 0) {
        ...
        if (0 > (nSmoke = particleManager.GetObjectSystem(nObject))) {
            nSmoke = particleManager.Create(
                ...
                F2X(-4.0),
                /*-1, 1,*/
                gameOpts->render.particles.nSize[3] * BOT_PART_LIFE,
                BOT_PART_SPEED,
                SMOKE_PARTICLES,
                nObject,
                smokeColors,
...
void DoMissileSmoke(CObject *pObj) {
    ...
    if ((pObj->info.xShield < 0) || (pObj->info.nFlags & (OF_SHOULD_BE_DEAD | OF_DESTROYED)))
        ...
        nLife = float(gameOpts->render.particles.nLife[3]) *
                sqrt(float(nSpeed) / float(WI_Speed(CONCUSSION_ID, gameStates.app.nDifficultyLevel)));
...
void DoDebrisSmoke(CObject *pObj) {
    ...
    if ((pObj->info.xShield < 0) || (pObj->info.nFlags & (OF_SHOULD_BE_DEAD | OF_DESTROYED)) ||
        (gameOpts->render.nDebrisLife &&
         (I2X(nDebrisLife[gameOpts->render.nDebrisLife]) - pObj->info.xLifeLeft > I2X(10))))
        nParts = 0;
    else
    ...
    if (nParts) {
        ...
        if (0 > (nSmoke = particleManager.GetObjectSystem(nObject))) {
            nSmoke = particleManager.Create(
                ...
                nScale,
                /*-1, 1,*/
                gameOpts->render.particles.nSize[4] * DEBRIS_PART_LIFE / 2,
                DEBRIS_PART_SPEED,
                SMOKE_PARTICLES,
                nObject,
                smokeColors,
...
void DoStaticParticles(CObject *pObj) {
#if STATIC_SMOKE
    int32_t i, j, nObject, nSmoke, nType, nFadeType;
    CFixVector pos, offs, dir;

    static CFloatVector defaultColors[7] = {
        {{{0.5f, 0.5f, 0.5f, 0.0f}}},
        {{{0.8f, 0.9f, 1.0f, 1.0f}}},
        {{{1.0f, 1.0f, 1.0f, 1.0f}}},
        {{{1.0f, 1.0f, 1.0f, 1.0f}}},
    ...
    static int32_t particleTypes[7] = {
        SMOKE_PARTICLES,
        BUBBLE_PARTICLES,
        FIRE_PARTICLES,
        WATERFALL_PARTICLES,
        SIMPLE_SMOKE_PARTICLES,
        RAIN_PARTICLES,
        SNOW_PARTICLES};
    ...
    if (pObj->rType.particleInfo.nType == SMOKE_TYPE_WATERFALL) {
        ...
        (pObj->rType.particleInfo.nType == SMOKE_TYPE_RAIN) || (pObj->rType.particleInfo.nType == SMOKE_TYPE_SNOW)) {
    ...
    if (!(SHOW_SMOKE && pObj->rType.particleInfo.bEnabled &&
          ((nType == 1) ? gameOpts->render.particles.bBubbles : gameOpts->render.particles.bStatic))) {
        if (particleManager.GetObjectSystem(nObject) >= 0)
            KillObjectSmoke(nObject);
        return;
    }
    if (0 > (nSmoke = particleManager.GetObjectSystem(nObject))) {
        ...
        nSmoke = particleManager.Create(
            ...
            -pObj->rType.particleInfo.nParts /
                ((nType == SMOKE_TYPE_RAIN) ? 10 : ((nType == SMOKE_TYPE_FIRE) || (nType == SMOKE_TYPE_SNOW)) ? 10 : 1),
            -PARTICLE_SIZE(
                pObj->rType.particleInfo.nSize[gameOpts->render.particles.bDisperse],
                (nType == 1) ? 4.0f : 2.0f,
                1),
            /*-1, 3,*/
            (nType == 2) ? FIRE_PART_LIFE * int32_t(sqrt(double(pObj->rType.particleInfo.nLife)))
                         : STATIC_SMOKE_PART_LIFE * pObj->rType.particleInfo.nLife,
            (nType == SMOKE_TYPE_RAIN) ? (int32_t)sqrt(float(pObj->rType.particleInfo.nDrift)) /** 64*/
                                       : pObj->rType.particleInfo.nDrift,
            particleTypes[nType],
            nObject,
            bColor ? &color : defaultColors + nType,
            nType != 2,
            pObj->rType.particleInfo.nSide - 1);
        if (nSmoke < 0)
            return;
        particleManager.SetObjectSystem(nObject, nSmoke);
        if (nType == 1)
            audio.CreateObjectSound(
                ...
                -1,
                -1,
                AddonSoundName(SND_ADDON_AIRBUBBLES));
        else
...
void DoParticleTrail(CObject *pObj) {
#if PARTICLE_TRAIL
    int32_t nParts, nObject, nSmoke, id = pObj->info.nId, bGatling = pObj->IsGatlingRound(), bOmega = (id == OMEGA_ID);
    float nScale;
    CFixVector pos;
    CFloatVector c;

    if (!(SHOW_OBJ_FX && (bGatling ? EGI_FLAG(bUseParticles, 0, 0, 0) && EGI_FLAG(bGatlingTrails, 1, 1, 0)
                                   : EGI_FLAG(bLightTrails, 1, 1, 0))))
        return;
    nObject = pObj->Index();
    ...
    if (0 > (nSmoke = particleManager.GetObjectSystem(nObject))) {
        if (bGatling) {
            ...
            if (((id >= LASER_ID) && (id <= LASER_ID + 3)) || (id == SUPERLASER_ID) || (id == SUPERLASER_ID + 1) ||
                (id == ROBOT_BLUE_LASER_ID) || (id == ROBOT_GREEN_LASER_ID) || (id == ROBOT_RED_LASER_ID) ||
                (id == ROBOT_WHITE_LASER_ID))
                nScale = 3;
        ...
        nSmoke = particleManager.Create(
            ...
            pObj->info.nSegment,
            1,
            bGatling ? nParts * 4 : nParts,
            -PARTICLE_SIZE(1, nScale, 1),
            /*gameOpts->render.particles.nSize [3], 1,*/
            (gameOpts->render.particles.nLife[3] + 1) *
                (bGatling ? 3 * LASER_PART_LIFE / 2 : LASER_PART_LIFE >> bOmega) /*<< bGatling*/,
            LASER_PART_SPEED,
            bGatling ? GATLING_PARTICLES : LIGHT_PARTICLES,
            nObject,
            &c,
            0,
            -1);
...
int32_t DoObjectSmoke(CObject *pObj) {
    ...
    if (t == OBJ_PLAYER)
        ...
        if (pObj->IsMissile())
            ...
            gameOpts->render.particles.bPlasmaTrails && gameStates.app.bHaveExtraGameInfo[IsMultiGame] &&
            EGI_FLAG(bLightTrails, 0, 0, 0) && pObj->HasLightTrail(nId))
            DoParticleTrail(pObj);
        else
            return 0;
