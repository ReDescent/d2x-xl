...
inline int32_t CParticleEmitter::MayBeVisible(int32_t nThread) {
    return (m_nSegment < 0) || (SEGMENT(m_nSegment)->m_function == SEGMENT_FUNC_SKYBOX) ||
           SegmentMayBeVisible(m_nSegment, 5, -1, nThread);
}
...
float CParticleEmitter::Brightness(void) {
    ...
    if (!pObj || (pObj->info.nType != m_nObjType) ||
        (pObj->info.nFlags & (OF_EXPLODING | OF_SHOULD_BE_DEAD | OF_DESTROYED | OF_ARMAGEDDON)))
        return m_fBrightness;
...
int32_t CParticleEmitter::Create(
    CFixVector *vPos,
    CFixVector *vDir,
    CFixMatrix *mOrient,
    int16_t nSegment,
    ...
    m_nParts = 0;
    m_nMoved = nCurTime;
    m_nPartLimit = m_nMaxParts = nMaxParts;
    m_nFirstPart = 0;
    m_fScale = fScale;
    // m_nDensity = nDensity;
        ...
        if ((m_nPartsPerPos = (int32_t)(m_fPartsPerTick * m_nTicks)) >= 1) {
            ...
            if (IsAlive(nCurTime)) {
                ...
                for (i = 0; i < h; i++) {
                    ...
                    if (m_particles[(j + i) % m_nPartLimit].Create(
                            &vPos,
                            vDir,
                            &mOrient,
                            m_nSegment,
...
int32_t CParticleEmitter::Render(int32_t nThread) {
...
#endif
    {
        ...
        for (h = 0, i = m_nParts, j = m_nFirstPart; i; i--, j = (j + 1) % m_nPartLimit)
            if ((bVisible || m_particles[j].IsVisible(nThread)) &&
                transparencyRenderer.AddParticle(m_particles + j, fBrightness, nThread))
                h++;
...
void CParticleEmitter::SetBrightness(int32_t nBrightness) { m_nDefBrightness = nBrightness; }

//------------------------------------------------------------------------------

void CParticleEmitter::SetFadeType(int32_t nFadeType) { m_nFadeType = nFadeType; }

//------------------------------------------------------------------------------

void CParticleEmitter::SetBlowUp(int32_t bBlowUpParts) { m_bBlowUpParts = bBlowUpParts; }

//------------------------------------------------------------------------------

void CParticleEmitter::SetSpeed(int32_t nSpeed) { m_nSpeed = nSpeed; }

//------------------------------------------------------------------------------

void CParticleEmitter::SetType(int32_t nType) { m_nType = nType; }
...
void CParticleEmitter::SetScale(float fScale) { m_fScale = fScale; }
