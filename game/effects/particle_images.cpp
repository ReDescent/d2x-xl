...
int32_t CParticleImageManager::LoadAll(void) {
    ...
    SetupMultipleTextures(
        ParticleImageInfo(SPARK_PARTICLES).pBm,
        ParticleImageInfo(SMOKE_PARTICLES).pBm,
        ParticleImageInfo(BUBBLE_PARTICLES).pBm);
    return 1;
}
...
bool CParticleImageManager::SetupMultipleTextures(CBitmap *bmP1, CBitmap *bmP2, CBitmap *bmP3) {
    ...
    if ((bmP2->Width() != nWidth) || (bmP2->Height() != nHeight) || (bmP3->Width() != nWidth) ||
        (bmP3->Height() != nHeight))
        return false;
