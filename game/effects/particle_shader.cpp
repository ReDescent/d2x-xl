...
bool CParticleManager::LoadShader(int32_t nShader, vec3 &dMax) {
    ...
    if ((nShader > 1) && !ogl.CopyDepthTexture(0, (nShader & 1) ? GL_TEXTURE2 : GL_TEXTURE3))
        nShader -= 2;
    // ogl.DrawBuffer ()->FlipBuffers (0, 1); // color buffer 1 becomes render target, color buffer 0 becomes render
    // source (scene texture) ogl.DrawBuffer ()->SelectColorBuffers ();
...
const char *particleVS = "void main (void){\r\n"
...
void CParticleManager::InitShader(void) {
    if (ogl.m_features.bRenderToTexture.Available() && ogl.m_features.bShaders) {
        ...
        for (i = 0; i < j; i++)
            if (((i & 1) == 0) &&
                !shaderManager.Build(hParticleShader[i], particleFS[i], particleVS)) // skip the texture array shaders
                break;
        if (i == 4)
