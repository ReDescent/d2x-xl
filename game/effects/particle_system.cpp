...
int32_t CParticleSystem::Create(
    CFixVector *vPos,
    CFixVector *vDir,
    CFixMatrix *mOrient,
    int16_t nSegment,
        ...
        if (m_nObject < 0x70000000) {
            ...
            if (!pObj)
                ...
                if ((pObj == LOCALOBJECT) && !gameStates.render.bChaseCam && !gameStates.render.bFreeCam)
                    return 0;
                if ((pObj->Type() == OBJ_NONE) || (pObj->Signature() != m_nSignature) ||
                    ((particleManager.GetObjectSystem(m_nObject, 0) < 0) &&
                     (particleManager.GetObjectSystem(m_nObject, 1) < 0)))
                    SetLife(0);
            }
        }
...
int32_t CParticleSystem::Update(int32_t nThread) {
    ...
    if ((pEmitter = m_emitters.Buffer()) && emitters.Create(m_nEmitters)) {
        CObject *pObj;
        bool bKill =
            (m_nObject < 0x70000000) &&
            (!(pObj = OBJECT(m_nObject)) || (pObj->info.nSignature != m_nSignature) || (pObj->info.nType == OBJ_NONE));
