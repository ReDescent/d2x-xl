...
static inline int32_t RandN(int32_t n) { return n ? int32_t(RandFloat() * float(n)) : 0; }
...
#if 1
    return sqrt(
               (sqr(Min(color.v.color.r, 1.0f) * 255.0f) + sqr(Min(color.v.color.g, 1.0f) * 255.0f) +
                sqr(Min(color.v.color.b, 1.0f) * 255.0f)) /
               3.0f) /
           255.0f;
#else
    return pow((sqr(Min(color.v.color.r, 1.0f) * 255.0f, 3.0f) + sqr(Min(color.v.color.g, 1.0f) * 255.0f, 3.0f) +
                sqr(Min(color.v.color.b, 1.0f) * 255.0f, 3.0f)) /
                   3.0f,
               1.0f / 3.0f) /
           255.0f;
#endif
...
static inline float SmokeStartAlpha(char bBlowUp, char nClass) {
    int32_t alpha = smokeStartAlpha[int32_t(bBlowUp)]
                                   [gameOpts->render.particles.nAlpha[gameOpts->app.bExpertMode ? int32_t(nClass) : 0]];
...
void CParticle::InitColor(CFloatVector *pColor, float fBrightness, char nParticleSystemType) {
    ...
    m_bEmissive = (nParticleSystemType == LIGHT_PARTICLES)
                      ...
                      : (nParticleSystemType == FIRE_PARTICLES) ? 2
                                                                //: ((nParticleSystemType <= SMOKE_PARTICLES) ||
                                                                //(nParticleSystemType <= WATERFALL_PARTICLES)) 	? 3
                                                                : 0;
    m_color[0] = m_color[1] = color =
        CFloatVector::Min((pColor && (m_bEmissive != 2)) ? *pColor : defaultParticleColor, 1.0f);
...
#if 1
                    m_color[0].Red() = m_color[0].Green() = 1.0;
#else
    ...
    if (nParticleSystemType == SIMPLE_SMOKE_PARTICLES)
        m_color[1].Alpha() /=
            3.5f - float(1 + int32_t(gameOpts->render.particles.nQuality > 1)) /
                       2.0f; // pColor ? 2.0f + (color.Red () + color.Green () + color.Blue ()) / 3.0f : 2.0f;
        ...
        (nParticleSystemType == BUBBLE_PARTICLES) || (nParticleSystemType == RAIN_PARTICLES) ||
...
bool CParticle::InitPosition(CFixVector *vPos, CFixVector *vEmittingFace, CFixMatrix *mOrient, bool bPointSource) {
    if (vEmittingFace)
        ...
        CFixVector v = CFixVector::Avg(
            (*mOrient).m.dir.r * (nSpeed - RandN(2 * nSpeed)),
            (*mOrient).m.dir.u * (nSpeed - RandN(2 * nSpeed)));
...
void CParticle::InitSize(float nScale, CFixMatrix *mOrient) {
    ...
    if (m_bBlowUp) {
        m_nWidth = (m_nType == WATERFALL_PARTICLES) ? m_nRad * 0.6666667f : m_nRad;
        m_nHeight = m_nRad;
    } else {
        m_nWidth = (m_nType == WATERFALL_PARTICLES) ? m_nRad * 0.3333333f : m_nRad /* * 2*/;
        m_nHeight = m_nRad /** 2*/;
    }
    m_nWidth /= 65536.0f;
    m_nHeight /= 65536.0f;
...
void CParticle::InitAnimation(void) {
    ...
    m_bRotate = ((m_nRenderType <= SMOKE_PARTICLES) || (m_nRenderType == SNOW_PARTICLES))
                    ? 1
                    : (m_nRenderType == FIRE_PARTICLES + PARTICLE_TYPES) ? -1 : 0;
}
...
void CParticle::UpdateTexCoord(void) {
    if ((m_nType <= WATERFALL_PARTICLES) &&
        ((m_nType != BUBBLE_PARTICLES) || gameOpts->render.particles.bWobbleBubbles)) {
...
int32_t CParticle::Bounce(int32_t nThread) {
    ...
    if (!CollideWithWall(nThread)) {
        m_nBounce = ((m_nType == BUBBLE_PARTICLES) || (m_nType == RAIN_PARTICLES) || (m_nType == SNOW_PARTICLES) ||
...
#else
        nSegment = FindSegByPos(
            m_vPos,
            m_nSegment,
            1,
            -1,
            ((m_nType == BUBBLE_PARTICLES) || (m_nType == RAIN_PARTICLES) || (m_nType == SNOW_PARTICLES)) ? 0
                                                                                                          : fix(m_nRad),
            nThread);
#endif
        if (nSegment < 0)
...
int32_t CParticle::Update(int32_t nCurTime, float fBrightness, int32_t nThread) {
    ...
    m_nLife -= t;
    m_decay =
        ((m_nType == BUBBLE_PARTICLES) || (m_nType == WATERFALL_PARTICLES)) ? 1.0f : float(m_nLife) / float(m_nTTL);
#else
...
void CParticle::Setup(
    ...
    if ((m_nType <= SMOKE_PARTICLES) && m_bBlowUp) {
        ...
        float fFade =
            (m_nFadeType == 3)
                ? 1.0f
                : (m_decay >
                   0.9f) // start from zero size by scaling with pow (m_decay, 44f) which is < 0.01 for m_decay == 0.9f
    ...
    pb[m_nOrient].texCoord.v.u = pb[(m_nOrient + 3) % 4].texCoord.v.u = m_texCoord.v.u + h;
    pb[(m_nOrient + 1) % 4].texCoord.v.u = pb[(m_nOrient + 2) % 4].texCoord.v.u = m_texCoord.v.u + m_deltaUV - h;
    h = ParticleImageInfo(m_nType).yBorder;
    pb[m_nOrient].texCoord.v.v = pb[(m_nOrient + 1) % 4].texCoord.v.v = m_texCoord.v.v + h;
    pb[(m_nOrient + 2) % 4].texCoord.v.v = pb[(m_nOrient + 3) % 4].texCoord.v.v = m_texCoord.v.v + m_deltaUV - h;
    ...
    if (m_bRotate && gameOpts->render.particles.bRotate) {
        ...
        pb[0].vertex.v.coord.x = pb[3].vertex.v.coord.x = vCenter.v.coord.x - vOffset.v.coord.x;
        pb[1].vertex.v.coord.x = pb[2].vertex.v.coord.x = vCenter.v.coord.x + vOffset.v.coord.x;
        pb[0].vertex.v.coord.y = pb[1].vertex.v.coord.y = vCenter.v.coord.y + vOffset.v.coord.y;
        pb[2].vertex.v.coord.y = pb[3].vertex.v.coord.y = vCenter.v.coord.y - vOffset.v.coord.y;
    }
    pb[0].vertex.v.coord.z = pb[1].vertex.v.coord.z = pb[2].vertex.v.coord.z = pb[3].vertex.v.coord.z =
        vCenter.v.coord.z;
}
...
vCenter += fVec;
    ...
    if (m_bEmissive < 0) {
        ...
        pb[0].texCoord.v.u = pb[1].texCoord.v.u = nCol + 1.0f / 64.0f;
        pb[1].texCoord.v.v = pb[2].texCoord.v.v = nRow;
        pb[2].texCoord.v.u = pb[3].texCoord.v.u = nCol + 7.0f / 64.0f;
        pb[0].texCoord.v.v = pb[3].texCoord.v.v = nRow + 1 / 8.0f;
        ...
        pb[int32_t(m_nOrient + 1) % 4].texCoord.v.u = pb[(m_nOrient + 2) % 4].texCoord.v.u =
        ...
        pb[int32_t(m_nOrient + 2) % 4].texCoord.v.v = pb[(m_nOrient + 3) % 4].texCoord.v.v =
            m_texCoord.v.v + m_deltaUV - hy;
        pb[0].texCoord.v.l = pb[1].texCoord.v.l = pb[2].texCoord.v.l = pb[3].texCoord.v.l =
            (((m_nType == SMOKE_PARTICLES) || (m_nType == WATERFALL_PARTICLES)) ? 1.0f : 2.0f);
    }
