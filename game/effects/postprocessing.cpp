...
const char *shockwaveVS = "void main(void) {\r\n"
...
float CPostEffectShockwave::Life(void) { return float(m_nLife) * gameStates.gameplay.slowmo[0].fSpeed; }
...
bool CPostEffectShockwave::Update(void) {
    ...
    p[1].v.coord.x = p[4].v.coord.x = p[0].v.coord.x - size;
    p[1].v.coord.y = p[2].v.coord.y = p[0].v.coord.y - size;
    p[2].v.coord.x = p[3].v.coord.x = p[0].v.coord.x + size;
    p[3].v.coord.y = p[4].v.coord.y = p[0].v.coord.y + size;
    p[1].v.coord.z = p[2].v.coord.z = p[3].v.coord.z = p[4].v.coord.z = p[0].v.coord.z;
...
#if SHOCKWAVE_STYLE
    glLightf(
        GL_LIGHT0 + m_nShockwaves,
        GL_QUADRATIC_ATTENUATION,
        (float)pow(0.5f - (float)cos(2.0 * PI * (1.0f - m_ttl)) * 0.5f, 0.25f) * m_nBias);
#else
...
static inline bool Available(void) {
    return ogl.m_features.bShaders && ogl.m_features.bRenderToTexture && (gameOpts->render.nQuality > 1);
}
...
const char *fogFS = "uniform sampler2D fogTex, depthTex;\r\n"
                    "uniform vec2 windowScale;\r\n"
                    "uniform vec4 fogColor1, fogColor2;\r\n"
                    "#define ZNEAR 1.0\r\n"
                    "#define ZFAR 5000.0\r\n"
...
const char *fogVS = "void main (void){\r\n"
                    "gl_TexCoord [0] = gl_MultiTexCoord0;\r\n"
