...
class CPostEffect {
    ...
    public:
    CPostEffect(int32_t nType = 0) : m_prev(0), m_next(0), m_nType(nType), m_bValid(false) {}
    ...
    inline void Link(CPostEffect *prev, CPostEffect *next) { m_prev = prev, m_next = next; }
...
class CPostEffectShockwave : public CPostEffect {
    ...
    public:
    CPostEffectShockwave(
        int32_t nStart = 0,
        int32_t nLife = 0,
        int32_t nSize = 0,
        int32_t nBias = 1,
        CFixVector pos = CFixVector::ZERO,
        int32_t nObject = -1)
        : CPostEffect(PP_EFFECT_SHOCKWAVE),
          m_nStart(nStart),
          m_nLife(int32_t(1000 * X2F(nLife))),
          m_nSize(nSize),
          m_nBias(nBias),
          m_pos(pos),
          m_nObject(nObject),
