...
void CShrapnel::Create(CObject *pParentObj, CObject *pObj, float fScale) {
    ...
    m_info.nSmoke = particleManager.Create(
        ...
        pObj->info.nSegment,
        1,
        int32_t(float(-SHRAPNEL_MAX_PARTS) / fSpeedScale) * Max(gameOpts->render.particles.nQuality, 2),
...
int32_t CShrapnelCloud::Create(CObject *pParentObj, CObject *pObj) {
    ...
    int32_t i, h = (int32_t)FRound(fScale * fShrapnelScale[gameOpts->render.effects.nShrapnels]);
...
void CShrapnelManager::Reset(void) { CArray<CShrapnelCloud>::Destroy(); }
...
void CShrapnelManager::Draw(CObject *pObj) { m_data.buffer[pObj->Index()].Draw(); }
