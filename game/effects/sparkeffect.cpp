    ...
    if (m_tRender) {
        if (m_nFrame > 31)
            ...
            transparencyRenderer
                .AddSpark(m_vPos, (char)m_nType, m_xSize, (char)m_nFrame, (char)m_nRotFrame, (char)m_nOrient);
    }
}
...
inline CSparks &CSparkManager::Sparks(int16_t nObjProducer) { return m_sparks[nObjProducer]; }
...
void CSparkManager::UpdateSparks(int16_t nObjProducer) { m_sparks[nObjProducer].Update(); }

//-----------------------------------------------------------------------------

void CSparkManager::RenderSparks(int16_t nObjProducer) { m_sparks[nObjProducer].Render(); }

//-----------------------------------------------------------------------------

void CSparkManager::DestroySparks(int16_t nObjProducer) { m_sparks[nObjProducer].Destroy(); }
...
void CSparkManager::Init(void) { m_nSegments = 0; }
...
void CSparkManager::Setup(void) {
    ...
    if ((gameStates.render.bHaveSparks =
             (gameOpts->render.effects.bEnabled && gameOpts->render.effects.bEnergySparks)) &&
        sparks.Bitmap() && (BuildSegList() > 0)) {
