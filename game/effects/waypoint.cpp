...
CObject *CWayPointManager::Current(CObject *pObj) { return WayPoint((uint32_t)*pObj->WayPoint()); }
...
CObject *CWayPointManager::Successor(CObject *pObj) {
    return WayPoint(
        (uint32_t)Current(pObj)->cType.wayPointInfo.nSuccessor[(int32_t)pObj->rType.lightningInfo.bDirection]);
}
