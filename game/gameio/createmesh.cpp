...
int32_t CTriMeshBuilder::AllocData(void) {
    ...
    if (m_nMaxTriangles && m_nMaxEdges) {
        if (!(m_edges.Resize(m_nMaxEdges * 2) && m_triangles.Resize(m_nMaxTriangles * 2))) {
            PrintLog(
                0,
                "Not enough memory for building the triangle mesh (%d edges, %d tris).\n",
                m_nMaxEdges * 2,
                m_nMaxTriangles * 2);
            FreeData();
            RETVAL(0)
...
int32_t CTriMeshBuilder::SplitTriangleByEdge(int32_t nTri, uint16_t nVert1, uint16_t nVert2, int16_t nPass) {
    ...
    if (!(pTriangle =
              CreateTriangle(pTriangle, index, nFace, nIndex))) // create a new triangle at this location (insert)
        RETVAL(0)
...
int32_t CTriMeshBuilder::SplitEdge(CSegFace *pFace, tEdge *pEdge, int16_t nPass) {
    ...
    if (gameData.segData.nVertices >= i) {
        i *= 2;
        if (!(gameData.segData.fVertices.Resize(i) && gameData.segData.vertices.Resize(i) &&
              gameData.segData.vertexOwners.Resize(i) && gameData.renderData.mine.Resize(i)))
            RETVAL(0)
    }
    gameData.segData.fVertices[gameData.segData.nVertices] =
        CFloatVector::Avg(gameData.segData.fVertices[verts[0]], gameData.segData.fVertices[verts[1]]);
    gameData.segData.vertices[gameData.segData.nVertices].Assign(
        gameData.segData.fVertices[gameData.segData.nVertices]);
...
void CTriMeshBuilder::QSortTriangles(int32_t left, int32_t right) {
    int32_t l = left, r = right, m = m_triangles[(l + r) / 2].nFace;
...
int32_t CTriMeshBuilder::InsertTriangles(void) {
    ...
    for (h = 0; h < m_nTriangles; h++, pTriangle++, pFaceTriangle++) {
        ...
        FACES.normals[nIndex] =
            CFloatVector3::Normal(FACES.vertices[nIndex], FACES.vertices[nIndex + 1], FACES.vertices[nIndex + 2]);
#if DBG
        ...
        if ((nIndex >= FACES.texCoord.Length()) || (nIndex >= FACES.ovlTexCoord.Length()) ||
            (nIndex >= FACES.color.Length()))
            RETVAL(0)
    ...
    PrintLog(
        -1,
        "created %d new triangles and %d new vertices\n",
        m_nTriangles - m_nTris,
...
void CTriMeshBuilder::SortFaceVertList(uint16_t *vertList, int32_t left, int32_t right) {
    int32_t l = left, r = right, m = vertList[(l + r) / 2];
...
bool CTriMeshBuilder::Load(int32_t nLevel, bool bForce) {
    ...
    if (bOk)
        bOk = (mdh.nVersion == MESH_DATA_VERSION) && (mdh.nCheckSum == CalcSegmentCheckSum()) &&
              (mdh.nSegments == gameData.segData.nSegments) && (mdh.nFaces == FACES.nFaces);
    ...
    if (bOk) {
        ...
        memcpy(
            gameData.segData.vertices.Buffer(),
            pBuffer,
            nSize = sizeof(gameData.segData.vertices[0]) * mdh.nVertices);
        pBuffer += nSize;
        gameData.segData.fVertices.Create(mdh.nVertices);
        memcpy(
            gameData.segData.fVertices.Buffer(),
            pBuffer,
            nSize = sizeof(gameData.segData.fVertices[0]) * mdh.nVertices);
        pBuffer += nSize;
        ...
        memcpy(
            FACES.lMapTexCoord.Buffer(),
            pBuffer,
            nSize = sizeof(FACES.lMapTexCoord[0]) * FACES.lMapTexCoord.Length());
        pBuffer += nSize;
...
bool CTriMeshBuilder::Save(int32_t nLevel) {
    ...
    if (bOk)
        bOk = (FACES.tris.Write(cf, mdh.nTris, 0, mdh.bCompressed) == uint32_t(mdh.nTris)) &&
              ...
              (FACES.lMapTexCoord.Write(cf, FACES.lMapTexCoord.Length(), 0, mdh.bCompressed) ==
               FACES.lMapTexCoord.Length()) &&
...
int32_t CQuadMeshBuilder::IsBigFace(uint16_t *sideVerts) {
    for (int32_t i = 0; i < 4; i++)
        if (CFloatVector::Dist(
                gameData.segData.fVertices[sideVerts[i]],
                gameData.segData.fVertices[sideVerts[(i + 1) % 4]]) > MAX_EDGE_LEN(gameStates.render.nMeshQuality))
            return 1;
    return 0;
}
...
CFloatVector3 *CQuadMeshBuilder::SetTriNormals(tFaceTriangle *pTriangle, CFloatVector3 *m_pNormal) {
    CFloatVector vNormalf = CFloatVector::Normal(
        gameData.segData.fVertices[pTriangle->index[0]],
        gameData.segData.fVertices[pTriangle->index[1]],
        gameData.segData.fVertices[pTriangle->index[2]]);
...
void CQuadMeshBuilder::InitFace(int16_t nSegment, uint8_t nSide, bool bRebuild) {
    ...
    m_pFace->m_info.bAnimation =
        IsAnimatedTexture(m_pFace->m_info.nBaseTex) || IsAnimatedTexture(m_pFace->m_info.nOvlTex);
...
void CQuadMeshBuilder::InitTexturedFace(void) {
    ...
    if ((m_pFace->m_info.nOvlTex = m_pSide->m_nOvlTex))
        m_nOvlTexCount++;
    m_pFace->m_info.bSlide =
        (gameData.pigData.tex.pTexMapInfo[m_pFace->m_info.nBaseTex].slide_u ||
         gameData.pigData.tex.pTexMapInfo[m_pFace->m_info.nBaseTex].slide_v);
    m_pFace->m_info.nCamera = IsMonitorFace(m_pFace->m_info.nSegment, m_pFace->m_info.nSide, 1);
    m_pFace->m_info.bIsLight =
        IsLight(m_pFace->m_info.nBaseTex) || (m_pFace->m_info.nOvlTex && IsLight(m_pFace->m_info.nOvlTex));
    ...
    char *pszName =
        (nTexture < MAX_WALL_TEXTURES)
            ? gameData.pigData.tex.bitmapFiles[gameStates.app.bD1Mission][gameData.pigData.tex.pBmIndex[nTexture].index]
                  .name
            : szEmpty;
...
#define LMAP_SIZE 0.0f //(1.0f / 16.0f)
    ...
    pTexCoord[2].v.v = pTexCoord[3].v.v = (y + 1) * step - border;
    RETURN
}
        ...
        *m_pNormal++ = vNormalf;
        ...
        if (!gameStates.app.bNostalgia)
            *m_pFaceColor = gameData.renderData.color.ambient[h];
            ...
            m_pFaceColor->Red() = m_pFaceColor->Green() = m_pFaceColor->Blue() = X2F(m_pSide->m_uvls[i].l);
...
void CQuadMeshBuilder::ComputeFaceKeys(void) {
    ...
    for (int32_t i = 0; i < FACES.nFaces; i++, pFace++) {
        ...
        if (pFace->m_info.nBaseTex < 0)
            ...
            if (pFace->bmTop && (pFace->bmTop->Flags() & BM_FLAG_SUPER_TRANSPARENT))
                pFace->m_info.nKey = int32_t(pFace->m_info.nBaseTex) +
                                     int32_t(pFace->m_info.nOvlTex) * MAX_WALL_TEXTURES * MAX_WALL_TEXTURES;
            else
                pFace->m_info.nKey =
                    int32_t(pFace->m_info.nBaseTex) + int32_t(pFace->m_info.nOvlTex) * MAX_WALL_TEXTURES;
        }
    }
    ...
    qs.SortAscending(
        ...
        0,
        FACES.nFaces - 1,
        reinterpret_cast<CQuickSort<CSegFace *>::comparator>(CQuadMeshBuilder::CompareFaceKeys));
...
void CQuadMeshBuilder::SplitIn4Tris(void) {
    ...
    vSide[0] =
        CFloatVector::Avg(gameData.segData.fVertices[m_sideVerts[0]], gameData.segData.fVertices[m_sideVerts[1]]);
    vSide[2] =
        CFloatVector::Avg(gameData.segData.fVertices[m_sideVerts[2]], gameData.segData.fVertices[m_sideVerts[3]]);
    vSide[1] =
        CFloatVector::Avg(gameData.segData.fVertices[m_sideVerts[1]], gameData.segData.fVertices[m_sideVerts[2]]);
    vSide[3] =
        CFloatVector::Avg(gameData.segData.fVertices[m_sideVerts[3]], gameData.segData.fVertices[m_sideVerts[0]]);

    VmLineLineIntersection(
        vSide[0],
        vSide[2],
        vSide[1],
        vSide[3],
...
int32_t CQuadMeshBuilder::Build(int32_t nLevel, bool bRebuild) {
    ...
    for (nSegment = 0; nSegment < gameData.segData.nSegments; nSegment++, m_pSeg++) {
        if (IsColoredSeg(nSegment) != 0)
            ...
            for (nSide = 0, m_pSide = m_pSeg->m_sides; nSide < SEGMENT_SIDE_COUNT; nSide++, m_pSide++) {
                ...
                m_nWallType =
                    IS_WALL(m_nWall) ? WALL(m_nWall)->IsInvisible() ? 0 : 2 : (m_pSeg->m_children[nSide] == -1) ? 1 : 0;
                if (m_nWallType)
                    FACES.nFaces++;
    ...
    if (gameStates.render.bSplitPolys)
        gameStates.render.bSplitPolys =
            (gameStates.render.bPerPixelLighting || !gameStates.render.nMeshQuality) ? 1 : -1;
    if (gameStates.render.bTriangleMesh)
    ...
    for (nSegment = 0; nSegment < gameData.segData.nSegments; nSegment++, m_pSeg++, m_pSegFace++) {
        ...
        for (nSide = 0, m_pSide = m_pSeg->m_sides; nSide < SEGMENT_SIDE_COUNT; nSide++, m_pSide++) {
            ...
            if ((nSegColor != nChildColor) || m_nWallType) {
                ...
                if ((nSegColor != nChildColor) &&
                    (!m_pSeg->HasWaterProp() ||
                     (!m_pSeg->IsWater(nSide)) && (m_pSeg->HasLavaProp() || !m_pSeg->IsLava(nSide))))
