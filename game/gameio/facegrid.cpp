...
void CFaceGridSegment::Setup(CFaceGridSegment *pParent, CFloatVector &vMin, CFloatVector &vMax) {
    ...
    for (int32_t i = 0; i < 6; i++) {
        ...
        m_faces[i].m_vNormal =
            CFloatVector::Normal(m_faces[i].m_vertices[0], m_faces[i].m_vertices[1], m_faces[i].m_vertices[2]);
    }
}

//------------------------------------------------------------------------------

bool CFaceGridSegment::ContainsPoint(CFloatVector &v) { return (v >= m_vMin) && (v <= m_vMax); }

//------------------------------------------------------------------------------
// Check whether the line between v1 and v2 intersects with at least one of the grid segment's faces

bool CFaceGridSegment::LineIntersectsFace(
    CFloatVector *vertices,
    CFloatVector &vNormal,
    CFloatVector &v1,
    CFloatVector &v2) {
    CFloatVector vIntersect;
    return FindPlaneLineIntersectionf(vIntersect, vertices[0], vNormal, v1, v2, 0.0f) &&
           !PointToFaceRelationf(vIntersect, vertices, 3, vNormal);
}
...
bool CFaceGridSegment::ContainsTriangle(CFloatVector vertices[]) {
    ...
    for (int32_t i = 0; i < 3; i++) {
        ...
        vMin.Set(
            Min(vMin.v.coord.x, vertices[i].v.coord.x),
            Min(vMin.v.coord.x, vertices[i].v.coord.y),
            Min(vMin.v.coord.x, vertices[i].v.coord.z));
        vMax.Set(
            Max(vMax.v.coord.x, vertices[i].v.coord.x),
            Max(vMax.v.coord.x, vertices[i].v.coord.y),
            Max(vMax.v.coord.x, vertices[i].v.coord.z));
    }
...
CFaceGridSegment::~CFaceGridSegment() { Destroy(); }
...
bool CFaceGridSegment::Split(int32_t nMaxFaces) {
    ...
    vOffs.Set(
        (m_vMax.v.coord.x - m_vMin.v.coord.x) / 2,
        (m_vMax.v.coord.y - m_vMin.v.coord.y) / 2,
        (m_vMax.v.coord.z - m_vMin.v.coord.z) / 2);
    ...
    for (int32_t nChild = 0, z = 0; z < 2; z++) {
        for (int32_t y = 0; y < 2; y++) {
            for (int32_t x = 0; x < 2; x++, nChild++) {
                ...
                CFloatVector vMin, vMax;
                vMin.Set(
                    m_vMin.v.coord.x + vOffs.v.coord.x * x,
                    m_vMin.v.coord.y + vOffs.v.coord.y * y,
                    m_vMin.v.coord.z + vOffs.v.coord.z * z);
                vMax.Set(
                    m_vMin.v.coord.x + vOffs.v.coord.x * (x + 1),
                    m_vMin.v.coord.y + vOffs.v.coord.y * (y + 1),
                    m_vMin.v.coord.z + vOffs.v.coord.z * (z + 1));
    ...
    for (;;) {
        ...
        for (int32_t i = 0; i < 8; i++) {
            if (bPropagated
                    ? !m_pChildren[i]->AddFace(pFace->m_nSegment, pFace->m_nSide, pFace->m_vertices, pFace->m_vNormal)
                    : !m_pChildren[i]->AddFace(pFace))
                return false;
    ...
    for (CGridFace *pFace = m_pFaces; pFace; pFace = pFace->m_pNextFace) {
        if (pFace->LineIntersects(line.m_vStart, line.m_vEnd)) {
            ...
            if (!pOccluder)
                ...
                (pFace->m_fDist == pOccluder->m_fDist) && (pFace->m_nSegment == line.m_nSegment) &&
                ((line.m_nSide < 0) || (pFace->m_nSide == line.m_nSide)))
                pOccluder = pFace;
            else
                continue;
...
CFaceGridSegment *CFaceGrid::Origin(CFloatVector v) { return m_pRoot ? m_pRoot->Origin(v) : NULL; }
...
bool CFaceGrid::AddFace(uint16_t nSegment, uint8_t nSide, uint16_t vertices[], CFloatVector vNormal) {
    ...
    for (int32_t i = 0; i < 3; i++) {
        v[i] = gameData.segData.fVertices[vertices[i]];
        m_vMin.Set(
            Min(m_vMin.v.coord.x, v[i].v.coord.x),
            Min(m_vMin.v.coord.x, v[i].v.coord.y),
            Min(m_vMin.v.coord.x, v[i].v.coord.z));
        m_vMax.Set(
            Max(m_vMax.v.coord.x, v[i].v.coord.x),
            Max(m_vMax.v.coord.x, v[i].v.coord.y),
            Max(m_vMax.v.coord.x, v[i].v.coord.z));
    }
