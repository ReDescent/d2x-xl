...
int32_t InitMonsterball(int32_t nBitmap) {
    ...
    memcpy(
        &gameData.effectData.animations[0][gameData.hoardData.monsterball.nClip],
    ...
    if (!tga.Read("monsterball.tga", gameFolders.game.szTextures[1], -1, 1.0, 0)) {
        ...
        if (pAltBm && (tga.Read("mballgold#0.tga", gameFolders.game.szTextures[1], -1, 1.0, 0) ||
            ...
            for (i = 0; i < gameData.hoardData.orb.nFrames; i++, nBitmap++) {
                ...
                InitHoardBitmap(
                    &gameData.pigData.tex.bitmaps[0][nBitmap],
...
void InitHoardData(void) {
    ...
    if (!gameData.hoardData.bInitialized) {
        ...
        gameData.hoardData.goal.nWidth = gameData.hoardData.goal.nHeight = 64;
        ...
        gameData.hoardData.orb.bm.Init(
            BM_LINEAR,
            0,
            0,
            gameData.hoardData.orb.nWidth,
            gameData.hoardData.orb.nHeight * gameData.hoardData.orb.nFrames);
        ...
        gameData.hoardData.goal.bm.Init(
            BM_LINEAR,
            0,
            0,
...
for (i = 0; i < 2; i++) {
    ...
    if (!gameData.hoardData.bInitialized) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            AltSounds[0][SOUND_YOU_GOT_ORB + i] = Sounds[0][SOUND_YOU_GOT_ORB + i] = j;
        }
...
void FreeHoardData(void) {
    ...
    if (!gameData.hoardData.bInitialized)
        return;
    pBm = gameData.hoardData.monsterball.bm.Override() ? gameData.hoardData.monsterball.bm.Override()
                                                       : &gameData.hoardData.monsterball.bm;
