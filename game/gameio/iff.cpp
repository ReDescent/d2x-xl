...
int32_t PutByte(uint8_t c, FILE *fp) { return fputc(c, fp); }
...
#define INDEX_TO_15BPP(i) \
    ((int16_t)((((palptr[(i)].r / 2) & 31) << 10) + (((palptr[(i)].g / 2) & 31) << 5) + ((palptr[(i)].b / 2) & 31)))
...
int32_t CIFF::ParseBitmap(CBitmap *pBm, int32_t bitmapType, CBitmap *pPrevBm) {
    ...
    if ((bmHeader.masking != mskHasTransparentColor) || (bmHeader.transparentColor < 0))
        ...
        pBm->SetPalette(
            paletteManager.Add(reinterpret_cast<uint8_t *>(&bmHeader.palette), bmHeader.transparentColor, -1),
            bmHeader.transparentColor,
            -1);
...
int32_t CIFF::WritePbm(FILE *fp, tIFFBitmapHeader *bitmap_header, int32_t bCompression) /* writes a pbm iff file */
{
    ...
    int32_t body_size, tiny_size,
        pbm_size = 4 + BMHD_SIZE + 8 + EVEN(raw_size) + sizeof(tPalEntry) * int32_t(1 << bitmap_header->nplanes) + 8;
...
int32_t CIFF::WriteBitmap(const char *cfname, CBitmap *pBm, uint8_t *palette) {
    ...
    if (pBm->Mode() == BM_RGB15)
        return IFF_BAD_BM_TYPE;
#if COMPRESS
...
const char error_messages[] = {"No error.\0"
