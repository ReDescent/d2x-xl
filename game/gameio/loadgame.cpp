...
void GameStartInitNetworkPlayers(void) {
    int32_t i, j, t, bCoop = IsCoopGame, segNum, segType, playerObjs[MAX_PLAYERS], startSegs[MAX_PLAYERS], nPlayers,
                     nMaxPlayers = bCoop ? MAX_COOP_PLAYERS + 1 : MAX_PLAYERS;
...
#endif
    if (IS_D2_OEM && IsMultiGame && (missionManager.nCurrentMission == missionManager.nBuiltInMission[0]) &&
        (missionManager.nCurrentLevel == 8)) {
        for (i = 0; i < nPlayers; i++)
    ...
    if (IS_MAC_SHARE && IsMultiGame && (missionManager.nCurrentMission == missionManager.nBuiltInMission[0]) &&
        (missionManager.nCurrentLevel == 4)) {
        for (i = 0; i < nPlayers; i++)
            if (PLAYER(i).connected && !(NETPLAYER(i).versionMinor & 0xF0)) {
                TextBox(
                    "Warning!",
...
void ResetPlayerData(bool bNewGame, bool bSecret, bool bRestore, int32_t nPlayer) {
    ...
    if (bNewGame) {
        ...
        player.flags = 0;
        player.nCloaks = player.nInvuls = 0;
        ...
        gameData.laserData.xLastFiredTime = gameData.laserData.xNextFireTime = gameData.missileData.xLastFiredTime =
...
void AddPlayerLoadout(bool bRestore) {
    if (gameStates.app.bHaveExtraGameInfo[IsMultiGame]) {
        ...
        if (gameStates.app.bD1Mission)
            LOCALPLAYER.primaryWeaponFlags &=
                ~(HAS_FLAG(HELIX_INDEX) | HAS_FLAG(GAUSS_INDEX) | HAS_FLAG(PHOENIX_INDEX) | HAS_FLAG(OMEGA_INDEX));
        ...
        if (gameStates.app.bD1Mission) {
            LOCALPLAYER.primaryWeaponFlags &=
                ~(HAS_FLAG(HELIX_INDEX) | HAS_FLAG(GAUSS_INDEX) | HAS_FLAG(PHOENIX_INDEX) | HAS_FLAG(OMEGA_INDEX));
            LOCALPLAYER.flags &=
                ~(PLAYER_FLAGS_FULLMAP | PLAYER_FLAGS_AMMO_RACK | PLAYER_FLAGS_CONVERTER | PLAYER_FLAGS_AFTERBURNER |
...
void ResetShipData(int32_t nPlayer, int32_t nMode) {
    ...
    player.flags &=
        ~(PLAYER_FLAGS_QUAD_LASERS | PLAYER_FLAGS_AFTERBURNER | PLAYER_FLAGS_CLOAKED | PLAYER_FLAGS_INVULNERABLE |
          PLAYER_FLAGS_FULLMAP | PLAYER_FLAGS_CONVERTER | PLAYER_FLAGS_AMMO_RACK | PLAYER_FLAGS_HEADLIGHT |
          PLAYER_FLAGS_HEADLIGHT_ON | PLAYER_FLAGS_FLAG);
    player.cloakTime = 0;
    player.invulnerableTime = 0;
    player.homingObjectDist = -I2X(1); // Added by RH
    ...
    if (nPlayer == N_LOCALPLAYER) {
        ...
        if (gameOpts->gameplay.nShip[1] < 0)
            gameOpts->gameplay.nShip[1] = (missionConfig.m_playerShip < 0)
                                              ? missionConfig.SelectShip(gameOpts->gameplay.nShip[0])
                                              : missionConfig.SelectShip(missionConfig.m_playerShip);
...
void UnloadLevelData(int32_t bRestore, bool bQuit) {
    ...
    if (!bRestore) {
        gameStates.gameplay.slowmo[0].fSpeed = gameStates.gameplay.slowmo[1].fSpeed = 1.0f;
        gameStates.gameplay.slowmo[0].nState = gameStates.gameplay.slowmo[1].nState = 0;
...
int32_t LoadModData(char *pszLevelName, int32_t bLoadTextures, int32_t nStage) {
	...
	LoadD2Sounds (true);
        ...
        if (!gameStates.app.bD1Mission /*missionManager.nEnhancedMission*/) {
            ...
            nLoadRes =
                LoadRobotExtensions("d2x.ham", gameFolders.missions.szRoot, /*missionManager.nEnhancedMission*/ 2);
            PrintLog(-1);
        }
        ...
        if (missionManager.nEnhancedMission) {
            ...
            if (nLoadRes == 0) {
                ...
                /*---*/ PrintLog(
                    1,
                    "trying custom robots (ham) from level '%s'\n",
        ...
        gameData.botData.nCamBotId =
            (LoadRobotReplacements("cambot.hxm", NULL, 1, 0) > 0) ? gameData.botData.nTypes[0] - 1 : -1;
        PrintLog(-1);
        ...
        if (*gameFolders.mods.szModels[0]) {
            sprintf(
                gameFolders.mods.szModels[1],
                "%s%s",
                gameFolders.mods.szModels[0],
                LevelFolder(missionManager.nCurrentLevel));
...
int32_t LoadLevel(int32_t nLevel, bool bLoadTextures, bool bRestore) {
    ...
    SetDataVersion(-1);
    memcpy(
        gameData.pigData.tex.brightness.Buffer(),
    ...
    if (!gameStates.app.bPrecomputeLightmaps) {
        ...
        if (IsEntropyGame) {
            ...
            (gameData.appData.GameMode(GM_CAPTURE | GM_HOARD)) ||
...
#define N_GLITZITEMS 11
    ...
    if (nMineLevel < 0)
        ...
        nMineLevel = 1;
    nEndGamePoints = nSkillPoints = nShieldPoints = nEnergyPoints = nHostagePoints = nAllHostagePoints = 0;
    bIsLastLevel = 0;
    ...
    if (!gameStates.app.cheats.bEnabled) {
        ...
        if (!IsMultiGame && LOCALPLAYER.lives &&
            (missionManager.nCurrentLevel == missionManager.nLastLevel)) { // player has finished the game!
            nEndGamePoints = LOCALPLAYER.lives * 10000;
            sprintf(szEndGame, "%s%i  ", TXT_SHIP_BONUS, nEndGamePoints);
            bIsLastLevel = 1;
        }
        cockpit->AddBonusPointsToScore(
            nSkillPoints + nEnergyPoints + nShieldPoints + nHostagePoints + nAllHostagePoints + nEndGamePoints);
    }
    c = 0;
...
void PlayerFinishedLevel(int32_t bSecret) {
    ...
    gameStates.render.cockpit.nLastDrawn[0] = gameStates.render.cockpit.nLastDrawn[1] = -1;
...
void PlayLevelIntroMovie(int32_t nLevel) { PlayLevelMovie(".mvi", nLevel); }

//------------------------------------------------------------------------------

void PlayLevelExtroMovie(int32_t nLevel) { PlayLevelMovie(".mvx", nLevel); }
...
void DoEndGame(void) {
    ...
    KeyFlush();
    if (!IsMultiGame) {
        if (missionManager.nCurrentMission ==
            (gameStates.app.bD1Mission ? missionManager.nBuiltInMission[1] : missionManager.nBuiltInMission[0])) {
            ...
            if (!bPlayed) {
                if (IS_D2_OEM) {
                    ...
                    briefing.Run(
                        gameStates.app.bD1Mission ? "endreg.tex" : "ending2.tex",
    ...
    if (gameData.appData.GameMode(GM_SERIAL | GM_MODEM))
        gameData.appData.SetGameMode(
            gameData.appData.GameMode() | GM_GAME_OVER); // preserve modem setting so go back into modem menu
    else
        gameData.appData.SetGameMode(GM_GAME_OVER);
    longjmp(gameExitPoint, 0); // Exit out of game loop
}
...
void AdvanceLevel(int32_t bSecret, int32_t bFromSecret) {
    ...
    Assert(!bSecret);
    if (!bFromSecret &&
        ((missionManager.nCurrentLevel != missionManager.nLastLevel) || extraGameInfo[IsMultiGame].bRotateLevels)) {
        if (IsMultiGame)
            MultiEndLevelScore();
        else {
            PlayLevelExtroMovie(missionManager.nCurrentLevel);
    ...
    if ((missionManager.NextLevel() > 0) && (missionManager.NextLevel() > missionManager.LastLevel()) &&
        (missionManager.NextLevel(1) < 1) && !extraGameInfo[IsMultiGame].bRotateLevels) // player has finished the game!
        DoEndGame();
    else {
...
} szIntroMovies[] = {{1, "pla"}, {5, "plb"}, {9, "plc"}, {13, "pld"}, {17, "ple"}, {21, "plf"}, {24, "plg"}};
        ...
        for (j = 0; j < N_PLAYERS; j++) {
            if (j != N_LOCALPLAYER) {
                ...
                if ((pObj->info.nType == OBJ_PLAYER)) {
                    xDist = simpleRouter[0].PathLength(
...
void DoPlayerDead(void) {
    ...
    if (IsCoopGame && gameStates.app.bHaveExtraGameInfo[1])
        LOCALPLAYER.score = (LOCALPLAYER.score * (100 - nCoopPenalties[(int32_t)extraGameInfo[1].nCoopPenalty])) / 100;
