...
int32_t d1_tmap_num_unique(int16_t nD1Texture) {
    ...
    static int16_t unique_tmap_nums[] = {
        0,   2,   4,   5,   6,   7,   9,   10,  11,  12,  17,  18,  20,  21,  25,  28,  38,  39,  41,  44,
        49,  50,  55,  57,  88,  132, 141, 147, 154, 155, 158, 159, 160, 161, 167, 168, 169, 170, 171, 174,
        175, 185, 193, 194, 195, 198, 199, 200, 202, 210, 211, 220, 226, 227, 228, 229, 230, 240, 241, 242,
        243, 246, 250, 251, 252, 253, 257, 258, 259, 260, 263, 266, 283, 298, 305, 308, 311, 312, 315, 317,
        319, 320, 321, 330, 331, 332, 333, 349, 351, 352, 353, 354, 355, 357, 358, 359, 362, 370, -1};
...
void ReadColor(CFile &cf, CFaceColor *pColor, int32_t bFloatData, int32_t bRegisterColor) {
    ...
    if (bRegisterColor &&
        (((pColor->Red() > 0) && (pColor->Red() < 1)) || ((pColor->Green() > 0) && (pColor->Green() < 1)) ||
...
CSegFace *FindDupFace(int16_t nSegment, int16_t nSide) {
    ...
    for (i = 0, pSegFace = SEGFACES.Buffer(); i < gameData.segData.nSegments; i++, pSegFace++) {
        for (j = pSegFace->nFaces, faceP1 = pSegFace->pFace; j; faceP1++, j--) {
            if (faceP1 == faceP0)
                continue;
            if ((faceP1->m_info.nIndex == faceP0->m_info.nIndex) ||
                !memcmp(faceP1->m_info.index, faceP0->m_info.index, sizeof(faceP0->m_info.index)))
                return faceP1;
        }
    }
    return 0;
...
void LoadTexColorsCompiled(int32_t i, CFile &cf) {
    ...
    if (gameStates.app.bD2XLevel) {
        ...
        for (; i < j; i++)
            ReadColor(
                cf,
                gameData.renderData.color.textures + i,
                gameData.segData.nLevelVersion <= 15,
...
int32_t LoadMineSegmentsCompiled(CFile &cf) {
    ...
    if (nCompiledVersion != COMPILED_MINE_VERSION)
        console.printf(
            CON_DBG,
            "compiled mine version=%i\n",
            nCompiledVersion); // many levels have "wrong" versions.  Theres no point in aborting because of it, I
                               // think.
