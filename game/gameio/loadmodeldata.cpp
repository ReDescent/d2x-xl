...
void _CDECL_ FreeObjExtensionBitmaps(void) {
    ...
    for (i = gameData.pigData.tex.nBitmaps[0], pBm = gameData.pigData.tex.bitmaps[0] + i;
         i < gameData.pigData.tex.nExtraBitmaps;
         i++, pBm++) {
...
int32_t LoadRobotExtensions(const char *fname, char *folder, int32_t nType) {
    ...
    if (bVertigoData) {
        ...
        memcpy(
            &gameData.botData.defaultInfo[N_D2_ROBOT_TYPES],
            &gameData.botData.info[0][N_D2_ROBOT_TYPES],
            sizeof(gameData.botData.info[0][0]) * t);
    }
    ...
    if (bVertigoData) {
        ...
        memcpy(
            &gameData.botData.defaultJoints[N_D2_ROBOT_TYPES],
            &gameData.botData.joints[N_D2_ROBOT_TYPES],
            sizeof(gameData.botData.joints[0]) * t);
    }
    ...
    if (gameData.modelData.nPolyModels >= MAX_POLYGON_MODELS) {
        Warning(
            "Too many polygon models (%d)\nin <%s>.\nMax is %d.",
            t,
            fname,
            MAX_POLYGON_MODELS - N_D2_POLYGON_MODELS);
        return -1;
    }
    ...
    if (bVertigoData) {
        memcpy(
            &gameData.pigData.tex.defaultObjBmIndex[N_D2_OBJBITMAPS],
            &gameData.pigData.tex.objBmIndex[N_D2_OBJBITMAPS],
            sizeof(gameData.pigData.tex.objBmIndex[0]) * t);
    }

    t = cf.ReadInt();
    if (N_D2_OBJBITMAPPTRS + t >= MAX_OBJ_BITMAPS) {
        Warning(
            "Too many object bitmap pointers (%d) in <%s>.  Max is %d.",
            t,
            fname,
            MAX_OBJ_BITMAPS - N_D2_OBJBITMAPPTRS);
        return -1;
    }
...
int32_t LoadRobotReplacements(
    ...
    CPolyModel *pModel;
    int32_t t, i, j;
    int32_t nBotTypeSave = gameData.botData.nTypes[gameStates.app.bD1Mission], nBotJointSave = gameData.botData.nJoints,
    ...
    if (!cf.Open(
            szFile,
            szFolder ? szFolder : gameFolders.game.szData[0],
            "rb",
            bUseHog ? 0 : -1)) // no robot replacement file
        return 0;
    ...
    for (j = 0; j < t; j++) {
        ...
        if (bAddBots) {
            if (gameData.botData.nJoints < MAX_ROBOT_JOINTS)
                ...
                Warning("%s: Robots joint (%d) out of range (valid range = 0 - %d).", szLevel, i, MAX_ROBOT_JOINTS - 1);
                gameData.botData.nTypes[gameStates.app.bD1Mission] = nBotTypeSave;
                gameData.botData.nJoints = nBotJointSave;
                gameData.modelData.nPolyModels = nPolyModelSave;
                return -1;
            ...
            Warning(
                "%s: Robots joint (%d) out of range (valid range = 0 - %d).",
                szLevel,
                i,
                gameData.botData.nJoints - 1);
            gameData.botData.nTypes[gameStates.app.bD1Mission] = nBotTypeSave;
            gameData.botData.nJoints = nBotJointSave;
            gameData.modelData.nPolyModels = nPolyModelSave;
            return -1;
    ...
    for (j = 0; j < t; j++) {
        ...
        if (bAddBots) {
            if (gameData.modelData.nPolyModels < MAX_POLYGON_MODELS)
                ...
                Warning(
                    "%s: Polygon model (%d) out of range (valid range = 0 - %d).",
                    szLevel,
                    i,
                    gameData.modelData.nPolyModels - 1);
                gameData.botData.nTypes[gameStates.app.bD1Mission] = nBotTypeSave;
                gameData.botData.nJoints = nBotJointSave;
                gameData.modelData.nPolyModels = nPolyModelSave;
                return -1;
            ...
            if (bAltModels) {
                ...
                (gameData.modelData.nPolyModels > N_D2_POLYGON_MODELS) ||
                (i >= N_D2_POLYGON_MODELS + N_VERTIGO_POLYGON_MODELS)) {
                Warning(
                    "%s: Polygon model (%d) out of range (valid range = 0 - %d).",
                    szLevel,
                    i,
    ...
    for (j = 0; j < t; j++) {
        ...
        if (!bAddBots && ((i < 0) || (i >= MAX_OBJ_BITMAPS))) {
            Warning(
                "%s: Object bitmap number (%d) out of range (valid range = 0 - %d).",
                szLevel,
                i,
                MAX_OBJ_BITMAPS - 1);
            gameData.botData.nTypes[gameStates.app.bD1Mission] = nBotTypeSave;
    ...
    for (j = 0; j < t; j++) {
        ...
        if ((i < 0) || (i >= MAX_OBJ_BITMAPS)) {
            Warning(
                "%s: Object bitmap pointer (%d) out of range (valid range = 0 - %d).",
                szLevel,
                i,
                MAX_OBJ_BITMAPS - 1);
            gameData.botData.nTypes[gameStates.app.bD1Mission] = nBotTypeSave;
...
int32_t LoadExitModels(void) {
    ...
    static const char *szExitBm[] =
        {"steel1.bbm", "rbot061.bbm", "rbot062.bbm", "steel1.bbm", "rbot061.bbm", "rbot063.bbm", NULL};
    ...
    if (cf.Open("exit.ham", gameFolders.game.szData[0], "rb", 0)) {
        ...
        cf.Exist("exit01.pof", gameFolders.game.szData[0], 0) &&
...
int32_t SaveModelData(void) {
    ...
    bOk = (cf.Write(&mdh, sizeof(mdh), 1) == 1) &&
          (gameData.modelData.spheres.Write(cf) == gameData.modelData.spheres.Length()) && cf.Close();
    return bOk;
}
