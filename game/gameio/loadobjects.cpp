...
void CObject::Verify(void) {
    ...
    if (Type() == OBJ_POWERUP) {
        ...
        if (this == gameData.objData.pConsole)
            InitPlayerObject();
        else if (info.renderType == RT_POLYOBJ) // recover from Matt's pof file matchup bug
...
static int32_t ReadObjectInfo(CFile &cf) {
    ...
    gameData.objData.nFirstDropped = gameData.objData.nLastDropped = -1;
...
static void CheckTriggerInfo(void) {
    ...
    for (i = 0, pTrigger = GEOTRIGGERS.Buffer(); i < gameFileInfo.triggers.count; i++, pTrigger++) {
        ...
        for (h = pTrigger->m_nLinks, j = 0; j < h;) {
            if ((pTrigger->m_segments[j] >= 0) && (pTrigger->m_sides[j] >= 0)
...
int32_t CmpObjTriggers(const CTrigger *pv, const CTrigger *pm) {
    return (pv->m_info.nObject < pm->m_info.nObject)
               ? -1
               : (pv->m_info.nObject > pm->m_info.nObject)
                     ? 1
            ...
            if (nObject != pTrigger->m_info.nObject) {
                if (nObject >= 0)
                    gameData.trigData.objTriggerRefs[nObject].nCount =
                        i - gameData.trigData.objTriggerRefs[nObject].nFirst;
...
AssignProducer(tObjectProducerInfo &producerInfo, int32_t nObjProducer, int32_t nFunction, int8_t bFlag) {
    ...
    for (int32_t i = 0, j = gameData.segData.nSegments; i < j; i++, pSeg++) {
        if ((pSeg->Function() == nFunction) && (pSeg->m_nObjProducer == nObjProducer)) {
            ...
            producer.bFlag &= (pSeg->Function() == SEGMENT_FUNC_ROBOTMAKER)
                                  ? ~1
                                  : (pSeg->Function() == SEGMENT_FUNC_EQUIPMAKER) ? ~2 : ~0;
...
static int32_t ReadLightDeltaIndexInfo(CFile &cf) {
    ...
    if ((gameFileInfo.lightDeltaIndices.offset > -1) && gameFileInfo.lightDeltaIndices.count) {
        ...
        if (gameTopFileInfo.fileinfoVersion < 29) {
            ...
            console.printf(
                CON_DBG,
                "Warning: Old mine version.  Not reading gameData.renderData.lights.deltaIndices info.\n");
#endif
            ...
            if (pTrigger->m_sides[j] >= 0) {
                ...
                if (nSegment >= gameData.segData.nSegments) {
                    ...
                    if (pTrigger->m_info.nType == TT_OBJECT_PRODUCER) {
                        if ((SEGMENT(nSegment)->m_function != SEGMENT_FUNC_ROBOTMAKER) &&
                            (SEGMENT(nSegment)->m_function != SEGMENT_FUNC_EQUIPMAKER)) {
                            if (j < --h) {
                                pTrigger->m_segments[j] = pTrigger->m_segments[h];
                                pTrigger->m_sides[j] = pTrigger->m_sides[h];
                            }
                        ...
                        (pTrigger->m_info.nType != TT_LIGHT_OFF) &&
    ...
    for (;;) {
        ...
        if ((gameData.segData.nLevelVersion <= 1) || (szCurrentLevelPalette[0] == 0)) // descent 1 level
            strcpy(
                ...
                /*gameStates.app.bD1Mission ? D1_PALETTE :*/ DEFAULT_LEVEL_PALETTE); // D1_PALETTE looks off in D2X-XL
