...
void PiggyInitSound(void) { memset(gameData.pigData.sound.sounds, 0, sizeof(gameData.pigData.sound.sounds)); }
...
int32_t LoadHiresSound(CSoundSample *pSound, char *pszSoundName, bool bCustom) {
    ...
    if (!((*gameFolders.mods.szSounds[1] && cf.Open(szSoundFile, gameFolders.mods.szSounds[1], "rb", 0)) ||
          ...
          ((/*bCustom ||*/ gameOpts->UseHiresSound()) &&
           cf.Open(szSoundFile, gameFolders.game.szSounds[HIRES_SOUND_FOLDER(gameStates.sound.bD1Sound)], "rb", 0))))
        return 0;
...
bool VerifyHiresSound(bool bCustom, int32_t nLoadedSounds) {
    if (bCustom) {
        // -> all default sounds are hires
        if (!(nLoadedSounds & 0xFFFF)) {
            gameOpts->sound.bHires[0] = gameOpts->sound.bHires[1];
        }
    } else if (gameOpts->sound.bHires[0]) {
        // -> not all default sounds are hires
        if (nLoadedSounds & 0xFFFF) {
...
int32_t LoadD2Sounds(bool bCustom) {
    ...
    if (bCustom) {
        ...
        sprintf(
            ...
            (gameOpts->sound.audioSampleRate >= SAMPLE_RATE_22K) ? ".s22" : ".s11");
        pszFile = szFile;
