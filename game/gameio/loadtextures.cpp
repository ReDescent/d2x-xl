...
const char *szAddonTextures[MAX_ADDON_BITMAP_FILES] = {
    "slowmotion#0",    "bullettime#0",    "targ01b#0-green", "targ01b#1-green", "targ02b#0-green", "targ02b#1-green",
    "targ02b#2-green", "targ03b#0-green", "targ03b#1-green", "targ03b#2-green", "targ03b#3-green", "targ03b#4-green",
    "targ01b#0-red",   "targ01b#1-red",   "targ02b#0-red",   "targ02b#1-red",   "targ02b#2-red",   "targ03b#0-red",
    "targ03b#1-red",   "targ03b#2-red",   "targ03b#3-red",   "targ03b#4-red"};

static int16_t d2OpaqueDoors[] = {440,          451, 463, 477, /*483,*/ 488, 500,          508, 523, 550, 556, 564, 572,
                                  579,          585, 593, 536, 600,          608,          615, 628, 635, 6,   649, 664,
                                  /*672,*/ 687, 702, 717, 725, 731,          738,          745, 754, 763, 772, 780, 790,
                                  806,          817, 827, 838, 849,          /*858,*/ 863, 871, 886, 901, -1};
...
#define BM_FLAGS_TO_COPY \
    (BM_FLAG_TRANSPARENT | BM_FLAG_SUPER_TRANSPARENT | BM_FLAG_NO_LIGHTING | BM_FLAG_RLE | BM_FLAG_RLE_BIG)
...
#define CHANGING_TEXTURE(_eP) \
    (((_eP)->changing.nWallTexture >= 0) ? (_eP)->changing.nWallTexture : (_eP)->changing.nObjectTexture)
...
void UnloadTextures(void) {
    ...
    for (bD1 = 0; bD1 < 2; bD1++) {
        ...
        for (i = 0, pBm = gameData.pigData.tex.bitmaps[bD1].Buffer(); i < gameData.pigData.tex.nBitmaps[bD1];
             i++, pBm++) {
#if DBG
            if (i == nDbgTexture)
                BRP;
    ...
    return (nTexture > 0) &&
           (strchr(
                gameData.pigData.tex
                    .bitmapFiles[gameStates.app.bD1Mission][gameData.pigData.tex.pBmIndex[nTexture].index]
                    .name,
                '#') != NULL);
}
...
static bool IsPowerup(const char *bmName) {
    static const char *szNames[] = {"invuln", "pwr", "flare"};
...
static bool IsWeapon(const char *bmName) {
    ...
    static const int nModels[] =
        {-1, -1, -1, -1, -1, BLUEKEY_MODEL, GOLDKEY_MODEL, REDKEY_MODEL, -1, -1, -1, -1, -1, HOSTAGE_MODEL};
...
static void MakeBitmapFilenames(
    const char *bmName,
    char *rootFolder,
    char *cacheFolder,
    char *fn,
    char *fnShrunk,
    int32_t nShrinkFactor) {
    if (!*rootFolder)
        *fn = *fnShrunk = '\0';
    else {
        CFile cf;
    ...
    for (int32_t i = 0; i < 2; i++) {
        ...
        if (*gameFolders.mods.szTextures[0]) {
            ...
            sprintf(
                ...
                gameFolders.mods.szLevel); // e.g. /var/cache/d2x-xl/mods/mymod/textures/level01
        } else
            *gameFolders.mods.szTextures[1] = *gameFolders.var.szTextures[4] = '\0';
        ...
        MakeBitmapFilenames(
            baseName,
            gameFolders.game.szTextures[bD1 + 1],
            gameFolders.var.szTextures[bD1 + 1],
            fn[5],
            fn[4],
            nShrinkFactor);
...
int32_t PageInBitmap(CBitmap *pBm, const char *bmName, int32_t nIndex, int32_t bD1, bool bHires) {
    ...
    if (/*gameStates.render.bBriefing ||*/ !(
        bHaveTGA || HaveHiresBitmap(bmName, bD1) || HaveHiresModel(bmName))) // hires addon texture not loaded
