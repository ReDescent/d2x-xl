...
int32_t MsnIsTok(char *buf, const char *tok) { return !strnicmp(buf, tok, strlen(tok)); }
...
int32_t _CDECL_ MLSortFunc(tMsnListEntry *e0, tMsnListEntry *e1) {
    char *s1 = e0->szMissionName, *s2 = e1->szMissionName;
...
void CMissionManager::Add(
    int32_t bAnarchy,
    int32_t bD1Mission,
    int32_t bSubFolder,
    int32_t bHaveSubFolders,
    int32_t nLocation) {
    CFile cf;
    FFS ffs;
    char lvlName[255];
            ...
            do {
                ...
                if (nFolderNameLen + strlen(m_list[m_nCount].szMissionName) > FILENAME_LEN) {
                    if (bFindDirs)
                        PrintLog(
                            0,
                            "   Error: Mission %s name length exceeds internal limits\n(%s%s)\n",
                            bFindDirs ? "folder" : "file",
                            gameFolders.missions.szCurrent[nLocation],
                            ffs.name);
                    continue;
                }
                if (bFindDirs) {
...
void CMissionManager::MoveFolderUp(void) {
    ...
    if (gameOpts->app.bSinglePlayer && !gameStates.app.bPrecomputeLightmaps &&
        !strcmp(gameFolders.missions.szSubFolder, "single/"))
...
int32_t CMissionManager::BuildList(int32_t bAnarchy, int32_t nSubFolder) {
    ...
    if (!bDownloadFolder && gameStates.app.bDemoData) {
        ...
        if (!bDownloadFolder && !bSubFolder) { // || (gameOpts->app.bSinglePlayer && !strcmp
                                               // (gameFolders.missions.szSubFolder, "single"))) {
        ...
        if (bDownloadFolder)
            ...
            sprintf(
                gameFolders.missions.szCurrent[0],
                "%s%s",
                gameFolders.missions.szRoot,
                gameFolders.missions.szSubFolder);
        bHaveSubFolders = 0;
        if (gameOpts->app.nVersionFilter & 2) {
        ...
        if (!bDownloadFolder && gameFolders.bAltHogDirInited &&
            strcmp(gameFolders.game.szAltHogs, gameFolders.game.szRoot)) {
            bHaveSubFolders = 0;
            if (strlen(gameFolders.game.szAltHogs) + strlen(MISSION_FOLDER) +
                    strlen(gameFolders.missions.szSubFolder) >=
                FILENAME_LEN - 5) {
                PrintLog(0, "   Error: Current mission folder name length exceeds internal limits\n");
                PrintLog(
                    0,
                    "   (%s%s%s)\n",
                    gameFolders.game.szAltHogs,
                    MISSION_FOLDER,
                    gameFolders.missions.szSubFolder);
                return 0;
            }
            sprintf(
                gameFolders.missions.szCurrent[1],
                "%s%s%s",
                gameFolders.game.szAltHogs,
                MISSION_FOLDER,
                gameFolders.missions.szSubFolder);
    ...
    nTopPlace = 0;
    if (bSubFolder) {
        nBuiltInMission[0] = nBuiltInMission[1] = -1;
    } else {
...
int32_t CMissionManager::Load(int32_t nMission) {
    ...
    if (!strcmp(m_list[nMission].filename, szBuiltinMissionFilename[0]))
        ...
        if (bFoundHogFile) {
            ...
            if ((m_list[nMission].nDescentVersion == 1) && strcmp(m_list[nMission].filename, "descent"))
...
int32_t CMissionManager::IsBuiltIn(const char *pszMission) {
    ...
    return (strstr(pszMission, "Descent: First Strike") != NULL)
               ...
               : (strstr(pszMission, "Descent 2: Counterstrike!") != NULL)
                     ? 2
                     : (strstr(pszMission, "Descent 2: Vertigo") != NULL) ? 3 : 0;
}
...
char *CMissionManager::LevelStateName(char *szFile, int32_t nLevel) {
    ...
    sprintf(
        szFile,
        "%s-player%d.level%d",
        m_list[nCurrentMission].szMissionName + 4,
        IsMultiGame ? N_LOCALPLAYER + 1 : 0,
        nLevel ? nLevel : nCurrentLevel);
#else
...
int32_t CMissionManager::SaveLevelStates(void) {
    if (!gameStates.app.bReadOnly) {
        ...
        sprintf(
            szFile,
            "%s.state",
            m_list[nCurrentMission].szMissionName + ((m_list[nCurrentMission].szMissionName[0] == '[') ? 4 : 0));
...
int32_t CMissionManager::LoadLevelStates(void) {
    ...
    sprintf(
        ...
        "%s.state",
        m_list[nCurrentMission].szMissionName + ((m_list[nCurrentMission].szMissionName[0] == '[') ? 4 : 0));
    if (!cf.Open(szFile, gameFolders.missions.szStates, "rb", 0) && !cf.Open(szFile, gameFolders.var.szCache, "rb", 0))
        return 0;
    for (int32_t i = 0; i < MAX_LEVELS_PER_MISSION; i++)
...
void CMissionManager::AdvanceLevel(int32_t nNextLevel) {
    if (gameStates.app.bD1Mission)
        missionManager.SetNextLevel(
            ((missionManager.nCurrentLevel < 0) ? missionManager.nEntryLevel : missionManager.nCurrentLevel) + 1);
