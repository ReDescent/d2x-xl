    ...
    for (int32_t i = gameData.effectData.nEffects[gameStates.app.bD1Data]; i; i--, pEffectInfo++) {
        if (pEffectInfo->changing.nWallTexture == nTexture) {
            ...
            if (pEffectInfo->destroyed.nTexture >= 0)
                LoadTexture(
                    gameData.pigData.tex.pBmIndex[pEffectInfo->destroyed.nTexture].index,
                    0,
                    gameStates.app.bD1Data); // use this bitmap when monitor destroyed
            if (pEffectInfo->destroyed.nAnimation >= 0)
                LoadAnimationTextures(
                    &gameData.effectData.vClipP[pEffectInfo->destroyed.nAnimation],
                    gameStates.app.bD1Data); // what animation to play when exploding
            if (pEffectInfo->destroyed.nEffect >= 0)
                LoadAnimationTextures(
                    &gameData.effectData.pEffect[pEffectInfo->destroyed.nEffect].animationInfo,
                    gameStates.app.bD1Data); // what effect to play when exploding
            if (pEffectInfo->nCriticalAnimation >= 0)
                LoadAnimationTextures(
                    &gameData.effectData.pEffect[pEffectInfo->nCriticalAnimation].animationInfo,
                    gameStates.app.bD1Data); // what effect to play when mine critical
        }
    }
}
...
void LoadModelTextures(int32_t nModel) {
    ...
    for (int32_t h = pModel->TextureCount(), i = 0; i < h; i++, pi++) {
        j = *pi;
        LoadTexture(
            gameData.pigData.tex.objBmIndex[j].index,
            0 /*i*/,
            0); // don't pass frame number to avoid checking for hires replacement textures
...
void CObject::LoadTextures(void) {
    ...
    case RT_POWERUP:
        if (PowerupToDevice())
            ...
            (info.nId >= MAX_POWERUP_TYPES_D2) && (rType.animationInfo.nClipIndex >= gameData.effectData.nClips[0]))
...
int32_t PagingGaugeSize(void) {
    return PROGRESS_STEPS(gameData.segData.nSegments) + PROGRESS_STEPS(gameFileInfo.walls.count) +
           PROGRESS_STEPS(gameData.objData.pwrUp.nTypes) * 2 + PROGRESS_STEPS(gameData.weaponData.nTypes[0]) +
    ...
    if (nTouchSeg < gameData.segData.nSegments) {
        ...
        for (i = 0; (i < PROGRESS_INCR) && (nTouchPowerup1 < gameData.objData.pwrUp.nTypes); i++, nTouchPowerup1++)
            if (gameData.objData.pwrUp.info[nTouchPowerup1].nClipIndex >= 0)
                LoadAnimationTextures(
                    &gameData.effectData.animations[0][gameData.objData.pwrUp.info[nTouchPowerup1].nClipIndex],
                    0);
        ...
        for (i = 0; (i < PROGRESS_INCR) && (nTouchPowerup2 < gameData.objData.pwrUp.nTypes); i++, nTouchPowerup2++)
            if (gameData.objData.pwrUp.info[nTouchPowerup2].nClipIndex >= 0)
                LoadAnimationTextures(
                    &gameData.effectData.animations[0][gameData.objData.pwrUp.info[nTouchPowerup2].nClipIndex],
                    0);
...
void LoadLevelTextures(void) {
    ...
    if (gameStates.app.bProgressBars && gameOpts->menus.nStyle) {
        ...
        ProgressBar(
            ...
            1,
            i,
            i + PagingGaugeSize() + SortLightsGaugeSize() + SegDistGaugeSize(),
            LoadTexturesPoll);
    } else
        LoadAllTextures();
}
