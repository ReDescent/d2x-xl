...
char *DefaultPigFile(int32_t bDemoData) { return szPigFiles[(bDemoData < 0) ? gameStates.app.bDemoData : bDemoData]; }

//------------------------------------------------------------------------------

static char szHamFiles[2][13] = {"descent2.ham", "d2demo.ham"};

char *DefaultHamFile(void) { return szHamFiles[gameStates.app.bDemoData]; }
...
int32_t PiggyInit(void) {
    ...
    if (!bogusBitmap.FrameSize()) {
        ...
        bitmapOffsets[0][0] = bitmapOffsets[1][0] = 0;
    }
...
void PiggyCriticalError(void) {
    ...
    int32_t i = InfoBox(
        ...
        "Exit",
        "%s\non drive %c:",
        szCriticalErrors[descent_critical_errcode & 0xf],
        (descent_critical_deverror & 0xf) + 'A');
...
int32_t LoadD1PigHeader(
    CFile &cf,
    int32_t *soundNumP,
    int32_t *pBmHdrOffs,
    int32_t *pBmDataOffs,
    int32_t *pBitmapNum,
    int32_t bReadTMapNums) {
    int32_t nPigDataStart, nHeaderSize, nBmHdrOffs, nBmDataOffs, nSoundNum, nBitmapNum;
    ...
    if (gameStates.app.bD1Mission && gameStates.app.bHaveD1Data && !gameStates.app.bHaveD1Textures) {
        ...
        for (i = 0; i < nBitmaps; i++) {
            ...
            memset(
                ...
                0,
                sizeof(
                    bm)); // important, because this texture will be cloned and must not contain any pointers to itself
            if (bmh.dflags & DBM_FLAG_ABM) {
                sprintf(szName, "%s#%d", szNameRead, bmh.dflags & DBM_NUM_FRAMES);
            } else
                strcpy(szName, szNameRead);
 ...
 */
void ReadBitmapIndex(tBitmapIndex *bi, CFile &cf) { bi->index = cf.ReadShort(); }
