...
RetrySelection:
    ...
    gameOpts->input.joystick.sensitivity[0] = gameOpts->input.joystick.sensitivity[1] =
        gameOpts->input.joystick.sensitivity[2] = gameOpts->input.joystick.sensitivity[3] =
            gameOpts->input.joystick.sensitivity[4] = 8;
    gameOpts->input.mouse.sensitivity[0] = gameOpts->input.mouse.sensitivity[1] = gameOpts->input.mouse.sensitivity[2] =
        8;
...
int32_t LoadPlayerProfile(int32_t nStage) {
    ...
    if (gameStates.gfx.bOverride) {
        ...
        gameStates.video.nDefaultDisplayMode =
            FindDisplayMode(gameData.renderData.screen.Width(), gameData.renderData.screen.Height());
    ...
    if (gameStates.input.nPlrFileVersion >= 23) {
        if (gameData.appData.nLifetimeChecksum !=
            ...
            GetLifetimeChecksum(networkData.nNetLifeKills, networkData.nNetLifeKilled);
#else
            networkData.nNetLifeKills = networkData.nNetLifeKilled = 0;
            gameData.appData.nLifetimeChecksum = 0;
            bRewriteIt = 1;
#endif
        }
    ...
    if ((gameOpts->render.stereo.xSeparation[1] < MM2X(RIFT_MIN_IPD)) ||
        (gameOpts->render.stereo.xSeparation[1] > MM2X(RIFT_MAX_IPD)))
...
int32_t SelectPlayer(void) {
    ...
    if (LOCALPLAYER.callsign[0] == 0) {
        ...
        gameOpts->input.joystick.sensitivity[0] = gameOpts->input.joystick.sensitivity[1] =
            gameOpts->input.joystick.sensitivity[2] = gameOpts->input.joystick.sensitivity[3] = 8;
        gameOpts->input.mouse.sensitivity[0] = gameOpts->input.mouse.sensitivity[1] =
