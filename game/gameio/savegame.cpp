        ...
        if (gameStates.menus.bHires) {
            ...
            if (gameOpts->menus.nStyle) {
                ...
                OglDrawEmptyRect(
                    x - 1,
                    y - 1,
                    x + CMenu::Scaled(image->Width()) + 1,
                    y + CMenu::Scaled(image->Height()) + 1);
            }
            gameData.SetStereoOffsetType(nOffsetSave);
        } else {
            saveGameInfo[nCurItem - 1].Image()->BlitClipped(
                (CCanvas::Current()->Width() - THUMBNAIL_W) / 2,
                menu[0].m_y - 5);
        }
    } else { // input
        if (nCurItem < 2)
...
void CSaveGameManager::Backup(void) {
    if (!m_override) {
        ...
        if (cf.Exist(m_filename, gameFolders.user.szSavegames, 0) &&
            cf.Open(m_filename, gameFolders.user.szSavegames, "wb", 0)) {
...
CSaveGameManager::Save(int32_t bBetweenLevels, int32_t bSecret, int32_t bQuick, const char *pszFilenameOverride) {
    ...
    if ((missionManager.nCurrentLevel < 0) &&
        !((m_bSecret > 0) || (gameOpts->gameplay.bSecretSave && missionConfig.m_bSecretSave) ||
...
void CSaveGameManager::SaveNetGame(void) {
    ...
    m_cf.Write(
        netGameInfo.AuxData(),
        NETGAME_AUX_SIZE,
        1); // Storage for protocol-specific data (e.g., multicast session and port)
}
...
void CSaveGameManager::SavePlayer(CPlayerData *pPlayer) {
    ...
    m_cf.WriteInt(pPlayer->nObject); // What CObject number this CPlayerData is. (made an int32_t by mk because it's
                                     // very often referenced)
...
int32_t CSaveGameManager::SetServerPlayer(
    CPlayerData *restoredPlayers,
    int32_t nPlayers,
    const char *pszServerCallSign,
    int32_t *pnOtherObjNum,
    int32_t *pnServerObjNum) {
    int32_t i, nServerPlayer = -1, nOtherObjNum = -1, nServerObjNum = -1;
...
void CSaveGameManager::LoadPlayer(CPlayerData *pPlayer) {
    ...
    pPlayer->nObject = m_cf.ReadInt(); // What CObject number this CPlayerData is. (made an int32_t by mk because it's
                                       // very often referenced)
...
int32_t CSaveGameManager::LoadSpawnPoint(int32_t i) {
    ...
    return (gameData.multiplayer.playerInit[i].nSegment >= 0) &&
           ...
           (gameData.multiplayer.playerInit[i].nSegment == FindSegByPos(
...
int32_t CSaveGameManager::LoadUniFormat(int32_t bMulti, fix xOldGameTime, int32_t *nLevel) {
    ...
    if (!m_bBetweenLevels) {
        ...
        for (i = 0, pWall = WALLS.Buffer(); i < gameData.wallData.nWalls; i++, pWall++) {
            ...
            if (pWall->nType == WALL_OPEN)
                audio.DestroySegmentSound(
                    (int16_t)pWall->nSegment,
                    (int16_t)pWall->nSide,
                    -1); //-1 means kill any sound
        }
        ...
        gameData.laserData.xLastFiredTime = gameData.missileData.xLastFiredTime = gameData.timeData.xGame;
    }
    ...
    if (m_bSecret != 1)
        gameData.omegaData.xCharge[0] = gameData.omegaData.xCharge[1] = m_cf.ReadFix();
    else
        m_cf.ReadFix();
    if (m_nVersion > 27)
    ...
    *nLevel = nCurrentLevel;
    ...
    if (LOCALPLAYER.numRobotsLevel >
        LOCALPLAYER.numKillsLevel + CountRobotsInLevel()) // fix for a bug affecting savegames
    ...
    if (!m_bBetweenLevels) {
        ...
        if (m_nVersion >= 26) {
            ...
            if (gameData.trigData.m_nTriggers[1] > 0) {
                ...
                m_cf.Seek(
                    ((m_nVersion > 39) ? LEVEL_OBJECTS : (m_nVersion > 34) ? MAX_OBJECTS_D2X : 700) * sizeof(int16_t),
                    SEEK_CUR);
        }
        // Restore tmap info
        ...
        for (i = 0; i < gameData.producerData.nRobotMakers; i++) {
            ...
            m_cf.Read(
                &gameData.producerData.robotMakers[i].xHitPoints,
                sizeof(tObjectProducerInfo) -
                    (reinterpret_cast<char *>(&gameData.producerData.robotMakers[i].xHitPoints) -
                     reinterpret_cast<char *>(&gameData.producerData.robotMakers[i])),
                1);
        }
...
int32_t CSaveGameManager::GetGameId(char *filename, int32_t bSecret) {
    ...
    m_cf.Seek(
        DESC_LENGTH + ImageSize() + 768 + 9 * sizeof(char) + (5 + (m_nVersion >= 55)) * sizeof(int32_t),
        SEEK_CUR);
