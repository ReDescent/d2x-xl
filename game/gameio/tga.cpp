...
int32_t CTGAHeader::Read(CFile &cf, CBitmap *pBm) {
    ...
    return (m_data.width <= 4096) && (m_data.height <= 32768) && (m_data.xStart < m_data.width) &&
           (m_data.yStart < m_data.height) &&
...
#pragma omp for reduction(+ : nVisible)
                    for (int32_t i = 0; i < j; i++) {
                        if (bSwapRB)
                            ::Swap(p[i].Red(), p[i].Blue());
                        if (bGrayScale) {
                            p[i].Red() = p[i].Green() = p[i].Blue() = uint8_t(
                                (int32_t(p[i].Red()) + int32_t(p[i].Green()) + int32_t(p[i].Blue())) / 3 * brightness);
                ...
                for (i = j; i; i--, p++) {
                    ...
                    if (bGrayScale) {
                        p->Red() = p->Green() = p->Blue() =
                            uint8_t((int32_t(p->Red()) + int32_t(p->Green()) + int32_t(p->Blue())) / 3 * brightness);
...
int32_t CTGA::Write(void) { return m_header.Write(m_cf, m_pBm) && WriteData(); }
...
int32_t
CTGA::ReadImage(const char *pszFile, const char *pszFolder, int32_t alpha, double brightness, int32_t bGrayScale) {
...
CBitmap *CTGA::ReadModelTexture(const char *pszFile, int32_t bCustom) {
    ...
    if (!(Read(
              pszFile,
              bCustom ? gameFolders.mods.szModels[bCustom - 1] : gameFolders.game.szModels,
              -1,
              1.0,
              0,
              false) ||
        ...
        return NULL;
    UseBitmapCache(m_pBm, int32_t(m_pBm->Height()) * int32_t(m_pBm->RowSize()));
    if (gameStates.app.bCacheTextures && (nShrinkFactor > 1) && (m_pBm->Width() == 512) &&
        Shrink(nShrinkFactor, nShrinkFactor, 1)) {
        strcat(fn, ".tga");
