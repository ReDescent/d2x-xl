...
void ConquerRoom(int32_t newOwner, int32_t oldOwner, int32_t roomId) {
    ...
    for (i = 0, j = jj = 0, k = kk = MAX_FUEL_CENTERS, pSeg = SEGMENTS.Buffer(); i < gameData.segData.nSegments;
         i++, pSeg++) {
    ...
    if (extraGameInfo[1].entropy.bRevertRooms && (jj + (MAX_FUEL_CENTERS - kk)) && ((j > jj) || (k < kk))) {
        if (kk < MAX_FUEL_CENTERS) {
            ...
            for (j = 0; j < jj; j++) {
                ...
                pFuelSeg->m_function =
                    gameData.producerData.origProducerTypes[uint32_t(pFuel - gameData.producerData.producers.Buffer())];
...
void StartConquerWarning(void) {
    if (extraGameInfo[1].entropy.bDoCaptureWarning) {
        gameStates.sound.nConquerWarningSoundChannel =
