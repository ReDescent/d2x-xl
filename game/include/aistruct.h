...
//#define PA_PLAYER_VISIBLE         2   // Level of robot awareness if robot is looking towards player, and player not
//hidden
...
#define SUB_FLAGS_CAMERA_AWAKE \
    0x04 // If set, a camera (on a missile) woke this robot up, so don't fire at player.  Can look real stupid!
...
typedef struct tAILocalInfo {
    ...
    // -- MK, 10/21/95, unused -- fix     last_seeTime, last_attackTime; // For sound effects, time at which player last
    // seen, attacked
    ...
    fix timeTargetSeen; // absolute time in seconds at which player was last seen, might cause to go into follow_path
                        // mode
