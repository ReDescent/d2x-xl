...
#define AM_SHOW_PLAYER(_i) \
    (!IsMultiGame || IsCoopGame || (netGameInfo.m_info.gameFlags & NETGAME_FLAG_SHOW_MAP) || \
...
class CAutomap {
    ...
    inline int32_t Visible(int32_t nSegment) {
        return m_bFull || m_visited[nSegment] || (OBSERVING && IsMultiGame && !IsCoopGame);
    }
    int32_t Active(void);
#if DBG
