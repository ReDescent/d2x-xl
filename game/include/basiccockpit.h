...
#define SHOW_COCKPIT \
    ((gameStates.render.cockpit.nMode == CM_FULL_COCKPIT) || (gameStates.render.cockpit.nMode == CM_STATUS_BAR))
#define SHOW_HUD \
    (!gameStates.app.bEndLevelSequence && \
     (!gameStates.app.bNostalgia || gameOpts->render.cockpit.bHUD || !SHOW_COCKPIT))
#define HIDE_HUD \
    (gameStates.app.bEndLevelSequence || (!(gameStates.app.bNostalgia || gameOpts->render.cockpit.bHUD) && \
...
class CGenericCockpit {
    ...
    void CGenericCockpit::DrawTimerCount(void) void DrawNumericalDisplay(int shield, int energy);
    void DrawKeys(void);
