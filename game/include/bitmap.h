...
class CBitmap : public CArray<uint8_t> {
    ...
    inline CBitmap *HasParent(void) { return (m_info.nType == BM_TYPE_STD) ? m_info.pParent : NULL; }
    ...
    inline void SetColor(CFloatVector *pColor = NULL, int32_t nColors = 1) {
        m_info.pColor = pColor, m_info.nColors = nColors;
    }
    ...
    inline uint8_t FrameCount(void) {
        return ((m_info.nType != BM_TYPE_ALT) && Parent()) ? m_info.pParent->FrameCount() : m_info.frames.nCount;
    }
    ...
    inline int32_t FrameSize(void) {
        return static_cast<int32_t>(m_info.props.h) * static_cast<int32_t>(m_info.props.rowSize);
    }
    inline void SetKey(uint16_t nId) { m_info.nId = nId; }
    void SetName(const char *pszName);
    inline void SetWidth(int16_t w) {
        m_info.props.w = w;
        m_info.props.rowSize = w * m_info.nBPP;
    }
    ...
    inline void SetBPP(uint8_t nBPP) {
        m_info.nBPP = nBPP;
        m_info.props.rowSize = m_info.props.w * m_info.nBPP;
    }
    ...
    inline void
    Render(CRectangle *pDest, int32_t bTransp = 0, int32_t bMipMaps = 0, int32_t bSmoothe = 0, float fAlpha = 1.0f) {
        Render(
            pDest,
            0,
            0,
