...
class CCamera : public CCanvas {
    ...
    inline GLuint DrawBuffer(void) {
        return m_data.bShadowMap ? FrameBuffer().DepthBuffer() : FrameBuffer().ColorBuffer();
    }
...
class CCameraManager {
    ...
    inline CCamera *Add(void) {
        return ((m_cameras.Buffer() || Create()) && m_cameras.Grow()) ? m_cameras.Top() : NULL;
    }
    ...
    inline CDynLight *ShadowLightSource(int32_t i) {
        return (m_shadowMaps[i].nCamera < 0) ? NULL : m_shadowMaps[i].pLight;
    }
