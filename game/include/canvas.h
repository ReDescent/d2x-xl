...
class CViewport : public CRectangle {
    ...
    CViewport(int32_t x = 0, int32_t y = 0, int32_t w = 0, int32_t h = 0, int32_t t = 0)
        : CRectangle(x, y, w, h),
          m_t(t) {}
...
class CGameScreenData : public CCanvas {
    public:
    void Set(int16_t l, int16_t t, int16_t w, int16_t h) {
        CViewport::SetLeft(l), CViewport::SetTop(t), CViewport::SetWidth(w), CViewport::SetHeight(h);
    }
...
class CScreen : public CGameScreenData {
    ...
    inline int16_t Width(bool bScale = true) {
        return bScale ? (int16_t)FRound(m_info.width * GetScale()) : m_info.width;
    }
    inline int16_t Height(bool bScale = true) {
        return bScale ? (int16_t)FRound(m_info.height * GetScale()) : m_info.height;
    }
