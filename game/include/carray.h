...
template <class _T> class CArray : public CQuickSort<_T> {
    ...
    template <class _U> class CArrayData {
        public:
        char szName[256];
        _U *buffer;
        _U null;
    ...
    public:
    template <class _V> class Iterator {
        private:
        _V *m_start;
        _V *m_end;
        _V *m_p;
    ...
    inline bool IsElement(_T *elem, bool bDiligent = false) {
        ...
        if (bDiligent) {
            uint32_t i =
                static_cast<uint32_t>(reinterpret_cast<uint8_t *>(elem) - reinterpret_cast<uint8_t *>(m_data.buffer));
            if (i % sizeof(_T))
    ...
    _T &Copy(CArray<_T> const &source, uint32_t offset = 0) {
        ...
        if (((static_cast<int32_t>(m_data.length)) >= 0) && (static_cast<int32_t>(source.m_data.length) > 0)) {
            ...
            if ((m_data.buffer && (m_data.length >= source.m_data.length + offset)) ||
                Resize(source.m_data.length + offset, false)) {
                memcpy(
                    m_data.buffer + offset,
                    source.m_data.buffer,
                    ((m_data.length - offset < source.m_data.length) ? m_data.length - offset : source.m_data.length) *
                        sizeof(_T));
            }
        }
        return m_data.buffer[0];
    ...
    inline int32_t BinSearch(_T key, int32_t left = 0, int32_t right = -1) {
        return m_data.buffer
                   ? CQuickSort<_T>::BinSearch(m_data.buffer, left, (right >= 0) ? right : m_data.length - 1, key)
                   : -1;
    }
};
...
template <class _T, uint32_t length> class CStaticArray : public CArray<_T> {

    template <class _U, uint32_t _length> class CStaticArrayData {
        public:
        _U buffer[_length];
    };
