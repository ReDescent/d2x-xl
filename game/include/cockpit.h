...
class CGenericCockpit {
    ...
    inline float Aspect(void) {
        return float(gameData.renderData.screen.Height()) / float(gameData.renderData.screen.Width()) / 0.75f;
    }
    ...
    inline void PageInGauge(int32_t nGauge) {
        LoadTexture(gameData.cockpitData.gauges[!gameStates.render.fonts.bHires][nGauge].index, 0, 0);
    }
    inline uint16_t GaugeIndex(int32_t nGauge) {
        return gameData.cockpitData.gauges[!gameStates.render.fonts.bHires][nGauge].index;
    }
    CBitmap *BitBlt(
        int32_t nGauge,
        int32_t x,
    ...
    inline bool ShowAlways(void) {
        return (gameStates.render.cockpit.nType == CM_FULL_COCKPIT) ||
               (gameStates.render.cockpit.nType == CM_STATUS_BAR);
    }

    inline bool Show(void) {
        return !gameStates.app.bEndLevelSequence &&
               (!gameStates.app.bNostalgia || gameOpts->render.cockpit.bHUD || !ShowAlways());
    }

    inline bool Hide(void) {
        return (gameStates.app.bEndLevelSequence >= EL_LOOKBACK) ||
               (!(gameStates.app.bNostalgia || gameOpts->render.cockpit.bHUD) &&
