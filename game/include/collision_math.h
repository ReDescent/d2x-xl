...
class CFixVector2 {
    ...
    inline fix Cross(CFixVector2 &other) {
        return fix(double(m_x) / 65536.0 * double(other.m_y) - double(m_y) / 65536.0 * double(other.m_x));
    }
};
...
class CHitQuery {
    ...
    public:
    CHitQuery() : flags(0), p0(NULL), p1(NULL), nSegment(-1), nObject(-1), radP0(0), radP1(0), bIgnoreObjFlag(0) {}
    CHitQuery(
        int32_t flags,
        CFixVector *p0,
        CFixVector *p1,
...
class CPhysSimData {
    ...
    explicit CPhysSimData(int16_t nObject = -1)
        : nObject(nObject),
          bUpdateOffset(1),
          bStopped(0),
          bBounced(0),
...
int32_t FindPlaneLineIntersection(
    ...
    CFixVector *vPlanePoint,
    CFixVector *vPlaneNorm,
    CFixVector *p0,
    CFixVector *p1,
    fix rad);

int32_t FindPlaneLineIntersectionf(
    CFloatVector &intersection,
    CFloatVector &vPlanePoint,
    CFloatVector &vPlaneNorm,
    CFloatVector &p0,
    CFloatVector &p1,
    float rad);
...
static inline fix RegisterHit(
    CFixVector *vBestHit,
    CFixVector *vBestNormal,
    CFixVector *vCurHit,
    CFixVector *vCurNormal,
    CFixVector *vRef,
    fix &dMin) {
    fix d = CFixVector::Dist(*vRef, *vCurHit);
