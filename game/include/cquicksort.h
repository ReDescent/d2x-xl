...
template <class _T> class CQuickSort {
    public:
...
template <typename _T> inline void CQuickSort<_T>::Swap(_T *left, _T *right) {
    _T h = *left;
    *left = *right;
    *right = h;
}

template <typename _T> void CQuickSort<_T>::SortAscending(_T *buffer, int32_t left, int32_t right) {
...
template <typename _T> void CQuickSort<_T>::SortDescending(_T *buffer, int32_t left, int32_t right) {
    int32_t l = left, r = right;
    _T median = buffer[(l + r) / 2];
...
template <typename _T> void CQuickSort<_T>::SortAscending(_T *buffer, int32_t left, int32_t right, comparator compare) {
    int32_t l = left, r = right;
    _T median = buffer[(l + r) / 2];
...
template <typename _T> int32_t CQuickSort<_T>::BinSearch(_T *buffer, int32_t l, int32_t r, _T key) {
    int32_t m;
