...
class CString : public CArray<char> {
    ...
    CString(const CString &other) : CArray<char>(other) { m_length = other.Length(); }
    ...
    inline const bool operator==(CString const &other) { return strcmp(Buffer(), other.Buffer()) == 0; }

    inline bool operator!=(CString const &other) { return strcmp(Buffer(), other.Buffer()) != 0; }
