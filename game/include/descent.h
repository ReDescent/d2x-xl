...
#define DEFAULT_CONTROL_CENTER_EXPLOSION_TIME \
    30 // Note: Usually uses Alan_pavlish_reactorTimes, but can be overridden in editor.
...
class CRenderStates {
    ...
    inline void ResetOutlineColor(void) { SetOutlineColor(); }
    ...
    inline int32_t DisableCartoonStyle(void) { return SetCartoonStyle(0); }

    inline int32_t CartoonStyle(void) { return bCartoonize; }
...
class CObjRenderList {
    public:
    CStaticArray<int16_t, MAX_SEGMENTS_D2X>
        ref; // [MAX_SEGMENTS_D2X];	//points to each segment's first render object list entry in renderObjs
...
class CRenderData {
    ...
    CGameScreenData
        frame; // current part of the screen being rendered too (usually entire screen or left and right halves of it)
...
class CSegDistHeader {
    ...
    inline bool Read(CFile &cf) {
        return (cf.Read(&offset, sizeof(offset), 1, 0) == 1) && (cf.Read(&length, sizeof(length), 1, 0) == 1) &&
               (cf.Read(&scale, sizeof(scale), 1, 0) == 1);
    }

    inline bool Write(CFile &cf) {
        return (cf.Write(&offset, sizeof(offset), 1, 0) == 1) && (cf.Write(&length, sizeof(length), 1, 0) == 1) &&
...
#define TEXTURE_ID_MASK \
    0x3FFF // mask off the part of a texture number containing the actual id. The upper 2 bits contain the orientation
           // (0 / 90 / 180 / 270 deg)
...
class CSegmentData {
    ...
    inline int32_t SegVisIdx(int32_t i, int32_t j) { return TRIAMATIDX(i, j); }
...
#pragma omp atomic
    ...
    inline bool BuildSegmentGrid(int32_t nSize, int32_t bSkyBox) {
        return segmentGrids[bSkyBox].Create(nSize, bSkyBox);
    }
    inline int32_t GetSegList(CFixVector vPos, int16_t *&listP, int32_t bSkyBox) {
        return segmentGrids[bSkyBox].GetSegList(vPos, listP);
    }
...
#else
    inline void Connect(int32_t nPlayer, int8_t nStatus) { players[nPlayer].Connect(nStatus); }
#endif
...
class CApplicationData {
    ...
    inline void SetGameMode(int32_t nMode) { nGameMode = nMode; }
};
...
#define PROF_TOGGLE \
    if (gameData.profilerData.bToggle) { \
...
class CGameData {
    ...
    CObject *
    Object(int32_t nObject, int32_t nChecks = GAMEDATA_ERRLOG_DEFAULT, const char *pszFile = "", int32_t nLine = 0);
    CSegment *
    Segment(int32_t nSegment, int32_t nChecks = GAMEDATA_ERRLOG_DEFAULT, const char *pszFile = "", int32_t nLine = 0);
    CWall *Wall(int32_t nWall, int32_t nChecks = GAMEDATA_ERRLOG_DEFAULT, const char *pszFile = "", int32_t nLine = 0);
    CTrigger *Trigger(
        int32_t nType,
        int32_t nTrigger,
        int32_t nChecks = GAMEDATA_ERRLOG_DEFAULT,
        const char *pszFile = "",
        int32_t nLine = 0);
    CTrigger *GeoTrigger(
        int32_t nTrigger,
...
#endif
    inline int32_t X(int32_t x, bool bForce = false) {
        return renderData.nStereoOffsetType
                   ? x - ((renderData.nStereoOffsetType == STEREO_OFFSET_FLOATING) ? FloatingStereoOffset2D(x, bForce)
                                                                                   : StereoOffset2D())
                   : x;
    }
};
...
#define SPECTATOR(_objP) \
    ((gameStates.render.bFreeCam > 0) && OBJECTS.IsElement(_objP) && (OBJ_IDX(_objP) == LOCALPLAYER.nObject))
...
#define SEM_ENTER(_sem) \
    if (gameStates.app.bMultiThreaded) \
        SemEnter(_sem, __FILE__, __LINE__);

#define SEM_LEAVE(_sem) \
    if (gameStates.app.bMultiThreaded) \
        SemLeave(_sem, __FILE__, __LINE__);
...
inline CTrigger *CGameData::Trigger(int32_t nType, int32_t nTrigger) {
    return ((nTrigger < 0) || (nTrigger == NO_TRIGGER) || (nTrigger >= trigData.m_nTriggers[nType]))
               ? NULL
               : trigData.triggers[nType] + nTrigger;
}

inline CTrigger *CGameData::GeoTrigger(int32_t nTrigger) { return Trigger(0, nTrigger); }

inline CTrigger *CGameData::ObjTrigger(int32_t nTrigger) { return Trigger(1, nTrigger); }
