...
class CDynLight {
    ...
    int32_t SeesPoint(
        ...
        int32_t nThread);
    int32_t
    SeesPoint(const int16_t nSegment, const int8_t nSide, CFixVector *vPoint, const int32_t nLevel, int32_t nThread);
    int32_t LightPathLength(
    ...
    inline int32_t Illuminate(int16_t nSegment, int16_t nSide) {
        return !info.bAmbient || (nSegment < 0) ||
               ((info.nSegment == nSegment) && ((nSide < 0) || (info.nSide == nSide)));
    }
    int32_t ComputeVisibleVertices(int32_t nThread);
    int32_t Compare(CDynLight &other);
    inline bool operator<(CDynLight &other) { return Compare(other) < 0; }
    inline bool operator>(CDynLight &other) { return Compare(other) > 0; }
    inline bool operator==(CDynLight &other) { return Compare(other) == 0; }
};
...
class CLightManager {
    ...
    int32_t
    SetActive(CActiveDynLight *pActiveLights, CDynLight *pLight, int16_t nType, int32_t nThread, bool bForce = false);
...
#define SHOW_DYN_LIGHT \
    (!(gameStates.app.bNostalgia || gameStates.render.bBriefing || \
       (gameStates.app.bEndLevelSequence >= EL_OUTSIDE)) && /*gameStates.render.bHaveDynLights &&*/ \
     gameOpts->render.nLightingMethod)

#define HAVE_DYN_LIGHT (gameStates.render.bHaveDynLights && SHOW_DYN_LIGHT)

#define APPLY_DYN_LIGHT (gameStates.render.bUseDynLight && (gameOpts->ogl.bLightObjects || gameStates.render.nState))
