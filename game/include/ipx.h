...
#define UDP_PAYLOAD_SIZE \
    (UDP_PACKET_SIZE - UDP_HEADER_SIZE - \
     sizeof(int32_t)) // the network thread will add its own 32 bit frame count to each data packet
...
class CNetworkAddress /*: public tNetworkAddress*/ {
    ...
    CNetworkAddress() {}
    CNetworkAddress(tNetworkAddress &address) { /**this*/
        m_address = address;
    }
    ...
    inline void SetNetwork(void *network) {
        memcpy(m_address.network.octets, (uint8_t *)network, sizeof(m_address.network));
    }
    ...
    inline void SetServer(void *ip) {
        memcpy(m_address.node.portAddress.ip.octets, (uint8_t *)ip, sizeof(m_address.node.portAddress.ip.octets));
    }
    inline void SetServer(uint32_t ip) { m_address.node.portAddress.ip.a = ip; }
    inline void SetPort(void *port) {
        memcpy(m_address.node.portAddress.port.b, (uint8_t *)port, sizeof(m_address.node.portAddress.port.b));
    }
    inline void SetPort(uint16_t port) { m_address.node.portAddress.port.p = port; }

    inline void ResetServer(uint8_t filler = 0) {
        memset(m_address.node.portAddress.ip.octets, filler, sizeof(m_address.node.portAddress.ip.octets));
    }
    inline void ResetNetwork(uint8_t filler = 0) {
        memset(m_address.network.octets, filler, sizeof(m_address.network));
    }
    inline void ResetNode(uint8_t filler = 0) { memset(m_address.node.b, filler, sizeof(m_address.node.b)); }
    inline void ResetPort(uint8_t filler = 0) {
        memset(m_address.node.portAddress.port.b, filler, sizeof(m_address.node.portAddress.port));
    }
    ...
    inline bool operator==(CNetworkAddress &other) {
        return ((m_address.node.portAddress.ip.octets[0] == 127) &&
                (other.m_address.node.portAddress.ip.octets[0] == 127))
    ...
    inline bool IsBroadcast(void) {
        return (GetNetwork() == 0xFFFFFFFF) && (GetServer() == 0xFFFFFFFF) && (GetPort() == 0xFFFF);
    }
...
class CNetworkInfo {
    ...
    inline void SetNetwork(void *network) {
        memcpy(m_info.address.network.octets, (uint8_t *)network, sizeof(m_info.address.network));
    }
    ...
    inline void SetServer(void *ip) {
        memcpy(
            m_info.address.node.portAddress.ip.octets,
            (uint8_t *)ip,
            sizeof(m_info.address.node.portAddress.ip.octets));
    }
    inline void SetServer(uint32_t ip) { m_info.address.node.portAddress.ip.a = ip; }
    inline void SetPort(void *port) {
        memcpy(m_info.address.node.portAddress.port.b, (uint8_t *)port, sizeof(m_info.address.node.portAddress.port.b));
    }
