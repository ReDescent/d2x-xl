...
class CControlConfig : public CMenu {
    ...
    inline int32_t ChangeKey(kcItem *item) {
        return ChangeControl(item, BT_KEY, &CControlConfig::KeyCtrlFunc, TXT_PRESS_NEW_KEY);
    }
    inline int32_t ChangeJoyButton(kcItem *item) {
        return ChangeControl(item, BT_JOY_BUTTON, &CControlConfig::JoyBtnCtrlFunc, TXT_PRESS_NEW_JBUTTON);
    }
    inline int32_t ChangeMouseButton(kcItem *item) {
        return ChangeControl(item, BT_MOUSE_BUTTON, &CControlConfig::MouseBtnCtrlFunc, TXT_PRESS_NEW_MBUTTON);
    }
    inline int32_t ChangeJoyAxis(kcItem *item) {
        return ChangeControl(item, BT_JOY_AXIS, &CControlConfig::JoyAxisCtrlFunc, TXT_MOVE_NEW_JOY_AXIS);
    }
    inline int32_t ChangeMouseAxis(kcItem *item) {
        return ChangeControl(item, BT_MOUSE_AXIS, &CControlConfig::MouseAxisCtrlFunc, TXT_MOVE_NEW_MSE_AXIS);
    }
