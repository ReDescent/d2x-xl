...
typedef uint32_t (*mve_cb_Read)(void *stream, void *buffer, uint32_t count);

typedef void *(*mve_cb_Alloc)(uint32_t size);
typedef void (*mve_cb_Free)(void *ptr);

typedef void (*mve_cb_ShowFrame)(
    uint8_t *buffer,
    uint32_t bufw,
    uint32_t bufh,
    uint32_t sx,
...
typedef void (*mve_cb_SetPalette)(uint8_t *p, uint32_t start, uint32_t count);
