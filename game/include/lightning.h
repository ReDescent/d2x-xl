...
class CLightningNode : public tLightningNode {
    ...
    CFixVector *
    Attract(CFixVector *vOffs, CFixVector *vAttract, CFixVector *vPos, int32_t nDist, int32_t i, int32_t bJoinPaths);
    CFixVector CreateJaggy(
        CFixVector *vPos,
        CFixVector *vDest,
        CFixVector *vBase,
        CFixVector *vPrevOffs,
        int32_t nSteps,
        int32_t nAmplitude,
        int32_t nMinDist,
        int32_t i,
    ...
    void Move(
        const CFixVector &vOldPos,
        const CFixVector &vOldEnd,
        const CFixVector &vNewPos,
        const CFixVector &vNewEnd,
        float fScale,
        int16_t nSegment);
    ...
    inline fix Offset(CFixVector &vStart, CFixVector &vEnd) { return VmLinePointDist(vStart, vEnd, m_vNewPos); }
};
...
class CLightning : public tLightning {
    ...
    int32_t
    ComputeChildEnd(CFixVector *vPos, CFixVector *vEnd, CFixVector *vDir, CFixVector *vParentDir, int32_t nLength);
...
class CLightningManager : public tLightningData {
    ...
    inline int16_t GetObjectSystem(int16_t nObject) {
        return (m_objects.Buffer() && (nObject >= 0)) ? m_objects[nObject] : -1;
    }
    inline void SetObjectSystem(int16_t nObject, int32_t i) {
        if (m_objects.Buffer() && (nObject >= 0))
            m_objects[nObject] = i;
    }
...
#define SHOW_LIGHTNING(_nQuality) \
    (gameOpts->render.effects.bEnabled && !(gameStates.app.bNostalgia || COMPETITION) && \
     (EGI_FLAG(bUseLightning, 1, 1, 0) >= (_nQuality)))
