...
#define WORLDSCALE \
    1000 // one world unit (65536 in fixed point arithmetic) corresponds to 1000 mm (1 m) of the real world
...
static inline int32_t Rand(int32_t nMax) { return int32_t(rand()) % nMax; }
...
static inline int32_t RandShort(void) { return Rand(SHORT_RAND_MAX + 1); }

//-----------------------------------------------------------------------------
// return a random integer in the range [-SHORT_RAND_MAX / 2 .. +SHORT_RAND_MAX / 2]

static inline int32_t SRandShort(void) { return SHORT_RAND_MAX / 2 - RandShort(); }

//-----------------------------------------------------------------------------
// return a random float in the range [0.0f .. 1.0f]

static inline float RandFloat(float scale = 1.0f) { return float(rand()) / (float(RAND_MAX) * scale); }

//------------------------------------------------------------------------------
// return a random double in the range [0.0 .. 1.0]

inline double RandDouble(void) { return double(rand()) / double(RAND_MAX); }

//-----------------------------------------------------------------------------

template <typename _T> inline void Swap(_T &a, _T &b) {
    _T h = a;
    a = b;
    b = h;
}

template <typename _T> inline const _T Clamp(const _T v, const _T vMin, const _T vMax) {
    return (v < vMin) ? vMin : (v > vMax) ? vMax : v;
}

template <typename _T> inline const _T Min(const _T a, const _T b) { return (a <= b) ? a : b; }

template <typename _T> inline const _T Max(const _T a, const _T b) { return (a >= b) ? a : b; }
