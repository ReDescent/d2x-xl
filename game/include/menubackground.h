...
class CBackground : public CCanvas {
    ...
    inline void SetBoxColor(uint8_t red = PAL2RGBA(22), uint8_t green = PAL2RGBA(22), uint8_t blue = PAL2RGBA(38)) {
        m_boxColor.Set(red, green, blue);
    }
...
class CBackgroundManager : public CStack<CBackground> {
    ...
    bool
    Setup(CBackground &bg, int32_t width, int32_t height, int32_t nType = BG_SUBMENU, int32_t nWallPaper = BG_STANDARD);
    ...
    void
    DrawBox(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t nLineWidth, float fAlpha, int32_t bForce);
