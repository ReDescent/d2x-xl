...
#define MISSION_FOLDER \
    ((gameOpts->app.bSinglePlayer && !gameStates.app.bPrecomputeLightmaps) ? BASE_MISSION_FOLDER "single/" \
                                                                           : BASE_MISSION_FOLDER)
...
class CMissionManager : public CMissionData {
    ...
    inline int32_t GetLevelState(int32_t nLevel) {
        return ((nLevel < 1) || (nLevel > nLastLevel))
                   ? 0
                   : nLevelState[nLevel]; // 0: not yet been there, -1: destroyed, 1: been there already
    }
