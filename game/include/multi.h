...
#define NETGAME_FLAG_MONSTERBALL \
    (NETGAME_FLAG_HOARD | NETGAME_FLAG_ENTROPY) // ugly hack, but we only have a single byte ... :-/
...
#pragma pack(push, 1)
enum compType { DOS, WIN_32, WIN_95, MAC };
...
class CAllNetPlayersInfo {
    ...
    inline size_t Size(void) {
        return sizeof(m_info) - sizeof(m_info.players) + sizeof(tNetPlayerInfo) * (MAX_NUM_NET_PLAYERS + 4);
    }
...
class CNetGameInfo {
    ...
    inline size_t Size(void) {
        return (gameStates.multi.nGameType == UDP_GAME)
                   ? sizeof(tNetGameInfo)
                   : sizeof(tNetGameInfo) - sizeof(tNetGameInfoD2X) + sizeof(tNetGameInfoD2);
    }
    ...
    inline int32_t *Locations(int32_t i = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.locations + i : m_info.versionSpecific.d2.locations + i;
    }
    inline int16_t *Kills(int32_t i = 0, int32_t j = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.kills[i] + j : m_info.versionSpecific.d2.kills[i] + j;
    }
    inline int16_t *TeamKills(int32_t i = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.teamKills + i : m_info.versionSpecific.d2.teamKills + i;
    }
    inline int16_t *Killed(int32_t i = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.killed + i : m_info.versionSpecific.d2.killed + i;
    }
    inline int16_t *PlayerKills(int32_t i = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.playerKills + i : m_info.versionSpecific.d2.playerKills + i;
    }
    inline int32_t *PlayerScore(int32_t i = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.playerScore + i : m_info.versionSpecific.d2.playerScore + i;
    }
    inline uint8_t *PlayerFlags(int32_t i = 0) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.playerFlags + i : m_info.versionSpecific.d2.playerFlags + i;
    }
    inline uint8_t *AuxData(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.auxData : m_info.versionSpecific.d2.auxData;
    }

    inline uint16_t GetSegmentCheckSum(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.nSegmentCheckSum : m_info.versionSpecific.d2.nSegmentCheckSum;
    }
    inline int32_t GetScoreGoal(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.nScoreGoal : m_info.versionSpecific.d2.nScoreGoal;
    }
    inline fix GetPlayTimeAllowed(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.xPlayTimeAllowed : m_info.versionSpecific.d2.xPlayTimeAllowed;
    }
    inline fix GetLevelTime(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.xLevelTime : m_info.versionSpecific.d2.xLevelTime;
    }
    inline int32_t GetControlInvulTime(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.controlInvulTime : m_info.versionSpecific.d2.controlInvulTime;
    }
    inline int32_t GetMonitorVector(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.monitorVector : m_info.versionSpecific.d2.monitorVector;
    }
    inline int16_t GetMinPPS(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.nMinPPS : m_info.versionSpecific.d2.nMinPPS;
    }
    inline uint8_t GetShortPackets(void) {
        return IsUdpGame() ? m_info.versionSpecific.d2x.bShortPackets : m_info.versionSpecific.d2.bShortPackets;
    }

    inline void SetSegmentCheckSum(uint16_t n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.nSegmentCheckSum = n;
        else
            m_info.versionSpecific.d2.nSegmentCheckSum = n;
    }
    inline void SetScoreGoal(int32_t n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.nScoreGoal = n;
        else
            m_info.versionSpecific.d2.nScoreGoal = n;
    }
    inline void SetPlayTimeAllowed(fix n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.xPlayTimeAllowed = n;
        else
            m_info.versionSpecific.d2.xPlayTimeAllowed = n;
    }
    inline void SetLevelTime(fix n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.xLevelTime = n;
        else
            m_info.versionSpecific.d2.xLevelTime = n;
    }
    inline void SetControlInvulTime(int32_t n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.controlInvulTime = n;
        else
            m_info.versionSpecific.d2.controlInvulTime = n;
    }
    inline void SetMonitorVector(int32_t n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.monitorVector = n;
        else
            m_info.versionSpecific.d2.monitorVector = n;
    }
    inline void SetMinPPS(int16_t n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.nMinPPS = n;
        else
            m_info.versionSpecific.d2.nMinPPS = n;
    }
    inline void SetShortPackets(uint8_t n) {
        if (IsUdpGame())
            m_info.versionSpecific.d2x.bShortPackets = 0;
        else
            m_info.versionSpecific.d2.bShortPackets = n;
    } // always long packets for UDP/IP
};
...
// Reserve space for the message id in the message. This macro will place the message id right after the one byte
// message type in the message buffer. pBuffer must point to the byte in the message buffer following the player id. The
// variable _msgId is created and points to the first byte of the reserved space in the message buffer.
#define ADD_MSG_ID \
    uint8_t *_msgIdP = gameData.multigame.msg.buf + pBuffer - 1; \
...
// Check whether the message with the message id contained in it has already been processed. If yes, exit the processing
// function. If no, add the sender's player id and the message id to the list of important messages the receipt of which
// needs to be confirmed and send a receipt confirmation. That list entry is subsequently being used to determine
// whether a message has already been processed and has been resent by its originator because it hasn't received a
// receipt confirmation for it from all game participants
#define CHECK_MSG_ID \
    if (!MultiProcessMessage(buf + pBuffer)) \
        return; \
    pBuffer += 4;

// Same as above, just returns the value specified by _r
#define CHECK_MSG_ID_RVAL(_r) \
    if (!MultiProcessMessage(buf + pBuffer)) \
        return (_r); \
    pBuffer += 4;
