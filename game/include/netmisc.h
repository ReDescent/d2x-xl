...
#define SendBroadcastNetPlayersPacket() BESendNetPlayersPacket(NULL, NULL)
#define SendInternetPlayerSyncData(playerSyncData, server, node) \
    BESendSequencePacket(playerSyncData, server, node, NULL)
#define SendBroadcastPlayerSyncData(playerSyncData) BESendSequencePacket(playerSyncData, NULL, NULL, NULL)
#define SendFullNetGamePacket(server, node, netAddress) BESendNetGamePacket(server, node, netAddress, 0)
#define SendLiteNetGamePacket(server, node, netAddress) BESendNetGamePacket(server, node, netAddress, 1)
#define SendInternetFullNetGamePacket(server, node) BESendNetGamePacket(server, node, NULL, 0)
#define SendInternetLiteNetGamePacket(server, node) BESendNetGamePacket(server, node, NULL, 1)
#define SendBroadcastFullNetGamePacket() BESendNetGamePacket(NULL, NULL, NULL, 0)
#define SendBroadcastLiteNetGamePacket() BESendNetGamePacket(NULL, NULL, NULL, 1)
#define ReceiveFullNetGamePacket(data, netgame) BEReceiveNetGamePacket(data, netgame, 0)
#define ReceiveLiteNetGamePacket(data, netgame) BEReceiveNetGamePacket(data, netgame, 1)

#define SendExtraGameInfoPacket(server, node, netAddress) BESendExtraGameInfo(server, node, netAddress)
#define SendInternetExtraGameInfoPacket(server, node) BESendExtraGameInfo(server, node, NULL)
#define SendBroadcastExtraGameInfoPacket() BESendExtraGameInfo(NULL, NULL, NULL)
#define ReceiveExtraGameInfoPacket(data, _extraGameInfo) \
    BEReceiveExtraGameInfo(data, _extraGameInfo); \
    AddPlayerLoadout(false)

#define SendBroadcastMissingObjFramesPacket() BESendMissingObjFrames(NULL, NULL, NULL)
#define ReceiveMissingObjFramesPacket(data, missingObjFrames) \
    BEReceiveMissingObjFrames((uint8_t *)data, missingObjFrames);
#define SendInternetMissingObjFramesPacket(server, node) BESendMissingObjFrames(server, node, NULL)

#else

#define ReceiveNetPlayersPacket(data, pinfo) memcpy(pinfo, data, int32_t(netPlayers[0].Size()))
#define SendNetPlayersPacket(server, node) \
    networkThread.Send((uint8_t *)&netPlayers[0], int32_t(netPlayers[0].Size()), server, node)
#define SendBroadcastNetPlayersPacket() IPXSendBroadcastData((uint8_t *)&netPlayers[0], int32_t(netPlayers[0].Size()))
...
#define SendBroadcastLiteNetGamePacket() IPXSendBroadcastData((uint8_t *)&netGameInfo.m_info, sizeof(tNetGameInfoLite))
#define ReceiveFullNetGamePacket(data, netgame) memcpy(&(netgame)->m_info, data, (netgame)->Size())
#define ReceiveLiteNetGamePacket(data, netgame) memcpy(&(netgame)->m_info, data, sizeof(tNetGameInfoLite))
...
#define SendBroadcastExtraGameInfoPacket() IPXSendBroadcastData((uint8_t *)(extraGameInfo + 1), sizeof(tExtraGameInfo))
#define ReceiveExtraGameInfoPacket(data, _extraGameInfo) \
    memcpy((uint8_t *)(_extraGameInfo), data, sizeof(tExtraGameInfo)); \
    AddPlayerLoadout(false)
