...
static inline int32_t CollisionModel(int32_t nCollisionModel = -1) {
    return (nCollisionModel < 0) ? missionConfig.m_nCollisionModel
                                       ? COLLISION_MODEL_BOXES
                                       : EGI_FLAG(nHitboxes, 0, 0, extraGameInfo[0].nHitboxes)
                                 : nCollisionModel;
}
...
class CEndLevelInfo {
    ...
    inline uint8_t *SecondsLeft(void) {
        return gameStates.app.bD2XLevel ? &m_info.d2x.secondsLeft : &m_info.d2.secondsLeft;
    }
    inline int16_t *ScoreMatrix(int32_t i = 0, int32_t j = 0) {
        return gameStates.app.bD2XLevel ? m_info.d2x.scoreMatrix[i] + j : m_info.d2.scoreMatrix[i] + j;
    }
