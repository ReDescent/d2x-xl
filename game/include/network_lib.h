...
#define NW_SET_SHORT(_b, _loc, _i) \
    *(reinterpret_cast<int16_t *>((_b) + (_loc))) = INTEL_SHORT((int16_t)(_i)); \
    (_loc) += 2
#define NW_GET_INT(_b, _loc, _i) \
    (_i) = INTEL_INT(*(reinterpret_cast<int32_t *>((_b) + (_loc)))); \
    (_loc) += 4
...
#define NW_GET_BYTES(_b, _loc, _p, _n) \
    memcpy(_p, (_b) + (_loc), _n); \
    (_loc) += (_n)
...
class CSyncPackLong : public CSyncPack {
    ...
    virtual void SetInfo(void *info) {
        memcpy(&m_info, info, HeaderSize() + reinterpret_cast<tFrameInfoLong *>(info)->data.dataSize);
    }
...
class CSyncPackShort : public CSyncPack {
    ...
    virtual tFrameInfoData &Data(void) { return m_info.data; }
    virtual void SetInfo(void *info) {
        memcpy(&m_info, info, HeaderSize() + reinterpret_cast<tFrameInfoShort *>(info)->data.dataSize);
    }
...
class CMineSyncData {
    ...
    inline int32_t MaxDataSize(void) {
        return (gameStates.multi.nGameType == UDP_GAME) ? UDP_PAYLOAD_SIZE - HeaderSize() : NET_XDATA_SIZE + 4;
    }
