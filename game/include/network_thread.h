...
class CNetworkPacketData {
    ...
    struct {
        int32_t nId;
        uint8_t buffer[MAX_PACKET_SIZE]; // buffer must be big enough to hold the amount of data read by the UDP
                                         // interface, which contains some additional info
    } m_data;
    ...
    inline bool operator==(CNetworkPacketData &other) {
        return (m_size == other.m_size) && !memcmp(Buffer(), other.Buffer(), m_size);
    }
};
...
class CNetworkPacket : public CNetworkPacketData {
    ...
    bool HasId(void);
    inline bool operator==(CNetworkPacket &other) {
        return (m_owner.m_address == other.m_owner.m_address) &&
               ((CNetworkPacketData &)*this == (CNetworkPacketData &)other);
    }
...
class CNetworkThread {
    ...
    bool Send(
        ...
        uint8_t *network,
        uint8_t *srcNode,
        uint8_t *destNode = NULL,
        int32_t bTrackerCall = 0);
