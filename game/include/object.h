...
#define OF_SILENT \
    8 // this makes no sound when it hits a CWall.  Added by MK for weapons, if you extend it to other types, do it
      // completely!
...
class CObject : public CObjectInfo {
    ...
    inline bool IsShot(CObject *pObj) {
        return (m_shots.nObject == pObj->Index()) && (m_shots.nSignature == pObj->Signature());
    }
    ...
    int32_t
    FindAnyHomingTarget(CFixVector &vTrackerPos, int32_t nTargetType1, int32_t nTargetType2, int32_t nThread = 0);
    ...
    inline bool IsProjectile(void) {
        return (Type() == OBJ_WEAPON) && (Id() < m_weaponInfo.Length()) && IsProjectile(Id());
    }
    inline bool IsEnergyProjectile(void) {
        return (Type() == OBJ_WEAPON) && (Id() < m_weaponInfo.Length()) && IsEnergyProjectile(Id());
    }
    inline bool HasLightTrail(void) {
        return (Type() == OBJ_WEAPON) && (Id() < m_weaponInfo.Length()) && HasLightTrail(Id());
    }
    ...
    inline bool IsEquipment(void) {
        return (Type() == OBJ_WEAPON) && (Id() < m_weaponInfo.Length()) && IsEquipment(Id());
    }
    ...
    void
    AddHomingTarget(CObject *pTarget, CFixVector *vTrackerPos, fix maxTrackableDist, fix &xBestDot, int32_t &nBestObj);
...
#define FORALL_PLAYER_OBJS(_objP) \
    for (CPlayerIterator playerIter((_objP)); !playerIter.Done(); (_objP) = playerIter.Step())
#define FORALL_ROBOT_OBJS(_objP) for (CRobotIterator robotIter((_objP)); !robotIter.Done(); (_objP) = robotIter.Step())
#define FORALL_WEAPON_OBJS(_objP) \
    for (CWeaponIterator weaponIter((_objP)); !weaponIter.Done(); (_objP) = weaponIter.Step())
#define FORALL_POWERUP_OBJS(_objP) \
    for (CPowerupIterator powerIter((_objP)); !powerIter.Done(); (_objP) = powerIter.Step())
#define FORALL_EFFECT_OBJS(_objP) \
    for (CEffectIterator effectIter((_objP)); !effectIter.Done(); (_objP) = effectIter.Step())
...
#define FORALL_STATIC_OBJS(_objP) \
    for (CStaticObjectIterator staticsIter((_objP)); !staticsIter.Done(); (_objP) = staticsIter.Step())
...
int32_t CreatePowerup(
    uint8_t nId,
    int16_t nCreator,
    int16_t nSegment,
    const CFixVector &vPos,
    int32_t bIgnoreLimits,
    bool bForce = false);
...
//               const CFixMatrix& orient, fix size, uint8_t ctype, uint8_t mtype, uint8_t rtype, int32_t
//               bIgnoreLimits);
