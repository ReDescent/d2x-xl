...
static inline int32_t FaceIsAdditive(CSegFace *pFace) {
    return (int32_t)(
        (pFace->m_info.bAdditive == 1) ? !(pFace->bmBot && pFace->bmBot->FromPog()) : pFace->m_info.bAdditive);
}

//------------------------------------------------------------------------------

typedef int32_t (
    *tFaceRenderFunc)(CSegFace *pFace, CBitmap *bmBot, CBitmap *bmTop, int32_t bTextured, int32_t bLightmaps);

extern tFaceRenderFunc faceRenderFunc;

static inline void SetFaceDrawer(int32_t nType) { faceRenderFunc = RenderFace; }
