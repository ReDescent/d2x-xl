...
//#define OGL_TEXCLAMP() OGL_ENABLE2(GL_texclamp,glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
//GL_CLAMP);glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);) #define OGL_TEXREPEAT()
//OGL_DISABLE2(GL_texclamp,glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);glTexParameteri(GL_TEXTURE_2D,
...
#define OGL_VIEWPORT(_x, _y, _w, _h) \
    { \
        if (((int)(_x) != ogl.m_states.nLastX) || ((int)(_y) != ogl.m_states.nLastY) || \
            ...
            glViewport( \
                (GLint)(_x), \
                (GLint)(grdCurScreen->scCanvas.cvBitmap.props.h - (_y) - (_h)), \
                (GLsizei)(_w), \
                (GLsizei)(_h)); \
...
// void ogl_loadtexture(ubyte * data, int width, int height,int dxo,int dyo, int *texid,double *u,double *v,char
// bMipMap,double prio);
...
void G3VertexColor(
    CFloatVector *pvVertNorm,
    CFloatVector *pVertPos,
    int nVertex,
    tFaceColor *pVertColor,
    tFaceColor *pBaseColor,
    float fScale,
    int bSetColor,
    int nThread);
...
typedef int
tTexPolyMultiDrawer(int, g3sPoint **, tUVL *, tUVL *, CBitmap *, CBitmap *, CTexture *, CFixVector *, int, int);
...
static inline int
G3DrawTexPoly(int nVerts, g3sPoint **pointList, tUVL *uvlList, CBitmap *bmP, CFixVector *pvNormal, int bBlend) {
