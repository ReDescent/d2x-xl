...
class COGL {
    ...
    int32_t RenderArrays(
        int32_t nPrimitive,
        CFloatVector *pVertex,
        int32_t nVertices,
        int32_t nDimensions = 3,
        tTexCoord2f *pTexCoord = NULL,
        CFloatVector *pColor = NULL,
        int32_t nColors = 1,
        CBitmap *pBm = NULL,
        int32_t nFrame = 0,
        int32_t nWrap = GL_REPEAT);
    int32_t RenderQuad(
        CBitmap *pBm,
        CFloatVector *pVertex,
        int32_t nDimensions = 3,
    ...
    inline bool SetPolyOffsetFill(bool bPolyOffsetFill) {
        return Enable(m_data.bPolyOffsetFill, bPolyOffsetFill, GL_POLYGON_OFFSET_FILL);
    }
    ...
    inline CFBO *DrawBuffer(int32_t nBuffer = -1) {
        return (nBuffer < 0) ? m_data.pDrawBuffer : m_data.GetDrawBuffer(nBuffer);
    }

    CFBO *BlurBuffer(int32_t nBuffer);

    inline CFBO *DepthBuffer(int32_t nBuffer = -1) {
        return (nBuffer < 0) ? m_data.pDrawBuffer : m_data.GetDepthBuffer(nBuffer + 5);
    }
...
#if DBG_OGL
    void
    VertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer, const char *pszFile, int32_t nLine);
    void
    ColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer, const char *pszFile, int32_t nLine);
    void
    TexCoordPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer, const char *pszFile, int32_t nLine);
