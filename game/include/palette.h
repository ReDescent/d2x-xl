...
class CPalette {
    ...
    inline int32_t ClosestColor(CFloatVector3 *pColor) {
        return ClosestColor(
            (int32_t)(pColor->Red() * 63.0f),
            (int32_t)(pColor->Green() * 63.0f),
            (int32_t)(pColor->Blue() * 63.0f));
    }
    ...
    inline bool operator==(CPalette &source) {
        return (TransparentColor() == source.TransparentColor()) && (SuperTranspColor() == source.SuperTranspColor()) &&
...
class CPaletteManager {
    ...
    CPalette *Load(
        const char *pszPaletteName,
        const char *pszLevelName,
        int32_t nUsedForLevel,
        int32_t bNoScreenChange,
        int32_t bForce);
    ...
    void Push(void) { m_save.Push(m_data.current); }
    void Pop(void) { m_data.current = m_save.Pop(); }
    ...
    inline int8_t RedEffect(bool bFade = false) {
        return int8_t(m_data.effect.Red() * 64.0f * (bFade ? FadeScale() : 1.0f));
    }
    inline int8_t GreenEffect(bool bFade = false) {
        return int8_t(m_data.effect.Green() * 64.0f * (bFade ? FadeScale() : 1.0f));
    }
    inline int8_t BlueEffect(bool bFade = false) {
        return int8_t(m_data.effect.Blue() * 64.0f * (bFade ? FadeScale() : 1.0f));
    }
    ...
    inline float FadeScale(void) {
        return m_data.xFadeDuration[1] ? (float)m_data.xFadeDuration[0] / (float)m_data.xFadeDuration[1] : 0.0f;
    }
};
