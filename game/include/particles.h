...
class CParticleEmitter : public tParticleEmitter {
    ...
    int32_t Create(
        ...
        int32_t nMaxParts,
        float fScale,
        /*int32_t nDensity, int32_t nPartsPerPos,*/ int32_t nLife,
        int32_t nSpeed,
        char nType,
        CFloatVector *pColor,
        int32_t nCurTime,
    ...
    inline bool IsAlive(int32_t nCurTime) { return (m_nLife < 0) || (m_nBirth + m_nLife > nCurTime); }
    inline bool IsDead(int32_t nCurTime) { return !(IsAlive(nCurTime) || m_nParts); }
...
class CParticleSystem : public tParticleSystem {
    ...
    inline CParticleEmitter *GetEmitter(int32_t i) { return m_emitters + i; }
...
class CParticleManager {
    ...
    inline int16_t GetObjectSystem(int16_t nObject, int32_t i = 0) {
        return m_objectSystems[i].Buffer() ? m_objectSystems[i][nObject] : -1;
    }

    inline CParticleEmitter *GetEmitter(int32_t i, int32_t j) { return GetSystem(i).GetEmitter(j); }
    ...
    inline void SetScale(int32_t i, float fScale) { GetSystem(i).SetScale(fScale); }

    inline void SetLife(int32_t i, int32_t nLife) { GetSystem(i).SetLife(nLife); }

    inline void SetBlowUp(int32_t i, int32_t bBlowUpParts) { GetSystem(i).SetBlowUp(bBlowUpParts); }

    inline void SetBrightness(int32_t i, int32_t nBrightness) { GetSystem(i).SetBrightness(nBrightness); }

    inline void SetFadeType(int32_t i, int32_t nFadeType) { GetSystem(i).SetFadeType(nFadeType); }

    inline void SetType(int32_t i, int32_t nType) { GetSystem(i).SetType(nType); }

    inline void SetSpeed(int32_t i, int32_t nSpeed) { GetSystem(i).SetSpeed(nSpeed); }

    inline void SetDir(int32_t i, CFixVector *vDir) { GetSystem(i).SetDir(vDir); }
    ...
    inline int32_t GetType(int32_t i) { return GetSystem(i).GetType(); }

    inline int32_t MaxParticles(int32_t nParts, int32_t nDens) {
        nParts = ((nParts < 0) ? -nParts : nParts * (nDens + 1)) >>
                 int32_t(!gameOpts->render.particles.bDisperse); //(int32_t) (nParts * pow (1.2, nDens));
    ...
    inline int32_t RemoveEmitter(int32_t i, int32_t j) { return GetSystem(i).RemoveEmitter(j); }

    inline int32_t RenderBufPtr(void) { return m_iRenderBuffer; }

    inline void IncRenderBufPtr(int32_t i = 1) { m_iRenderBuffer += i; }
