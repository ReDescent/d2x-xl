...
class CVertexOwner {
    ...
    public:
    inline bool operator<(CVertexOwner &other) {
        return (m_nOwner < other.m_nOwner) || ((m_nOwner == other.m_nOwner) && (m_nVertex < other.m_nVertex));
    }
    inline bool operator>(CVertexOwner &other) {
        return (m_nOwner > other.m_nOwner) || ((m_nOwner == other.m_nOwner) && (m_nVertex > other.m_nVertex));
    }
    inline bool operator!=(CVertexOwner &other) {
        return (m_nOwner != other.m_nOwner) || (m_nVertex != other.m_nVertex);
    }
};
...
class CModel {
    ...
    int32_t BuildFromPOF(
        CObject *pObj,
        int32_t nModel,
        CPolyModel *pp,
        CArray<CBitmap *> &modelBitmaps,
        CFloatVector *objColorP);
    ...
    int32_t CountPOFModelItems(
        void *modelDataP,
        uint16_t *pnSubModels,
        uint16_t *pnVerts,
        uint16_t *pnFaces,
        uint16_t *pnFaceVerts);
    CFace *AddPOFFace(
        CSubModel *psm,
        CFace *pmf,
        CFixVector *pn,
