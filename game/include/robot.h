...
typedef struct tRobotInfo {
    ...
    int8_t pursuit; //  !0 means pursues player after he goes around a corner.  4 = 4/2 pursue up to 4/2 seconds after
                    //  becoming invisible if up to 4 segments away
    ...
    uint8_t aim; //  255 = perfect, less = more likely to miss.  0 != Random, would look stupid.  0=45 degree spread.
                 //  Specify in bitmaps.tbl in range 0.0..1.0
...
//      robotType      nType of robot for which to get joint information.  A particular nType, not an instance of a
//      robot. gun_num         gun number.  If in 0..Robot_info[robotType].nGuns-1, then it is a gun, else it refers to
//      non-animating parts of robot. state           state about which to get information.  Legal states in range
//      0..N_ANIM_STATES-1, defined in robot.h, are:
