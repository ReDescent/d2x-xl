...
static inline int32_t IsWaterTexture(int16_t nTexture) { return ((nTexture >= 399) && (nTexture <= 403)); }
...
class CSide {
    ...
    int32_t CheckLineToFaceRegularf(
        CFloatVector &vIntersection,
        CFloatVector *p0,
        CFloatVector *p1,
        float rad,
...
class CSegment {
    ...
    void
    OverrideTextures(int16_t nTexture, int16_t nOldTexture, int16_t nTexture2, int32_t bFullBright, int32_t bForce);
    ...
    inline void SetTextures(int32_t nSide, int32_t nBaseTex, int32_t nOvlTex) {
        m_sides[nSide].SetTextures(nBaseTex, nOvlTex);
    }
    ...
    inline CFixVector *GetCornerVertices(int32_t nSide, CFixVector *vertices) {
        return m_sides[nSide].GetCornerVertices(vertices);
    }
    ...
    inline int32_t CheckLineToFaceRegular(
    ...
    inline int32_t CheckLineToFaceEdges(
        CFixVector &intersection,
        CFixVector *p0,
        CFixVector *p1,
        fix rad,
    ...
    inline int32_t HasDamageProp(int32_t i = -1) {
        return (i < 0) ? (m_xDamage[0] | m_xDamage[1]) != 0 : (m_xDamage[i] != 0);
    }
    ...
    inline int32_t FogType(void) {
        return HasWaterProp() ? 1 : HasLavaProp() ? 2 : HasLightFogProp() ? 3 : HasDenseFogProp() ? 4 : 0;
    }
...
class CSegFace {
    ...
    inline CFloatVector Normal(void) {
        CSide *pSide = Side();
        return (pSide->m_nType == 1) ? pSide->m_fNormals[0]
                                     : CFloatVector::Avg(pSide->m_fNormals[0], pSide->m_fNormals[1]);
    }
};
...
//--repair-- 	int16_t   special_segment; // if specialType indicates repair center, this is the base of the repair
//center
//--repair-- } lsegment;

// Globals from mglobal.c
extern uint16_t sideVertIndex[SEGMENT_SIDE_COUNT]
                             [4]; // sideVertIndex[my_side] is list of vertices forming side my_side.
