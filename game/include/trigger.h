...
class CTrigger : public CTriggerTargets {
    ...
    inline bool ClientOnly(void) {
        return (m_info.nType == TT_SPEEDBOOST) || (m_info.nType == TT_SHIELD_DAMAGE) ||
               (m_info.nType == TT_ENERGY_DRAIN);
    }
    int32_t Delay(void);
    bool IsDelayed(void);
    bool IsExit(void);
    ...
    inline bool Flagged(uint16_t mask, uint16_t match = 0) {
        return match ? (Flags() & mask) == match : (Flags() & mask) != 0;
    }
