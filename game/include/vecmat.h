...
class CFixVector {
    ...
    // returned.
    static const fixang DeltaAngle(
        const CFixVector &v0,
        const CFixVector &v1,
        CFixVector *fVec); // computes the delta angle between two normalized vectors.
...
class CFloatVector {
    ...
    inline float Max(void) {
        return (v.coord.x > v.coord.y) ? (v.coord.x > v.coord.z) ? v.coord.x : v.coord.z
                                       : (v.coord.y > v.coord.z) ? v.coord.y : v.coord.z;
    }
...
class CAngleVector {
    ...
    void Set(const CAngleVector &other) {
    ...
    inline const float Mag(void) const { return (const float)sqrt(SqrMag()); }
};
#pragma pack(pop)
...
inline const CFloatVector CFloatVector::Avg(const CFloatVector &src0, const CFloatVector &src1) {
    CFloatVector vec;
    vec.Set(
        (src0.v.coord.x + src1.v.coord.x) / 2,
...
inline CFloatVector &CFloatVector::Cross(CFloatVector &dest, const CFloatVector &v0, const CFloatVector &v1) {
    dest.Set(
        v0.v.coord.y * v1.v.coord.z - v0.v.coord.z * v1.v.coord.y,
...
inline const float CFloatVector::Dist(const CFloatVector &v0, const CFloatVector &v1) { return (v0 - v1).Mag(); }
...
inline CFloatVector &
CFloatVector::Perp(CFloatVector &dest, const CFloatVector &p0, const CFloatVector &p1, const CFloatVector &p2) {
...
inline bool CFloatVector::IsZero(void) const {
    return (v.coord.x == 0.0f) && (v.coord.y == 0.0f) && (v.coord.z == 0.0f) && (v.coord.w == 0.0f);
}
...
inline const float CFloatVector::Mag(void) const { return (const float)sqrt(SqrMag()); }
...
inline const float CFloatVector3::Dist(const CFloatVector3 &v0, const CFloatVector3 &v1) { return (v0 - v1).Mag(); }
...
inline CFloatVector3 &
CFloatVector3::Perp(CFloatVector3 &dest, const CFloatVector3 &p0, const CFloatVector3 &p1, const CFloatVector3 &p2) {
    return Cross(dest, p1 - p0, p2 - p1);
}

inline const CFloatVector3
CFloatVector3::Perp(const CFloatVector3 &p0, const CFloatVector3 &p1, const CFloatVector3 &p2) {
    return Cross(p1 - p0, p2 - p1);
}

inline const CFloatVector3
CFloatVector3::Normal(const CFloatVector3 &p0, const CFloatVector3 &p1, const CFloatVector3 &p2) {
...
inline bool CFloatVector3::IsZero(void) const {
    return (v.coord.x == 0.0f) && (v.coord.y == 0.0f) && (v.coord.z == 0.0f);
}
...
inline const float CFloatVector3::Mag(void) const { return (const float)sqrt(SqrMag()); }
...
inline const CFixVector CFixVector::Avg(CFixVector &src0, CFixVector &src1, CFixVector &src2, CFixVector &src3) {
    CFixVector vec;
    vec.Set(
        (src0.v.coord.x + src1.v.coord.x + src2.v.coord.x + src3.v.coord.x) / 4,
...
inline const fix CFixVector::Dist(const CFixVector &vec0, const CFixVector &vec1) { return (vec0 - vec1).Mag(); }

inline const fix CFixVector::Dot(const CFixVector &v0, const CFixVector &v1) {
    return fix(
        (double(v0.v.coord.x) * double(v1.v.coord.x) + double(v0.v.coord.y) * double(v1.v.coord.y) +
         double(v0.v.coord.z) * double(v1.v.coord.z)) /
        65536.0);
}

inline const fix CFixVector::Dot(const fix x, const fix y, const fix z, const CFixVector &v) {
    return fix(
        (double(x) * double(v.v.coord.x) + double(y) * double(v.v.coord.y) + double(z) * double(v.v.coord.z)) /
        65536.0);
}
...
inline CFixVector &
CFixVector::Perp(CFixVector &dest, const CFixVector &p0, const CFixVector &p1, const CFixVector &p2) {
...
inline bool CFixVector::IsZero(void) const { return !(v.coord.x || v.coord.y || v.coord.z); }

inline void CFixVector::SetZero(void) { memset(&v, 0, sizeof(v)); }

inline const int32_t CFixVector::Sign(void) const { return (v.coord.x * v.coord.y * v.coord.z < 0) ? -1 : 1; }
...
inline const fix CFixVector::operator*(const CFixVector &other) const {
    return fix(
        (double(v.coord.x) * double(other.v.coord.x) + double(v.coord.y) * double(other.v.coord.y) +
         double(v.coord.z) * double(other.v.coord.z)) /
        65536.0);
}
...
inline void CFixMatrix::CheckAndFix(void) { *this = CreateFU(m.dir.f, m.dir.u); }
...
class CFloatMatrix {
    ...
    static const CFloatMatrix
    Create(const CFloatVector &r, const CFloatVector &u, const CFloatVector &f, const CFloatVector &w);
...
inline const CFloatVector3 CFloatMatrix::operator*(const CFloatVector3 &v) {
    CFloatVector3 vec;
    vec.Set(
        CFloatVector3::Dot(v, *m.dir.r.XYZ()),
...
// make sure this m.matrix is orthogonal
inline void CFloatMatrix::CheckAndFix(void) { *this = CreateFU(m.dir.f, m.dir.u); }
...
const int32_t FindPointLineIntersection(
    ...
    const CFixVector &p2,
    const CFixVector &p3,
    int32_t bClampToFarthest);
// const int32_t FindPointLineIntersection (CFixVector& hitP, const CFixVector& p1, const CFixVector& p2, const
// CFixVector& p3, const CFixVector& vPos, int32_t bClampToFarthest);
