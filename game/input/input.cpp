...
int32_t CControlsManager::ReadKeyboard(void) { return 1; }
...
int32_t CControlsManager::ReadJoyAxis(int32_t i, int32_t rawJoyAxis[]) {
    ...
    if (gameOpts->input.joystick.bSyncAxis && (dz < 16384) &&
...
void CControlsManager::SetFCSButton(int32_t btn, int32_t button) { JoyGetButtonState(btn); }
...
inline int32_t HaveKey(kcItem *k, int32_t i) { return k[i].value; }
...
void CControlsManager::DoD2XKeys(
    int32_t *bSlideOn,
    int32_t *bBankOn,
    fix *pitchTimeP,
    fix *headingTimeP,
    int32_t *nCruiseSpeed,
...
int32_t CControlsManager::LimitTurnRate(int32_t bUseMouse) {
    if (!(gameOpts->input.bLimitTurnRate || IsMultiGame))
        return 0;
    if (automap.Active() || gameOpts->input.mouse.bJoystick || gameStates.app.bNostalgia || COMPETITION ||
...
void CControlsManager::DoKeyboard(
    int32_t *bSlideOn,
    int32_t *bBankOn,
    fix *pitchTimeP,
    fix *headingTimeP,
    int32_t *nCruiseSpeed,
    int32_t bGetSlideBank) {
    int32_t i, v, pitchScale = (!(gameStates.app.bNostalgia || COMPETITION) && (FastPitch() == 1)) ? 2 * PH_SCALE : 1;
    ...
    if (bGetSlideBank == 2) {
        ...
        for (i = 0; i < 2; i++)
            if (((v = HaveKey(kcKeyboard, +i)) < 255) && KeyDownCount(v))
                *nCruiseSpeed = 0;
    }
...
void CControlsManager::DoJoystick(
    int32_t *bSlideOn,
    int32_t *bBankOn,
    fix *pitchTimeP,
    fix *headingTimeP,
                ...
                if (!*bBankOn) {
                    if ((v = kcJoystick[i + 14].value) < 255) {
                        if (kcJoystick[61].value) // If inverted...
                            ...
                            m_info[2].headingTime +=
                                DeltaAxis(v); // (m_joyAxis [v] * 16 / joySensMod); // m_frameCount;
                    }
                }
            }
            if (*bBankOn) {
...
void CControlsManager::DoMouse(
    int32_t *mouseAxis,
    int32_t nMouseButtons,
    int32_t *bSlideOn,
    int32_t *bBankOn,
    fix *pitchTimeP,
    fix *headingTimeP,
        ...
        if (*bSlideOn) {
            ...
            if (*bBankOn) {
                ...
                if (!gameStates.app.bNostalgia && gameOpts->input.mouse.bJoystick) {
                    ...
                    if (((v = kcMouse[15].value) < 255) && mouseAxis[v]) {
                        if (kcMouse[16].value) // If inverted...
                            m_info[3].headingTime -=
                                (mouseAxis[v] * gameOpts->input.mouse.sensitivity[0]) / nMouseSensMod;
                        else
                            m_info[3].headingTime +=
                                (mouseAxis[v] * gameOpts->input.mouse.sensitivity[0]) / nMouseSensMod;
                    }
...
int32_t CControlsManager::ReadOculusRift(void) {
    return transformation.m_info.bUsePlayerHeadAngles =
               ogl.IsOculusRift() && gameData.renderData.rift.GetHeadAngles(&transformation.m_info.playerHeadAngles);
}
...
void CControlsManager::DoTrackIR(void) {
    int32_t dx = (int32_t)((float)tirInfo.fvRot.z * (float)gameData.renderData.screen.Width() / 16384.0f),
            dy = (int32_t)((float)tirInfo.fvRot.y * (float)gameData.renderData.screen.Height() / 16384.0f), dz;
    int32_t x, y;
    ...
    if (gameOpts->input.trackIR.nMode == 0) {
        ...
        if (gameOpts->input.trackIR.bMove[1])
            m_info[0].bankTime +=
                (int32_t)(tirInfo.fvRot.x * m_pollTime / 131072.0f * (gameOpts->input.trackIR.sensitivity[2] + 1));
        ...
        if (gameOpts->input.trackIR.bMove[0]) {
            transformation.m_info.playerHeadAngles.v.coord.h =
                fixang(-tirInfo.fvRot.z / 4 * (gameOpts->input.trackIR.sensitivity[0] + 1));
            transformation.m_info.playerHeadAngles.v.coord.p =
                fixang(tirInfo.fvRot.y / 4 * (gameOpts->input.trackIR.sensitivity[1] + 1));
        } else
            transformation.m_info.playerHeadAngles.v.coord.h = transformation.m_info.playerHeadAngles.v.coord.p = 0;
        if (gameOpts->input.trackIR.bMove[1])
            transformation.m_info.playerHeadAngles.v.coord.b =
                (fixang)tirInfo.fvRot.x / 4 * (gameOpts->input.trackIR.sensitivity[2] + 1);
        else
    ...
    if (gameOpts->input.trackIR.bMove[2] && (float(fabs(tirInfo.fvTrans.x) > fDeadzone))) {
        if (tirInfo.fvTrans.x < 0)
            ...
            tirInfo.fvTrans.x -= fDeadzone;
        m_info[0].sidewaysThrustTime -= int32_t(
            (tirInfo.fvTrans.x - fDeadzone) * m_pollTime / 65536.0f * (gameOpts->input.trackIR.sensitivity[0] + 1));
    }
    if (gameOpts->input.trackIR.bMove[3] && (float(fabs(tirInfo.fvTrans.y) > fDeadzone))) {
        if (tirInfo.fvTrans.y < 0)
            ...
            tirInfo.fvTrans.y -= fDeadzone;
        m_info[0].verticalThrustTime += (int32_t)(
            (tirInfo.fvTrans.y - fDeadzone) * m_pollTime / 65536.0f * (gameOpts->input.trackIR.sensitivity[1] + 1));
    }
    if (gameOpts->input.trackIR.bMove[4] && ((float)fabs(tirInfo.fvTrans.z) > fDeadzone)) {
        if (tirInfo.fvTrans.z < 0)
            ...
            tirInfo.fvTrans.z -= fDeadzone;
        m_info[0].forwardThrustTime -= (int32_t)(
            (tirInfo.fvTrans.z - fDeadzone) * m_pollTime / 8192.0f * (gameOpts->input.trackIR.sensitivity[2] + 1));
    }
}
...
void CControlsManager::DoSlideBank(int32_t bSlideOn, int32_t bBankOn, fix pitchTime, fix headingTime) {
    if (bSlideOn)
        m_info[0].headingTime = m_info[0].pitchTime = 0;
    else {
        if (pitchTime == 0)
...
void CControlsManager::SetType(void) {
    ...
    if (gameStates.input.nJoyType < 0)
        gameStates.input.nJoyType = ((gameConfig.nControlType >= CONTROL_FLIGHTSTICK_PRO) &&
...
int32_t CControlsManager::Read(void) {
    ...
    if (!gameOpts->legacy.bInput)
        // FIXME: why poll 2 dozen times in the following code when input polling calls all necessary input handlers
        // anyway?
        event_poll();
    ...
    if (gameOpts->input.mouse.bUse)
        ...
        mouseAxis[0] = mouseAxis[1] = mouseAxis[2] = 0;
        nMouseButtons = 0;
        bUseMouse = 0;
    }
    ...
    for (i = 0; i < 3; i++) {
        DoKeyboard(
            ...
            &pitchTime,
            &headingTime,
            reinterpret_cast<int32_t *>(&gameStates.input.nCruiseSpeed),
            i);
        if (bUseJoystick)
            DoJoystick(
                &bSlideOn,
        ...
        if (bUseMouse)
            DoMouse(
                reinterpret_cast<int32_t *>(&mouseAxis[0]),
                nMouseButtons,
                &bSlideOn,
                &bBankOn,
                &pitchTime,
...
char CControlsManager::GetKeyValue(char key) { return (kcKeyboard[int32_t(key)].value); }
