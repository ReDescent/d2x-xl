...
void JoyHatHandler(SDL_JoyHatEvent *jhe) {
    ...
    for (int32_t hbi = 0; hbi < 4; hbi++) {
        if (!joyInfo.buttons[hat + hbi].lastState &&
            joyInfo.buttons[hat + hbi].state) { // lastState up, current state down
            joyInfo.buttons[hat + hbi].xTimeWentDown = TimerGetFixedSeconds();
            joyInfo.buttons[hat + hbi].numDowns++;
        } else if (joyInfo.buttons[hat + hbi].lastState && !joyInfo.buttons[hat + hbi].state) { // lastState down,
                                                                                                // current state up
