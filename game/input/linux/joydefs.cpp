...
// char *control_text[CONTROL_MAX_TYPES] = { "Keyboard only", "Joystick", "Flightstick Pro", "Thrustmaster FCS", "Gravis
// Gamepad", "Mouse", "Cyberman" };
...
void InputDeviceConfig() {
    ...
    m[0].group = 0;
    m[0].key = KEY_K;
    m[1].nType = NM_TYPE_RADIO;
    m[1].text = "JOYSTICK";
    m[1].value = 0;
    m[1].group = 0;
    m[1].key = KEY_J;
    m[2].nType = NM_TYPE_RADIO;
    m[2].text = "MOUSE";
    m[2].value = 0;
    m[2].group = 0;
    m[2].key = KEY_M;
    m[3].nType = NM_TYPE_TEXT;
    m[3].text = "";
    m[4].nType = NM_TYPE_MENU;
    m[4].text = "CUSTOMIZE ABOVE";
    m[4].key = KEY_A;
    m[5].nType = NM_TYPE_MENU;
    m[5].text = "CUSTOMIZE KEYBOARD";
    m[5].key = KEY_C;
    // added on 2/5/99 by Victor Rachels for D1X keys menu
    m[6].nType = NM_TYPE_MENU;
    m[6].text = "CUSTOMIZE D1X KEYS";
    m[6].key = KEY_X;
