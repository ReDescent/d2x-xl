...
#define key_ismod(k) \
    ((k & 0xff) == KEY_LALT || (k & 0xff) == KEY_RALT || (k & 0xff) == KEY_LSHIFT || (k & 0xff) == KEY_RSHIFT || \
     (k & 0xff) == KEY_LCTRL || (k & 0xff) == KEY_RCTRL)
...
void UpdateVCRState(void) {
    ...
    if ((gameStates.input.keys.pressed[KEY_LSHIFT] || gameStates.input.keys.pressed[KEY_RSHIFT]) &&
        gameStates.input.keys.pressed[KEY_RIGHT])
        ...
        gameStates.input.keys.pressed[KEY_LEFT])
        ...
        gameStates.input.keys.pressed[KEY_RIGHT] && ((TimerGetFixedSeconds() - newdemo_single_frameTime) >= I2X(1)))
...
void HandleDemoKey(int32_t key) {
    ...
    case KEY_F3:
        if (!(GuidedInMainView() || gameStates.render.bChaseCam || (gameStates.render.bFreeCam > 0) ||
              gameStates.app.bEndLevelSequence || gameStates.app.bPlayerIsDead))
            cockpit->Toggle();
        break;
    ...
    case KEY_F7:
        gameData.multigame.score.bShowList =
            (gameData.multigame.score.bShowList + 1) % ((gameData.demoData.nGameMode & GM_TEAM) ? 4 : 3);
        break;
...
void ReadControls(void) {
    ...
    if (!gameStates.app.bEndLevelSequence && !gameStates.app.bPlayerIsDead) {
        if ((gameData.demoData.nState == ND_STATE_PLAYBACK) || (markerManager.DefiningMsg()) ||
            gameData.multigame.msg.bSending || gameData.multigame.msg.bDefining) // WATCH OUT!!! WEIRD CODE ABOVE!!!
            controls.Reset();
        else
    ...
    if (LOCALPLAYER
            .m_bExploded) { // gameStates.app.bPlayerIsDead && (gameData.objData.pConsole->flags & OF_EXPLODING)) {
        if (!explodingFlag) {
