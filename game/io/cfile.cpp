...
char *GameDataFilename(char *pszFilename, const char *pszExt, int32_t nLevel, int32_t nType) {
    ...
    SplitPath(
        *hogFileManager.AltFiles().szName
            ? hogFileManager.AltFiles().szName
...
int32_t CFile::Open(const char *filename, const char *folder, const char *mode, int32_t nHogType) {
    ...
    if (*filename != '\x01') {
        ...
        if (!fp && ((pszFileExt = strstr(filename, ".rdl")) || (pszFileExt = strstr(filename, ".rl2"))) &&
            (pszHogExt = strchr(hogFileManager.AltHogFile(), '.')) && !stricmp(pszFileExt, pszHogExt))
...
size_t CFile::Length(void) { return m_info.size; }
...
size_t CFile::WriteCompressed(const void *buf, uint32_t bufLen) {
    ...
    CByteArray compressedBuffer;
    if (compressedBuffer.Create(nCompressedSize, "WriteCompressed::compressedBuffer") &&
        (compress(compressedBuffer.Buffer(), &nCompressedSize, (uint8_t *)buf, bufLen) == Z_OK)) {
...
size_t CFile::Tell(void) { return m_info.rawPosition; }
...
int32_t CFile::WriteByte(int8_t b) { return (int32_t)Write(&b, sizeof(b), 1); }
