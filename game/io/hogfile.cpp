...
class CLevelHeader {
    ...
    public:
    explicit CLevelHeader(int32_t bExtended = 0) : m_size(0), m_bExtended(bExtended) {
        m_name[0] = '\0';
        m_longName[0] = '\0';
    }
...
tHogFile *CHogFile::BinSearch(tHogFile *hogFiles, int32_t nFiles, const char *pszFile) {
    ...
    do {
        int m = (l + r) / 2;
        int i = stricmp(hogFiles[m].name, pszFile);
        if (i < 0)
            l = m + 1;
        else if (i > 0)
            r = m - 1;
...
void CHogFile::UseAltDir(const char *path) {
    gameFolders.bAltHogDirInited = (strcmp(path, gameFolders.game.szData[0]) != 0) &&
                                   (GetAppFolder("", gameFolders.game.szAltHogs, path, "descent2.hog") == 0);
}
...
int32_t CHogFile::UseD2X(const char *name) { return Use(&m_files.D2XHogFiles, name, gameFolders.missions.szRoot); }

// ----------------------------------------------------------------------------

int32_t CHogFile::UseXL(const char *name) { return Use(&m_files.XLHogFiles, name, gameFolders.game.szData[0]); }

// ----------------------------------------------------------------------------

int32_t CHogFile::UseExtra(const char *name) {
    return gameStates.app.bHaveExtraData =
               !gameStates.app.bNostalgia && Use(&m_files.ExtraHogFiles, name, gameFolders.game.szData[0]);
}

// ----------------------------------------------------------------------------

int32_t CHogFile::UseD1(const char *name) { return Use(&m_files.D1HogFiles, name, gameFolders.game.szData[0]); }
