...
int32_t mvefile_fetch_next_chunk(MVEFILE *movie) { return _mvefile_fetch_next_chunk(movie); }
...
static int16_t _mve_get_short(uint8_t *data) { return int16_t(data[0]) | (int16_t(data[1]) << 8); }

//-----------------------------------------------------------------------

static uint16_t _mve_get_ushort(uint8_t *data) { return uint16_t(data[0]) | (uint16_t(data[1]) << 8); }
...
void *MVE_Alloc(uint32_t size) { return reinterpret_cast<void *>(new uint8_t[size]); }
