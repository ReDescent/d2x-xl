...
static int32_t end_movie_handler(uint8_t major, uint8_t minor, uint8_t *data, int32_t len, void *context) { return 0; }
...
static void _CDECL_ mve_audio_callback(void *userdata, uint8_t *stream, int32_t len) {
    ...
    while (mve_audio_bufhead != mve_audio_buftail &&
           len > (mve_audio_buflens[mve_audio_bufhead] - mve_audio_curbuf_curpos)) {
        length = mve_audio_buflens[mve_audio_bufhead] - mve_audio_curbuf_curpos;
        memcpy(
            stream,
            reinterpret_cast<uint8_t *>(mve_audio_buffers[mve_audio_bufhead]) + mve_audio_curbuf_curpos,
            length);
        total += length;
        stream += length;
        len -= length;
    ...
    /* ospace remaining && buffers remaining */
    if ((len > 0) && (mve_audio_bufhead != mve_audio_buftail)) {
        memcpy(
            stream,
            reinterpret_cast<uint8_t *>(mve_audio_buffers[mve_audio_bufhead]) + mve_audio_curbuf_curpos,
            len);
        mve_audio_curbuf_curpos += len;
        stream += len;
        len -= len;
        if (mve_audio_curbuf_curpos >= mve_audio_buflens[mve_audio_bufhead]) {
...
static int32_t display_video_handler(uint8_t major, uint8_t minor, uint8_t *data, int32_t len, void *context) {
    ...
    mve_showframe(
        reinterpret_cast<uint8_t *>(g_vBackBuf1),
        g_width,
        g_height,
        0,
        0,
...
void MVE_sndInit(int32_t x) { mve_audio_enabled = (x != -1); }
