...
void CLightManager::Init(void) { m_data.Init(); }

//------------------------------------------------------------------------------

void CLightManager::Destroy(void) { m_data.Destroy(); }
...
void CLightManager::SetColor(int16_t nLight, float red, float green, float blue, float fBrightness) {
    ...
    if ((pLight->info.nType == 2) ? (gameOpts->render.color.nLevel >= 1) : (gameOpts->render.color.nLevel == 2)) {
        ...
        pLight->info.color.Red() = pLight->info.color.Green() = pLight->info.color.Blue() = 1.0f;
    } else {
        pLight->info.color.Red() = pLight->info.color.Green() = pLight->info.color.Blue() =
            red * 0.30f + green * 0.584f + blue * 0.116f; //(red + green + blue) / 3.0f;
    }
...
int16_t
CLightManager::Update(CFloatVector *pColor, float fBrightness, int16_t nSegment, int16_t nSide, int16_t nObject) {
    ...
    if (nLight >= 0) {
        ...
        if ((pLight->info.fBrightness != fBrightness) || (pLight->info.color.Red() != pColor->Red()) ||
            (pLight->info.color.Green() != pColor->Green()) || (pLight->info.color.Blue() != pColor->Blue())) {
...
int32_t CLightManager::IsTriggered(int16_t nSegment, int16_t nSide, bool bOppSide) {
    ...
    int32_t i = 0;
    bool bForceField =
        (gameData.pigData.tex.pTexMapInfo[SEGMENT(nSegment)->m_sides[nSide].m_nBaseTex].flags & TMI_FORCE_FIELD) != 0;
...
if (xBrightness > I2X (1))
	...
	CSide			*pSide = SEGMENT (nSegment)->m_sides + nSide;
        ...
        if (nSide < 0) {
            ...
            light.info.bVariable = IsDestructible(nTexture) || IsFlickering(nSegment, nSide) ||
                                   IsTriggered(nSegment, nSide) || SEGMENT(nSegment)->Side(nSide)->IsVolatile();
...
void CLightManager::Delete(int16_t nLight) {
    if ((nLight >= 0) && (nLight < m_data.nLights[0])) {
        ...
        if ((nDbgSeg >= 0) && (nDbgSeg == pLight->info.nSegment) &&
            ((nDbgSide < 0) || (nDbgSide == pLight->info.nSide)))
            BRP;
#endif
...
int32_t CDynLight::LightPathLength(
    const int16_t nLightSeg,
    const int16_t nDestSeg,
    const CFixVector &vDestPos,
    fix xMaxLightRange,
    int32_t bFastRoute,
    int32_t nThread) {
    fix dist = (bFastRoute == 1) ? gameData.segData.SegDist(nLightSeg, nDestSeg, info.vPos, vDestPos)
                                 : (bFastRoute == 0) ? simpleRouter[nThread].PathLength(
                                                           info.vPos,
                                                           nLightSeg,
                                                           vDestPos,
                                                           nDestSeg,
                                                           X2I(xMaxLightRange / 5),
...
int32_t CDynLight::SeesPoint(
    const int16_t nDestSeg,
    const int8_t nDestSide,
    const CFixVector *pvNormal,
    CFixVector *pvPoint,
    int32_t nLevel,
    int32_t nThread) {
    ENTER(0, 0);
    ...
    if (info.nSide < 0) {
        ...
        RETVAL(0)
#if DBG
        if ((nDbgSeg >= 0) && (nDbgVertex >= 0) && (info.nSegment == nDbgSeg) &&
            ((nDbgSide < 0) || (info.nSide == nDbgSide)) && (nDbgVertex >= 0) && (*pvPoint == VERTICES[nDbgVertex]))
            BRP;
#endif
    }
...
int32_t CDynLight::SeesPoint(
    const int16_t nSegment,
    const int8_t nSide,
    CFixVector *vPoint,
    const int32_t nLevel,
    int32_t nThread) {
...
int32_t CDynLight::Contribute(
    const int16_t nDestSeg,
    const int8_t nDestSide,
    const int16_t nDestVertex,
    CFixVector &vDestPos,
    const CFixVector *vNormal,
    fix xMaxLightRange,
    float fRangeMod,
    fix xDistMod,
    int32_t nThread) {
    ...
    if (nLightSeg == nDestSeg)
        ...
        if (nDestVertex >= 0) {
            if (info.visibleVertices && info.visibleVertices->Buffer())
                ...
                info.bDiffuse[nThread] =
                    SeesPoint(nDestSeg, nDestSide, vNormal, &vDestPos, gameOpts->render.nLightmapPrecision, nThread);
        ...
        // if bDiffuse == 0 then point is completely occluded (determined by above call to SeesPoint ()), otherwise use
        // SeesPoint() to test occlusion
    }
    ...
    if (info.bDiffuse[nThread] == 2)
        ...
        if (xDistance < xPathLength) {
            // adjust pathlength using distance when going via segments (which actually is quite accurate) and
            // geometrical distance of the two points involved
#if LIGHTDIST_MATH == 1
...
void CLightManager::GatherStaticVertexLights(int32_t nVertex, int32_t nMax, int32_t nThread) {
    ...
    for (; nVertex < nMax; nVertex++, pf++) {
        ...
        GetVertexColor(
            ...
            -1,
            nVertex,
            RENDERPOINTS[nVertex].GetNormal()->XYZ(),
            vVertex.XYZ(),
            pf,
            NULL,
            1,
