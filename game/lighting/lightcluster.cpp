...
CLightClusterManager::CLightClusterManager() { m_bUse = 1; }
...
void CLightClusterManager::Destroy(void) { m_objects.Destroy(); }
        ...
        if ((pObj->info.nType == OBJ_LIGHT) && (pObj->info.nId == CLUSTER_LIGHT_ID)) {
            ...
            if (!(h = pObj->cType.lightInfo.nObjects)) {
                ...
                if (1 || (pObj->cType.lightInfo.nSegment < 0)) {
                    int16_t nSegment =
                        FindSegByPos(pObj->info.position.vPos, abs(pObj->cType.lightInfo.nSegment), 0, 0);
                ...
                lightManager
                    .Add(NULL, &pObj->cType.lightInfo.color, pObj->cType.lightInfo.intensity, -1, -1, i, -1, NULL);
            }
        }
    }
    RETURN
...
void CLightClusterManager::AddForAI(CObject *pObj, int16_t nObject, int16_t nShot) {
    ...
    if (pPrevShot && (pPrevShot->info.nSignature ==
                      pObj->Shots().nSignature)) { // the previous shot fired by that object is still alive
        ...
        if (CFixVector::Dist(pShot->info.position.vPos, pLight->info.position.vPos) < I2X(15)) {
            if (nLight >= 0) {
                ...
                m_objects[nShot].nObject = m_objects[nPrevShot].nObject = nLight;
                if (nLight >= 0) {
                    pLight = OBJECT(nLight);
                    m_objects[nShot].nSignature = m_objects[nPrevShot].nSignature = pLight->info.nSignature;
