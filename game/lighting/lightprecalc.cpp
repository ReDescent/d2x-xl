...
int32_t CSegmentData::SegDist(uint16_t i, uint16_t j, const CFixVector &vStart, const CFixVector &vDest) {
    ...
    if (nDestSeg < 0)
        return CFixVector::Dist(SEGMENT(nStartSeg)->Center(), vStart) +
               CFixVector::Dist(SEGMENT(nStartSeg)->Center(), vDest);
    return nDistance + CFixVector::Dist(SEGMENT(nStartSeg)->Center(), vStart) +
           CFixVector::Dist(SEGMENT(nDestSeg)->Center(), vDest);
}
...
int32_t ComputeNearestSegmentLights(int32_t i, int32_t nThread) {
    ...
    for (pSeg = SEGMENT(i); i < j; i++, pSeg++) {
        ...
        for (l = n = 0; l < lightManager.LightCount(0); l++, pLight++) {
            if (!((pLight->info.nSegment < 0) ? gameData.segData.SegVis(OBJECT(pLight->info.nObject)->info.nSegment, i)
                                              : gameData.segData.LightVis(pLight->Index(), i)))
                continue;
...
#define VERTVIS(_nSegment, _nVertex) \
    (gameData.segData.bVertVis.Buffer() \
         ? gameData.segData.bVertVis[(_nSegment)*VERTVIS_FLAGS + ((_nVertex) >> 3)] & (1 << ((_nVertex)&7)) \
         : 0)
...
static void SetLightVis(int16_t nLight, int16_t nSegment) {
    ENTER(0, 0);
#if DBG
    if ((lightManager.Lights(nLight)->info.nSegment == nDbgSeg) &&
        (lightManager.Lights(nLight)->info.nSide == nDbgSide))
        BRP;
    if (nSegment == nDbgSeg)
        BRP;
    ...
    for (nFaces = pSegFace->nFaces, pFace = pSegFace->pFace; nFaces; nFaces--, pFace++) {
        ...
        if (gameStates.render.bTriangleMesh) {
            for (nTris = pFace->m_info.nTris, pTriangle = FACES.tris + pFace->m_info.nTriIndex; nTris;
                 nTris--, pTriangle++)
                for (i = 0; i < 3; i++)
...
static void ComputeSingleSegmentVisibility(
    int32_t nThread,
    int16_t nStartSeg,
    int16_t nFirstSide,
    int16_t nLastSide,
    ...
    for (nSide = nFirstSide; nSide <= nLastSide; nSide++, pSide++) {
        ...
        SetupTransformation(
            transformation,
            viewer.info.position.vPos,
            viewer.info.position.mOrient,
...
static void CheckLightVisibility(int16_t nLight, int16_t nDestSeg, fix xMaxDist, float fRange, int32_t nThread) {
    ...
    if ((0 < dPath) || // path from light to dest blocked
        ...
        (CFixVector::Dist(SEGMENT(nLightSeg)->Center(), SEGMENT(nDestSeg)->Center()) - SEGMENT(nLightSeg)->MaxRad() -
             SEGMENT(nDestSeg)->MaxRad() >=
         xMaxDist)) { // distance to great
...
#pragma omp atomic
    ...
    // cast rays from light segment (corners and center) to target segment and see if at least one of them isn't blocked
    // by geometry
    ...
    for (i = 4; i >= -4; i--) {
        if (i == 4)
            ...
            v0 = CFloatVector::Avg(
                FVERTICES[pSide->m_corners[4 + i]],
                FVERTICES[pSide->m_corners[(5 + i) & 3]]); // center of face's edges
...
static bool InitLightVisibility(int32_t nStage) {
    ...
    if (nStage) {
        ...
        if (lightManager.GeometryLightCount()) {
            if (!gameData.segData.bLightVis.Create(
                    (lightManager.GeometryLightCount() * LEVEL_SEGMENTS + 3) / 4,
                    "gameData.segData.bLightVis"))
                RETVAL(false);
...
int32_t SegDistGaugeSize(void) { return PROGRESS_STEPS(gameData.segData.nSegments); }
...
int32_t LoadLightData(int32_t nLevel) {
    ...
    if (bOk)
        bOk = (gameData.segData.bSegVis.Read(cf, gameData.segData.SegVisSize(ldh.nSegments), 0, ldh.bCompressed) ==
              ...
              (gameData.segData.bLightVis.Read(cf, size_t((ldh.nLights * ldh.nSegments + 3) / 4), 0, ldh.bCompressed) ==
               size_t((ldh.nLights * ldh.nSegments + 3) / 4)) &&
              (nDistances = LoadSegDistData(cf, ldh)) &&
              (lightManager.NearestSegLights().Read(cf, ldh.nSegments * MAX_NEAREST_LIGHTS, 0, ldh.bCompressed) ==
               size_t(ldh.nSegments * MAX_NEAREST_LIGHTS)) &&
              (lightManager.NearestVertLights().Read(cf, ldh.nVertices * MAX_NEAREST_LIGHTS, 0, ldh.bCompressed) ==
               size_t(ldh.nVertices * MAX_NEAREST_LIGHTS));
    cf.Close();
...
int32_t SaveLightData(int32_t nLevel) {
    ...
    bOk = (cf.Write(&ldh, sizeof(ldh), 1) == 1) &&
          (gameData.segData.bSegVis.Write(cf, gameData.segData.SegVisSize(ldh.nSegments), 0, ldh.bCompressed) ==
           size_t(gameData.segData.SegVisSize(ldh.nSegments))) &&
          (gameData.segData.bLightVis.Write(cf, size_t((ldh.nLights * ldh.nSegments + 3) / 4), 0, ldh.bCompressed) ==
           size_t((ldh.nLights * ldh.nSegments + 3) / 4)) &&
          (nDistances = SaveSegDistData(cf, ldh)) &&
          (lightManager.NearestSegLights().Write(cf, ldh.nSegments * MAX_NEAREST_LIGHTS, 0, ldh.bCompressed) ==
           size_t(ldh.nSegments * MAX_NEAREST_LIGHTS)) &&
          (lightManager.NearestVertLights().Write(cf, ldh.nVertices * MAX_NEAREST_LIGHTS, 0, ldh.bCompressed) ==
           size_t(ldh.nVertices * MAX_NEAREST_LIGHTS));
    cf.Close();
...
void _CDECL_ SegVisThread(int32_t nId) {
    ComputeSegmentVisibilityMT(
        nId * (gameData.segData.nSegments + gameStates.app.nThreads - 1) / gameStates.app.nThreads,
        nId);
}
...
void _CDECL_ LightVisThread(int32_t nId) {
    ComputeLightVisibilityMT(
        nId * (lightManager.LightCount(0) + gameStates.app.nThreads - 1) / gameStates.app.nThreads,
        nId);
}
...
void _CDECL_ VertLightsThread(int32_t nId) {
    ComputeNearestVertexLights(
        nId * (gameData.segData.nVertices + gameStates.app.nThreads - 1) / gameStates.app.nThreads,
        nId);
}
...
void ComputeNearestLights(int32_t nLevel) {
    ENTER(0, 0);
    if (!(SHOW_DYN_LIGHT || (gameStates.render.bAmbientColor && !gameStates.render.bColored) || !COMPETITION))
        RETURN
    loadOp = 0;
    loadIdx = 0;
...
#endif
    {
        ...
        if (gameStates.app.bProgressBars && gameOpts->menus.nStyle)
            ProgressBar(
                ...
                LoadMineGaugeSize() + PagingGaugeSize() + SortLightsGaugeSize() + SegDistGaugeSize(),
                PrecalcLightPollST);
        else {
            PrintLog(0, "Computing segment visibility\n");
    ...
    PrintLog(
        0,
        "Distance table usage = %1.2f %%\n",
        100.0f * float(nDistCount) /
            (float(gameData.segData.nSegments) * float(gameData.segData.nSegments) - float(nSavedCount)));
#endif
    ...
    if (nLevel > -1) {
        if (gameStates.app.bProgressBars) {
            ...
            ProgressBar(
                ...
                2,
                0,
                (missionManager.nLastLevel - nLevel + 1 - missionManager.nLastSecretLevel) *
                    lightmapManager.Progress().Scale(),
