...
void CLightManager::Transform(int32_t bStatic, int32_t bVariable) {
    ...
    for (i = 0; i < m_data.nLights[0]; i++, pLight++) {
        ...
        pLight->render.bState =
            pLight->info.bState &&
            (pLight->info.color.Red() + pLight->info.color.Green() + pLight->info.color.Blue() > 0.0);
        pLight->render.bLightning = (pLight->info.nObject < 0) && (pLight->info.nSide < 0);
        for (int32_t j = 0; j < gameStates.app.nThreads; j++)
            ResetUsed(pLight, j);
        pLight->render.bShadow = pLight->render.bExclusive = 0;
...
static void QSortDynamicLights(int32_t left, int32_t right, int32_t nThread) {
    ...
    int32_t l = left, r = right, mat = pActiveLights[(l + r) / 2]->xDistance;
...
int32_t CLightManager::SetActive(
    ...
    else if (pLight->info.nSegment >= 0) xDist /= 2;
    else if (!pLight->info.bSpot) xDist += (MAX_OGL_LIGHTS - xDist) / 2;
#endif
    pActiveLights += xDist;
...
void CLightManager::SetNearestToVertex(
    int32_t nSegment,
    int32_t nSide,
    int32_t nVertex,
    CFixVector *vNormal,
...
void CLightManager::SetNearestStatic(int32_t nSegment, int32_t bStatic, int32_t nThread) {
    ...
    if (gameStates.render.nLightingMethod) {
        ...
        CActiveDynLight *pActiveLights = m_data.active[nThread].Buffer();
        fix xMaxLightRange =
            pSeg->AvgRad() + (/*(gameStates.render.bPerPixelLighting == 2) ? MAX_LIGHT_RANGE * 2 :*/ MAX_LIGHT_RANGE);
        ...
        for (i = MAX_NEAREST_LIGHTS; i; i--, pNearestLight++) {
            ...
            pLight->render.xDistance[nThread] = (fix)(
                (CFixVector::Dist(c, pLight->info.vPos) /*- F2X (pLight->info.fRad)*/) /
...
int16_t
CLightManager::SetNearestToSegment(int32_t nSegment, int32_t nFace, int32_t bVariable, int32_t nType, int32_t nThread) {
    ENTER(0, nThread);
    ...
    if (gameStates.render.nLightingMethod) {
        ...
        int32_t bSkipHeadlight = (gameStates.render.nType != RENDER_TYPE_OBJECTS) &&
                                 ((gameStates.render.bPerPixelLighting == 2) || gameOpts->ogl.bHeadlight);
        fix xMaxLightRange = SEGMENT(nSegment)->AvgRad() +
                             (/*(gameStates.render.bPerPixelLighting == 2) ? MAX_LIGHT_RANGE * 2 :*/ MAX_LIGHT_RANGE);
        CDynLight *pLight;
...
int16_t CLightManager::SetNearestToPixel(
    int16_t nSegment,
    int8_t nSide,
    CFixVector *vNormal,
    CFixVector *vPixelPos,
    float fLightRad,
    int32_t nThread) {
    ENTER(0, nThread);
#if DBG
    ...
    if (gameStates.render.nLightingMethod) {
        ...
        fix xMaxLightRange =
            F2X(fLightRad) + (/*(gameStates.render.bPerPixelLighting == 2) ? MAX_LIGHT_RANGE * 2 :*/ MAX_LIGHT_RANGE);
        CDynLight *pLight;
        CActiveDynLight *pActiveLights = m_data.active[nThread].Buffer();
...
CFaceColor *CLightManager::AvgSgmColor(int32_t nSegment, CFixVector *vPosP, int32_t nThread) {
    ...
    if (nSegment == nDbgSeg)
        BRP;
#endif
    if (!vPosP && (pSegColor->index == (char)(gameData.appData.nFrameCount & 0xff)) &&
        (pSegColor->Red() + pSegColor->Green() + pSegColor->Blue() != 0))
        return pSegColor;
#if DBG
    if (nSegment == nDbgSeg)
        BRP;
