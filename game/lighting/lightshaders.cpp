...
// 2 - 8 light sources
// Implementation hint: dist = max (lightDist - lightRad, /*gl_LightSource [0].constantAttenuation **/ lightDist /
// lightRad) means that as soon as a face to be lit is inside the light's assumed radius (i.e. lightDist <= lightRad),
// we will assume lightDist / lightRad as distance. This has the effect of creating a soft light edge even in such a
// case. The use of a light radius (volume) is required because the Descent engine makes an entire geometry face that
// bears a light emitting texture emit light, so the light sources actually aren't point lights, but light areas.
...
int32_t CreatePerPixelLightingShader(int32_t nType, int32_t nLights) {
    ...
    if (!(ogl.m_features.bShaders && gameStates.render.bUsePerPixelLighting && (ogl.m_features.bPerPixelLighting == 2)))
        ogl.m_features.bPerPixelLighting = gameStates.render.bPerPixelLighting = 0;
    ...
    for (h = 0; h <= 3; h++) {
...
#endif
        {
            ...
            bOk = (pszFS != NULL) && (pszVS != NULL) &&
                  shaderManager.Build(perPixelLightingShaderProgs[i][h], pszFS, pszVS);
            delete[] pszFS;
            delete[] pszVS;
            if (!bOk) {
...
int32_t SetupHardwareLighting(CSegFace *pFace, int32_t nType) {
    ...
    for (nLights = 0; (ogl.m_states.iLight < ogl.m_states.nLights) & (nLightRange > 0) &&
                      (nLights < gameStates.render.nMaxLightsPerPass);
        ...
        specular.Alpha() =
            (psl->info.nSegment >= 0) ? psl->info.fRad : psl->info.fRad * psl->info.fBoost; // krasser Missbrauch!
...
int32_t SetupPerPixelLightingShader(CSegFace *pFace, int32_t nType, bool bHeadlight) {
    ...
    if (!SetupLightmap(pFace))
        return 0;
    GLhandleARB shaderProg = GLhandleARB(
        shaderManager.Deploy(perPixelLightingShaderProgs[gameStates.render.nMaxLightsPerPass][3 /*nType*/]));
    if (!shaderProg) {
    ...
    if (!nType)
        glUniform4fv(
            glGetUniformLocation(shaderProg, "matColor"),
            1,
            reinterpret_cast<GLfloat *>(&pFace->m_info.color));
    ...
    glUniform1f(
        glGetUniformLocation(shaderProg, "fLightScale"),
        (nLights ? GLfloat(nLights) / GLfloat(ogl.m_states.nLights) : 1.0f) * gameData.renderData.fBrightness);
    ogl.ClearError(0);
    PROF_END(ptShaderStates)
