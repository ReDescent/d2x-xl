...
#define BRIEFING_SECRET_NUM \
    31 //  This must correspond to the first secret level which must come at the end of the list.
...
int32_t CBriefing::StartSound(int32_t nChannel, int16_t nSound, fix nVolume, const char *pszWAV) {
    if (nChannel < 0) {
        ...
        nChannel = audio.StartSound(
            audio.XlatSound(nSound),
            SOUNDCLASS_GENERIC,
            nVolume,
            0xFFFF / 2,
            1,
...
void CBriefing::InitCharPos(tBriefingScreen *pScreen, int32_t bRescale) {
    ...
    m_info.briefingTextY =
        gameStates.app.bD1Mission ? pScreen->textTop : pScreen->textTop - (8 * (1 + gameStates.menus.bHires));
}
...
void CBriefing::RenderRobotMovie(void) { RenderElement(5); }
...
int32_t CBriefing::LoadImage(char *szBriefScreen) {
    ...
    if (pcxResult != PCX_ERROR_NONE) {
        ...
        if (strncmp(szErrScreen, szBriefScreen, sizeof(szErrScreen))) {
            Error(
                "Error loading briefing screen <%s>, \nPCX load error: %s (%i)\n",
                szBriefScreen,
                PcxErrorMsg(pcxResult),
                pcxResult);
...
int32_t CBriefing::LoadImage(int32_t nScreen) {
    ...
    szBriefScreen =
        gameStates.app.bD1Mission ? briefingScreens[nScreen % MAX_BRIEFING_SCREENS].szName : m_info.szCurImage;
    if (!*szBriefScreen)
        return 0;
...
int32_t CBriefing::HandleC(void) {
    ...
    if ((c == 'F') || (c == 'B')) {
        m_info.message++;
        CRGBAColor *pColor =
            (c == 'F') ? briefFgColors[gameStates.app.bD1Mission] : briefBgColors[gameStates.app.bD1Mission];
...
int32_t CBriefing::HandleInput(void) {
    if (!m_info.bRedraw && m_info.nDelayCount) {
        ...
        if ((keypress == KEY_ALTED + KEY_ENTER) || (keypress == KEY_ALTED + KEY_PADENTER))
...
int32_t CBriefing::ShowMessage(int32_t nScreen, char *message, int32_t nLevel) {
    ...
    m_info.bOnlyRobots =
        movieManager.m_bHaveExtras && m_info.bExtraSounds && (m_info.nLevel == 1) && (m_info.nScreen < 4);
        ...
        while (m_info.bRedraw) {
            ...
            if (m_info.ch == '$') {
                ...
                for (i = sizeofa(briefingHandlers2), pInfo = briefingHandlers2; i; i--, pInfo++)
                    if (!pInfo->ch ||
                        ((m_info.ch == pInfo->ch) && (!pInfo->prevCh || (m_info.prevCh == pInfo->prevCh)))) {
                        pInfo->handlerP();
                        break;
                    }
            }
...
void CBriefing::Run(const char *filename, int32_t nLevel) {
    ...
    strcpy(
        fnBriefing,
        ((nLevel != 0x7e) && *missionManager.szBriefingFilename[0]) ? missionManager.szBriefingFilename[0] : filename);
    ...
    if (gameStates.app.bD1Mission) {
        ...
        if (nLevel == 1) {
            while (
                !bAbortBriefing && (nCurBriefingScreen < int32_t(MAX_BRIEFING_SCREENS)) &&
