...
void KillEverything(int32_t bVerbose) {
    ...
    extraGameInfo[0].nBossCount[0] = extraGameInfo[0].nBossCount[1] = extraGameInfo[1].nBossCount[0] =
            ...
            for (j = 0; j < gameData.wallData.nWalls; j++) {
                if (WALL(j)->nTrigger == i) {
                    ...
                    gameData.objData.pConsole->info.position.mOrient.m.dir.f =
                        SEGMENT(nSegment)->m_sides[WALL(j)->nSide].m_normals[0];
...
void AccessoryCheat(int32_t bVerbose) {
    if (!gameStates.app.bD1Mission) {
        LOCALPLAYER.flags |= PLAYER_FLAGS_HEADLIGHT | PLAYER_FLAGS_AFTERBURNER | PLAYER_FLAGS_AMMO_RACK |
                             PLAYER_FLAGS_CONVERTER | PLAYER_FLAGS_SLOWMOTION | PLAYER_FLAGS_BULLETTIME;
...
void ExitPathCheat(int32_t bVerbose) { MarkPathToExit(); }
...
void FinishLevelCheat(int32_t bVerbose) { KillEverything(bVerbose); }

//------------------------------------------------------------------------------

void FramerateCheat(int32_t bVerbose) { gameStates.render.bShowFrameRate = !gameStates.render.bShowFrameRate; }
...
void DoWowieCheat(int32_t bVerbose, int32_t bInitialize) {
    ...
    if (gameStates.app.bD1Mission) {
        LOCALPLAYER.primaryWeaponFlags =
            (1 << LASER_INDEX | (1 << VULCAN_INDEX) | (1 << SPREADFIRE_INDEX) | (1 << PLASMA_INDEX)) |
            (1 << FUSION_INDEX);
        LOCALPLAYER.secondaryWeaponFlags = (1 << CONCUSSION_INDEX) | (1 << HOMING_INDEX) | (1 << PROXMINE_INDEX) |
                                           (1 << SMART_INDEX) | (1 << MEGA_INDEX);
        ...
        if (gameData.pigData.tex.nHamFileVersion < 3) { // SHAREWARE
            LOCALPLAYER.primaryWeaponFlags =
                ~((1 << PHOENIX_INDEX) | (1 << OMEGA_INDEX) | (1 << FUSION_INDEX) | HAS_FLAG(SUPER_LASER_INDEX));
...
void SuperWowieCheat(int32_t bVerbose) {
    if (gameStates.gameplay.bMineMineCheat) {
        ...
        LOCALPLAYER.invulnerableTime = LOCALPLAYER.cloakTime = 0;
        OBJECT(N_LOCALPLAYER)->ResetDamage();
    } else {
        AccessoryCheat(bVerbose);
        WowieCheat(bVerbose);
        LOCALPLAYER.flags |= /*PLAYER_FLAGS_CLOAKED |*/ PLAYER_FLAGS_INVULNERABLE;
        LOCALPLAYER.invulnerableTime = LOCALPLAYER.cloakTime = 0x7fffffff;
...
void EnableD1Cheats(int32_t bVerbose) {
    ...
    if (bVerbose)
        HUDInitMessage(
            gameStates.app.cheats.bD1CheatsEnabled ? TXT_WANNA_CHEAT : LOCALPLAYER.score ? TXT_GOODGUY : TXT_TOOLATE);
}
...
inline int32_t Cheat(tCheat *pCheat) {
    ...
    if (pCheat->bPunish && IsMultiGame &&
        !(gameStates.app.bHaveExtraGameInfo[1] &&
          extraGameInfo[1].bEnableCheats)) { // trying forbidden cheatcode in multiplayer
        MultiDoCheatPenalty();
        return 1;
    }
    if ((pCheat->bD1Cheat != -1) &&
        (pCheat->bD1Cheat != gameStates.app.bD1Mission)) { // trying cheat code from other game version
        MultiDoCheatPenalty();
        return 1;
    }
...
void DoCheatMenu() {
    ...
    if (mmn > -1) {
        ...
        if (m[2].Value())
            LOCALPLAYER.flags |= PLAYER_FLAGS_BLUE_KEY | PLAYER_FLAGS_RED_KEY | PLAYER_FLAGS_GOLD_KEY;
