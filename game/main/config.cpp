...
int32_t ReadConfigFile(void) {
    ...
    if (i > 0) {
        ...
        if (i < 0)
            ...
            i = 100;
        gameConfig.nAudioVolume[0] = gameConfig.nAudioVolume[1] = gameConfig.nMidiVolume = (i * 8) / 100;
    }
...
int32_t WriteConfigFile(bool bExitProgram) {
    ...
    sprintf(
        ...
        "%s=%d\n",
        pszHiresMovies,
        (FindArg("-nohires") || FindArg("-nohighres") || FindArg("-lowresmovies")) ? bHiresMoviesSave
