...
CObject *FindInitObject(CObject *pObj) {
    ENTER(0, 0);
    int32_t h, i, j, bUsed, bUseFree, objCount = InitObjectCount(pObj);
    int16_t nType = pObj->info.nType;
    int16_t id = pObj->info.nId;
...
int32_t ChooseDropSegment(CObject *pObj, int32_t *pbFixedPos, int32_t nDropState) {
    ...
    fix nDist = 0;
    int32_t bUseInitSgm = pObj && (EGI_FLAG(bFixedRespawns, 0, 0, 0) ||
                                   (EGI_FLAG(bEnhancedCTF, 0, 0, 0) && (pObj->info.nType == OBJ_POWERUP) &&
                                    ((pObj->info.nId == POW_BLUEFLAG) || (pObj->info.nId == POW_REDFLAG))));
#if TRACE
    ...
    while (nSegment == -1) {
        if (!IsMultiGame)
            ...
            while ((count < N_PLAYERS) && // make sure player is not the local player or on his team
                   (!PLAYER(nPlayer).connected || (nPlayer == N_LOCALPLAYER) ||
                    ((gameData.appData.GameMode(GM_TEAM | GM_CAPTURE | GM_ENTROPY)) &&
                     (GetTeam(nPlayer) == GetTeam(N_LOCALPLAYER))))) {
        ...
        if (pSeg->HasBlockedProp() || (nSegFunc == SEGMENT_FUNC_REACTOR) || (nSegFunc == SEGMENT_FUNC_GOAL_BLUE) ||
            (nSegFunc == SEGMENT_FUNC_GOAL_RED) || (nSegFunc == SEGMENT_FUNC_TEAM_BLUE) ||
...
void RespawnDestroyedWeapon(int16_t nObject) {
    ...
    while (h >= 0) {
        i = h;
        h = gameData.objData.dropInfo[i].nNextPowerup;
        if ((gameData.objData.dropInfo[i].nObject == nObject) && (gameData.objData.dropInfo[i].nDropTime < 0)) {
...
int32_t AddDropInfo(int16_t nObject, int16_t nPowerupType, int32_t nDropTime) {
    ...
    gameData.objData.dropInfo[h].nDropTime =
        (nDropTime > 0) ? nDropTime : (extraGameInfo[IsMultiGame].nSpawnDelay <= 0) ? -1 : gameStates.app.nSDLTicks[0];
        ...
        if (IsMultiGame && (extraGameInfo[IsMultiGame].nSpawnDelay != 0)) {
            if (nDropState == CHECK_DROP) {
                if ((gameData.objData.dropInfo[nObject].nDropTime < 0) ||
                    (gameStates.app.nSDLTicks[0] - gameData.objData.dropInfo[nObject].nDropTime <
            ...
            if (IsMultiGame && (gameStates.multi.nGameType == UDP_GAME) && (nDropState == INIT_DROP) &&
                OBJECT(nObject)->IsMissile()) {
...
void MaybeReplacePowerupWithEnergy(CObject *pDelObj) {
    ...
    if (((nWeapon == VULCAN_INDEX) || (pDelObj->info.contains.nId == POW_VULCAN_AMMO)) &&
        ...
        ((nWeapon == GAUSS_INDEX) || (pDelObj->info.contains.nId == POW_VULCAN_AMMO)) &&
        (LOCALPLAYER.primaryAmmo[VULCAN_INDEX] >= VULCAN_AMMO_MAX))
        pDelObj->info.contains.nCount = 0;
    else if (nWeapon != -1) {
        if ((PlayerHasWeapon(nWeapon, 0, -1, 1) & HAS_WEAPON_FLAG) ||
            WeaponNearby(pDelObj, pDelObj->info.contains.nId)) {
            if (RandShort() > 16384) {
    ...
    if ((pDelObj->info.nCreator == BOSS_GATE_PRODUCER_NUM) && (pDelObj->info.contains.nId == POW_ENERGY) &&
        ...
        console.printf(
            CON_DBG,
            "Converting energy powerup to nothing because robot %i gated in by boss.\n",
            OBJ_IDX(pDelObj));
#endif
        pDelObj->info.contains.nCount = 0;
    }
...
int32_t CObject::CreateEgg(bool bLocal, bool bUpdateLimits) {
    ...
    for (i = info.contains.nCount; i; i--) {
        nObject = DropPowerup(
            info.contains.nType,
            uint8_t(info.contains.nId),
            Index(),
            i == 1, // drop extra powerups?
...
void DropAfterburnerBlobs(
    ...
    for (i = 0; i < nThrusters; i++) {
        ...
        if (xLifeTime != -1) {
            ...
            pBlobObj->rType.animationInfo.xFrameTime = FixMulDiv(
                gameData.effectData.animations[0][ANIM_AFTERBURNER_BLOB].xFrameTime,
                xLifeTime,
                pBlobObj->info.xLifeLeft);
...
void DropMissile1or4(CObject *pPlayerObj, int32_t nMissileIndex) {
    ...
    if (0 < (nMissiles = PLAYER(pPlayerObj->info.nId).secondaryAmmo[nMissileIndex])) {
        ...
        if ((nMissileIndex == CONCUSSION_INDEX) && (pPlayerObj->Id() == N_LOCALPLAYER))
            nMissiles -= gameData.multiplayer.weaponStates[pPlayerObj->Id()]
                             .nBuiltinMissiles; // player gets 4 concs anyway when respawning, so avoid them building up
        if (nMissiles > 0) {
    ...
    if (pPlayerObj && ((pPlayerObj->info.nType == OBJ_PLAYER) || (pPlayerObj->info.nType == OBJ_GHOST))) {
        ...
        if (pPlayer->LaserLevel(0) > 0) {
            if (!(IsBuiltinWeapon(LASER_INDEX) || IsBuiltinWeapon(SUPER_LASER_INDEX))) {
                PrepareObjectCreateEgg(
                    pPlayerObj,
                    pPlayer->LaserLevel(0),
                    OBJ_POWERUP,
                    POW_LASER); // Note: laserLevel = 0 for laser level 1.
                if (bResetLasers)
        ...
        pPlayer->nInvuls = pPlayer->nCloaks = 0;
        ...
        if ((gameData.appData.GameMode(GM_CAPTURE)) && (pPlayer->flags & PLAYER_FLAGS_FLAG))
            PrepareObjectCreateEgg(
                ...
                1,
                OBJ_POWERUP,
                (GetTeam(nPlayer) == TEAM_RED) ? POW_BLUEFLAG : POW_REDFLAG);
        ...
        if (!IsMultiGame || gameStates.app.bHaveExtraGameInfo[1]) {
            ...
            if (0 < (i = nVulcanAmmo / VULCAN_CLIP_CAPACITY -
                         1)) { // drop ammo in excess of presupplied Vulcan/Gauss ammo as vulcan ammo packs
...
int32_t ReturnFlagHome(CObject *pObj) {
    ...
    if (gameStates.app.bHaveExtraGameInfo[1] && extraGameInfo[1].bEnhancedCTF) {
        if (SEGMENT(pObj->info.nSegment)->m_function ==
            ((pObj->info.nId == POW_REDFLAG) ? SEGMENT_FUNC_GOAL_RED : SEGMENT_FUNC_GOAL_BLUE))
