...
void ResetPogEffects(void) {
    ...
    for (bD1 = 0; bD1 <= gameStates.app.bD1Data; bD1++)
        for (i = gameData.effectData.nEffects[bD1], pEffectInfo = gameData.effectData.effects[bD1].Buffer(); i;
             i--, pEffectInfo++) {
            // if (pEffectInfo->flags & EF_FROMPOG)
            pEffectInfo->flags &= ~(EF_ALTFMT | EF_FROMPOG | EF_INITIALIZED);
            pEffectInfo->nCurFrame = 0;
        }
    for (i = gameData.wallData.nAnims[gameStates.app.bD1Data], pWallEffect = gameData.wallData.pAnim.Buffer(); i;
         i--, pWallEffect++)
...
void DoSpecialEffects(bool bSetup) {
    ...
    // if (gameStates.app.tick40fps.bTick)
    {
        ...
        for (i = 0, pEffectInfo = gameData.effectData.pEffect.Buffer();
             i < gameData.effectData.nEffects[gameStates.app.bD1Data];
             i++, pEffectInfo++) {
            ...
            if (pEffectInfo->flags & EF_ALTFMT) {
                if ((pEffectInfo->flags & EF_INITIALIZED) &&
                    ...
                    pBm = SetupHiresAnim(
                        reinterpret_cast<int16_t *>(pEffectInfo->animationInfo.frames),
                        nFrames,
                        t,
                        0,
                        0,
            ...
            while (ft && (pEffectInfo->xTimeLeft < 0)) {
                ...
                if (++(pEffectInfo->nCurFrame) >= nFrames) {
                    if (pEffectInfo->flags & EF_ONE_SHOT) {
                        ...
                        Assert(
                            pEffectInfo->destroyed.nTexture != 0 &&
                            SEGMENT(pEffectInfo->nSegment)->m_sides[pEffectInfo->nSide].m_nOvlTex);
#endif
                        SEGMENT(pEffectInfo->nSegment)->m_sides[pEffectInfo->nSide].m_nOvlTex =
                            pEffectInfo->destroyed.nTexture; // replace with destroyed
        ...
        for (i = 0, pEffectInfo = gameData.effectData.effects[0].Buffer(); i < gameData.effectData.nEffects[0];
             i++, pEffectInfo++) {
...
#endif
                        SEGMENT(pEffectInfo->nSegment)->m_sides[pEffectInfo->nSide].m_nOvlTex =
                            pEffectInfo->destroyed.nTexture; // replace with destoyed
                        pEffectInfo->flags &= ~EF_ONE_SHOT;
                        pEffectInfo->nSegment = -1; // done with this
                    }
                    pEffectInfo->nCurFrame = 0;
            ...
            if ((pEffectInfo->nCriticalAnimation != -1) && gameData.reactorData.bDestroyed) {
                int32_t n = pEffectInfo->nCriticalAnimation;
                bmi =
                    gameData.effectData.effects[0][n].animationInfo.frames[gameData.effectData.effects[0][n].nCurFrame];
                ...
                bmi = pEffectInfo->animationInfo.frames[pEffectInfo->nCurFrame];
                //*pEffectInfo->bm_ptr = &gameData.pigData.tex.bitmaps [gameData.effectData.effects
                //[0][n].animationInfo.frames [gameData.effectData.effects [0][n]..nCurFrame].index]; if
                // (pEffectInfo->changing.nObjectTexture != -1)
                gameData.pigData.tex.objBmIndex[t] = bmi;
            }
        }
        xEffectTime = 0;
...
void RestoreEffectBitmapIcons(void) {
    ...
    for (i = 0,
        j = gameData.effectData.nEffects[gameStates.app.bD1Data],
    ...
    for (i = 0, j = gameData.effectData.nEffects[0], pEffectInfo = gameData.effectData.effects[0].Buffer(); i < j;
         i++, pEffectInfo++)
        if (!(pEffectInfo->flags & EF_CRITICAL)) {
            bmi = pEffectInfo->animationInfo.frames[0];
            if (pEffectInfo->changing.nWallTexture != -1)
