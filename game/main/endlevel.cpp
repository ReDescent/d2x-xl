...
char movieTable[2][30] = {
    {'a', 'b', 'c', 'a', 'd', 'f', 'd', 'f', 'g', 'h', 'i', 'g', 'j', 'k', 'l',
     'j', 'm', 'o', 'm', 'o', 'p', 'q', 'p', 'q', 0,   0,   0,   0,   0,   0},
    {'f', 'f', 'f', 'a', 'a', 'b', 'b', 'c', 'c', 'c', 'g', 'f', 'g', 'g', 'f',
     'g', 'f', 'g', 'f', 'f', 'f', 'g', 'f', 'g', 'd', 'd', 'f', 'e', 'e', 'e'}};
...
void StartEndLevelSequence(int32_t bSecret) {
    ...
    if ((nMoviePlayed == MOVIE_NOT_PLAYED) &&
        gameStates.app.bEndLevelDataLoaded) { // don't have movie.  Do rendered sequence, if available
...
int32_t ChaseAngles(CAngleVector *cur_angles, CAngleVector *desired_angles) {
    ...
    if (abs(delta_angs.v.coord.p) < frame_turn) {
        ...
        mask |= 1;
    } else if (delta_angs.v.coord.p > 0)
    ...
    if (abs(delta_angs.v.coord.b) < frame_turn) {
        ...
        mask |= 2;
    } else if (delta_angs.v.coord.b > 0)
    ...
    if (abs(delta_angs.v.coord.h) < frame_turn) {
        ...
        mask |= 4;
    } else if (delta_angs.v.coord.h > 0)
...
void DoEndLevelFrame(void) {
    ...
    if (!gameStates.render.bOutsideMine) {
        if (gameStates.app.bEndLevelSequence == EL_OUTSIDE) {
            ...
            if (CFixVector::Dot(tvec, gameData.endLevelData.exit.mOrient.m.dir.f) > 0) {
                ...
                pObj = CreateExplosion(
                    gameData.endLevelData.exit.nSegNum,
                    gameData.endLevelData.exit.vSideExit,
                    I2X(50),
                    ANIM_BIG_PLAYER_EXPLOSION);
                if (pObj) {
        ...
        if ((explosion_wait1 -= gameData.timeData.xFrame) < 0) {
            ...
            vPos =
                gameData.objData.pConsole->info.position.vPos +
                gameData.objData.pConsole->info.position.mOrient.m.dir.f * (-gameData.objData.pConsole->info.xSize * 5);
        ...
        if (gameData.objData.pConsole->info.nSegment == gameData.endLevelData.exit.nTransitSegNum) {
            if ((missionManager.nCurrentMission == missionManager.nBuiltInMission[0]) &&
                ...
                gameData.objData.endLevelCamera->info.position.vPos +=
                    gameData.objData.endLevelCamera->info.position.mOrient.m.dir.f * (I2X(7));
                timer = 0x20000;
            }
        }
        break;
    ...
    case EL_LOOKBACK: {
        ...
        if (gameData.objData.endLevelCamera->info.nSegment == gameData.endLevelData.exit.nSegNum) {
            ...
            gameData.objData.endLevelCamera->info.position.mOrient.m.dir.f =
                -gameData.objData.endLevelCamera->info.position.mOrient.m.dir.f;
            gameData.objData.endLevelCamera->info.position.mOrient.m.dir.r =
                -gameData.objData.endLevelCamera->info.position.mOrient.m.dir.r;
...
#endif
        gameData.objData.pConsole->info.position.vPos +=
            gameData.objData.pConsole->info.position.mOrient.m.dir.f *
            (FixMul(gameData.timeData.xFrame, gameData.endLevelData.xCurFlightSpeed));
#ifndef SLEW_ON
        gameData.objData.endLevelCamera->info.position.vPos +=
            gameData.objData.endLevelCamera->info.position.mOrient.m.dir.f *
    ...
    case EL_STOPPED: {
        GetAnglesToObject(
            &vPlayerDestAngles,
            &gameData.endLevelData.station.vPos,
            &gameData.objData.pConsole->info.position.vPos);
...
void DrawExitModel(void) {
    ...
    DrawPolyModel(
        NULL,
        &vModelPos,
        &gameData.endLevelData.exit.mOrient,
        NULL,
        gameStates.gameplay.bMineDestroyed ? gameData.endLevelData.exit.nDestroyedModel
...
void RenderExternalScene(fix xEyeOffset) {
    ...
    if (!(p.Codes() & (CC_BEHIND | PF_OVERFLOW))) {
        ...
        if (!gameData.endLevelData.satellite.pBm->Bind(1))
            G3DrawRodTexPoly(
                gameData.endLevelData.satellite.pBm,
                &p,
                SATELLITE_WIDTH,
                &pTop,
                SATELLITE_WIDTH,
    ...
    DrawPolyModel(
        ...
        &vmdIdentityMatrix,
        NULL,
        gameData.endLevelData.station.nModel,
        0,
        I2X(1),
        NULL,
        NULL);
...
void RenderEndLevelMine(fix xEyeOffset, int32_t nWindowNum) {
    ...
    if (gameData.objData.pViewer->info.nType == OBJ_PLAYER)
        gameData.renderData.mine.viewer.vPos +=
            gameData.objData.pViewer->info.position.mOrient.m.dir.f * ((gameData.objData.pViewer->info.xSize * 3) / 4);
    if (xEyeOffset)
        gameData.renderData.mine.viewer.vPos += gameData.objData.pViewer->info.position.mOrient.m.dir.r * (xEyeOffset);
    if (gameStates.app.bEndLevelSequence >= EL_OUTSIDE)
    ...
    if (pExitFlightData->bStart || (UpdateObjectSeg(pObj, false) > 0)) {
        ...
        if (pExitFlightData->bStart || (pObj->info.nSegment != nOldPlayerSeg)) { // moved into new seg
            ...
            if ((nExitSide >= 0) && (pSeg->m_children[nExitSide] >= 0)) {
                ...
                if (xSegTime) {
                    pExitFlightData->aStep.v.coord.x =
                        Max(-MAX_ANGSTEP,
                            Min(MAX_ANGSTEP,
                                FixDiv(DeltaAng(pExitFlightData->angles.v.coord.p, aDest.v.coord.p), xSegTime)));
                    pExitFlightData->aStep.v.coord.z =
                        Max(-MAX_ANGSTEP,
                            Min(MAX_ANGSTEP,
                                FixDiv(DeltaAng(pExitFlightData->angles.v.coord.b, aDest.v.coord.b), xSegTime)));
                    pExitFlightData->aStep.v.coord.y =
                        Max(-MAX_ANGSTEP,
                            Min(MAX_ANGSTEP,
                                FixDiv(DeltaAng(pExitFlightData->angles.v.coord.h, aDest.v.coord.h), xSegTime)));
                } else {
...
int32_t DoSlewMovement(CObject *pObj, int32_t check_keys, int32_t check_joy) {
    ...
    if (check_joy && bJoyPresent) {
        ...
        if (abs(joy_y) < JOY_NULL)
            joy_y = 0;
        if (btns)
            if (!rotang.dir.coord.pitch)
                rotang.dir.coord.pitch = FixMul(-joy_y * 512, gameData.timeData.xFrame);
            else
                ;
        else if (joyy_moved)
            pObj->physInfo.velocity.z = -joy_y * 8192;
        if (!rotang.head)
            rotang.head = FixMul(joy_x * 512, gameData.timeData.xFrame);
        if (joyx_moved)
            old_joy_x = joy_x;
        if (joyy_moved)
            old_joy_y = joy_y;
...
try_again:
    ...
    gameData.endLevelData.exit.vGroundExit =
        gameData.endLevelData.exit.vMineExit + gameData.endLevelData.exit.mOrient.m.dir.u * (-I2X(20));
