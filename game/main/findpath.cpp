...
void CSimpleHeap::Setup(int16_t nStartSeg, int16_t nDestSeg, uint32_t flag, int32_t dir) {
    ...
    m_nStartSeg = m_queue[0] = nStartSeg;
    m_nDestSeg = nDestSeg;
    m_nTail = 0;
    m_nHead = 1;
    m_bFlag = flag;
...
bool CSimpleUniDirHeap::Match(int16_t nSegment, CScanInfo &scanInfo) { return (nSegment == m_nDestSeg); }
...
fix CSimpleBiDirRouter::FindPath(void) {
    ...
    if (gameStates.app.nThreads > 1) {
        ...
        if (((0 < (m_scanInfo.m_nLinkSeg = m_heap[0].nLinkSeg)) &&
             (m_scanInfo.m_nLinkSeg != m_heap[0].m_nDestSeg + 1)) ||
...
fix CDACSUniDirRouter::BuildPath(int16_t nSegment) {
    ...
    for (int32_t i = 1, j; i < h; i = j) {
        // beginning at segment route [i].node, traverse the route until the center of route segment route [j].nNode
        // cannot be seen from the center of segment route [i].nNode. That way, the distance calculation is corrected by
        // using direct lines of sight between segments of the route that can "see" each other even if they aren't
        // directly connected.
    ...
    if (m_nDestSeg >= 0) {
        xDist += CFixVector::Dist(m_p0, SEGMENT(route[1].nNode)->Center()) +
                 CFixVector::Dist(m_p1, SEGMENT(route[h].nNode)->Center());
        if (m_cacheType >= 0)
...
#endif
                if (m_heap
                        .Push(pSeg->m_children[nSide], nSegment, nSide, nDist + uint16_t(pSeg->m_childDists[1][nSide])))
                    ++nExpanded;
            }
        }
    }
...
fix CDACSUniDirRouter::Distance(int16_t nSegment) { return BuildPath(nSegment); }
...
fix CDACSBiDirRouter::BuildPath(int16_t nSegment) {
    ...
    xDist += CFixVector::Dist(m_p0, SEGMENT(m_route[1].nNode)->Center()) +
             CFixVector::Dist(m_p1, SEGMENT(m_route[j].nNode)->Center());
    if (m_cacheType >= 0)
