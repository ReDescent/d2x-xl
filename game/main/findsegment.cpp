...
static int32_t TraceSegs(
    ...
    for (;;) {
        ...
        for (nSide = 0, bit = 1; nSide < SEGMENT_SIDE_COUNT; nSide++, bit <<= 1)
            if (pSeg->Side(nSide)->FaceCount() && (centerMask & bit) &&
                (xTolerance || (pSeg->m_children[nSide] > -1)) && (xSideDists[nSide] < xMaxDist)) {
                if (xTolerance && (xTolerance >= -xSideDists[nSide]) &&
                    (xTolerance >= pSeg->Side(nSide)->DistToPoint(vPos)))
                    return nCurSeg;
        ...
        if (0 <= (nMatchSeg = TraceSegs(
                      vPos,
                      pSeg->m_children[nMaxSide],
                      nTraceDepth + 1,
                      bVisited,
                      bFlag,
                      xTolerance))) // trace into adjacent CSegment
            break;
    }
...
static int32_t TraceSegsf(
    ...
    for (;;) {
        ...
        if (0 <= (nMatchSeg = TraceSegsf(
                      vPos,
                      pSeg->m_children[nMaxSide],
                      nTraceDepth + 1,
                      bVisited,
                      bFlag,
                      fTolerance))) // trace into adjacent CSegment
            break;
    }
