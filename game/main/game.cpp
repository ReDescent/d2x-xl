...
void GameInitRenderBuffers(
    int32_t nScreenSize,
    int32_t render_w,
    int32_t render_h,
    int32_t render_method,
    int32_t flags) {
...
void DoInvulnerableStuff(void) {
    ENTER(0, 0);
    if ((LOCALPLAYER.flags & PLAYER_FLAGS_INVULNERABLE) && (LOCALPLAYER.invulnerableTime != 0x7fffffff)) {
...
void DoAfterburnerStuff(void) {
    ...
    } else /*if ((gameStates.gameplay.xLastAfterburnerCharge && (controls [0].afterburnerState !=
              gameStates.gameplay.bLastAfterburnerState)) || (gameStates.gameplay.bLastAfterburnerState &&
              (gameStates.gameplay.xLastAfterburnerCharge && !gameData.physicsData.xAfterburnerCharge)))*/
    {
        ...
        if (gameData.physicsData.xAfterburnerCharge && controls[0].afterburnerState &&
            (LOCALPLAYER.flags & PLAYER_FLAGS_AFTERBURNER)) {
            if (nSoundObj < 0) {
                audio.CreateObjectSound(
                    (int16_t)SOUND_AFTERBURNER_IGNITE,
                    SOUNDCLASS_PLAYER,
...
CCanvas *CurrentGameScreen(void) { return &gameData.renderData.scene; }
...
void CGameLoop::Setup(void) {
    ...
    gameStates.app.nSDLTicks[0] = gameStates.app.nSDLTicks[1] = gameData.timeData.xGameStart = SDL_GetTicks();
...
int32_t CGameLoop::Preprocess(void) {
    ...
    if (IsMultiGame) {
        ...
        MultiCheckForScoreGoalWinner(
            netGameInfo.GetPlayTimeAllowed() &&
            (gameStates.app.xThisLevelTime >= I2X((netGameInfo.GetPlayTimeAllowed() * 5 * 60))));
...
void CGameLoop::HandleAutomap(void) {
    ENTER(0, 0);
    if (automap.Active()) {
        int32_t save_w = gameData.renderData.frame.Width(), save_h = gameData.renderData.frame.Height();
        automap.DoFrame(0, 0);
...
void ComputeSlideSegs(void) {
    ...
    for (nSegment = 0; nSegment <= gameData.segData.nLastSegment; nSegment++, pSeg++) {
        ...
        for (nSide = 0; nSide < SEGMENT_SIDE_COUNT; nSide++) {
            ...
            if (gameData.pigData.tex.pTexMapInfo[nTexture].slide_u ||
                gameData.pigData.tex.pTexMapInfo[nTexture].slide_v) {
                if (!bIsSlideSeg) {
                    bIsSlideSeg = 1;
...
int32_t MarkPlayerPathToSegment(int32_t nSegment) {
    ...
    if (int32_t(gameData.aiData.routeSegs.Index(gameData.aiData.freePointSegs)) + MAX_PATH_LENGTH * 2 >
        MAX_POINT_SEGS) {
#if TRACE
