...
void EvalAutoNetGameArgs(void) {
    ...
    if (gameData.multiplayer.autoNG.bHost)
        gameData.multiplayer.autoNG.bValid =
            *gameData.multiplayer.autoNG.szPlayer && *gameData.multiplayer.autoNG.szName &&
            *gameData.multiplayer.autoNG.szFile && *gameData.multiplayer.autoNG.szMission;
    else
        gameData.multiplayer.autoNG.bValid =
...
void EvalRenderArgs(void) {
    ...
    if ((t = FindArg("-hires_textures")))
        gameOptions[0].render.textures.bUseHires[0] = gameOptions[0].render.textures.bUseHires[1] = NumArg(t, 1);
...
void EvalAppArgs(void) {
    ...
    if ((t = FindArg("-autodemo"))) {
        ...
        strncpy(
            gameData.demoData.fnAuto,
            *appConfig[t + 1] ? appConfig[t + 1] : "descent.dem",
            sizeof(gameData.demoData.fnAuto));
    } else
