...
void CGameData::SetStereoSeparation(int32_t nFrame) {
    ogl.SetStereoSeparation(
        ogl.IsSideBySideDevice() ? gameOpts->render.stereo.xSeparation[ogl.IsOculusRift()] * nFrame : 0);
}
...
CDynLightData::CDynLightData() { Init(); }
...
bool CLightData::Resize(void) {
    if (!(/*gameStates.app.bNostalgia ||*/ lightManager.Create()))
        return false;
    return dynamicLight.Resize(LEVEL_VERTICES) && dynamicColor.Resize(LEVEL_VERTICES) &&
           bGotDynColor.Resize(LEVEL_VERTICES) && vertices.Resize(LEVEL_VERTICES) && vertexFlags.Resize(LEVEL_VERTICES);
}
...
void CShadowData::Destroy(void) { DESTROY(objLights); }
...
CHUDData::CHUDData() { bPlayerMessage = 1; }
...
CFaceListIndex::~CFaceListIndex() { Destroy(); }
...
void CFaceListIndex::Create(void) {
    ...
    roots.Create(
        ...
        "CFaceListIndex::roots"); //((MAX_WALL_TEXTURES  + MAX_WALL_TEXTURES / 10) * 3);
    tails.Create(
        gameData.segData.nFaceKeys,
        "CFaceListIndex::tails"); //((MAX_WALL_TEXTURES  + MAX_WALL_TEXTURES / 10) * 3);
    usedKeys.Create(
        gameData.segData.nFaceKeys,
        "CFaceListIndex::usedKeys"); //((MAX_WALL_TEXTURES  + MAX_WALL_TEXTURES / 10) * 3);
    Init();
}
...
bool CSegmentData::Resize(void) {
    return gameData.renderData.mine.Resize() && gameData.segData.vertices.Resize(LEVEL_VERTICES) &&
           gameData.segData.fVertices.Resize(LEVEL_VERTICES) && gameData.segData.vertexOwners.Resize(LEVEL_VERTICES);
}
...
inline fix Floor(fix v) { return (v >= 0) ? v & 0xFFFF0000 : -Ceil(-v); }

// ----------------------------------------------------------------------------

inline fix Ceil(fix v) { return (v >= 0) ? Floor(v + 0xFFFF) : -Floor(-v); }
...
CTextureData::CTextureData() {
    for (int32_t i = 0; i < 2; i++) {
        ...
        tMapInfo[i].Create(
            MAX_TEXTURES + MAX_TEXTURES / 10,
            szLabel); // add some room for extra textures like e.g. from the hoard data
        tMapInfo[i].Clear();
...
CShipData::CShipData() { player = &only; }
...
CMorphData::CMorphData() { gameData.renderData.morph.xRate = MORPH_RATE; }
...
int32_t CObjectData::RebuildEffects(void) {
    ...
    if (nEffects && effects.Buffer()) {
        for (int32_t i = 0; i < nEffects; i++) {
            ...
            int32_t nObject = CreateObject(
                bo.info.nType,
                bo.info.nId,
                -1,
                -bo.info.nSegment - 2,
                bo.info.position.vPos,
                bo.info.position.mOrient,
                bo.info.xSize,
                bo.info.controlType,
                bo.info.movementType,
                bo.info.renderType);
            if (nObject >= 0) {
...
bool CObjectData::IsGuidedMissile(CObject *pObj) { return pObj && pObj->IsGuidedMissile(); }

// ----------------------------------------------------------------------------

bool CObjectData::HasGuidedMissile(uint8_t nPlayer) { return IsGuidedMissile(guidedMissile[nPlayer].pObj); }
...
bool CColorData::Resize(void) {
    return vertices.Resize(LEVEL_VERTICES) && vertBright.Resize(LEVEL_VERTICES) && ambient.Resize(LEVEL_VERTICES);
}
...
CPhysicsData::CPhysicsData() { Init(); }
...
bool CPhysicsData::Create(void) { return true; }
...
void CWeaponData::Destroy(void) { color.Destroy(); }

// ----------------------------------------------------------------------------

COmegaData::COmegaData() {
    memset(this, 0, sizeof(*this));
    xCharge[0] = xCharge[1] = xMaxCharge = DEFAULT_MAX_OMEGA_CHARGE;
}
...
void CMultiplayerData::Destroy(void) { DESTROY(gameData.multiplayer.leftoverPowerups); }
...
bool CMultiGameData::Create(void) {
    for (int32_t i = 0; i < MAX_NUM_NET_PLAYERS; i++)
        CREATE(
            gameData.multigame.remoteToLocal[i],
            LEVEL_OBJECTS,
            0xff); // Remote CObject number for each local CObject
    CREATE(gameData.multigame.localToRemote, LEVEL_OBJECTS, 0xff);
    CREATE(
        gameData.multigame.nObjOwner,
        LEVEL_OBJECTS,
        0xff); // Who created each CObject in my universe, -1 = loaded at start
    return true;
}
...
CTerrainRenderData::CTerrainRenderData() {
    ...
    pBm = NULL;
    CLEAR(uvlList);
    uvlList[0][1].u = uvlList[0][2].v = uvlList[1][0].u = uvlList[1][1].u = uvlList[1][1].v = uvlList[1][2].v = I2X(1);
}
...
fix CGameData::FusionDamage(fix xBaseDamage) {
    return (gameStates.app.bHaveExtraGameInfo[IsMultiGame] && !COMPETITION && !EGI_FLAG(bUnnerfD1Weapons, 0, 0, 0))
...
bool CGameData::Create(int32_t nSegments, int32_t nVertices) {
    ENTER(0, 0);
    if (!(gameData.segData.Create(nSegments, nVertices) && gameData.objData.Create() &&
          gameData.renderData.color.Create() && gameData.renderData.lights.Create() &&
          gameData.renderData.shadows.Create() && gameData.weaponData.Create() && gameData.physicsData.Create() &&
          gameData.aiData.Create() && gameData.multiplayer.Create() && gameData.multigame.Create() &&
...
void SetDataVersion(int32_t v) {
    ...
    PrintLog(
        0,
        "Setting data version %d (D%d) (D1 mission = %d, have D1 data = %d)\n",
        v,
        2 - gameStates.app.bD1Data,
        gameStates.app.bD1Mission,
        gameStates.app.bHaveD1Data);
...
void DefaultKeyboardSettings(bool bSetup) { gameOptions[0].input.bUseHotKeys = 1; }

// ----------------------------------------------------------------------------

void DefaultApplicationSettings(bool bSetup) { gameOptions[0].app.nVersionFilter = 3; }
...
tRobotInfo *CGameData::RobotInfo(int32_t nId, int32_t nChecks, const char *pszFile, int32_t nLine) {
    ...
    if (nId < 0)
        return (
            tRobotInfo *)GameDataError("robot info", "underflow", nChecks & GAMEDATA_ERRLOG_UNDERFLOW, pszFile, nLine);
    if ((uint32_t)nId >= a.Length())
        return (
            tRobotInfo *)GameDataError("robot info", "overflow", nChecks & GAMEDATA_ERRLOG_OVERFLOW, pszFile, nLine);
...
CWeaponInfo *CGameData::WeaponInfo(int32_t nId, int32_t bD1, int32_t nChecks, const char *pszFile, int32_t nLine) {
    ...
    if (nId < 0)
        return (CWeaponInfo *)
            GameDataError("weapon info", "underflow", nChecks & GAMEDATA_ERRLOG_UNDERFLOW, pszFile, nLine);
    if (nId >= gameData.weaponData.nTypes[gameStates.app.bD1Mission])
        return (
            CWeaponInfo *)GameDataError("weapon info", "overflow", nChecks & GAMEDATA_ERRLOG_OVERFLOW, pszFile, nLine);
    return a + nId;
}
...
static inline CWeaponInfo *WI_Unnerfed(int32_t nId) {
    return gameData.WeaponInfo(
        nId,
        ((nId < gameData.weaponData.nTypes[1]) && EGI_FLAG(bUnnerfD1Weapons, 0, 0, 0)) ? 1 : -1);
}
