...
static int32_t GetSystemFolders(int32_t &nSharedFolderMode, int32_t &nUserFolderMode) {
    ...
    if (!FindDataFolder(appConfig.Text("-datadir")) && !FindDataFolder(getenv("DESCENT2")) &&
        !FindDataFolder(appConfig[1], true) && !FindDataFolder(DEFAULT_GAME_FOLDER))
        return 0;
    ...
    *gameFolders.szSharePath = '\0';
    ...
    if (!FindDataFolder(appConfig.Text("-datadir")) && !FindDataFolder(gameFolders.szSharePath) &&
        !FindDataFolder(getenv("DESCENT2")) && !FindDataFolder(appConfig[1], true) &&
    ...
    if (!FindDataFolder(appConfig.Text("-datadir")) && !FindDataFolder(appConfig[1], true) &&
        !FindDataFolder(DEFAULT_GAME_FOLDER)) {
...
static int32_t MakeCacheFolders(int32_t nSharedFolderMode, int32_t nUserFolderMode) {
    ...
    if (*gameFolders.user.szRoot)
        ...
        if (!MakeFolder(gameFolders.user.szCache, gameFolders.user.szRoot, CACHE_FOLDER))
            strcpy(gameFolders.user.szCache, gameFolders.user.szRoot); // if user cache folder cannot be created, put
    ...
    if (*gameFolders.var.szRoot)
        ...
        if (MakeFolder(gameFolders.var.szRoot))
            ...
            // fall back to user cache folder if folder cannot be created
    ...
    if (!nSharedFolderMode)
        ...
        (nSharedFolderMode > 0) && !MakeFolder(gameFolders.var.szCache, gameFolders.var.szRoot, SHARED_CACHE_FOLDER)) {
...
static int32_t MakeGameFolders(void) {
    ...
    if (!GetAppFolder(gameFolders.game.szRoot, gameFolders.game.szTextures[0], TEXTURE_FOLDER, "")) {
        if (GetAppFolder(gameFolders.game.szTextures[0], gameFolders.game.szTextures[1], TEXTURE_FOLDER_D2, ""))
            MakeFolder(
                gameFolders.game.szTextures[0],
                gameFolders.game.szTextures[0],
                TEXTURE_FOLDER_D2); // older D2X-XL installations may have a different D2 texture folder
...
static int32_t MakeSharedFolders(void) {
    ...
    int32_t i = MakeFolder(gameFolders.var.szModels[0], gameFolders.var.szCache, MODEL_FOLDER) &&
                ...
                MakeTexSubFolders(gameFolders.var.szTextures[1]) && MakeTexSubFolders(gameFolders.var.szTextures[2]) &&
...
#else
    MakeFolder(
        gameFolders.user.szWallpapers,
        strcmp(gameFolders.game.szRoot, gameFolders.user.szRoot) ? gameFolders.user.szCache
                                                                 : gameFolders.game.szTextures[0],
        WALLPAPER_FOLDER);
#endif
...
void GetAppFolders(bool bInit) {
    ...
    *gameFolders.user.szRoot = *gameFolders.game.szRoot = *gameFolders.game.szData[0] = *gameFolders.game.szData[1] =
        *gameFolders.var.szCache = *gameFolders.user.szCache = '\0';
...
void ResetModFolders(void) {
    ...
    *gameFolders.mods.szName = *gameFolders.mods.szCache = *gameFolders.mods.szMusic = *gameFolders.mods.szSounds[0] =
        *gameFolders.mods.szSounds[1] = *gameFolders.mods.szCurrent = *gameFolders.mods.szTextures[0] =
            *gameFolders.mods.szTextures[1] = *gameFolders.var.szTextures[3] = *gameFolders.var.szTextures[4] =
                *gameFolders.mods.szModels[0] = *gameFolders.var.szModels[1] = *gameFolders.var.szModels[2] =
    ...
    if (GetAppFolder(gameFolders.mods.szRoot, gameFolders.mods.szCurrent, gameFolders.mods.szName, "")) {
        *gameFolders.mods.szName = '\0';
        ...
        sprintf(
            ...
            "%s%s/",
            gameFolders.var.szMods,
            gameFolders.mods.szName); // e.g. /var/cache/d2x-xl/mods/mymod/
        ...
        if (GetAppFolder(gameFolders.mods.szCurrent, gameFolders.mods.szTextures[0], TEXTURE_FOLDER, "")) {
            *gameFolders.mods.szTextures[0] = '\0';
            ...
            sprintf(
                ...
                "%s%s/",
                gameFolders.mods.szCache,
                TEXTURE_FOLDER); // e.g. /var/cache/d2x-xl/mods/mymod/textures/
        ...
        if (GetAppFolder(gameFolders.mods.szCurrent, gameFolders.mods.szWallpapers, WALLPAPER_FOLDER, "*.tga")) {
            ...
            *gameOpts->menus.altBg.szName[1] = '\0';
            ...
            if (nLevel < 0)
                ...
                // chose a random custom loading screen for missions other than D2:CS that do not have their own custom
                // loading screens
                if ((bBuiltIn == 2) && (missionManager.IsBuiltIn(hogFileManager.MissionName()) != 2)) {
                    if (nLoadingScreen <
                        0) { // create a random offset the first time this function is called and use it later on
                        int32_t i;
                        for (i = 0; i < 24; i++)
