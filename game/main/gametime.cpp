...
template <class _T> class CFrameTime : public CGenericFrameTime {
    protected:
    _T m_tLast;
    _T m_tick;
...
void CWindowsFrameTime::GetTick(void) { QueryPerformanceCounter(&m_tick); }
...
void CUnixFrameTime::Setup(void) { CFrameTime<int64_t>::Setup(); }
...
void CSDLFrameTime::GetTick(void) { m_tick = SDL_GetTicks(); }
...
int32_t TimeStopped(void) { return gameData.timeData.nPaused > 0; }
...
void CalcFrameTime(int32_t fps) {
    ...
    fix timerValue, xLastFrameTime = gameData.timeData.xFrame;
