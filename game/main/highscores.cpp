...
void CScoreTable::DrawItem(int32_t i) {
    ...
    if (!gameData.appData.GameMode(GM_MODEM | GM_SERIAL))
        GrPrintF(
            NULL,
            LHX(CENTERING_OFFSET(N_PLAYERS) - 15),
            y,
            "%c",
            szConditionLetters[PLAYER(m_sorted[i]).connected]);
    ...
    if (PLAYER(m_sorted[i]).netKilledTotal + PLAYER(m_sorted[i]).netKillsTotal == 0)
        ...
        sprintf(
            ...
            "%d%%",
            (int32_t)(
                (double)PLAYER(m_sorted[i]).netKillsTotal /
                (double)(PLAYER(m_sorted[i]).netKilledTotal + PLAYER(m_sorted[i]).netKillsTotal) * 100.0));
...
void CScoreTable::DrawCoopItem(int32_t i) {
    ...
    GrPrintF(
        NULL,
        LHX(CENTERING_OFFSET(N_PLAYERS) - 15) + xOffs,
        y,
        "%c",
        szConditionLetters[PLAYER(m_sorted[i]).connected]);
    ...
    for (j = 0; j < N_PLAYERS; j++) {
        ...
        if (PLAYER(m_sorted[j]).connected == CONNECT_DISCONNECTED)
            ...
            fontManager.SetColorRGBi(
                RGBA_PAL2(playerColors[color].Red(), playerColors[color].Green(), playerColors[color].Blue()),
                1,
                0,
                0);
...
void CScoreTable::DrawDeaths(void) {
    ...
    if (gameData.reactorData.countdown.nSecsLeft <= 0)
        ...
        sprintf(
            reinterpret_cast<char *>(&reactor_message),
            "%s: %d %s  ",
            TXT_TIME_REMAINING,
            gameData.reactorData.countdown.nSecsLeft,
            TXT_SECONDS);
