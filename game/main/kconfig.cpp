...
uint8_t system_keys[] = {
    ...
    (uint8_t)KEY_MINUS,
    (uint8_t)KEY_EQUALS,
    (uint8_t)KEY_ALTED + KEY_F9};

#define TABLE_CREATION 0

// Array used to 'blink' the cursor while waiting for a keypress.
int8_t fades[64] = {1,  1,  1,  2,  2,  3,  4,  4,  5,  6,  8,  9,  10, 12, 13, 15, 16, 17, 19, 20, 22, 23,
...
// char * joybutton_text [28] = { "TRIG", "BTN 1", "BTN 2", "BTN 3", "BTN 4", "", "LEFT", "HAT ", "RIGHT", "", "", "HAT
// ", "MID", "", "", "HAT ", "", "", "", "HAT ", "TRIG", "LEFT", "RIGHT", "", "UP","DOWN","LEFT", "RIGHT" }; char *
// JOYAXIS_TEXT [4] = { "X1", "Y1", "X2", "Y2" }; char * mouseAxisTextIndex [2] = { "L/R", "F/B" }; char *
// mouseButtonTextIndex [3] = { "Left", "Right", "Mid" };

int32_t yesNoTextIndex[2] = {TNUM_N, TNUM_Y};

#ifndef USE_LINUX_JOY
int32_t joybutton_text[28] = {TNUM_BTN_1,  TNUM_BTN_2, TNUM_BTN_3,  TNUM_BTN_4,  -1,          TNUM_TRIG,  TNUM_LEFT,
                              TNUM_HAT_L,  TNUM_RIGHT, -1,          TNUM_HAT2_D, TNUM_HAT_R,  TNUM_MID,   -1,
                              TNUM_HAT2_R, TNUM_HAT_U, TNUM_HAT2_L, -1,          TNUM_HAT2_U, TNUM_HAT_D, TNUM_TRIG,
...
const char *extraMouseButtonTextIndex[13] =
    {"MW UP", "MW DN", "M6", "M7", "M8", "M9", "M10", "M11", "M12", "M13", "M14", "M15", "M16"}; // text for buttons
                                                                                                 // above 3. -MPM
...
tControlSettings controlSettings = {
    {
        {0xc8, 0x48, 0xd0, 0x50, 0xcb, 0x4b, 0xcd, 0x4d, 0x38, 0xff, 0xff, 0x4f, 0xff, 0x51, 0xff,
         0x4a, 0xff, 0x4e, 0xff, 0xff, 0x10, 0x47, 0x12, 0x49, 0x1d, 0x9d, 0x39, 0xff, 0x21, 0xff,
         0x1e, 0xff, 0x2c, 0xff, 0x30, 0xff, 0x13, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf,
         0xff, 0x1f, 0xff, 0x33, 0xff, 0x34, 0xff, 0x23, 0xff, 0x14, 0xff, 0xff, 0xff, 0x0,  0x0},
    ...
    {0x2,  0xff, 0x3,  0xff, 0x4,  0xff, 0x5,  0xff, 0x6,  0xff, 0x7,  0xff, 0x8,  0xff,
     0x9,  0xff, 0xa,  0xff, 0xb,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
const char *CControlConfig::YesNoText(int32_t i) { return baseGameTexts[yesNoTextIndex[i != 0]][0]; }
...
void CControlConfig::DrawItem(kcItem *item, int32_t is_current, int32_t bRedraw) {
    ...
    *szText = '\0';
    ...
    if (item->w1) {
        ...
        OglDrawFilledRect(
            KC_LHX(item->x + item->w1),
            KC_LHY(item->y - 1),
            KC_LHX(item->x + item->w1 + item->w2),
            KC_LHY(item->y) + h);
...
void CControlConfig::DrawQuestion(kcItem *item) {
    ...
    if (++looper > 63)
        looper = 0;
    OglDrawFilledRect(
        KC_LHX(item->w1 + item->x),
        KC_LHY(item->y - 1),
        KC_LHX(item->w1 + item->x + item->w2),
        KC_LHY(item->y) + h);
    fontManager.SetColorRGBi(RGBA_PAL2(28, 28, 28), 1, 0, 0);
    x = LHX(item->w1 + item->x) + ((LHX(item->w2) - w) / 2);
...
void CControlConfig::Edit(kcItem *items, int32_t nItems) {
    ...
    m_nPrevItem = -1;
    m_nCurItem = 0;
    m_nPrevMouseState = m_nMouseState = 0;
    m_nPrevChangeMode = m_nChangeMode = BT_NONE;
    m_bTimeStopped = 0;
    m_bRedraw = 0;
    ...
    if (m_version == 0)
        ...
        if (m_version >= 4)
            ...
            memset(
                m_info,
                0,
                sizeof(ext_control_info) + sizeof(CAngleVector) + 64 + sizeof(CFixVector) + sizeof(CFixMatrix) + 4);
        ...
        if (m_version > 1) {
            ...
            memset(
                m_info,
                0,
                sizeof(ext_control_info) + sizeof(CAngleVector) + 64 + sizeof(CFixVector) + sizeof(CFixMatrix));
