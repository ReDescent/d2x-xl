...
float CMeshEdge::PlanarAngle(void) { return 0.97f; }

//------------------------------------------------------------------------------

float CMeshEdge::PartialAngle(void) { return 0.9f; }

//------------------------------------------------------------------------------

int32_t CMeshEdge::Planar(void) { return m_fDot >= PlanarAngle(); }

//------------------------------------------------------------------------------

int32_t CMeshEdge::Partial(void) { return m_fDot >= PartialAngle(); }

//------------------------------------------------------------------------------

CFloatVector &CMeshEdge::Normal(int32_t i) { return m_faces[i].m_vNormal[0]; }
...
int32_t CMeshEdge::Prepare(CFloatVector vViewer, int32_t nFilter, float fDistance) {
    ...
    int32_t nDistScale = DistToScale(
        bAutoScale ? Min(CFloatVector::Dist(vViewer, Vertex(0)), CFloatVector::Dist(vViewer, Vertex(1))) : fDistance);
...
#if DBG
    if ((m_faces[0].m_nItem == 166) && (m_faces[0].m_nFace == 3) && (m_faces[1].m_nItem == 176) &&
        (m_faces[1].m_nFace == 3))
        BRP;
#endif
...
#endif
#if DBG
                if ((gameData.segData.edgeVertexData[bPartial].m_nVertices >=
                     (int32_t)gameData.segData.edgeVertexData[bPartial].m_vertices.Length()) &&
                    !gameData.segData.edgeVertexData[bPartial].m_vertices.Resize(
                        2 * gameData.segData.edgeVertexData[bPartial].m_nVertices))
                    return -1;
#endif
...
void SortEdgeFaces(CEdgeFaceInfo faces[]) {
    if ((faces[1].m_nItem < faces[0].m_nItem) ||
        ((faces[1].m_nItem == faces[0].m_nItem) && ((faces[1].m_nFace < faces[0].m_nFace)))) {
...
int32_t CSegmentData::CreateEdgeBuffers(int32_t nLength) {
    for (int32_t i = 0; i < 2; i++) {
        ...
        if ((gameData.segData.edgeVertexData[i].m_vertices.Length() < uint32_t(nLength)) &&
            !gameData.segData.edgeVertexData[i].m_vertices.Resize(nLength, false))
            return 0;
        sprintf(szLabel, "gameData.segData.edgeVertexData [%d].m_dists", i);
        gameData.segData.edgeVertexData[i].m_dists.SetName(szLabel);
        if ((gameData.segData.edgeVertexData[i].m_dists.Length() < uint32_t(nLength)) &&
            !gameData.segData.edgeVertexData[i].m_dists.Resize(nLength, false))
            return 0;
    }
    return 1;
}
...
#if POLYGONAL_OUTLINE
    if (!gameData.segData.edgeVertices.Create(
            gameData.segData.nEdges * (bPolygonalOutline ? 8 : 4),
            "gameData.segData.edgeVertices"))
#else
