...
void *CMovie::Alloc(uint32_t size) { return reinterpret_cast<void *>(new uint8_t[size]); }

// ----------------------------------------------------------------------

void CMovie::Free(void *p) { delete[] reinterpret_cast<uint8_t *>(p); }

//-----------------------------------------------------------------------

void CMovie::ShowFrame(
    uint8_t *buf,
    uint32_t wBuffer,
    uint32_t hBuffer,
    uint32_t xSrc,
    ...
    for (fc.Begin(); fc.Continue(); fc.End()) {
        ...
        if (gameOpts->movies.bFullScreen > 0) {
            ...
            if (!gameOpts->movies.bFullScreen &&
                (((uint32_t)CCanvas::Current()->Width() > w) || ((uint32_t)CCanvas::Current()->Height() > h))) {
...
int32_t CMovieManager::RequestCD(void) { return -1; }
...
void CMovieManager::InitLibs(void) {
    ...
    int32_t j = (m_bHaveExtras = !gameStates.app.bNostalgia) ? N_BUILTIN_MOVIE_LIBS : FIRST_EXTRA_MOVIE_LIB;
...
int32_t CMovieManager::Run(char *filename, int32_t bHires, int32_t bRequired, int32_t dx, int32_t dy) {
    ...
    if (MVE_rmPrepMovie(
            reinterpret_cast<void *>(pMovie ? &pMovie->m_cf : &cf),
            dx,
            dy,
            track,
            pLib ? pLib->m_bLittleEndian : 1)) {
        Int3();
        return MOVIE_NOT_PLAYED;
    }
    nFrame = 0;
...
int32_t CMovieManager::StartRobot(char *filename) {
    ...
    MVE_sfCallbacks(CMovie::ShowFrame);
    MVE_palCallbacks(CMovie::SetPalette);
    int32_t res = MVE_rmPrepMovie(
        reinterpret_cast<void *>(&m_pRobot->m_cf),
        gameStates.menus.bHires ? 280 : 140,
        gameStates.menus.bHires ? 200 : 80,
        0,
