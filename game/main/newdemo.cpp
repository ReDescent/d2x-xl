...
void DemoError(void) { PrintLog(0, "Error in demo playback\n"); }

#define CATCH_BAD_READ \
    if (bNDBadRead) { \
        DemoError(); \
        bDone = -1; \
        break; \
...
int32_t NDFindObject(int32_t nSignature) {
    ...
    FORALL_OBJSi(
        pObj,
        i) if ((pObj->info.nType != OBJ_NONE) && (pObj->info.nSignature == nSignature)) return pObj->Index();
    return -1;
}
...
void NDWritePosition(CObject *pObj) {
    ...
    if ((renderType == RT_POLYOBJ) || (renderType == RT_HOSTAGE) || (renderType == RT_MORPH) ||
        (pObj->info.nType == OBJ_CAMERA) || (!bOldFormat && (renderType == RT_POWERUP))) {
        if (bOldFormat)
...
static void NDReadPosition(CObject *pObj, int32_t bSkip) {
    ...
    if ((pObj->info.nId == ANIM_MORPHING_ROBOT) && (renderType == RT_FIREBALL) &&
...
void NDReadObject(CObject *pObj) {
    ...
    case CT_EXPLOSION:
        ...
        pObj->cType.explInfo.attached.nNext = pObj->cType.explInfo.attached.nPrev =
...
void NDSetPowerupClip(CObject *pObj) {
    ...
    if (pState->nClipIndex >= 0) {
        ...
        pState->nCurFrame = pAnimInfo->xFrameTime
                                ? ((gameData.timeData.xGame - gameData.demoData.xStartTime) / pAnimInfo->xFrameTime) %
                                      pAnimInfo->nFrameCount
                                : 0;
    }
}
...
int32_t NDWriteObject(CObject *pObj) {
    ...
    if (o.info.nType == OBJ_ROBOT) {
        if (ROBOTINFO(o.info.nId) && ROBOTINFO(o.info.nId)->bossFlag) {
            ...
            if ((i >= 0) && (gameData.timeData.xGame > gameData.bossData[i].m_nCloakStartTime) &&
...
void NDPopCtrlCenTriggers() {
    ...
    for (i = 0; i < gameData.reactorData.triggers.m_nLinks; i++) {
        ...
        if (gameData.wallData.pAnim[anim_num].flags & WCF_TMAP1)
            pSeg->m_sides[side].m_nBaseTex = pConnSeg->m_sides[nConnSide].m_nBaseTex =
                gameData.wallData.pAnim[anim_num].frames[n - 1];
        else
            pSeg->m_sides[side].m_nOvlTex = pConnSeg->m_sides[nConnSide].m_nOvlTex =
                gameData.wallData.pAnim[anim_num].frames[n - 1];
    }
}
...
int32_t NDReadFrameInfo(void) {
    ...
    while (!bDone) {
        ...
        case ND_EVENT_VIEWER_OBJECT: // Followed by an CObject structure
            ...
            if (WhichWindow & 15) {
                ...
                if (gameData.demoData.nVcrState != ND_STATE_PAUSED) {
                    ...
                    gameData.objData.pViewer->info.nNextInSeg = gameData.objData.pViewer->info.nPrevInSeg =
        ...
        case ND_EVENT_RENDER_OBJECT: // Followed by an CObject structure
            ...
            if (gameData.demoData.nVcrState != ND_STATE_PAUSED) {
                ...
                if ((pObj->info.nType == OBJ_PLAYER) && (gameData.demoData.nGameMode & GM_MULTI)) {
                    int32_t nPlayer = (gameData.demoData.nGameMode & GM_TEAM)
                                          ? GetTeam(pObj->info.nId)
                                          : (pObj->info.nId % MAX_PLAYER_COLORS) + 1;
                    if (nPlayer == 0)
                        break;
                    nPlayer--;
                    for (i = 0; i < N_PLAYER_SHIP_TEXTURES; i++)
                        mpTextureIndex[nPlayer][i] =
                            gameData.pigData.tex
                                .objBmIndex[gameData.pigData.tex.pObjBmIndex
                                                [gameData.modelData.polyModels[0][pObj->ModelId()].FirstTexture() + i]];
                    mpTextureIndex[nPlayer][4] =
                        gameData.pigData.tex.objBmIndex
                            [gameData.pigData.tex.pObjBmIndex[gameData.pigData.tex.nFirstMultiBitmap + nPlayer * 2]];
                    mpTextureIndex[nPlayer][5] =
                        gameData.pigData.tex
                            .objBmIndex[gameData.pigData.tex
                                            .pObjBmIndex[gameData.pigData.tex.nFirstMultiBitmap + nPlayer * 2 + 1]];
        ...
        case ND_EVENT_HOMING_DISTANCE: {
            ...
            LOCALPLAYER.homingObjectDist = I2X((int32_t)distance << 16);
        } break;
        ...
        case ND_EVENT_WALL_SET_TMAP_NUM1: {
            ...
            if ((nConnSeg >= 0) && (gameData.demoData.nVcrState != ND_STATE_PAUSED) &&
                (gameData.demoData.nVcrState != ND_STATE_REWINDING) &&
                (gameData.demoData.nVcrState != ND_STATE_ONEFRAMEBACKWARD)) {
                if (gameData.demoData.nVersion >= 18)
                    SEGMENT(nSegment)->SetTexture(nSide, SEGMENT(nConnSeg), nConnSide, nAnim, nFrame);
                else
                    SEGMENT(nSegment)->m_sides[nSide].m_nBaseTex = SEGMENT(nConnSeg)->m_sides[nConnSide].m_nBaseTex =
                        tmap;
            }
        } break;
        ...
        case ND_EVENT_DOOR_OPENING: {
            ...
            if ((gameData.demoData.nVcrState == ND_STATE_REWINDING) ||
                ...
                if (gameData.wallData.pAnim[anim_num].flags & WCF_TMAP1)
                    ...
                    pSeg->m_sides[nSide].m_nOvlTex = pOppSeg->m_sides[nConnSide].m_nOvlTex =
                        gameData.wallData.pAnim[anim_num].frames[0];
            } else
        ...
        case ND_EVENT_NEW_LEVEL: {
            ...
            if ((loadedLevel < missionManager.nLastSecretLevel) || (loadedLevel > missionManager.nLastLevel)) {
    ...
    for (i = curObjs + nCurObjs, pCurObj = curObjs; pCurObj < i; pCurObj++) {
        ...
        FORALL_OBJSi(pObj, h) {
            if (pCurObj->info.nSignature == pObj->info.nSignature) {
                ...
                if ((renderType != RT_LASER) && (renderType != RT_FIREBALL) && (renderType != RT_THRUSTER) &&
                    ...
                    if (mag1 > I2X(1) / 256) {
                        ...
                        rvec2 *= factor;
                        rvec1 += rvec2;
                        CFixVector::Normalize(rvec1); // Note: Doesn't matter if this is null, if null, VmVector2Matrix
                                                      // will just use fvec1
                ...
                // -- old fashioned way --				VmAngles2Matrix (&(curObjs [i].info.position.mOrient),
                // &cur_angles);
            }
        }
    }
    ...
    if ((gameData.demoData.nVcrState == ND_STATE_REWINDING) ||
        ...
        if ((gameData.demoData.nPlaybackStyle == NORMAL_PLAYBACK) && (gameData.demoData.nFrameCount > 10))
            if ((gameData.demoData.xPlaybackTotal * INTERPOL_FACTOR) < gameData.demoData.xRecordedTotal) {
                ...
                gameData.demoData.xPlaybackTotal =
                    gameData.demoData.xRecordedTotal + gameData.timeData.xFrame; // baseline playback time
