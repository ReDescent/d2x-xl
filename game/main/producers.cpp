...
bool CSegment::CreateProducer(int32_t nOldFunction) {
    ENTER(0, 0);
    if ((m_function != SEGMENT_FUNC_FUELCENTER) && (m_function != SEGMENT_FUNC_REPAIRCENTER) &&
        (m_function != SEGMENT_FUNC_REACTOR) && (m_function != SEGMENT_FUNC_ROBOTMAKER) &&
    ...
    if (nOldFunction == SEGMENT_FUNC_EQUIPMAKER) {
        ...
        if (m_nObjProducer < --gameData.producerData.nEquipmentMakers) {
            gameData.producerData.equipmentMakers[m_nObjProducer] =
                gameData.producerData.equipmentMakers[gameData.producerData.nEquipmentMakers];
            SEGMENT(gameData.producerData.equipmentMakers[gameData.producerData.nEquipmentMakers].nSegment)
                ->m_nObjProducer = m_nObjProducer;
            m_nObjProducer = -1;
        }
    } else if (nOldFunction == SEGMENT_FUNC_ROBOTMAKER) {
        gameData.producerData.origProducerTypes[m_value] = SEGMENT_FUNC_NONE;
        if (m_nObjProducer < --gameData.producerData.nRobotMakers) {
            gameData.producerData.robotMakers[m_nObjProducer] =
                gameData.producerData.robotMakers[gameData.producerData.nRobotMakers];
            SEGMENT(gameData.producerData.robotMakers[gameData.producerData.nRobotMakers].nSegment)->m_nObjProducer =
                m_nObjProducer;
            m_nObjProducer = -1;
        }
    }
...
bool CSegment::CreateObjectProducer(int32_t nOldFunction, int32_t nMaxCount) {
    ...
    if ((nOldFunction != SEGMENT_FUNC_FUELCENTER) && (nOldFunction != SEGMENT_FUNC_REPAIRCENTER) &&
        (nOldFunction != SEGMENT_FUNC_ROBOTMAKER) && (nOldFunction != SEGMENT_FUNC_EQUIPMAKER)) {
    ...
    producer.xMaxCapacity = producer.xCapacity = I2X(gameStates.app.nDifficultyLevel + 3);
...
int32_t StartObjectProducer(int16_t nSegment) {
    ...
    if (pSeg->m_function == SEGMENT_FUNC_EQUIPMAKER) { // toggle it on or off
        if (pSeg->m_nObjProducer >= gameData.producerData.nEquipmentMakers)
            RETVAL(0)
        pObjProducer =
            gameData.producerData.producers + gameData.producerData.equipmentMakers[pSeg->m_nObjProducer].nProducer;
...
void OperateRobotMaker(CObject *pObj, int16_t nSegment) {
    ...
    if (nSegment < 0)
        ...
        pObjProducer =
            gameData.producerData.producers + gameData.producerData.robotMakers[pSeg->m_nObjProducer].nProducer;
        nType = GetObjProducerObjType(
            pObjProducer,
            gameData.producerData.robotMakers[pSeg->m_nObjProducer].objFlags,
            MAX_ROBOT_TYPES);
        if (nType < 0)
            nType = 255;
    }
    ...
    if (pObj) {
        ...
        if (gameData.effectData.animations[0][nVideoClip].nSound > -1)
            audio.CreateSegmentSound(
                gameData.effectData.animations[0][nVideoClip].nSound,
                (int16_t)pObjProducer->nSegment,
                0,
                vPos,
                0,
                I2X(1));
...
void EquipmentMakerHandler(tProducerInfo *pObjProducer) {
    ...
    if (!pObjProducer->bFlag) {
        ...
        nType = GetObjProducerObjType(
            pObjProducer,
            gameData.producerData.equipmentMakers[nObjProducer].objFlags,
            MAX_POWERUP_TYPES);
        if (nType < 0)
            RETURN
...
void RobotMakerHandler(tProducerInfo *pObjProducer) {
    ...
    if (/*!gameStates.app.bD2XLevel &&*/ (
        LOCALPLAYER.RemainingRobots() >= gameData.objData.nInitialRobots + MAX_EXCESS_OBJECTS)) {
#if DBG
    ...
    if (!pObjProducer->bFlag) {
        ...
        vPos = SEGMENT(pObjProducer->nSegment)->Center();
        // If this is the first materialization, set to valid robot.
        nType = (int32_t)GetObjProducerObjType(
            pObjProducer,
            gameData.producerData.robotMakers[nObjProducer].objFlags,
            MAX_ROBOT_TYPES);
        if (nType < 0)
            RETURN
#if TRACE
        console.printf(
            CON_DBG,
            "Morph: (nType = %i) (seg = %i) (capacity = %08x)\n",
            nType,
            pObjProducer->nSegment,
            pObjProducer->xCapacity);
#endif
...
void UpdateAllProducers(void) {
    ...
    for (i = 0; i < gameData.producerData.nProducers; i++, pProducer++) {
        ...
        if (t == SEGMENT_FUNC_ROBOTMAKER) {
            ...
            (pProducer->xMaxCapacity > 0) && (gameData.producerData.playerSegP != SEGMENT(pProducer->nSegment))) {
...
fix CSegment::Refuel(fix nMaxFuel) {
    ...
    if (IsEntropyGame && ((m_owner < 0) || ((m_owner > 0) && (m_owner != GetTeam(N_LOCALPLAYER) + 1))))
        RETVAL(0)
...
tObjectProducerInfo *
CProducerData::Find(int16_t nSegment, CStaticArray<tObjectProducerInfo, MAX_ROBOT_CENTERS> &producers) {
