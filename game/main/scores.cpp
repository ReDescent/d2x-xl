...
CScoreManager::CScoreManager() : m_bHilite(false) { SetDefaultScores(); }
...
bool CScoreManager::Load(void) {
    ...
    if ((m_scores.version != VERSION_NUMBER) || (m_scores.nSignature[0] != 'D') || (m_scores.nSignature[1] != 'H') ||
...
void CScoreManager::InitStats(tStatsInfo &stats) {
    ...
    stats.killRatio =
        (LOCALPLAYER.numRobotsTotal > 0) ? (LOCALPLAYER.numKillsTotal * 100) / LOCALPLAYER.numRobotsTotal : 0;
    stats.hostageRatio =
        (LOCALPLAYER.hostages.nTotal > 0) ? (LOCALPLAYER.hostages.nRescued * 100) / LOCALPLAYER.hostages.nTotal : 0;
...
//------------------------------------------------------------------------------
// char * score_placement[10] = { TXT_1ST, TXT_2ND, TXT_3RD, TXT_4TH, TXT_5TH, TXT_6TH, TXT_7TH, TXT_8TH, TXT_9TH,
// TXT_10TH };
...
void CScoreManager::Render(int32_t nCurItem) {
    int8_t fadeValues[64] = {1,  1,  1,  2,  2,  3,  4,  4,  5,  6,  8,  9,  10, 12, 13, 15, 16, 17, 19, 20, 22, 23,
                             24, 26, 27, 28, 28, 29, 30, 30, 31, 31, 31, 31, 31, 30, 30, 29, 28, 28, 27, 26, 24, 23,
                             22, 20, 19, 17, 16, 15, 13, 12, 10, 9,  8,  6,  5,  4,  4,  3,  2,  2,  1,  1};
