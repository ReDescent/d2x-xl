...
int32_t CSegment::Index(void) { return this - SEGMENTS; }
...
int32_t CSegment::IsPassable(int32_t nSide, CObject *pObj, bool bIgnoreDoors) {
    ...
    if (SEGMENT(nChildSeg)->HasBlockedProp()) {
        ...
        // handle the player in a speed boost area. The player must only exit speed boost areas through a segment side
        // with a speed boost trigger
    ...
    if (pAnim->flags & WCF_ALTFMT) {
        if (gameData.demoData.nState == ND_STATE_RECORDING)
            NDRecordWallSetTMapNum1(
                SEG_IDX(this),
                (uint8_t)nSide,
                (int16_t)(pConnSeg ? SEG_IDX(pConnSeg) : -1),
                (uint8_t)nConnSide,
                nTexture,
                nAnim,
                nFrame);
...
int32_t CSegment::DoorIsBlocked(int32_t nSide, bool bIgnoreMarker) {
    ...
    for (nObject = m_objects; nObject != -1; nObject = OBJECT(nObject)->info.nNextInSeg) {
        ...
        if ((nType == OBJ_WEAPON) || (nType == OBJ_FIREBALL) || (nType == OBJ_EXPLOSION) || (nType == OBJ_EFFECT) ||
            (bIgnoreMarker && (nType == OBJ_MARKER)))
            continue;
...
void CSegment::OperateTrigger(int32_t nSide, CObject *pObj, int32_t bShot) {
    ...
    if (pObj ? pTrigger->Operate(
                   pObj->Index(),
                   pObj->IsPlayer() ? pObj->info.nId : pObj->IsGuideBot() ? N_LOCALPLAYER : -1,
                   bShot,
                   0)
...
int32_t CSegment::BlowupTexture(int32_t nSide, CFixVector &vHit, CObject *pBlower, int32_t bForceBlowup) {
    ...
    if (pParent) {
        ...
        if (pParent && (pParent->info.nType != OBJ_PLAYER) &&
            ((pParent->info.nType != OBJ_ROBOT) || !pParent->IsGuideBot()) && pWall &&
            (pWall->nTrigger < gameData.trigData.m_nTriggers[0]))
            RETVAL(0)
    }
    ...
    if (dtp.nSwitchType) {
        ...
        if (!bPermaTrigger && (pEffectInfo->destroyed.nEffect != -1) &&
            (gameData.effectData.pEffect[pEffectInfo->destroyed.nEffect].nSegment == -1)) {
...
CSegment *CSegFace::Segment(void) { return SEGMENT(m_info.nSegment); }
...
float CSegment::FaceSize(uint8_t nSide) {
    ...
    float nSize =
        TriangleSize(gameData.segData.vertices[v[0]], gameData.segData.vertices[v[1]], gameData.segData.vertices[v[2]]);
    if (nVertices == 4)
        nSize += TriangleSize(
            gameData.segData.vertices[v[0]],
            gameData.segData.vertices[v[2]],
            gameData.segData.vertices[v[3]]);
    return nSize;
}
