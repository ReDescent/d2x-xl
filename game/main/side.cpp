...
CFloatVector &CSide::Normalf(int32_t nFace) { return m_fNormals[nFace]; }
...
void CSide::SetupVertexList(uint16_t *verts, uint16_t *index) {
    ...
    if (m_nShape) {
        ...
        m_vertices[0] = m_vertices[5] = m_corners[0];
        m_vertices[1] = m_corners[1];
        m_vertices[2] = m_vertices[3] = m_corners[2];
        m_vertices[4] = m_corners[3];
        m_nMinVertex[1] = Min(m_vertices[1], m_vertices[4]);
        m_nFaces = 2;
    } else if (m_nType == SIDE_IS_TRI_13) {
        m_vertices[0] = m_vertices[5] = m_corners[3];
        m_vertices[1] = m_corners[0];
        m_vertices[2] = m_vertices[3] = m_corners[1];
...
void CSide::SetupFaceVertIndex(void) {
    if (m_nType == SIDE_IS_QUAD) {
        ...
        if (m_nFaces == 1) {
            ...
            m_faceVerts[0] = m_faceVerts[5] = 0;
            m_faceVerts[1] = 1;
            m_faceVerts[2] = m_faceVerts[3] = 2;
            m_faceVerts[4] = 3;
        }
    } else if (m_nType == SIDE_IS_TRI_13) {
        m_faceVerts[0] = m_faceVerts[5] = 3;
        m_faceVerts[1] = 0;
        m_faceVerts[2] = m_faceVerts[3] = 1;
        m_faceVerts[4] = 2;
    }
}
...
void CSide::SetupAsQuad(CFixVector &vNormal, CFloatVector &vNormalf, uint16_t *verts, uint16_t *index) {
    ...
    m_normals[0] = m_normals[1] = vNormal;
    m_fNormals[0] = m_fNormals[1] = vNormalf;
...
void CSide::SetupAsTriangles(bool bSolid, uint16_t *verts, uint16_t *index) {
    ...
    if (bSolid) {
		...
		VmVecNormalChecked (&vNormal, 
            ...
            m_fNormals[0] =
                CFloatVector::Normal(FVERTICES[m_corners[0]], FVERTICES[m_corners[1]], FVERTICES[m_corners[2]]);
            m_fNormals[1] =
                CFloatVector::Normal(FVERTICES[m_corners[0]], FVERTICES[m_corners[2]], FVERTICES[m_corners[3]]);
        } else {
            ...
            m_fNormals[0] =
                CFloatVector::Normal(FVERTICES[m_corners[0]], FVERTICES[m_corners[1]], FVERTICES[m_corners[3]]);
            m_fNormals[1] =
                CFloatVector::Normal(FVERTICES[m_corners[1]], FVERTICES[m_corners[2]], FVERTICES[m_corners[3]]);
        }
    } else {
        uint16_t vSorted[4];
...
CFixVector &CSide::MinVertex(void) { return Vertex(m_nMinVertex[0]); }
...
bool CSide::IsPlanar(void) { return (m_nFaces < 2) || (Height() <= PLANE_DIST_TOLERANCE); }
...
CSegMasks CSide::Masks(const CFixVector &refPoint, fix xRad, int16_t sideBit, int16_t faceBit, bool bCheckPoke) {
    ...
    masks.m_valid = 1;
    if (m_nFaces == 2) {
        int32_t nSideCount = 0, nCenterCount = 0;
...
int32_t CSide::SeesPoint(CFixVector &vPoint, int16_t nDestSeg, int8_t nDestSide, int32_t nLevel, int32_t nThread) {
    ...
    for (; i >= j; i--) {
        if (i == 4)
            ...
            v0 = CFloatVector::Avg(
                FVERTICES[m_corners[4 + i]],
                FVERTICES[m_corners[(5 + i) % 4]]); // center of face's edges
...
uint32_t CSide::PointToFaceRelationf(CFloatVector &vRef, int16_t nFace, CFloatVector &vNormal) {
    ...
    int32_t nProjPlane =
        (vt.v.coord.x > vt.v.coord.y) ? (vt.v.coord.x > vt.v.coord.z) ? 0 : 2 : (vt.v.coord.y > vt.v.coord.z) ? 1 : 2;
...
return nEdgeMask;
    ...
    for (nEdge = 1, nEdgeMask = 0; nEdge <= 3; nEdge++) {
        v0 = v1;
        v1 = gameStates.render.bRendering ? &RENDERPOINTS[m_vertices[h + nEdge % 3]].ViewPos()
...
int32_t CSide::CheckLineToFaceRegularf(
...
// iFace determines which of four possible faces we have
// note: the seg parm is temporary, until the face itself has a pRef field
int32_t CSide::CheckLineToFaceRegular(
...
// use lowest pRef number
    ...
    int32_t nVertex = m_nMinVertex[0];
    int32_t pli = FindPlaneLineIntersection(
        vIntersection,
        gameStates.render.bRendering ? &RENDERPOINTS[nVertex].ViewPos() : VERTICES + nVertex,
        &vNormal,
        p0,
        p1,
        rad);
...
// of faces
int32_t CSide::CheckLineToFaceEdgesf(
    CFloatVector &vIntersection,
    CFloatVector &p0,
    CFloatVector &p1,
    float rad,
...
// the plane of a CSide.  In this case, we must do checks against the edge
// of faces
int32_t CSide::CheckLineToFaceEdges(
    CFixVector &vIntersection,
    CFixVector *p0,
    CFixVector *p1,
    fix rad,
...
void CSide::HitPointUV(fix *u, fix *v, fix *l, CFixVector &vIntersection, int32_t iFace) {
    ...
    float k0 = (fabs(vec0.m_x) > fabs(vec0.m_y)) ? (k1 * vec1.m_x + vHit.m_x - vRef.m_x) / vec0.m_x
...
bool CSide::IsOpenableDoor(void) { return IS_WALL(m_nWall) ? WALL(m_nWall)->IsOpenableDoor() : false; }
...
bool CSide::IsWall(void) { return IS_WALL(m_nWall); }

//------------------------------------------------------------------------------

CTrigger *CSide::Trigger(void) { return IS_WALL(m_nWall) ? Wall()->Trigger() : NULL; }

//------------------------------------------------------------------------------

bool CSide::IsVolatile(void) { return IsWall() && WALL(m_nWall)->IsVolatile(); }

//------------------------------------------------------------------------------

bool CSide::IsSolid(void) { return IsWall() && Wall()->IsSolid(); }
...
bool CSide::IsTextured(void) { return IS_WALL(m_nWall); }
