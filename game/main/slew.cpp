...
void slew_reset_orient() {
    if (!pSlewObj || pSlewObj->info.controlType != CT_SLEW)
        return;
    pSlewObj->info.position.mOrient.m.dir.r.v.coord.x = pSlewObj->info.position.mOrient.m.dir.u.v.coord.y =
        pSlewObj->info.position.mOrient.m.dir.f.v.coord.z = I2X(1);
    pSlewObj->info.position.mOrient.m.dir.r.v.coord.y = pSlewObj->info.position.mOrient.m.dir.r.v.coord.z =
        pSlewObj->info.position.mOrient.m.dir.u.v.coord.x = pSlewObj->info.position.mOrient.m.dir.u.v.coord.z =
            pSlewObj->info.position.mOrient.m.dir.f.v.coord.x = pSlewObj->info.position.mOrient.m.dir.f.v.coord.y = 0;
}
...
int32_t do_slew_movement(CObject *pObj, int32_t check_keys, int32_t check_joy) {
    ...
    if (check_keys) {
        if (gameStates.app.nFunctionMode == FMODE_EDITOR) {
            ...
            pObj->mType.physInfo.velocity.v.coord.y +=
                VEL_SPEED * (KeyDownTime(KEY_PADMINUS) - KeyDownTime(KEY_PADPLUS));
    ...
    moved |= pObj->mType.physInfo.velocity.v.coord.x | pObj->mType.physInfo.velocity.v.coord.y |
             pObj->mType.physInfo.velocity.v.coord.z;
...
int32_t slew_frame(int32_t check_keys) { return do_slew_movement(pSlewObj, !check_keys, 1); }
