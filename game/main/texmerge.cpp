...
int32_t TexMergeInit(int32_t nCacheSize) {
    ...
    for (i = 0; i < nCacheEntries; i++, pCache++) {
        ...
        pCache->bmTop = pCache->bmBot = pCache->bitmap = NULL;
        pCache->nOrient = -1;
    }
...
void TexMergeFlush(void) {
    for (int32_t i = 0; i < nCacheEntries; i++) {
        ...
        texCache[i].bmTop = texCache[i].bmBot = NULL;
    }
}
...
CBitmap *TexMergeGetCachedBitmap(int32_t tMapBot, int32_t tMapTop, int32_t nOrient) {
    ...
    if (!pBm || (pBm->Width() != bmBot->Width()) || (pBm->Height() != bmBot->Height())) {
        if (pBm)
            delete pBm;
...
static inline int32_t TexScale(int32_t v, frac s) { return (v * s.c) / s.d; }

static inline int32_t TexIdx(int32_t x, int32_t y, int32_t w, frac s) { return TexScale(y * w + x, s); }
...
const char *texMergeFSData = "uniform sampler2D baseTex, decalTex, maskTex;\r\n"
