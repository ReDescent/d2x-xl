...
class CHomingTargetData {
    ...
    CHomingTargetData(CObject *pTracker, CFixVector &vTrackerPos, CStack<class CTarget> &targets)
        : m_pTracker(pTracker),
          m_vTrackerPos(vTrackerPos),
          m_nBestObj(-1),
          m_targets(targets) {
        m_vTrackerViewDir = SPECTATOR(m_pTracker) ? gameStates.app.playerPos.mOrient.m.dir.f
                                                  : m_pTracker->info.position.mOrient.m.dir.f;
...
int32_t CObject::SelectHomingTarget(CFixVector &vTrackerPos, int32_t nThread) {
    ...
    RETVAL(FindAnyHomingTarget(
        vTrackerPos,
        OBJ_PLAYER,
        (pParent && (pParent->Target()->Type() == OBJ_ROBOT)) ? OBJ_ROBOT : -1,
        nThread))
}
    ...
    FORALL_ACTOR_OBJS(pTarget) {
        ...
        if (nType == OBJ_WEAPON) {
            ...
            if (IsTeamGame) {
                ...
                if (pParentObj && (pParentObj->Type() == OBJ_PLAYER) &&
                    (GetTeam(pTarget->Id()) == GetTeam(pParentObj->Id())))
