...
int32_t oppTrigTypes[] = {
    TT_CLOSE_DOOR,      TT_OPEN_DOOR,     TT_OBJECT_PRODUCER,  TT_EXIT,           TT_SECRET_EXIT,
    TT_ILLUSION_ON,     TT_ILLUSION_OFF,  TT_LOCK_DOOR,        TT_UNLOCK_DOOR,    TT_CLOSE_WALL,
    TT_OPEN_WALL,       TT_ILLUSORY_WALL, TT_LIGHT_ON,         TT_LIGHT_OFF,      TT_TELEPORT,
    TT_SPEEDBOOST,      TT_CAMERA,        TT_SHIELD_DAMAGE,    TT_ENERGY_DRAIN,   TT_CHANGE_TEXTURE,
    TT_SMOKE_LIFE,      TT_SMOKE_SPEED,   TT_SMOKE_DENS,       TT_SMOKE_SIZE,     TT_SMOKE_DRIFT,
    TT_COUNTDOWN,       TT_SPAWN_BOT,     TT_SMOKE_BRIGHTNESS, TT_SET_SPAWN,      TT_MESSAGE,
    TT_SOUND,           TT_MASTER,        TT_DISABLE_TRIGGER,  TT_ENABLE_TRIGGER, TT_DISARM_ROBOT,
    TT_REPROGRAM_ROBOT, TT_SHAKE_MINE};
...
void CTriggerTargets::Check(void) {
    ...
    if (m_nLinks > MAX_TRIGGER_TARGETS) {
        ...
        PrintLog(
            0,
            "Invalid trigger target count (trigger #%d)\n",
            (CTrigger *)this - gameData.trigData.triggers[0].Buffer());
    }
    for (i = j = 0; i < m_nLinks; i++) {
        if (m_sides[i] >= 0) {
            if ((m_segments[i] < 0) || (m_segments[i] >= gameData.segData.nSegments) || (m_sides[i] < 0) ||
                (m_sides[i] > 5)) {
                PrintLog(
                    0,
                    "Invalid trigger target (trigger #%d, target %d)\n",
                    (CTrigger *)this - gameData.trigData.triggers[0].Buffer(),
                    i);
                continue;
            }
...
inline int32_t CTrigger::Index(void) { return this - GEOTRIGGERS; }
...
void CTrigger::DoChangeTexture(void) {
    int32_t baseTex = GetValue() & 0xffff, ovlTex = (GetValue() >> 16);
...
void CTrigger::DoSpawnBots(CObject *pObj) { OperateRobotMaker(pObj, m_nLinks ? m_segments[0] : -1); }
...
int32_t CTrigger::DoChangeWalls(void) {
    ...
    for (int32_t i = 0; i < m_nLinks; i++) {
        if ((m_segments[i] < 0) || (m_segments[i] >= gameData.segData.nSegments)) {
            PrintLog(
                0,
                "%s trigger %d has invalid target segment\n",
                gameData.trigData.triggers[0].IsElement(this) ? "Standard" : "Object",
                gameData.trigData.triggers[0].IsElement(this) ? gameData.trigData.triggers[0].Index(this)
                                                              : gameData.trigData.triggers[1].Index(this));
            continue;
        }
        ...
        bForceField =
            ((gameData.pigData.tex.pTexMapInfo[pSeg->m_sides[nSide].m_nBaseTex].flags & TMI_FORCE_FIELD) != 0);
...
void CTrigger::PrintMessage(int32_t nPlayer, int32_t bShot, const char *message) {
    ...
    if (!gameStates.app.bD1Mission && ((nPlayer < 0) || (nPlayer == N_LOCALPLAYER))) {
        if (!Flagged(TF_NO_MESSAGE) &&
            bShot) // only display if not a silent trigger and the trigger has been shot (not flown through)
...
void TriggerSetObjPos(int16_t nObject, int16_t nSegment) { OBJECT(nObject)->RelinkToSeg(nSegment); }
    ...
    if (!sbd.bBoosted) {
        ...
        if (pSrcPt && pDestPt) {
            ...
            if (!n.IsZero())
                ...
                controls[0].verticalThrustTime = controls[0].forwardThrustTime = controls[0].sidewaysThrustTime = 0;
                n = SEGMENT(nDestSeg)->Side(nDestSide)->Normal(2);
                // turn the ship so that it is facing the destination nSide of the destination CSegment
                // Invert the Normal as it points into the CSegment
                n.Neg();
...
void CTrigger::DoSpeedBoost(int16_t nObject) {
    if (!(COMPETITION /*|| IsCoopGame*/) || extraGameInfo[IsMultiGame].nSpeedBoost) {
        ...
        SetSpeedBoostVelocity(
            (int16_t)nObject,
            GetValue(),
            (int16_t)(pWall ? pWall->nSegment : -1),
            (int16_t)(pWall ? pWall->nSide : -1),
            m_segments[0],
            m_sides[0],
            NULL,
            NULL,
            Flagged(TF_SET_ORIENT) != 0);
    }
}
...
int32_t CTrigger::WallIsForceField(void) {
    for (int32_t i = 0; i < m_nLinks; i++)
        if ((gameData.pigData.tex.pTexMapInfo[SEGMENT(m_segments[i])->m_sides[m_sides[i]].m_nBaseTex].flags &
             TMI_FORCE_FIELD))
            return 1;
    return 0;
}
...
int32_t CTrigger::Operate(int16_t nObject, int32_t nPlayer, int32_t bShot, bool bObjTrigger) {
    ...
    if (bIsPlayer) {
        ...
        nPlayer = -1;
        if (pObj && (pObj->info.nType != OBJ_ROBOT) && (pObj->info.nType != OBJ_DEBRIS) && // exploded robot
            (pObj->info.nType != OBJ_REACTOR) && (pObj->info.nType != OBJ_HOSTAGE) &&
...
bool CTrigger::IsFlyThrough(void) { return !WALL(m_info.nWall)->IsSolid(true); }
...
CTrigger *FindObjTrigger(int16_t nObject, int16_t nType, int16_t nTrigger) {
    ...
    if (++nTrigger - gameData.trigData.objTriggerRefs[nObject].nFirst <
        gameData.trigData.objTriggerRefs[nObject].nCount)
...
void TriggersFrameProcess(void) {
    ...
    for (i = gameData.trigData.m_nTriggers[0]; i > 0; i--, pTrigger++) {
        ...
        if (pTrigger->Flagged(TF_DISABLED))
            continue;
        if (pTrigger->Flagged(TF_AUTOPLAY) && (pTrigger->m_info.tOperated < 0) &&
            (pTrigger->IsDelayed() || !pTrigger->Flagged(TF_PERMANENT))) {
            pTrigger->Operate(LOCALPLAYER.nObject, N_LOCALPLAYER, 0, false);
            if (!pTrigger->IsDelayed())
                pTrigger->SetFlags(TF_DISABLED);
        }
        if ((pTrigger->Type() == TT_SOUND) && (pTrigger->Flagged(TF_PLAYING_SOUND)) && (pTrigger->GetTime(1) > 0) &&
            ...
            if ((pTrigger->IsFlyThrough() ? !pTrigger->Flagged(TF_ONE_SHOT) : pTrigger->Flagged(TF_PERMANENT)))
                pTrigger->SetFlags(TF_DISABLED); // sound has been played; make sure it doesn't get played again when
                                                 // laoading a save game that may subsequently be now
        }
    ...
    for (i = gameData.trigData.m_nTriggers[1]; i > 0; i--, pTrigger++) {
        if (pTrigger->Flagged(TF_AUTOPLAY) && (pTrigger->m_info.nObject >= 0) && (pTrigger->m_info.tOperated < 0) &&
            (!IsMultiGame || IAmGameHost())) {
...
static void StartTriggeredSounds(CArray<CTrigger> &triggers) {
    ...
    for (int32_t i = triggers.Length(); i > 0; i--, pTrigger++)
        if ((pTrigger->Type() == TT_SOUND) && pTrigger->Flagged(TF_PLAYING_SOUND | TF_DISABLED, TF_PLAYING_SOUND) &&
            (pTrigger->m_info.nChannel < 0))
...
int32_t ObjTriggerIsValid(int32_t nTrigger) {
    ...
    FORALL_OBJS(pObj) {
        ...
        if ((nTrigger >= gameData.trigData.objTriggerRefs[j].nFirst) &&
            ...
            (pTrigger = OBJTRIGGER(nTrigger)) && (pTrigger->m_info.nObject >= 0))
            return 1;
    }
    return 0;
}
...
int32_t FindTriggerTarget(int16_t nSegment, int16_t nSide, int32_t i) {
    ...
    for (j = i - gameData.trigData.m_nTriggers[0];
         i < gameData.trigData.m_nTriggers[0] + gameData.trigData.m_nTriggers[1];
         i++, j++) {
...
int32_t FindNextLevel(void) {
    ...
    if (gameData.segData.nLevelVersion > 20) {
        ...
        for (int32_t i = 0; i < gameData.trigData.m_nTriggers[0]; i++, pTrigger++) {
            if (pTrigger->Type() == TT_EXIT) {
                int32_t l =
                    (X2I(pTrigger->GetValue()) > 0) ? X2I(pTrigger->GetValue()) : missionManager.nCurrentLevel + 1;
