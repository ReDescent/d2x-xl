...
CActiveDoor *CWall::OpenDoor(void) {
    ...
    if (state == WALL_DOOR_CLOSING) { // closing, so reuse door
        if ((pDoor = FindActiveDoor(this - WALLS))) {
            pDoor->time =
                (fix)(gameData.wallData.pAnim[nClip].xTotalTime * gameStates.gameplay.slowmo[0].fSpeed) - pDoor->time;
            if (pDoor->time < 0)
                pDoor->time = 0;
        }
...
bool DoDoorOpen(int32_t nDoor) {
    ...
    for (i = 0; i < pDoor->nPartCount; i++) {
        ...
        if (nConnSide < 0) {
            PrintLog(
                0,
                "Door %d @ %d,%d has no oppposite door in %d\n",
                pSeg->WallNum(nSide),
                pWall->nSegment,
                nSide,
                pSeg->m_children[nSide]);
            continue;
        }
        ...
        if ((gameData.demoData.nState != ND_STATE_PLAYBACK) && !(i || pDoor->time)) {
            if (gameData.wallData.pAnim[pWall->nClip].closeSound > -1)
                audio.CreateSegmentSound(
                    (int16_t)gameData.wallData.pAnim[pSeg->Wall(nSide)->nClip].closeSound,
                    pWall->nSegment,
                    nSide,
                    SEGMENT(pWall->nSegment)->SideCenter(nSide),
                    0,
                    I2X(1));
        }
        pDoor->time += gameData.timeData.xFrame;
...
bool DoCloakingWallFrame(int32_t nCloakingWall) {
    ...
    if (pCloakWall->time > CLOAKING_WALL_TIME) {
        ...
        for (int32_t i = 0; i < 4; i++) {
            SEGMENT(pFrontWall->nSegment)->m_sides[pFrontWall->nSide].m_uvls[i].l =
                FixMul(pCloakWall->front_ls[i], xLightScale);
            if (pBackWall)
                SEGMENT(pBackWall->nSegment)->m_sides[pBackWall->nSide].m_uvls[i].l =
                    FixMul(pCloakWall->back_ls[i], xLightScale);
        }
        if (gameData.demoData.nState == ND_STATE_RECORDING) {
            tUVL *pUVL = SEGMENT(pFrontWall->nSegment)->m_sides[pFrontWall->nSide].m_uvls;
            NDRecordCloakingWall(
                pCloakWall->nFrontWall,
                pCloakWall->nBackWall,
                pFrontWall->nType,
                pFrontWall->state,
...
void DoDecloakingWallFrame(int32_t nCloakingWall) {
    ...
    if (pCloakWall->time > CLOAKING_WALL_TIME) {
        ...
        pFrontWall->cloakValue =
            ((CLOAKING_WALL_TIME / 2 - pCloakWall->time) * (FADE_LEVELS - 2)) / (CLOAKING_WALL_TIME / 2);
        pFrontWall->nType = WALL_CLOAKED;
        if (pBackWall) {
            pBackWall->cloakValue = pFrontWall->cloakValue;
            pBackWall->nType = WALL_CLOAKED;
...
void WallFrameProcess(void) {
    ...
    for (i = 0; i < gameData.wallData.activeDoors.ToS(); i++) {
        ...
        pBackWall = NULL, pWall = WALL(pDoor->nFrontWall[0]);
        if (pWall->state == WALL_DOOR_OPENING) {
            ...
            if ((pDoor->time > DOOR_WAIT_TIME) &&
                !SEGMENT(pWall->nSegment)->DoorIsBlocked(pWall->nSide, pWall->IgnoreMarker())) {
    ...
    for (nSide = 0; nSide < SEGMENT_SIDE_COUNT; nSide++) {
        ...
        if ((tm = pSeg->m_sides[nSide].m_nOvlTex)) {
            ...
            if (((ec != -1) && (db != -1) && !(pEffectInfo->flags & EF_ONE_SHOT)) ||
                ...
                if (dist < damage / 2) {
                    dist = simpleRouter[0].PathLength(
                        pnt,
                        pSeg->Index(),
                        pObj->info.position.vPos,
                        pObj->info.nSegment,
                        MAX_BLAST_GLASS_DEPTH,
                        WID_TRANSPARENT_FLAG,
                        -1);
        ...
        if ((nType == WALL_OPEN) || (nType == WALL_DOOR) || (nType == WALL_BLASTABLE) || (nType == WALL_COLORED))
            ...
            for (int32_t i = 0; i < gameData.trigData.m_nTriggers[0]; i++, pTrigger++) {
                ...
                for (int32_t j = pTrigger->m_nLinks; j; j--, pSegNum++, pSideNum++) {
                    if ((*pSegNum != nSegment) && (*pSideNum != nSide))
                        continue;
                    if ((pTrigger->m_info.nType == TT_OPEN_DOOR) || (pTrigger->m_info.nType == TT_ILLUSION_OFF) ||
                        (pTrigger->m_info.nType == TT_OPEN_WALL)) {
                        bVolatile = 1;
                        break;
                    }
                }
...
CTrigger *CWall::Trigger(void) { return GEOTRIGGER(nTrigger); }
...
void InitExplodingWalls(void) { gameData.wallData.exploding.Reset(); }
...
void DoExplodingWallFrame(void) {
    ...
    for (uint32_t i = 0; i < gameData.wallData.exploding.ToS();) {
        ...
        if (gameData.wallData.exploding[i].time > EXPL_WALL_TIME)
            ...
            CSegment *pSeg = SEGMENT(nSegment), *pConnSeg = SEGMENT(pSeg->m_children[nSide]);
        ...
        for (e = oldCount; e < newCount; e++) {
            ...
            vPos += SEGMENT(nSegment)->m_sides[nSide].m_normals[0] *
                    (size * (EXPL_WALL_TOTAL_FIREBALLS - e) / EXPL_WALL_TOTAL_FIREBALLS);
            if (e & 3) // 3 of 4 are Normal
                CreateExplosion(
                    (int16_t)gameData.wallData.exploding[i].nSegment,
                    vPos,
                    size,
                    (uint8_t)ANIM_SMALL_EXPLOSION);
            else
...
void SetupWalls(void) {
    ...
    for (int32_t i = 0; i < gameData.wallData.nWalls; i++, pWall++) {
        if ((pWall->nType == WALL_DOOR) && (pWall->flags & WALL_DOOR_OPENED)) {
            if ((pWall->flags & WALL_DOOR_AUTO) &&
                !FindActiveDoor(i)) { // make sure pre-opened automatic doors will close after a while
                pWall->flags &= ~WALL_DOOR_OPENED;
