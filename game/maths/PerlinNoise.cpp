...
double CPerlinNoise::LinearInterpolate(double a, double b, double x) { return a * (1.0 - x) + b * x; }
...
double CPerlinNoise::SmoothedNoise(double v) { return Noise(v) / 2 + Noise(v - 1) / 4 + Noise(v + 1) / 4; }
