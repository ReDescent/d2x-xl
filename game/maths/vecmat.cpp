...
const CFixMatrix CFixMatrix::IDENTITY = CFixMatrix::Create(CFixVector::XVEC, CFixVector::YVEC, CFixVector::ZVEC);

const CFloatMatrix CFloatMatrix::IDENTITY = CFloatMatrix::Create(
    CFloatVector::Create(1.0f, 0, 0, 0),
...
fix CFixVector::Mag(void) const {
    if (gameOpts->render.nMathFormat == 2)
        return fix(sqrt(
            double(v.coord.x) * double(v.coord.x) + double(v.coord.y) * double(v.coord.y) +
            double(v.coord.z) * double(v.coord.z)));
    // The following code is vital for backwards compatibility: Side normals get computed after loading a level using
    // this fixed math. D2X-XL temporarily switches to math format 0 to enforce that. Otherwise level checksums would
    // differ from those of other Descent 2 versions, breaking multiplayer compatibility.
    tQuadInt q = {0, 0};
...
const CFloatMatrix
CFloatMatrix::Create(const CFloatVector &r, const CFloatVector &u, const CFloatVector &f, const CFloatVector &w) {
    CFloatMatrix m;
    m.m.dir.r = r;
    m.m.dir.u = u;
    m.m.dir.f = f;
...
const CFloatMatrix CFloatMatrix::Inverse(void) {
    ...
    m.m.dir.r.v.coord.x =
        (m.m.dir.u.v.coord.y * m.m.dir.f.v.coord.z - m.m.dir.u.v.coord.z * m.m.dir.f.v.coord.y) / fDet;
    m.m.dir.r.v.coord.y =
        (m.m.dir.r.v.coord.z * m.m.dir.f.v.coord.y - m.m.dir.r.v.coord.y * m.m.dir.f.v.coord.z) / fDet;
    m.m.dir.r.v.coord.z =
        (m.m.dir.r.v.coord.y * m.m.dir.u.v.coord.z - m.m.dir.r.v.coord.z * m.m.dir.u.v.coord.y) / fDet;
    m.m.dir.u.v.coord.x =
        (m.m.dir.u.v.coord.z * m.m.dir.f.v.coord.x - m.m.dir.u.v.coord.x * m.m.dir.f.v.coord.z) / fDet;
    m.m.dir.u.v.coord.y =
        (m.m.dir.r.v.coord.x * m.m.dir.f.v.coord.z - m.m.dir.r.v.coord.z * m.m.dir.f.v.coord.x) / fDet;
    m.m.dir.u.v.coord.z =
        (m.m.dir.r.v.coord.z * m.m.dir.u.v.coord.x - m.m.dir.r.v.coord.x * m.m.dir.u.v.coord.z) / fDet;
    m.m.dir.f.v.coord.x =
        (m.m.dir.u.v.coord.x * m.m.dir.f.v.coord.y - m.m.dir.u.v.coord.y * m.m.dir.f.v.coord.x) / fDet;
    m.m.dir.f.v.coord.y =
        (m.m.dir.r.v.coord.y * m.m.dir.f.v.coord.x - m.m.dir.r.v.coord.x * m.m.dir.f.v.coord.y) / fDet;
    m.m.dir.f.v.coord.z =
        (m.m.dir.r.v.coord.x * m.m.dir.u.v.coord.y - m.m.dir.r.v.coord.y * m.m.dir.u.v.coord.x) / fDet;
    return m;
}
...
const fix CFixMatrix::Det(void) {
    fix xDet = FixMul(
        m.dir.r.v.coord.x,
        FixMul(m.dir.u.v.coord.y, m.dir.f.v.coord.z) - FixMul(m.dir.u.v.coord.z, m.dir.f.v.coord.y));
    xDet += FixMul(
        m.dir.r.v.coord.y,
        FixMul(m.dir.u.v.coord.z, m.dir.f.v.coord.x) - FixMul(m.dir.u.v.coord.x, m.dir.f.v.coord.z));
    xDet += FixMul(
        m.dir.r.v.coord.z,
        FixMul(m.dir.u.v.coord.x, m.dir.f.v.coord.y) - FixMul(m.dir.u.v.coord.y, m.dir.f.v.coord.x));
    return xDet;
}
...
const CFixMatrix CFixMatrix::Inverse(void) {
    ...
    i.m.dir.r.v.coord.x =
        FixDiv(FixMul(m.dir.u.v.coord.y, m.dir.f.v.coord.z) - FixMul(m.dir.u.v.coord.z, m.dir.f.v.coord.y), xDet);
    i.m.dir.r.v.coord.y =
        FixDiv(FixMul(m.dir.r.v.coord.z, m.dir.f.v.coord.y) - FixMul(m.dir.r.v.coord.y, m.dir.f.v.coord.z), xDet);
    i.m.dir.r.v.coord.z =
        FixDiv(FixMul(m.dir.r.v.coord.y, m.dir.u.v.coord.z) - FixMul(m.dir.r.v.coord.z, m.dir.u.v.coord.y), xDet);
    i.m.dir.u.v.coord.x =
        FixDiv(FixMul(m.dir.u.v.coord.z, m.dir.f.v.coord.x) - FixMul(m.dir.u.v.coord.x, m.dir.f.v.coord.z), xDet);
    i.m.dir.u.v.coord.y =
        FixDiv(FixMul(m.dir.r.v.coord.x, m.dir.f.v.coord.z) - FixMul(m.dir.r.v.coord.z, m.dir.f.v.coord.x), xDet);
    i.m.dir.u.v.coord.z =
        FixDiv(FixMul(m.dir.r.v.coord.z, m.dir.u.v.coord.x) - FixMul(m.dir.r.v.coord.x, m.dir.u.v.coord.z), xDet);
    i.m.dir.f.v.coord.x =
        FixDiv(FixMul(m.dir.u.v.coord.x, m.dir.f.v.coord.y) - FixMul(m.dir.u.v.coord.y, m.dir.f.v.coord.x), xDet);
    i.m.dir.f.v.coord.y =
        FixDiv(FixMul(m.dir.r.v.coord.y, m.dir.f.v.coord.x) - FixMul(m.dir.r.v.coord.x, m.dir.f.v.coord.y), xDet);
    i.m.dir.f.v.coord.z =
        FixDiv(FixMul(m.dir.r.v.coord.x, m.dir.u.v.coord.y) - FixMul(m.dir.r.v.coord.y, m.dir.u.v.coord.x), xDet);
    return i;
}
...
const CFloatVector CFloatMatrix::ComputeAngles(void) const {
    ...
    a.v.coord.z = ((m.dir.f.v.coord.x == 0.0f) && (m.dir.f.v.coord.z == 0.0f))
                      ? 0.0f
                      : atan2(m.dir.f.v.coord.z, m.dir.f.v.coord.x);
...
const int32_t FindPointLineIntersection(
    CFixVector &hitP,
    const CFixVector &p1,
    const CFixVector &p2,
    const CFixVector &p3,
    ...
    double m = fabs(
        double(d21.v.coord.x) * double(d21.v.coord.x) + double(d21.v.coord.y) * double(d21.v.coord.y) +
        double(d21.v.coord.z) * double(d21.v.coord.z));
    if (m == 0.0) {
        hitP = p1;
        return 0;
    }
    CFixVector d31 = p3 - p1;
    double u = double(d31.v.coord.x) * double(d21.v.coord.x) + double(d31.v.coord.y) * double(d21.v.coord.y) +
               double(d31.v.coord.z) * double(d21.v.coord.z);
    u /= m;
...
const int32_t FindPointLineIntersection(
    CFloatVector &hitP,
    const CFloatVector &p1,
    const CFloatVector &p2,
    const CFloatVector &p3,
    const CFloatVector &vPos,
    int32_t bClamp) {
...
const int32_t FindPointLineIntersection(
    CFloatVector3 &hitP,
    const CFloatVector3 &p1,
    const CFloatVector3 &p2,
    const CFloatVector3 &p3,
    CFloatVector3 *vPos,
    int32_t bClamp) {
...
const float VmLineLineIntersection(
    const CFloatVector3 &v1,
    const CFloatVector3 &v2,
    const CFloatVector3 &v3,
    const CFloatVector3 &v4,
    CFloatVector3 &va,
    CFloatVector3 &vb) {
...
const float VmLineLineIntersection(
    const CFloatVector &v1,
    const CFloatVector &v2,
    const CFloatVector &v3,
    const CFloatVector &v4,
    CFloatVector &va,
    CFloatVector &vb) {
    ...
    CFloatVector vScale) // axes of ellipsoid
{
    ...
    float a = vDir.v.coord.x * vDir.v.coord.x / vScale.v.coord.x + vDir.v.coord.y * vDir.v.coord.y / vScale.v.coord.y +
              vDir.v.coord.z * vDir.v.coord.z / vScale.v.coord.z;

    float b =
        (vLine.v.coord.x * vDir.v.coord.x / vScale.v.coord.x + vLine.v.coord.y * vDir.v.coord.y / vScale.v.coord.y +
         vLine.v.coord.z * vDir.v.coord.z / vScale.v.coord.z) *
        2.0f;
