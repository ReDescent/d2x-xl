...
void SetMaxCustomDetails(void) {
    ...
    gameStates.render.detail.nMaxObjectsOnScreenDetailed =
        detailData.maxObjsOnScreenDetailed[DIFFICULTY_LEVEL_COUNT - 1];
...
void InitCustomDetails(void) {
    ...
    gameStates.render.detail.nMaxPerspectiveDepth =
        detailData.maxPerspectiveDepths[gameStates.render.detail.nWallDetail];
    ...
    gameStates.render.detail.nMaxObjectsOnScreenDetailed =
        detailData.maxObjsOnScreenDetailed[gameStates.render.detail.nObjectComplexity];
    gameData.modelData.nSimpleModelThresholdScale =
        detailData.simpleModelThresholdScales[gameStates.render.detail.nObjectComplexity];
    gameStates.render.detail.nMaxLinearDepthObjects =
        detailData.maxLinearDepthObjects[gameStates.render.detail.nObjectDetail];
    ...
    do {
        ...
        if (!gameStates.app.bGameRunning)
            m.AddSlider(
                ...
                0,
                sizeofa(detailData.nSoundChannels) - 1,
                0,
                HTX_ONLINE_MANUAL);
