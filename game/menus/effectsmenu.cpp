...
int32_t EffectOptionsCallback(CMenu &menu, int32_t &key, int32_t nCurItem, int32_t nState) {
    ...
    if ((m = menu["shield effect"])) {
        ...
        if (gameOpts->render.effects.bShields != v) {
            extraGameInfo[0].nShieldEffect = gameOpts->render.effects.bShields = v;
    ...
    do {
        ...
        *szSlider = *(TXT_RENDER_SPARKS - 1);
        m.AddSlider(
            "energy sparks",
            szSlider + 1,
            gameOpts->render.effects.bEnergySparks,
            0,
            2,
            KEY_P,
            HTX_RENDER_SPARKS);
        ...
        *szSlider = *(TXT_EXPLOSION_BLASTS - 1);
        m.AddSlider(
            "shockwaves",
            szSlider + 1,
            gameOpts->render.effects.nShockwaves,
            0,
            2,
            KEY_K,
            HTX_EXPLOSION_BLASTS);
        ...
        *szSlider = *(TXT_LIGHTTRAIL_QUAL - 1);
        m.AddSlider(
            ...
            nLightTrails,
            0,
            1 + extraGameInfo[0].bUseParticles,
            KEY_T,
            HTX_LIGHTTRAIL_QUAL);
        ...
        *szSlider = *(TXT_WARP_EFFECT - 1);
        m.AddSlider(
            "warp effect",
            szSlider + 1,
            gameOpts->render.effects.bWarpAppearance,
            0,
            2,
            KEY_W,
            HTX_WARP_EFFECT);
        ...
        if (extraGameInfo[0].bUseParticles) {
            m.AddCheck(
                ...
                KEY_X,
                HTX_ADVRND_STATICSMOKE);
            m.AddCheck(
                "gatling trails",
                TXT_GATLING_TRAILS,
                extraGameInfo[0].bGatlingTrails,
                KEY_G,
                HTX_GATLING_TRAILS);
        }
        if ((gameOptions[0].render.nQuality > 2) && (gameOpts->app.bExpertMode == SUPERUSER)) {
            m.AddCheck(
                softParticleIds[0],
                TXT_SOFT_SPRITES,
                gameOpts->SoftBlend(SOFT_BLEND_SPRITES) != 0,
                KEY_I,
                HTX_SOFT_SPRITES);
            m.AddCheck(
                softParticleIds[1],
                TXT_SOFT_PARTICLES,
                gameOpts->SoftBlend(SOFT_BLEND_PARTICLES) != 0,
                KEY_A,
                HTX_SOFT_PARTICLES);
        }
        ...
        if (m.Available("shadows")) {
            if ((extraGameInfo[0].bShadows = (nShadows != 0)))
                gameOpts->render.shadows.nReach = gameOpts->render.shadows.nClip = nShadows;
        }
