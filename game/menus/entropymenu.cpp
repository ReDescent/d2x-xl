...
void NetworkEntropyToggleOptions(void) {
    ...
    m.AddCheck(
        "capture warning",
        TXT_ENT_CONQWARN,
        extraGameInfo[0].entropy.bDoCaptureWarning,
        KEY_W,
        HTX_ONLINE_MANUAL);
    ...
    for (extraGameInfo[0].entropy.nVirusStability = 0; extraGameInfo[0].entropy.nVirusStability < 3;
...
void NetworkEntropyTextureOptions(void) {
    ...
    m.AddCheck(
        "brighten rooms",
        TXT_ENT_TEX_BRIGHTEN,
        extraGameInfo[0].entropy.bBrightenRooms,
        KEY_B,
        HTX_ONLINE_MANUAL);
    ...
    for (extraGameInfo[0].entropy.nOverrideTextures = 0; extraGameInfo[0].entropy.nOverrideTextures < 3;
...
void NetworkEntropyOptions(void) {
    CMenu m(25);
    int32_t i;
    char szCapVirLim[10], szCapTimLim[10], szMaxVirCap[10], szBumpVirCap[10], szBashVirCap[10], szVirGenTim[10],
        szVirLife[10], szEnergyFill[10], szShieldFill[10], szShieldDmg[10];

    m.AddInput(
        "capture virus threshold",
        TXT_ENT_VIRUS_THRESHOLD,
        szCapVirLim,
        extraGameInfo[0].entropy.nCaptureVirusThreshold,
        3,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "capture time threshold",
        TXT_ENT_CAPTURE_TIME,
        szCapTimLim,
        extraGameInfo[0].entropy.nCaptureTimeThreshold,
        3,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "virus capacity",
        TXT_ENT_MAXCAP,
        szMaxVirCap,
        extraGameInfo[0].entropy.nMaxVirusCapacity,
        6,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "virus capacity increase",
        TXT_ENT_BUMPCAP,
        szBumpVirCap,
        extraGameInfo[0].entropy.nBumpVirusCapacity,
        3,
        HTX_ONLINE_MANUAL); // cap. increase when scoring a kill
    m.AddInput(
        "virus capacity decrease",
        TXT_ENT_BASHCAP,
        szBashVirCap,
        extraGameInfo[0].entropy.nBashVirusCapacity,
        3,
        HTX_ONLINE_MANUAL); // cap. decrease when being killed
    m.AddInput(
        "virus creation delay",
        TXT_ENT_GENTIME,
        szVirGenTim,
        extraGameInfo[0].entropy.nVirusGenTime,
        3,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "virus lifetime",
        TXT_ENT_VIRLIFE,
        szVirLife,
        extraGameInfo[0].entropy.nVirusLifespan,
        3,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "refuel rate",
        TXT_ENT_EFILL,
        szEnergyFill,
        extraGameInfo[0].entropy.nEnergyFillRate,
        3,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "repair rate",
        TXT_ENT_SFILL,
        szShieldFill,
        extraGameInfo[0].entropy.nShieldFillRate,
        3,
        HTX_ONLINE_MANUAL);
    m.AddInput(
        "damage rate",
        TXT_ENT_DMGRATE,
        szShieldDmg,
        extraGameInfo[0].entropy.nShieldDamageRate,
        3,
        HTX_ONLINE_MANUAL);
    m.AddText("", "");
