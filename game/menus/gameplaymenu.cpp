...
static const char *pszGuns[] =
    {"Laser", "Vulcan", "Spreadfire", "Plasma", "Fusion", "Super Laser", "Gauss", "Helix", "Phoenix", "Omega"};
static const char *pszDevices[] =
    {"Full Map", "Ammo Rack", "Converter", "Quad Lasers", "Afterburner", "Headlight", "Slow Motion", "Bullet Time"};
...
static inline int32_t GetGunLoadoutFlag(int32_t i) { return (extraGameInfo[0].loadout.nGuns & (1 << i)) != 0; }
...
void LoadoutOptionsMenu(void) {
    ...
    for (i = 0; i < (int32_t)sizeofa(pszDevices); i++, nOptions++)
        m.AddCheck(
            pszDevices[i],
            pszDevices[i],
            (extraGameInfo[0].loadout.nDevice & (nDeviceFlags[i])) != 0,
            0,
            HTX_DEVICE_LOADOUT);
...
void AddShipSelection(CMenu &m, int32_t &optShip) {
    ...
    if (h < 2)
        ...
        optShip = (missionConfig.m_shipsAllowed[0])
                      ? m.AddRadio("standard ship", TXT_STANDARD_SHIP, 0, KEY_S, HTX_PLAYERSHIP)
                      : -1;
...
void GameplayOptionsMenu(void) {
    ...
    nAIAggressivity = (gameOpts->gameplay.nAIAggressivity && gameOpts->gameplay.nAIAwareness)
                          ? 5
                          : gameOpts->gameplay.nAIAggressivity;
    do {
        m.Destroy();
        ...
        *szSlider = *(TXT_WEAPON_SWITCH - 1);
        m.AddSlider(
            "weapon switch",
            szSlider + 1,
            gameOpts->gameplay.nAutoSelectWeapon,
            0,
            2,
            KEY_W,
            HTX_WEAPON_SWITCH);
        ...
        m.AddCheck(
            ...
            KEY_M,
            HTX_GPLAY_SMARTSWITCH);
        if (!bRestricted) {
            m.AddCheck(
                "headlight drains power",
                TXT_HEADLIGHT_POWERDRAIN,
                extraGameInfo[0].headlight.bDrainPower,
                KEY_O,
                HTX_HEADLIGHT_POWERDRAIN);
        ...
        if (!bRestricted) {
            ...
            m.AddCheck(
                "spinup gatling",
                TXT_SPINUP_GATLING,
                extraGameInfo[0].bGatlingSpeedUp,
                KEY_G,
                HTX_SPINUP_GATLING);
            ...
            m.AddCheck(
                "recharge energy",
                TXT_RECHARGE_ENERGY,
                extraGameInfo[0].bRechargeEnergy,
                KEY_R,
                HTX_RECHARGE_ENERGY);
            if (extraGameInfo[0].bRechargeEnergy) {
                if (gameOpts->app.bExpertMode == SUPERUSER) {
                    sprintf(
                        szSlider + 1,
                        TXT_RECHARGE_DELAY,
                        CShipEnergy::RechargeDelay(extraGameInfo[0].nRechargeDelay) / 1000);
                    *szSlider = *(TXT_RECHARGE_DELAY - 1);
                    m.AddSlider(
                        "recharge delay",
                        szSlider + 1,
                        extraGameInfo[0].nRechargeDelay,
                        0,
                        CShipEnergy::RechargeDelayCount() - 1,
                        KEY_Y,
                        HTX_RECHARGE_DELAY);
                    ...
                    *szSlider = *(TXT_RECHARGE_SPEED - 1);
                    m.AddSlider(
                        "recharge speed",
                        szSlider + 1,
                        extraGameInfo[0].nRechargeSpeed,
                        0,
                        2,
                        KEY_S,
                        HTX_RECHARGE_SPEED);
