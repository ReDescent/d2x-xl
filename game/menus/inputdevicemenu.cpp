...
int32_t AddAxisControls(
    CMenu &menu,
    const char *szType,
    char *szText,
    const char *szFmtSyncd,
    const char *szFmt,
    const char *szLabel,
    const char *szHelp,
    int32_t nControls,
    ...
    for (i = 0; i < j; i++) {
        ...
        opt =
            menu.AddSlider(szId, szText + i * 50 + 1, v, 0, nValues - 1, bSyncControls ? nKeySyncd : pnKeys[i], szHelp);
        if (!i)
            h = opt;
    }
    return h;
...
void MouseConfigMenu(void) {
    ...
    do {
        do {
            ...
            controls.SetType();
            mouseOpts.nUse =
                m.AddCheck("use device", TXT_USE_MOUSE, gameOpts->input.mouse.bUse, KEY_M, HTX_CONF_USEMOUSE);
            ...
            if (gameOpts->input.mouse.bUse || gameStates.app.bNostalgia) {
                ...
                if (gameStates.app.bNostalgia)
                    ...
                    m.AddCheck(
                        "sync axis",
                        TXT_SYNC_MOUSE_AXIS,
                        gameOpts->input.mouse.bSyncAxis,
                        KEY_A,
                        HTX_CONF_SYNCMOUSE);
                AddAxisControls(
                    m,
            ...
            if (gameOpts->input.joystick.bUse || gameStates.app.bNostalgia) {
                ...
                if (!gameStates.app.bNostalgia && gameOpts->app.bExpertMode) {
                    m.AddCheck(
                        "linear sensitivity",
                        TXT_JOY_LINSCALE,
                        gameOpts->input.joystick.bLinearSens,
                        KEY_I,
                        HTX_CONF_LINJOY);
                    m.AddCheck(
                        "sync axis",
                        TXT_SYNC_JOY_AXES,
                        gameOpts->input.joystick.bSyncAxis,
                        KEY_Y,
                        HTX_CONF_SYNCJOY);
                }
                h = gameOpts->input.joystick.bSyncAxis ? 1 : UNIQUE_JOY_AXES;
                AddAxisControls(
                    ...
                    HTX_CONF_JOYSENS,
                    UNIQUE_JOY_AXES,
                    gameOpts->input.joystick.sensitivity,
                    16,
                    NULL,
                    KEY_S,
                    joyHotkeys,
                ...
                if (!gameStates.app.bNostalgia) {
                    AddAxisControls(
                        ...
                        HTX_CONF_JOYDZONE,
                        UNIQUE_JOY_AXES,
                        gameOpts->input.joystick.deadzones,
                        16,
                        nJoyDeadzones,
                        KEY_S,
                        joyHotkeys,
...
void OculusRiftConfigMenu(void) {
    ...
    do {
        ...
        AddDeadzoneControl(
            ...
            HTX_RIFT_DEADZONE,
            szDZoneSizes,
            gameOpts->input.oculusRift.nDeadzone,
            KEY_U);
    ...
    if (gameOpts->input.trackIR.bUse) {
        for (i = 0; i < 3; i++) {
            if (menu.Value(trackirModeIds[i]) && (gameOpts->input.trackIR.nMode != i)) {
                ...
                if (i == 0) {
                    gameData.trackIR.x = gameData.trackIR.y = 0;
                }
                key = -2;
                return nCurItem;
            }
    ...
    do {
        ...
        if (gameOpts->input.trackIR.bUse) {
            ...
            if ((gameOpts->input.trackIR.nMode != 1) || gameOpts->input.trackIR.bMove[0] ||
                gameOpts->input.trackIR.bMove[1] || gameOpts->input.trackIR.bMove[2] ||
                gameOpts->input.trackIR.bMove[3] || gameOpts->input.trackIR.bMove[4]) {
                m.AddText("", "");
                m.AddCheck(
                    "sync axis",
                    TXT_SYNC_TRACKIR_AXES,
                    gameOpts->input.trackIR.bSyncAxis,
                    KEY_K,
                    HTX_SYNC_TRACKIR_AXES);
                AddAxisControls(
                    m,
                    "sensitivity",
                    &szTrackIRSens[0][0],
                    ...
                    HTX_TRACKIR_SENS,
                    3,
                    gameOpts->input.trackIR.sensitivity,
                    16,
                    NULL,
                    KEY_S,
                    axis3DHotkeys,
                ...
                AddDeadzoneControl(
                    m,
                    szTrackIRDeadzone,
                    TXT_TRACKIR_DEADZONE,
                    HTX_TRACKIR_DEADZONE,
                    szDZoneSizes,
                    gameOpts->input.trackIR.nDeadzone,
                    KEY_U);
            }
        }
                ...
                if (gameOpts->app.bExpertMode || gameOpts->app.bNotebookFriendly) {
                    ...
                    *szKeyRampScale = *(TXT_KBD_RAMP - 1);
                    m.AddSlider(
                        "ramp scale",
                        szKeyRampScale + 1,
                        gameOpts->input.keyboard.nRamp / 10,
                        0,
                        10,
                        KEY_R,
                        HTX_CONF_RAMPSCALE);
                    if (gameOpts->input.keyboard.nRamp > 0) {
                        m.AddCheck(
                            kbdRampIds[0],
                            TXT_RAMP_ACCEL,
                            gameOpts->input.keyboard.bRamp[0],
                            KEY_A,
                            HTX_CONF_RAMPACC);
                        m.AddCheck(
                            kbdRampIds[1],
                            TXT_RAMP_ROT,
                            gameOpts->input.keyboard.bRamp[1],
                            KEY_O,
                            HTX_CONF_RAMPROT);
                        m.AddCheck(
                            kbdRampIds[2],
                            TXT_RAMP_SLIDE,
                            gameOpts->input.keyboard.bRamp[2],
                            KEY_D,
                            HTX_CONF_RAMPSLD);
                    }
                }
    ...
    do {
        ...
        if (gameOpts->app.bExpertMode && (gameStates.app.bNostalgia < 3)) {
            m.AddCheck(
                "fast pitch",
                TXT_FASTPITCH,
                (extraGameInfo[0].bFastPitch == 1) ? 1 : 0,
                KEY_T,
                HTX_CONF_FASTPITCH);
