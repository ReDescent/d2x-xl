        ...
        if (((gameStates.input.keys.xLastPressTime + I2X(/*2*/ 5)) < curtime)
            ...
            for (;;) {
                if (Rand(nDemos + 1) == 0) {
                    ...
                    nLastKey = -3; // exit menu to force rebuild even if not going to game mode. -3 tells menu system
                                   // not to restore
                    SetScreenMode(SCREEN_MENU);
                    break;
                } else {
                    gameStates.input.keys.xLastPressTime =
                        curtime; // Reset timer so that disk won't thrash if no demos.
...
static void PlayMenuSong(void) {
    ...
    for (i = 0; i < songManager.TotalCount(); i++) {
        if (cf.Open(
                reinterpret_cast<char *>(songManager.SongData(i).filename),
                gameFolders.game.szData[0],
                "rb",
                i >= songManager.Count(0))) {
            cf.Close();
