...
void CMenu::DrawCloseBox(int32_t x, int32_t y) {
    ...
    OglDrawFilledRect(
        x + MENU_CLOSE_X,
        y + MENU_CLOSE_Y,
        x + MENU_CLOSE_X + Scaled(MENU_CLOSE_SIZE),
        y + MENU_CLOSE_Y + Scaled(MENU_CLOSE_SIZE));
    CCanvas::Current()->SetColorRGBi(RGBA_PAL2(21, 21, 21));
    OglDrawFilledRect(
        x + MENU_CLOSE_X + LHX(1),
        y + MENU_CLOSE_Y + LHX(1),
        x + MENU_CLOSE_X + Scaled(MENU_CLOSE_SIZE - LHX(1)),
        y + MENU_CLOSE_Y + Scaled(MENU_CLOSE_SIZE - LHX(1)));
}
...
int32_t CMenu::GetSize(int32_t &w, int32_t &h, int32_t &aw, int32_t &nMenus, int32_t &nOthers) {
    ...
    for (uint32_t i = 0; i < ToS(); i++) {
        int32_t nLineCount =
            Item(i)
                .GetSize(Scaled(h), aw, nStringWidth, nStringHeight, nAverageWidth, nMenus, nOthers, m_props.bTinyMode);
        if (w < nStringWidth)
...
int32_t CMenu::InitProps(const char *pszTitle, const char *pszSubTitle) {
    if ((m_props.screenWidth == gameData.renderData.frame.Width()) &&
    ...
    m_props.nStringHeight =
        GetSize(m_props.width, m_props.height, m_props.aw, m_props.nMenus, m_props.nOthers) + m_props.bTinyMode * 2;

    CFixFraction heightScales[2] = {CFixFraction(4, 5), CFixFraction(3, 5)};

    m_props.nMaxOnMenu =
        ((((m_props.screenHeight > 480)) ? heightScales[ogl.IsSideBySideDevice()] * m_props.screenHeight : 480) -
         m_props.titleHeight - Scaled(LHY(8))) /
            m_props.nStringHeight -
        2;
    if (/*!m_props.bTinyMode && */ (m_props.height > (m_props.nMaxOnMenu * (m_props.nStringHeight + 1)) + gap)) {
        m_props.bIsScrollBox = 1;
        m_props.height =
            (m_props.nMaxOnMenu * (m_props.nStringHeight + haveTitle) +
             haveTitle * Scaled(LHY(m_props.bTinyMode ? 12 : 8)));
    } else
...
void CMenu::SaveScreen(CCanvas **gameCanvasP) { gameData.renderData.frame.Activate("CMenu::SaveScreen (frame)"); }
...
void CMenu::Render(void) {
    ...
    for (i = 0; i < m_props.nMaxDisplayable + m_props.nScrollOffset - m_props.nMaxNoScroll; i++) {
        ...
        m_bRedraw = 1;
        if (item.m_bCentered)
            item.m_x = (item.m_nType == NM_TYPE_GAUGE) ? Max(m_props.width - item.m_w, 0) / 2
                                                       : fontManager.Current()->GetCenteredX(item.m_text);
    ...
    if (m_props.bIsScrollBox) {
        ...
        if (m_bRedraw || (m_nLastScrollCheck != m_props.nScrollOffset)) {
            ...
            int32_t sy = Item(m_props.nScrollOffset).m_y -
                         ((m_props.nStringHeight + 1) * (m_props.nScrollOffset - m_props.nMaxNoScroll));
...
bool CMenu::FadeIn(void) {
    if (gameOpts->menus.nFade && !gameStates.app.bNostalgia) {
        ...
        gameStates.render.grAlpha =
            (t < int32_t(gameOpts->menus.nFade)) ? float(t) / float(gameOpts->menus.nFade) : 1.0f;
        return true;
    } else {
        m_tEnter = 0;
        gameStates.render.grAlpha = 1.0f;
...
#define REDRAW_ALL \
    for (int32_t _i = 0; _i < int32_t(ToS()); _i++) \
        Item(_i).m_bRedraw = 1;
...
int32_t CMenu::KeyScrollValue(void) {
    ...
    if ((Item(m_nChoice).m_nType == NM_TYPE_SLIDER) || (Item(m_nChoice).m_nType == NM_TYPE_NUMBER))
        Item(m_nChoice).Value() =
            NMCLAMP(Item(m_nChoice).Value(), Item(m_nChoice).MinValue(), Item(m_nChoice).MaxValue());
    ...
    if (m_nChoice > -1) {
        ...
        case NM_TYPE_CHECK:
            if (item.Value())
                ...
                item.Value() = 1;
            if (m_props.bIsScrollBox) {
                if ((m_nChoice == m_props.nMaxOnMenu + m_props.nScrollOffset - 1) ||
                    (m_nChoice == m_props.nScrollOffset))
                    m_nLastScrollCheck = -1;
            }
        ...
        case NM_TYPE_RADIO:
            for (int32_t i = 0; i < int32_t(ToS()); i++) {
                if ((i != m_nChoice) && (Item(i).m_nType == NM_TYPE_RADIO) && (Item(i).m_group == item.m_group) &&
                    Item(i).Value()) {
                    Item(i).Value() = 0;
                    Item(i).m_bRedraw = 1;
                }
...
int32_t CMenu::QuickSelect(int32_t *pnCurItem) {
    ...
    if (ascii < 255) {
        ...
        do {
            ...
            if (ascii != toupper(ch))
                continue;
            if ((t != NM_TYPE_MENU) && (t != NM_TYPE_CHECK) && (t != NM_TYPE_RADIO) && (t != NM_TYPE_NUMBER) &&
                (t != NM_TYPE_SLIDER))
                continue;
...
int32_t CMenu::HandleKey(int32_t width, int32_t height, int32_t bTinyMode, int32_t *pnCurItem) {
    ...
    case KEY_SHIFTED + KEY_UP:
        if (gameStates.menus.bReordering && m_nChoice != m_nTopChoice) {
            ...
            Item(m_nChoice).m_bRebuild = Item(m_nChoice - 1).m_bRebuild = 1;
            m_nChoice--;
        }
        break;
...
#if DBG
    case KEY_BACKSPACE:
        if ((m_nChoice > -1) && (Item(m_nChoice).m_nType != NM_TYPE_INPUT) &&
            (Item(m_nChoice).m_nType != NM_TYPE_INPUT_MENU))
            Int3();
        break;
#endif
...
int32_t CMenu::MouseCheckOption(void) {
    ...
    case NM_TYPE_RADIO:
        for (i = 0; i < int32_t(ToS()); i++) {
            if ((i != m_nChoice) && (Item(i).m_nType == NM_TYPE_RADIO) &&
                (Item(i).m_group == Item(m_nChoice).m_group) && (Item(i).Value())) {
                Item(i).Value() = 0;
                Item(i).m_bRedraw = 1;
            }
        }
...
int32_t CMenu::MouseScrollUp(void) {
    ...
    if (m_props.bIsScrollBox) {
        if (m_props.nScrollOffset > m_props.nMaxNoScroll) {
            ...
            int32_t y1 = Item(m_props.nScrollOffset).m_y -
                         ((m_props.nStringHeight + 1) * (m_props.nScrollOffset - m_props.nMaxNoScroll));
...
int32_t CMenu::MouseScrollDown(void) {
    ...
    if (m_props.bIsScrollBox) {
        ...
        if (i < int32_t(ToS()) && Item(i).Selectable()) {
            ...
            int32_t y1 =
                Item(i - 1).m_y - ((m_props.nStringHeight + 1) * (m_props.nScrollOffset - m_props.nMaxNoScroll));
            int32_t x1 = x2 - arrowWidth;
            int32_t y2 = y1 + arrowHeight;
            if ((m_xMouse > x1) && (m_xMouse < x2) && (m_yMouse > y1) && (m_yMouse < y2)) {
                if (!to.Expired())
...
int32_t
CMenu::Setup(int32_t nType, int32_t width, int32_t height, int32_t bTinyMode, pMenuCallback callback, int32_t &nItem) {
...
int32_t
CMenu::AddInput(const char *szId, const char *szText, char *szValue, int32_t nValue, int32_t nLen, const char *szHelp) {
...
void ProgressBar(
    ...
    int32_t nBars,
    int32_t nCurProgress,
    int32_t nMaxProgress,
    ...
    menu.Create(
        3 + (nBars - 1) * 3 + (doProgress ? doProgress(menu, i, i, -1) : 0),
        "ProgressBar"); // doProgress can add menu items - ask it how many it will add
    ...
    for (i = 0; i < nBars; i++) {
        ...
        int32_t h = menu.AddGauge(
            szId,
            "                    ",
            -1,
            nMaxProgress); // the blank string denotes the screen width of the gauge
        menu[h].m_bCentered = 1;
