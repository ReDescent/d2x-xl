...
static const char *menuBgNames[4][2] =
    {{"menu.pcx", "menub.pcx"}, {"menuo.pcx", "menuob.pcx"}, {"menud.pcx", "menud.pcx"}, {"menub.pcx", "menub.pcx"}};
...
const char *WallpaperName(int32_t nType, int32_t bHires) { return szWallpapers[nType - 1][Hires(bHires)]; }
...
static inline void SetBoxBorderColor(CRGBColor &color) {
    CCanvas::Current()->SetColorRGB(
        ...
        color.Green(),
        color.Blue(),
        255); // PAL2RGBA (22), PAL2RGBA (22), PAL2RGBA (38), 255);
}
...
void CBackground::Setup(int32_t width, int32_t height) {
    ...
    CCanvas::Setup(
        ...
        (gameData.renderData.frame.Width() - width) / 2,
        (gameData.renderData.frame.Height() - height) / 2,
        width,
        height,
        true);
}
...
void CBackgroundManager::Draw(int32_t nWallpaper) { m_wallpapers[nWallpaper].Draw(false); }

//------------------------------------------------------------------------------

void CBackgroundManager::DrawBox(
    int32_t left,
    int32_t top,
    int32_t right,
    int32_t bottom,
...
CBitmap *CBackgroundManager::LoadCustomWallpaper(const char *filename) {
    ...
    if (!tga.Read(
            ...
            gameOpts->menus.altBg.brightness,
            gameOpts->menus.altBg.grayscale)) {
        delete pBm;
