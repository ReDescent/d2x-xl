...
void CMenuItem::Destroy(void) { FreeTextBms(); }
...
bool CMenuItem::DrawHotKeyStringBitmap(int32_t bIsCurrent, int32_t bTiny, int32_t bCreateTextBms, int32_t nDepth) {
    ...
    if (!m_bmText[bIsCurrent])
        m_bmText[bIsCurrent] = CreateStringBitmap(
            m_text,
            MENU_KEY(m_nKey, -1),
            gameData.menuData.keyColor,
            nTabs,
            m_bCentered,
            m_w,
            bTiny * 2,
    ...
    m_bmText[bIsCurrent]->Render(
        NULL,
        gameData.X(m_x),
        m_y,
        int32_t(m_bmText[bIsCurrent]->Width() * fScale),
        int32_t(m_bmText[bIsCurrent]->Height() * fScale),
        0,
        0,
        m_bmText[bIsCurrent]->Width(),
        m_bmText[bIsCurrent]->Height(),
        1,
        gameStates.app.bDemoData ? -1 : 0);
    return true;
}
...
void CMenuItem::DrawHotKeyString(int32_t bIsCurrent, int32_t bTiny, int32_t bCreateTextBms, int32_t nDepth) {
    ...
    if (!DrawHotKeyStringBitmap(bIsCurrent, bTiny, bCreateTextBms, nDepth)) {
        int32_t w, h, aw, l, i, x = m_x, y = m_y;
...
void CMenuItem::DrawString(int32_t bIsCurrent, int32_t bTiny) {
    ...
    // CHANGED
    if (RETRO_STYLE)
        backgroundManager.Top()
            ->Saved()
            ->BlitClipped(CCanvas::Current(), 5, y - 1, backgroundManager.Current()->Width() - 15, h + 2, 5, y - 1);
...
void CMenuItem::DrawSlider(int32_t bIsCurrent, int32_t bTiny) {
    ...
    if (RETRO_STYLE)
        backgroundManager.Current()
            ->BlitClipped(CCanvas::Current(), 5, y, backgroundManager.Current()->Width() - 15, h, 5, y);
    DrawHotKeyString(bIsCurrent, bTiny, 1, 0);
    if (p) {
        fontManager.Current()->StringSize(s1, w, h, aw);
...
void CMenuItem::DrawRightString(int32_t bIsCurrent, int32_t bTiny, char *s) {
    int32_t w, h, aw;
    int32_t w1 = m_rightOffset, x = m_x, y = m_y;
...
void CMenuItem::UpdateCursor(void) {
    ...
    if (time & 0x8000)
        ...
        OglDrawFilledRect(
            x,
            y,
            x + CCanvas::Current()->Font()->Width() - 1,
            y + CCanvas::Current()->Font()->Height() - 1);
    }
}
...
int32_t CMenuItem::GetSize(
    ...
    if (m_nType == NM_TYPE_SLIDER) {
        ...
        nOthers++;
        nStringWidth =
            m_nTextLen * CCanvas::Current()->Font()->Width() + ((gameStates.menus.bHires ? 3 : 1) * m_nTextLen);
...
void CMenuItem::SaveText(void) { SetText(m_text, m_savedText); }

//------------------------------------------------------------------------------

void CMenuItem::RestoreText(void) { SetText(m_savedText); }
