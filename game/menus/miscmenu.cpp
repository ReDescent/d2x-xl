    ...
    do {
        ...
        *szSlider = *(TXT_EXPERT_MODE - 1);
        m.AddSlider(
            "expert mode",
            szSlider + 1,
            gameOpts->app.bExpertMode,
            0,
            sizeofa(pszExpertMode) - 1,
            KEY_X,
            HTX_EXPERT_MODE);
        m.AddText("", "");
        if (gameStates.app.bNostalgia) {
            ...
            m.AddCheck(
                "show reticle",
                TXT_SHOW_RETICLE,
                gameOpts->render.cockpit.bReticle,
                KEY_R,
                HTX_CPIT_SHOWRETICLE);
            m.AddCheck(
                "missile view",
                TXT_MISSILE_VIEW,
            ...
            m.AddCheck(
                "headlight on",
                TXT_HEADLIGHT_ON,
                gameOpts->gameplay.bHeadlightOnWhenPickedUp,
                KEY_H,
                HTX_MISC_HEADLIGHT);
        } else {
            m.AddCheck(
                "epileptic friendly",
                TXT_EPILEPTIC_FRIENDLY,
                gameOpts->app.bEpilepticFriendly,
                KEY_E,
                HTX_EPILEPTIC_FRIENDLY);
            m.AddCheck(
                "colorblind friendly",
                TXT_COLORBLIND_FRIENDLY,
                gameOpts->app.bColorblindFriendly,
                KEY_C,
                HTX_COLORBLIND_FRIENDLY);
            m.AddCheck(
                "notebook friendly",
                TXT_NOTEBOOK_FRIENDLY,
                gameOpts->app.bNotebookFriendly,
                KEY_N,
                HTX_COLORBLIND_FRIENDLY);
        }
        ...
        if ((gameStates.app.bNostalgia < 2) && gameOpts->app.bExpertMode) {
            ...
            *szSlider = *(TXT_SCREENSHOTS - 1);
            m.AddSlider(
                "screenshots",
                szSlider + 1,
                gameOpts->app.nScreenShotInterval,
                0,
                7,
                KEY_S,
                HTX_MISC_SCREENSHOTS);
                ...
                if (extraGameInfo[0].bAutoDownload) {
                    ...
                    *szSlider = *(TXT_DL_TIMEOUT - 1);
                    m.AddSlider(
                        "download timeout",
                        szSlider + 1,
                        downloadManager.GetTimeoutIndex(),
                        0,
                        downloadManager.MaxTimeoutIndex(),
                        KEY_T,
                        HTX_MISC_DLTIMEOUT);
                }
            }
