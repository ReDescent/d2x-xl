...
const char *szModeLetters[] = {"ANRCHY", "TEAM", "ROBO", "COOP", "FLAG", "HOARD", "TMHOARD", "ENTROPY", "MONSTER"};
...
int32_t NetworkJoinPoll(CMenu &menu, int32_t &key, int32_t nCurItem, int32_t nState) {
    ...
    if (networkData.bGamesChanged || (networkData.nActiveGames != networkData.nLastActiveGames)) {
        ...
        for (i = 0; i < networkData.nActiveGames; i++, h++) {
            ...
            int32_t nLevelVersion = gameOpts->menus.bShowLevelVersion
                                        ? missionManager.FindByName(activeNetGames[i].m_info.szMissionName, -1)
                                        : -1;
            ...
            sprintf(
                ...
                i + 1,
                szGameName,
                szModeLetters[activeNetGames[i].m_info.gameMode],
                nplayers,
                activeNetGames[i].m_info.nMaxPlayers,
                szMissionName,
                szLevelName,
                psz);
...
int32_t NetworkBrowseGames(void) {
    ...
    do {
        ...
        for (;;) {
            ...
            memcpy(
                &networkData.serverAddress.m_address,
                activeNetGames[choice].m_server,
                sizeof(networkData.serverAddress.m_address));
            console.printf(CON_DBG, TXT_LOADING_MSN, AGI.m_info.szMissionName);
            if (!(missionManager.LoadByName(AGI.m_info.szMissionName, 0, gameFolders.missions.szDownloads) ||
                  missionManager.LoadByName(AGI.m_info.szMissionName, -1) ||
                  (downloadManager.DownloadMission(AGI.m_info.szMissionName) &&
                   missionManager.LoadByName(AGI.m_info.szMissionName, 0, gameFolders.missions.szDownloads)))) {
