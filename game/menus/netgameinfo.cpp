...
#define INITFLAGS(_t) \
    { \
        sprintf(mTexts[opt], _t); \
        strcat(mTexts[opt], szHighlight); \
        j = 0; \
    }
...
void ShowNetGameInfo(int32_t choice) {
...
#endif
    {
        ...
        opt++;
        if (!AXI.bCompetition && (AXI.bDualMissileLaunch || !AXI.bFriendlyFire || AXI.bInhibitSuicide ||
...
static int32_t WeaponFxCompMode(void) { return (!AXI.bCompetition && (AXI.bLightTrails || AXI.bTracers)) ? 2 : 0; }

//------------------------------------------------------------------------------

static int32_t ShipFxCompMode(void) {
    return (!AXI.bCompetition &&
            (AXI.nShieldEffect || AXI.bDamageExplosions || AXI.bShowWeapons || AXI.bGatlingSpeedUp))
               ? 2
               : 0;
}
...
static int32_t HUDCompMode(void) {
    return (!AXI.bCompetition && (AXI.bTargetIndicators || AXI.bDamageIndicators || AXI.bMslLockIndicators))
               ? 2
               : (AXI.nWeaponIcons != 0) ? 1 : 0;
}

//------------------------------------------------------------------------------

static int32_t RadarCompMode(void) {
    return (!AXI.bCompetition && AXI.bRadarEnabled) && (((AGI.gameFlags & NETGAME_FLAG_SHOW_MAP) != 0) ||
                                                        AXI.bPowerupsOnRadar || AXI.bRobotsOnRadar || (AXI.nRadar != 0))
               ? 2
               : 0;
}
...
static int32_t ControlsCompMode(void) { return (!AXI.bCompetition && (AXI.bMouseLook || AXI.bFastPitch)) ? 2 : 0; }

//------------------------------------------------------------------------------

static int32_t GameplayCompMode(void) {
    return (!AXI.bCompetition &&
            (AXI.bEnableCheats || AXI.bDarkness || AXI.bSmokeGrenades || (AXI.nFusionRamp != 2) || !AXI.bFriendlyFire ||
             AXI.bInhibitSuicide || AXI.bKillMissiles || AXI.bTripleFusion || AXI.bEnhancedShakers || AXI.nHitboxes))
               ? 2
               : 0;
}
...
static int32_t CompetitionMode(void) {
    return GraphicsFxCompMode() | WeaponFxCompMode() | ShipFxCompMode() | HUDCompMode() | RadarCompMode() |
           ControlsCompMode() | GameplayCompMode();
}
...
char *XMLPlayerInfo(char *xmlGameInfo) {
    ...
    for (int32_t i = 0; i < N_PLAYERS; i++) {
        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <Player%d name=\"%s\" ping=\"",
            i,
            NETPLAYER(N_LOCALPLAYER).callsign);
        ...
        uint32_t ip =
            (uint32_t(node[0]) << 24) + (uint32_t(node[1]) << 16) + (uint32_t(node[2]) << 8) + uint32_t(node[3]);

        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "\" score=\"%d\" kills=\"%d\" deaths=\"%d\" country=\"%s\" />\n",
            PLAYER(i).score,
...
char *XMLGameInfo(void) {
    ...
    sprintf(
        xmlGameInfo + strlen(xmlGameInfo),
        "    <Mission Type=\"%s\" Name=\"%s\" Level=\"%d\" ",
        gameStates.app.bD1Mission ? "D1" : gameStates.app.bD2XLevel ? "D2X" : "D2",
        AGI.szMissionTitle,
        AGI.nLevel);
    ...
    sprintf(
        xmlGameInfo + strlen(xmlGameInfo),
        "    <Player Current=\"%d\" Max=\"%d\" />\n",
        AGI.nNumPlayers,
        AGI.nMaxPlayers);

    sprintf(
        xmlGameInfo + strlen(xmlGameInfo),
        "    <Mode Type=\"%s\" Team=\"%d\" Robots=\"%d\" />\n",
        szGameType[(int32_t)extraGameInfo[0].bEnhancedCTF][(int32_t)mpParams.nGameMode],
        (mpParams.nGameMode != NETGAME_ANARCHY) && (mpParams.nGameMode != NETGAME_ROBOT_ANARCHY) &&
    ...
    sprintf(
        ...
        (AGI.nNumPlayers == AGI.nMaxPlayers)
            ? "full"
            : (networkData.nStatus == NETSTAT_STARTING) ? "forming" : szGameState[mpParams.nGameAccess]);
    strcat(xmlGameInfo, "  </Descent>\n");

    sprintf(
        xmlGameInfo + strlen(xmlGameInfo),
        "  <Server Type=\"D2X-XL\" Version=\"%s\" Extensions=\"%s\" />\n",
        VERSION,
        szCompMode[nExtensions = CompetitionMode()]);
    ...
    if (nExtensions) {
        ...
        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <GraphicsFx Shadows=\"%d\" Smoke=\"%d\" Lightning=\"%d\" />\n",
            AXI.bShadows,
            AXI.bUseParticles,
        ...
        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <WeaponFx LightTrails=\"%d\" Tracers=\"%d\" />\n",
            !AXI.bCompetition && AXI.bLightTrails,
            !AXI.bCompetition && AXI.bTracers);

        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <ShipFx Shield=\"%d\" Damage=\"%d\" Weapons=\"%d\" GatlingSpeedup=\"%d\" />\n",
        ...
        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <HUD Icons=\"%d\" TargetInd=\"%d\" DamageInd=\"%d\" LockInd=\"%d\" />\n",
            AXI.nWeaponIcons != 0,
        ...
        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <Radar Players=\"%d\" Powerups=\"%d\" Robots=\"%d\"  HUD=\"%d\" />\n",
        ...
        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <Controls MouseLook=\"%d\" FastPitch=\"%d\" />\n",
            !AXI.bCompetition && AXI.bMouseLook,
            !AXI.bCompetition && AXI.bFastPitch);

        sprintf(
            xmlGameInfo + strlen(xmlGameInfo),
            "    <GamePlay Cheats=\"%d\" Darkness=\"%d\" SmokeGrenades=\"%d\" FusionRamp=\"%d\" FriendlyFire=\"%d\" Suicide=\"%d\" KillMissiles=\"%d\" TriFusion=\"%d\" BetterShakers=\"%d\" HitBoxes=\"%d\" />\n",
