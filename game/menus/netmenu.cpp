...
int32_t NetworkEndLevelPoll2(CMenu &menu, int32_t &key, int32_t nCurItem, int32_t nState) {
    ...
    for (int32_t i = 0; i < N_PLAYERS; i++) {
        if ((PLAYER(i).connected != CONNECT_PLAYING) && (PLAYER(i).connected != CONNECT_ESCAPE_TUNNEL) &&
...
int32_t NetworkStartPoll(CMenu &menu, int32_t &key, int32_t nCurItem, int32_t nState) {
    ...
    if (nm > gameData.multiplayer.nMaxPlayers) {
        InfoBox(
            ...
            "%s %d %s",
            TXT_SORRY_ONLY,
            gameData.multiplayer.nMaxPlayers,
            TXT_NETPLAYERS_IN);
    ...
    if (n < netGameInfo.m_info.nNumPlayers) {
        ...
        if (gameOpts->multi.bNoRankings)
            ...
            sprintf(
                menu[N_PLAYERS - 1].Text(),
                "%d. %s%-20s",
                N_PLAYERS,
                pszRankStrings[NETPLAYER(N_PLAYERS - 1).rank],
                NETPLAYER(N_PLAYERS - 1).callsign);
...
void NetworkMoreGameOptions(void) {
    ...
    do {
        ...
        *szInvul = *(TXT_REACTOR_LIFE - 1);
        ...
        if (IsCoopGame) {
            nLastPlayTime = nLastScoreGoal = 0;
        } else {
        ...
        m.AddCheck(
            ...
            TXT_KEEP_LIGHTS,
            mpParams.bIndestructibleLights,
            KEY_L,
            HTX_MULTI2_KEEPLIGHTS);
        ...
        if (!gameStates.app.bGameRunning)
            m.AddCheck(
                ...
                TXT_ALLOW_CUSTOM_WEAPONS,
                extraGameInfo[0].bAllowCustomWeapons,
                KEY_C,
                HTX_ALLOW_CUSTOM_WEAPONS);
        m.AddText("", "");
        ...
        sprintf(
            szSocket,
            "%d",
            (gameStates.multi.nGameType == UDP_GAME) ? mpParams.udpPorts[0] + networkData.nPortOffset
    ...
    if (gameStates.multi.nGameType >= IPX_GAME) {
        ...
        if ((newSocket < -0xFFFF) || (newSocket > 0xFFFF))
            ...
            networkData.nPortOffset =
                (gameStates.multi.nGameType == UDP_GAME) ? newSocket - mpParams.udpPorts[0] : newSocket;
    ...
    if (extraGameInfo[1].bDarkness) {
        ...
        if (extraGameInfo[1].headlight.bAvailable && (m = menu["spot size"])) {
            ...
            if (v != extraGameInfo[1].nSpotSize) {
                extraGameInfo[1].nSpotSize = extraGameInfo[1].nSpotStrength = v;
...
void NetworkD2XOptions(void) {
    ...
    do {
        ...
        if (!extraGameInfo[1].bCompetition) {
            ...
            m.AddCheck(
                "fast pitch",
                TXT_FASTPITCH,
                (extraGameInfo[1].bFastPitch == 1) ? 1 : 0,
                KEY_P,
                HTX_MULTI2_FASTPITCH);
            // m.AddCheck (, TXT_DUAL_LAUNCH, extraGameInfo [1].bDualMissileLaunch, KEY_M, HTX_GPLAY_DUALLAUNCH, "dual
            // missile launch");
        }
        if (IsTeamGame) {
	...
	mat.AddCheck (TXT_NO_REACTOR, extraGameInfo [1].bDisableReactor, KEY_R, HTX_MULTI2_NOREACTOR, "disable reactor");
        ...
        m.AddCheck(
            "smoke grenades",
            TXT_GPLAY_SMOKEGRENADES,
            extraGameInfo[0].bSmokeGrenades,
            KEY_S,
            HTX_GPLAY_SMOKEGRENADES);
        ...
        *szSlider = *(TXT_RESPAWN_DELAY - 1);
        m.AddSlider(
            "spawn delay",
            szSlider + 1,
            extraGameInfo[0].nSpawnDelay / 5000,
            0,
            12,
            KEY_R,
            HTX_GPLAY_SPAWNDELAY);
    ...
    if ((nMission >= 0) && (missionManager.nLastLevel > 1)) {
        if (gameStates.app.bNostalgia) {
            ...
            m.AddSlider(
                "level number",
                szLevelText,
                NMCLAMP(mpParams.nLevel, 1, missionManager.nLastLevel) - 1,
                0,
                missionManager.nLastLevel - 1);
        }
    }
    m.AddText("", "");
    nGameTypes = m.ToS();
	...
	if (!gameStates.app.bNostalgia)
        ...
        *szSlider = *(TXT_GAME_TYPE - 1);
        m.AddSlider(
            "game type",
            szSlider + 1,
            NMCLAMP(mpParams.nGameType, 0, nGameTypes),
            0,
            nGameTypes,
            KEY_T,
            HTX_GAME_TYPE);
...
int32_t GameParamsMenu(
    ...
    if (m[pszMissionName]->Rebuilding()) {
        int32_t nOffset = 0;
        if ((nMission >= 0) && (strstr(missionManager[nMission].szMissionName, "(D1) ") ||
                                strstr(missionManager[nMission].szMissionName, "(D2) ") ||
                                strstr(missionManager[nMission].szMissionName, "(XL) ")))
            nOffset = 5;
        strncpy(
            netGameInfo.m_info.szMissionName,
            (nMission < 0) ? "" : missionManager[nMission].filename,
            sizeof(netGameInfo.m_info.szMissionName) - 1);
        if (gameStates.app.bNostalgia) {
            m["mission name"]->SetText(
                (nMission < 0) ? const_cast<char *>(TXT_NONE_SELECTED)
                               : const_cast<char *>(missionManager[nMission].szMissionName + nOffset));
            if ((nMission >= 0) && (missionManager.nLastLevel > 1)) {
    ...
    if (key != -1) {
        ...
        if ((missionManager.nLastLevel > 0) &&
            ((mpParams.nLevel < 1) || (mpParams.nLevel > missionManager.nLastLevel))) {
...
int32_t NetworkGetGameParams(int32_t bAutoRun) {
    ...
    *szLevelText = *szMaxPlayers = *szIpAddr = *szLevel = '\0';
    ...
    strncpy(
        netGameInfo.m_info.szMissionTitle,
        missionManager[nMission].szMissionName + (gameOpts->menus.bShowLevelVersion ? 4 : 0),
        sizeof(netGameInfo.m_info.szMissionTitle));
...
int32_t NetworkChooseConnect() { return 1; }
...
int32_t IpAddrMenuCallBack(CMenu &menu, int32_t &key, int32_t nCurItem, int32_t nState) { return nCurItem; }
    ...
    if (bUDP && !tracker.m_bUse) {
        if (!*mpParams.szServerIpAddr) {
            ...
            if (IpxInit(-1) != IPX_INIT_OK) {
                ...
                sprintf(szPort[1], "%s%d", (nSign < 0) ? "-" : (nSign > 0) ? "+" : "", mpParams.udpPorts[1]);
                IpxClose();
            }
        }
