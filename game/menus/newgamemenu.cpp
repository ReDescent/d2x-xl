...
int32_t SelectAndLoadMission(int32_t bMulti, int32_t *bAnarchyOnly) {
    ...
    do {
        ...
        nNewMission = lb.ListBox(
            bMulti ? TXT_MULTI_MISSION : menuTitles[gameOpts->app.nVersionFilter],
            msnNames,
            nDefaultMission);
        GameFlushInputs();
        if (nNewMission == -1)
    ...
    strncpy(
        gameConfig.szLastMission,
        msnNames[nNewMission] + (MsnHasGameVer(msnNames[nNewMission]) ? 4 : 0),
        sizeof(gameConfig.szLastMission));
    ...
    for (;;) {
        ...
        if (nMission >= 0) {
            ...
            if (!gameStates.app.bPrecomputeLightmaps) {
                ...
                m.AddCheck(
                    ...
                    extraGameInfo[IsMultiGame].bAllowCustomWeapons,
                    KEY_C,
        ...
        if (gameStates.app.bPrecomputeLightmaps) {
            ...
            if (gameOpts->render.color.bConfigurable) {
                ...
                m.AddCheck(
                    "rebuild lightmaps",
                    TXT_REBUILD_LIGHTMAPS,
                    gameStates.app.bRebuildLightmaps,
                    KEY_R,
                    HTX_REBUILD_LIGHTMAPS);
            }
        } else {
#if DBG
            m.AddText("", "");
...
int32_t ExecMultiMenuOption(CMenu &m, int32_t nChoice) {
    ...
    if ((nChoice == multiOpts.nStartUdpTracker) || (nChoice == multiOpts.nJoinUdpTracker)) {
        ...
        (nChoice == multiOpts.nStartIPX) || (nChoice == multiOpts.nStartKali) || (nChoice == multiOpts.nStartMCast4))
        bStart = 1;
    ...
    gameStates.multi.bServer[0] = gameStates.multi.bServer[1] = (nChoice & 1) == 0;
    ...
    do {
...
#endif
            nConnections = m.ToS();
            m[m.IndexOf("ipx game") + NMCLAMP(gameStates.multi.nConnection, 0, nConnections - m.IndexOf("ipx game"))]
                .Value() = 1;
        } else {
        ...
        if (i > -1) {
            if (!DBG && gameStates.app.bNostalgia > 1)
                ...
                for (gameStates.multi.nConnection = 0; gameStates.multi.nConnection < nConnections;
