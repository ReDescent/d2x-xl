...
void PhysicsOptionsMenu(void) {
    ...
    do {
        ...
        if (!gameStates.app.bGameRunning)
            m.AddCheck(
                "unnerf D1 weapons",
                TXT_UNNERF_D1_WEAPONS,
                extraGameInfo[0].bUnnerfD1Weapons,
                KEY_U,
                HTX_UNNERF_D1_WEAPONS);
        ...
        if (gameOpts->app.bExpertMode == SUPERUSER) {
            ...
            *szSlider = *(TXT_MSL_TURNSPEED - 1);
            m.AddSlider(
                "msl turn speed",
                szSlider + 1,
                extraGameInfo[0].nWeaponTurnSpeed,
                0,
                3,
                KEY_T,
                HTX_GPLAY_MSL_TURNSPEED);
            ...
            *szSlider = *(TXT_MSL_STARTSPEED - 1);
            m.AddSlider(
                "msl start speed",
                szSlider + 1,
                3 - extraGameInfo[0].nMslStartSpeed,
                0,
                3,
                KEY_S,
                HTX_MSL_STARTSPEED);
            ...
            *szSlider = *(TXT_SLOWMOTION_SPEEDUP - 1);
            m.AddSlider(
                "slow motion speedup",
                szSlider + 1,
                gameOpts->gameplay.nSlowMotionSpeedup - 4,
                0,
                4,
                KEY_M,
                HTX_SLOWMOTION_SPEEDUP);
            m.AddText("", "");
        ...
        *szSlider = *(TXT_COLLISION_HANDLING - 1);
        m.AddSlider(
            "collision handling",
            szSlider + 1,
            extraGameInfo[0].bUseHitAngles,
            0,
            1,
            KEY_C,
            HTX_GPLAY_COLLHANDLING);
