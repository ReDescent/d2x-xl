    ...
    *szSmokeSize[i] = *(TXT_SMOKE_SIZE - 1);
    ...
    if (i > 2) {
        sprintf(
            szSmokeLife[i] + 1,
            TXT_SMOKE_LIFE,
            pszSmokeLife[NMCLAMP(gameOpts->render.particles.nLife[i] - 1, 0, 3)]);
...
void SmokeDetailsMenu(void) {
    ...
    do {
        ...
        for (j = 1; j < 5; j++)
            smokeOpts.nSize[j] = smokeOpts.nDensity[j] = smokeOpts.nAlpha[j] = -1;
        ...
        if (gameOpts->render.particles.bSyncSizes) {
            ...
            smokeOpts.nDensity[0] = smokeOpts.nSize[0] = -1;
        }
        ...
        if (gameOpts->render.particles.bPlayers) {
            ...
            m.AddCheck(
                "reduce lag",
                TXT_SMOKE_DECREASE_LAG,
                gameOpts->render.particles.bDecreaseLag,
                KEY_R,
                HTX_ADVREND_DECSMOKELAG);
        }
