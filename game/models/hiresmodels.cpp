...
tReplacementModel replacementModels[] = {
    ...
    //{NULL, NULL, 110, 0, 0, -1}, 	//filename NULL means this is an additional model number to be used with the last
    // listed oof filename
    // robots
...
int32_t ReplacementModelCount(void) { return sizeofa(replacementModels); }
...
int16_t LoadLoresModel(int16_t i) {
    ...
    if (nModel == nDbgModel)
        BRP;
#endif
    pModel = ((gameStates.app.bFixModels && gameStates.app.bAltModels) ? gameData.modelData.polyModels[2]
...
#endif
    if ((j = LoadASEModel(
             gameData.modelData.aseModels[bCustom != 0] + gameData.modelData.nHiresModels,
             nModel,
             bCustom))) {
        if (bLog)
            PrintLog(-1);
