...
static void OOF_InitMinMax(CFloatVector *pvMin, CFloatVector *pvMax) {
    if (pvMin && pvMax) {
        (*pvMin).v.coord.x = (*pvMin).v.coord.y = (*pvMin).v.coord.z = 1000000;
        (*pvMax).v.coord.x = (*pvMax).v.coord.y = (*pvMax).v.coord.z = -1000000;
    }
}
...
static bool
OOF_ReadVertList(CFile &cf, CArray<CFloatVector> &list, int32_t nVerts, CFloatVector *pvMin, CFloatVector *pvMax) {
    char szId[20] = "";
...
void CAnim::Init(void) { m_nTicks = 0; }
...
void CRotAnim::Init(void) { CAnim::Init(); }
...
int32_t CRotAnim::Read(CFile &cf, CModel *po, int32_t bTimed) {
    ...
    if (!m_frames.Create(m_nFrames, "OOF::CRotAmin::m_frames"))
        return 0;
    m_frames.Clear(0);
    if (bTimed && (m_nTicks = abs(m_nLastFrame - m_nFirstFrame) + 1) &&
...
int32_t CPosAnim::Read(CFile &cf, CModel *po, int32_t bTimed) {
    if (!this->CFrameInfo::Read(cf, po, bTimed))
        return 0;
    if (bTimed && (m_nTicks = m_nLastFrame - m_nFirstFrame) &&
...
void CSpecialPoint::Init(void) { memset(this, 0, sizeof(*this)); }
...
void CFaceList::Init(void) { m_nFaces = 0; }
...
void CEdgeList::Init(void) { m_nEdges = m_nContourEdges = 0; }
...
int32_t CModel::ReloadTextures(void) { return m_textures.Bind(m_bCustom); }
...
int32_t CModel::Read(char *filename, int16_t nModel, int32_t bFlipV, int32_t bCustom) {
    ...
    if (bCustom ? !cf.Open(filename, gameFolders.mods.szModels[bCustom - 1], "rb", 0)
...
int32_t OOF_ReleaseTextures(void) {
    ...
    for (bCustom = 0; bCustom < 2; bCustom++)
        for (i = gameData.modelData.nHiresModels, pModel = gameData.modelData.oofModels[bCustom].Buffer(); i;
             i--, pModel++)
