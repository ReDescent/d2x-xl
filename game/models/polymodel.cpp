...
void CPolyModel::Parse(const char *filename, tRobotInfo *pRobotInfo) {
    ...
    while (POF_Read(&id, sizeof(id), 1, modelBuf) == 1) {
        ...
        case ID_OHDR: { // Object header
            ...
            m_info.rad[0] = m_info.rad[1] = POF_ReadInt(modelBuf);
...
void CPolyModel::FindMinMax(void) {
    ...
    for (int32_t i = 0; i < m_info.nModels; i++) {
        ...
        if (i == 0)
            big_mn = big_mx = mn;
        while (nVerts--) {
            if ((*vp).v.coord.x > mx.v.coord.x)
                mx.v.coord.x = (*vp).v.coord.x;
            if ((*vp).v.coord.y > mx.v.coord.y)
                mx.v.coord.y = (*vp).v.coord.y;
            if ((*vp).v.coord.z > mx.v.coord.z)
                mx.v.coord.z = (*vp).v.coord.z;
            if ((*vp).v.coord.x < mn.v.coord.x)
                mn.v.coord.x = (*vp).v.coord.x;
            if ((*vp).v.coord.y < mn.v.coord.y)
                mn.v.coord.y = (*vp).v.coord.y;
            if ((*vp).v.coord.z < mn.v.coord.z)
                mn.v.coord.z = (*vp).v.coord.z;
            if ((*vp).v.coord.x + ofs.v.coord.x > big_mx.v.coord.x)
                big_mx.v.coord.x = (*vp).v.coord.x + ofs.v.coord.x;
            if ((*vp).v.coord.y + ofs.v.coord.y > big_mx.v.coord.y)
                big_mx.v.coord.y = (*vp).v.coord.y + ofs.v.coord.y;
            if ((*vp).v.coord.z + ofs.v.coord.z > big_mx.v.coord.z)
                big_mx.v.coord.z = (*vp).v.coord.z + ofs.v.coord.z;
            if ((*vp).v.coord.x + ofs.v.coord.x < big_mn.v.coord.x)
                big_mn.v.coord.x = (*vp).v.coord.x + ofs.v.coord.x;
            if ((*vp).v.coord.y + ofs.v.coord.y < big_mn.v.coord.y)
                big_mn.v.coord.y = (*vp).v.coord.y + ofs.v.coord.y;
            if ((*vp).v.coord.z + ofs.v.coord.z < big_mn.v.coord.z)
                big_mn.v.coord.z = (*vp).v.coord.z + ofs.v.coord.z;
            vp++;
        }
    }
}
...
int32_t CPolyModel::LoadTextures(tBitmapIndex *altTextures) {
    ...
    if (altTextures) {
        for (i = 0; i < nTextures; i++) {
            ...
            gameData.modelData.textures[i] =
                gameData.pigData.tex.bitmaps[gameStates.app.bD1Model] + altTextures[i].index;
        ...
        for (i = 0, j = m_info.nFirstTexture; i < nTextures; i++, j++) {
            ...
            gameData.modelData.textures[i] =
                gameData.pigData.tex.bitmaps[gameStates.app.bD1Model] + gameData.modelData.textureIndex[i].index;
            if (gameStates.render.bBuildModels)
...
int32_t LoadPolyModel(const char *filename, int32_t nTextures, int32_t nFirstTexture, tRobotInfo *pRobotInfo) {
    ...
    if (gameData.modelData.nPolyModels > MAX_POLYGON_MODELS - 10)
        console.printf(
            CON_VERBOSE,
            "Used %d/%d polygon model slots\n",
            gameData.modelData.nPolyModels + 1,
            MAX_POLYGON_MODELS);
#endif
    Assert(strlen(filename) <= 12);
    strcpy(pofNames[gameData.modelData.nPolyModels], filename);
    gameData.modelData.polyModels[0][gameData.modelData.nPolyModels++]
        .Load(filename, nTextures, nFirstTexture, pRobotInfo);
