...
int32_t CDownloadManager::MaxTimeoutIndex(void) { return sizeofa(m_timeouts) - 1; }

//------------------------------------------------------------------------------

int32_t CDownloadManager::GetTimeoutIndex(void) { return m_iTimeout; }

//------------------------------------------------------------------------------

int32_t CDownloadManager::GetTimeoutSecs(void) { return m_timeouts[m_iTimeout]; }
...
int32_t CDownloadManager::FindClient(uint8_t *server, uint8_t *node) {
    for (int32_t i = 0; i < m_nClients; i++)
        if (m_clients[i].nState && !memcmp(m_clients[i].addr.Network(), server, 4) &&
...
int32_t CDownloadManager::AcceptClient(void) {
    ...
    if (!(m_clients[i].thread = SDL_CreateThread(UploadThread, "UploadThread", &i))) {
        RemoveClient(i);
        return -1;
    }
    return i;
...
int32_t CDownloadManager::RequestDownload(tClient *pClient) { return SendRequest(PID_DOWNLOAD, PID_DL_START, pClient); }
...
int32_t CDownloadManager::InitUpload(uint8_t *data) {
    if (m_nSemaphore)
        return 0;
    m_nSemaphore++;
    if (gameStates.app.bHaveSDLNet && extraGameInfo[0].bAutoDownload && (data[1] == PID_DL_START) &&
        (0 <= AcceptClient())) {
        m_nSemaphore--;
        return 0;
    }
