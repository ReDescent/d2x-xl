...
uint8_t *IpxGetMyServerAddress() { return reinterpret_cast<uint8_t *>(&ipxNetwork); }

//------------------------------------------------------------------------------

uint8_t *IpxGetMyLocalAddress() { return ipx_MyAddress.Node(); }
...
int32_t IpxGetPacketData(uint8_t *data) {
    ...
    while (driver->PacketReady(&ipxSocketData)) {
        ...
        if (dataSize < 6)
            continue;
        dataOffs =
            tracker.IsTracker(networkData.packetSource.GetServer(), networkData.packetSource.GetPort(), (char *)buf)
                ? 0
                : 4;
        ...
        for (i = 0; i < nIpxNetworks; i++) {
            if (memcmp(ipxNetworks + i, &ipxNetwork, 4)) {
                ...
                IPXSendPacketData(
                    data,
                    dataSize,
                    reinterpret_cast<uint8_t *>(ipxNetworks + i),
                    broadcast,
                    local_address);
            } else {
...
void IpxReadUserFile(const char *filename) {
    ...
    while (fgets(szTemp, sizeof(szTemp), fp)) {
        ...
        if (nIpxUsers < MAX_USERS) {
            ...
            // ipx_real_buffer[0],ipx_real_buffer[1],ipx_real_buffer[2],ipx_real_buffer[3]); printf
