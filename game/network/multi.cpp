...
static int32_t multiMessageLengths[MULTI_MAX_TYPE + 1][2] = {
    ...
    {6, 10}, // MULTI_FLAGS
    {2, -1}, // MULTI_DROP_BLOB
    {/*2 **/ MAX_POWERUP_TYPES * sizeof(uint16_t) + 1,
     MAX_POWERUP_TYPES * sizeof(uint16_t) + 5}, // MULTI_POWERUP_UPDATE
...
int16_t GetTeam(int32_t nPlayer) { return (netGameInfo.m_info.GetTeamVector() & (1 << nPlayer)) ? 1 : 0; }
...
void ChoseTeam(int32_t nPlayer, bool bForce) {
    if (gameStates.app.bHaveExtraGameInfo && (bForce || extraGameInfo[1].bAutoBalanceTeams) && IsTeamGame &&
        IAmGameHost()) {
...
void AutoBalanceTeams(void) {
    if (gameStates.app.bHaveExtraGameInfo && extraGameInfo[1].bAutoBalanceTeams && IsTeamGame && IAmGameHost()) {
        ...
        teamSize[0] = teamSize[1] = teamScore[0] = teamScore[1] = 0;
        for (i = 0; i < N_PLAYERS; i++) {
            t = GetTeam(i);
            teamSize[t]++;
            teamScore[t] = PLAYER(i).score;
...
static void QSortScoreList(int32_t *score, int32_t left, int32_t right) {
    int32_t l = left, r = right, m = score[(l + r) / 2];
    ...
    for (i = 0; i < MAX_NUM_NET_PLAYERS; i++) {
        if (IsCoopGame)
            ...
            if (PLAYER(i).netKilledTotal + PLAYER(i).netKillsTotal)
                score[i] = int32_t(
                    double(PLAYER(i).netKillsTotal) / double(PLAYER(i).netKilledTotal + PLAYER(i).netKillsTotal) /
                    100.0);
            else
...
void MultiComputeKill(int32_t nKiller, int32_t nKilled, int32_t nKillerPlayer = -1) {
    ...
    if ((nKilled < 0) || (nKilled > gameData.objData.nLastObject[0]) || (nKiller < 0) ||
        (nKiller > gameData.objData.nLastObject[0])) {
        ...
        if (IsHoardGame) {
            ...
            if (t0 == t1) {
                ...
                if (extraGameInfo[1].entropy.nMaxVirusCapacity)
                    if (PLAYER(nKillerPlayer).secondaryAmmo[SMARTMINE_INDEX] >
                        extraGameInfo[1].entropy.nMaxVirusCapacity)
                        PLAYER(nKillerPlayer).secondaryAmmo[SMARTMINE_INDEX] =
                            extraGameInfo[1].entropy.nMaxVirusCapacity;
            }
        } else if (IsTeamGame) {
            if (t0 == t1) {
...
void MultiSyncMonsterball(void) {
    if ((gameData.appData.GameMode(GM_MONSTERBALL)) && gameData.hoardData.pMonsterBall && IAmGameHost()) {
        ...
        if (gameStates.app.nSDLTicks[0] - t0 > 250) {
            ...
            i += sizeof(int16_t);
            memcpy(
                gameData.multigame.msg.buf + i,
                &gameData.hoardData.pMonsterBall->info.position.vPos,
                sizeof(gameData.hoardData.pMonsterBall->info.position.vPos));
            i += sizeof(gameData.hoardData.pMonsterBall->info.position.vPos);
            memcpy(
                gameData.multigame.msg.buf + i,
                &gameData.hoardData.pMonsterBall->info.position.mOrient,
                sizeof(gameData.hoardData.pMonsterBall->info.position.mOrient));
            i += sizeof(gameData.hoardData.pMonsterBall->info.position.mOrient);
            memcpy(
                gameData.multigame.msg.buf + i,
                &gameData.hoardData.pMonsterBall->mType.physInfo.velocity,
                sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.velocity));
            i += sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.velocity);
            memcpy(
                gameData.multigame.msg.buf + i,
                &gameData.hoardData.pMonsterBall->mType.physInfo.thrust,
                sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.thrust));
            i += sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.thrust);
            memcpy(
                gameData.multigame.msg.buf + i,
                &gameData.hoardData.pMonsterBall->mType.physInfo.rotVel,
                sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.rotVel));
            i += sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.rotVel);
            memcpy(
                gameData.multigame.msg.buf + i,
                &gameData.hoardData.pMonsterBall->mType.physInfo.rotThrust,
                sizeof(gameData.hoardData.pMonsterBall->mType.physInfo.rotThrust));
...
int32_t MultiEndLevel(int32_t *secret) { return IsNetworkGame ? NetworkEndLevel(secret) : 0; }
...
int32_t MultiMenuPoll(void) {
    ...
    if (gameStates.app.bEndLevelSequence || (gameData.reactorData.bDestroyed && !bReactorWasAlive) ||
...
void MultiDoFire(uint8_t *buf) {
    ...
    gameData.multigame.weapon.nFired[0] = gameData.multigame.weapon.nFired[1] = 0;
}
...
void MultiDestroyPlayerShip(int32_t nPlayer, int32_t bExplode, int32_t nRemoteCreated, int16_t *objList) {
    ...
    gameData.multigame.create.nCount = 0; // this variable gets incremented everytime a powerup is created; use it to
                                          // find out how many powerups actually get created
    DropPlayerEggs(pObj);
...
void MultiDoPlayerExplode(uint8_t *buf) {
    ...
    if (gameStates.multi.nGameType == UDP_GAME) {
        ...
        pObj->Velocity().Set(
            ...
            GET_INTEL_INT(buf + pBuffer + 4),
            GET_INTEL_INT(buf + pBuffer + 8));
        pBuffer += 12;
        pObj->Position().Set(
            GET_INTEL_INT(buf + pBuffer),
            GET_INTEL_INT(buf + pBuffer + 4),
...
void MultiDoInvul(uint8_t *buf) { MultiDoPowerState(buf, PLAYER_FLAGS_INVULNERABLE, true); }

//-----------------------------------------------------------------------------

void MultiDoDeInvul(uint8_t *buf) { MultiDoPowerState(buf, PLAYER_FLAGS_INVULNERABLE, false); }
...
void MultiDoCtrlcenFire(uint8_t *buf) {
    ...
    if (0 <= (i = FindReactor(OBJECT(nObject))))
        CreateNewWeaponSimple(
            &vTarget,
            gameData.reactorData.states[i].vGunPos + (int32_t)nGun,
            nObject,
    ...
    if (pObj) {
        if (nSound > 0)
            ...
            audio.CreateObjectSound(
                ...
                pObj->Index(),
                0,
                ((nSound >= SND_ADDON_SCRAPE) && (nSound <= SND_ADDON_SCRAPE4)) ? volume
                                                                                : gameOpts->sound.xCustomSoundVolume,
                I2X(256),
                -1,
                -1,
                AddonSoundName(nSound),
                1);
        }
    }
}
...
void MultiDoTrigger(uint8_t *buf) {
    ...
    int16_t nObject =
        (gameStates.multi.nGameType == UDP_GAME) ? GET_INTEL_SHORT(buf + pBuffer) : PLAYER(nPlayer).nObject;
...
void MultiDoDropMarker(uint8_t *buf) {
    ...
    if ((markerManager.Objects(nMarker) != -1) && (OBJECT(markerManager.Objects(nMarker))->info.nType != OBJ_NONE) &&
        (markerManager.Objects(nMarker) != 0))
        ReleaseObject(markerManager.Objects(nMarker));
    markerManager.SetObject(
        nPlayer * 2 + nMsg,
        DropMarkerObject(vPos, LOCALOBJECT->info.nSegment, LOCALOBJECT->info.position.mOrient, (uint8_t)nMarker));
...
void MultiSetObjectTextures(CObject *pObj) {
    ...
    if (!id)
        ...
        pBmIndex[4] =
            gameData.pigData.tex
                .objBmIndex[gameData.pigData.tex.pObjBmIndex[gameData.pigData.tex.nFirstMultiBitmap + 2 * id]];
        pBmIndex[5] =
            gameData.pigData.tex
                .objBmIndex[gameData.pigData.tex.pObjBmIndex[gameData.pigData.tex.nFirstMultiBitmap + 2 * id + 1]];
...
void MultiSendPlayerExplode(uint8_t nType) {
    ...
    gameData.multigame.msg.buf[pBuffer++] =
        uint8_t(gameData.multiplayer.SecondaryAmmo(N_LOCALPLAYER, CONCUSSION_INDEX, 0));
    ...
    gameData.multigame.msg.buf[pBuffer++] =
        uint8_t(gameData.multiplayer.SecondaryAmmo(N_LOCALPLAYER, EARTHSHAKER_INDEX));
    PUT_INTEL_SHORT(
        gameData.multigame.msg.buf + pBuffer,
        LOCALPLAYER.primaryAmmo[VULCAN_INDEX] + gameData.multiplayer.weaponStates[N_LOCALPLAYER].nAmmoUsed);
    pBuffer += 2;
...
// This function is called if a player ship is about to explode. It will make sure the ship doesn't drop more powerups
// of each kind than the entire mine initially contained to prevent rampant powerups in netgames
...
void MultiAdjustCapForPlayer(int32_t nPlayer) {
    ...
    AddAllowedPowerup(
        POW_VULCAN_AMMO,
        (gameData.multiplayer.PrimaryAmmo(nPlayer, VULCAN_INDEX) + VULCAN_CLIP_CAPACITY - 1) / VULCAN_CLIP_CAPACITY);
    for (i = 0; i < MAX_SECONDARY_WEAPONS; i++) {
        nType = int32_t(secondaryWeaponToPowerup[0][i]);
        AddAllowedPowerup(
            nType,
            i ? gameData.multiplayer.SecondaryAmmo(nPlayer, i) : gameData.multiplayer.SecondaryAmmo(nPlayer, i, 0));
    }

    for (i = 0; i < int32_t(sizeofa(nDeviceFlags)); i++)
        if (gameData.multiplayer.Flag(nPlayer, nDeviceFlags[i]) &&
            !(extraGameInfo[IsMultiGame].loadout.nDevice & nDeviceFlags[i]))
...
void MultiSendKill(int32_t nObject) {
    ...
    if (nKillerObj > -1) {
        ...
        int16_t nRemoteObj =
            (int16_t)GetRemoteObjNum(nKillerObj, reinterpret_cast<int8_t &>(gameData.multigame.msg.buf[pBuffer + 2]));
...
void MultiSendInvul(void) { MultiSendPowerState(MULTI_INVUL, true); }

//-----------------------------------------------------------------------------
// Broadcast a change in our pflags (made to support cloaking)

void MultiSendDeInvul(void) { MultiSendPowerState(MULTI_DEINVUL, false); }

//-----------------------------------------------------------------------------
// Broadcast a change in our pflags (made to support cloaking)

void MultiSendCloak(void) { MultiSendPowerState(MULTI_CLOAK, true); }

//-----------------------------------------------------------------------------
// Broadcast a change in our pflags (made to support cloaking)

void MultiSendDeCloak(void) { MultiSendPowerState(MULTI_DECLOAK, false); }
            ...
            if (!IsCoopGame) {
                ...
                if (nObject != -1) {
                    ...
                    if (pObj) {
                        ...
                        pObj->rType.animationInfo.xFrameTime =
                            gameData.effectData.animations[0][pObj->rType.animationInfo.nClipIndex].xFrameTime;
...
void CSegment::OverrideTextures(
    int16_t nTexture,
    int16_t nOldTexture,
    int16_t nTexture2,
    int32_t bFullBright,
    ...
    if (nTexture > -1) {
        ...
        for (int32_t j = 0; j < SEGMENT_SIDE_COUNT; j++, pSide++) {
            ...
            if (bForce || (m_sides[j].m_nBaseTex == nOldTexture)) {
                ...
                if ((extraGameInfo[1].entropy.nOverrideTextures == 1) && (pSide->m_nOvlTex > 0) && (nTexture2 > 0))
...
void CSegment::ChangeTexture(int32_t oldOwner) {
    ...
    case SEGMENT_FUNC_ROBOTMAKER:
        if (IsEntropyGame && (m_owner >= 0))
            OverrideTextures(
                texOverrides[(int32_t)m_owner],
                (int16_t)((oldOwner < 0) ? -1 : texOverrides[oldOwner]),
                316,
                bFullBright,
                oldOwner < 0);
        break;
...
int32_t MultiDeleteExtraObjects(void) {
    ...
    FORALL_OBJS(pObj) {
        ...
        if ((nType == OBJ_PLAYER) || (nType == OBJ_GHOST) || (nType == OBJ_CAMBOT) || (nType == OBJ_EFFECT))
            ...
            (nType != OBJ_NONE) && (nType != OBJ_PLAYER) && (nType != OBJ_POWERUP) && (nType != OBJ_MONSTERBALL) &&
            (nType != OBJ_EXPLOSION) && (nType != OBJ_REACTOR) && (nType != OBJ_HOSTAGE) &&
            ((nType != OBJ_WEAPON) || (pObj->info.nId != SMALLMINE_ID))) {
...
void ExtractNetPlayerStats(tNetPlayerStats *ps, CPlayerInfo *pd) {
    ...
    // memcpy (ps->primaryAmmo, pd->primaryAmmo, MAX_PRIMARY_WEAPONS*sizeof (int16_t));        // How much ammo of each
    // nType. memcpy (ps->secondaryAmmo, pd->secondaryAmmo, MAX_SECONDARY_WEAPONS*sizeof (int16_t)); // How much ammo of
    // each nType.
...
void UseNetPlayerStats(CPlayerInfo *ps, tNetPlayerStats *pd) {
    ...
    // memcpy (ps->secondaryAmmo, pd->secondaryAmmo, MAX_SECONDARY_WEAPONS*sizeof (int16_t)); // How much ammo of each
    // nType.
    ps->lastScore = INTEL_INT(pd->lastScore); // Score at beginning of current level.
    ps->score = INTEL_INT(pd->score); // Current score.
...
void MultiDoGuided(uint8_t *buf) {
    ...
    if ((missileP < OBJECTS.Buffer()) || (missileP - OBJECTS > gameData.objData.nLastObject[0])) {
...
void MultiSendJasonCheat(int32_t num) { return; }
...
void MultiSendLight(int32_t nSegment, uint8_t val) {
    ...
    for (int32_t i = 0; i < 6; i++, pBuffer += 2)
        PUT_INTEL_SHORT(
            gameData.multigame.msg.buf + pBuffer,
            (pSeg->m_sides[i].FaceCount()) ? pSeg->m_sides[i].m_nOvlTex : 0);
    SET_MSG_ID
    MultiSendData(gameData.multigame.msg.buf, pBuffer, 1);
}
...
void MultiSendMonsterball(int32_t bForce, int32_t bCreate) {
    ...
    if (bForce || (t - nTimeout > 1000)) {
        ...
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->info.position.vPos.v.coord.x);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->info.position.vPos.v.coord.y);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->info.position.vPos.v.coord.z);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->mType.physInfo.velocity.v.coord.x);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->mType.physInfo.velocity.v.coord.y);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->mType.physInfo.velocity.v.coord.z);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->mType.physInfo.rotVel.v.coord.x);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->mType.physInfo.rotVel.v.coord.y);
        pBuffer += 4;
        PUT_INTEL_INT(
            gameData.multigame.msg.buf + pBuffer,
            gameData.hoardData.pMonsterBall->mType.physInfo.rotVel.v.coord.z);
        pBuffer += 4;
        MultiSendData(gameData.multigame.msg.buf, pBuffer, 1);
    }
}
...
void MultiDoDropBlob(uint8_t *buf) { DropAfterburnerBlobs(PLAYEROBJECT(int32_t(buf[1])), 2, I2X(1), -1, NULL, 0); }
    ...
    if (nFunction == 0)
        ...
        if (nSound == SOUND_AFTERBURNER_PLAY)
            audio.CreateObjectSound(
                ...
                SOUNDCLASS_PLAYER,
                int16_t(PLAYER(nPlayer).nObject));
        else
            audio.CreateObjectSound(
                nSound,
                SOUNDCLASS_PLAYER,
                ...
                I2X(1),
                I2X(256),
                AFTERBURNER_LOOP_START,
                AFTERBURNER_LOOP_END);
    }
}
...
void DropFlag(void) {
    ...
    nObject = SpitPowerup(
        ...
        (uint8_t)((GetTeam(N_LOCALPLAYER) == TEAM_RED) ? POW_BLUEFLAG : POW_REDFLAG));
    if (!OBJECT(nObject))
        return;
...
void MultiSendRobotControls(uint8_t nPlayer) {
    ...
    memcpy(
        &(gameData.multigame.msg.buf[pBuffer]),
        &gameData.multigame.robots.controlledTime,
        MAX_ROBOTS_CONTROLLED * 4);
    pBuffer += (MAX_ROBOTS_CONTROLLED * 4);
...
void MultiDoRanking(uint8_t *buf) {
    ...
    if (!gameOpts->multi.bNoRankings)
        HUDInitMessage(TXT_RANKCHANGE2, PLAYER(nPlayer).callsign, rankstr, pszRankStrings[(int32_t)rank]);
}
...
void MultiDoStopConquerWarning(uint8_t *buf) { audio.StopSound(SOUND_CONTROL_CENTER_WARNING_SIREN); }
...
void MultiAdjustPowerups(void) {
    ...
    if (gameData.multiplayer.WaitingForExplosion() ||
        gameData.multiplayer.WaitingForWeaponInfo()) { // don't act if player ship status pending
