...
int32_t MultiAddControlledRobot(int32_t nObject, int32_t agitation) {
    ...
    if (firstFreeRobot != -1) // Room left for new robots
        i = firstFreeRobot;
    else if ((agitation > lowest_agitation)) { // && (lowest_agitation <= MAX_TO_DELETE)) // Replace some old robot with
                                               // a more agitated one
...
void MultiSendClaimRobot(int32_t nObject) {
    ...
    int16_t nRemoteObject =
        GetRemoteObjNum(nObject, reinterpret_cast<int8_t &>(gameData.multigame.msg.buf[pBuffer + 2]));
...
int32_t MultiSendRobotFrame(int32_t sent) {
    ...
    for (i = 0; i < MAX_ROBOTS_CONTROLLED; i++) {
        ...
        if ((gameData.multigame.robots.controlled[sending] != -1) &&
            ((gameData.multigame.robots.sendPending[sending] > sent) ||
             (gameData.multigame.robots.fired[sending] > sent))) {
...
void MultiSendCreateRobotPowerups(CObject *pDelObj) {
    ...
    gameData.multigame.msg.buf[pBuffer++] = N_LOCALPLAYER;
    hBufP =
        pBuffer++; // points to the buffer location where the number of powerups contained in the data packet is stored
    ...
    for (i = 0; i < gameData.multigame.create.nCount; i++) {
        PUT_INTEL_SHORT(
            gameData.multigame.msg.buf + pBuffer + 2 * i,
            gameData.multigame.create
                .nObjNums[i]); // pBuffer must always point to the start of the object data list here!
        SetLocalObjNumMapping(gameData.multigame.create.nObjNums[i]);
    }
    for (; i < MAX_ROBOT_POWERUPS; i++)
        PUT_INTEL_SHORT(
            gameData.multigame.msg.buf + pBuffer + 2 * i,
            -1); // pBuffer must always point to the start of the object data list here!
...
void MultiDoClaimRobot(uint8_t *buf) {
    ...
    if (pObj->cType.aiInfo.REMOTE_OWNER != -1)
        if (MULTI_ROBOT_PRIORITY(nRemoteBot, nPlayer) <=
            MULTI_ROBOT_PRIORITY(nRemoteBot, pObj->cType.aiInfo.REMOTE_OWNER))
            return;
...
void MultiDoRobotFire(uint8_t *buf) {
    ...
    if ((nRobot < 0) || (nRobot > gameData.objData.nLastObject[0]) || (pObj->info.nType != OBJ_ROBOT) ||
...
void MultiDoCreateRobot(uint8_t *buf) {
    ...
    pBuffer += 2;
    nType = buf[pBuffer];
    if ((nPlayer < 0) || (nRemoteObj < 0) || (nProducer < 0) || (nProducer >= gameData.producerData.nProducers) ||
        (nPlayer >= N_PLAYERS))
        return;
...
void MultiDoBossActions(uint8_t *buf) {
    ...
    case 2: // Cloak
        ...
        gameData.bossData[nBossIdx].m_nCloakEndTime =
            gameData.timeData.xGame + gameData.bossData[nBossIdx].m_nCloakDuration;
...
void MultiDoCreateRobotPowerups(uint8_t *buf) {
    ...
    if (gameStates.multi.nGameType != UDP_GAME)
        ...
        // MultiExplodeRobot will handle the cases of the robot not existing or still existing but already about to
        // being destroyed
        int8_t nOwner;
...
void MultiRobotRequestChange(CObject *robot, int32_t player_num) {
    ...
    if ((gameData.multigame.robots.agitation[slot] < 70) ||
        (MULTI_ROBOT_PRIORITY(nRemoteObj, player_num) > MULTI_ROBOT_PRIORITY(nRemoteObj, N_LOCALPLAYER)) ||
        (RandShort() > 0x4400)) {
