...
int32_t MultiMessageFeedback(void) {
    ...
    if (IsTeamGame && (atoi(gameData.multigame.msg.szMsg) > 0) && (atoi(gameData.multigame.msg.szMsg) < 3)) {
        sprintf(
            szFeedbackResult + strlen(szFeedbackResult),
            "%s '%s'",
            TXT_TEAM,
            netGameInfo.m_info.szTeamName[atoi(gameData.multigame.msg.szMsg) - 1]);
        bFound = 1;
    }
    if (!bFound)
        if (IsTeamGame) {
    ...
    if (!bFound)
        for (i = 0; i < N_PLAYERS; i++) {
            if ((!strnicmp(PLAYER(i).callsign, gameData.multigame.msg.szMsg, l)) && (i != N_LOCALPLAYER) &&
                (PLAYER(i).connected)) {
                if (bFound)
                    strcat(szFeedbackResult, ", ");
                bFound++;
                if (!(bFound % 4))
                    strcat(szFeedbackResult, "\n");
                sprintf(szFeedbackResult + strlen(szFeedbackResult), "%s", PLAYER(i).callsign);
            }
        }
    if (!bFound)
...
void MultiDoStartTyping(uint8_t *buf) { gameStates.multi.bPlayerIsTyping[int32_t(buf[1])] = 1; }

//-----------------------------------------------------------------------------

void MultiDoQuitTyping(uint8_t *buf) { gameStates.multi.bPlayerIsTyping[int32_t(buf[1])] = 0; }
...
int32_t KickPlayer(int32_t bBan) {
    ...
    for (i = 0; i < N_PLAYERS; i++)
        if ((!strnicmp(
                PLAYER(i).callsign,
                &gameData.multigame.msg.szMsg[name_index],
                strlen(gameData.multigame.msg.szMsg) - name_index)) &&
...
int32_t HandicapPlayer(void) {
    ...
    if (gameStates.gameplay.xInitialShield[0] < 10) {
        ...
        sprintf(
            gameData.multigame.msg.szMsg,
            TXT_NEW_HANDICAP,
            LOCALPLAYER.callsign,
            gameStates.gameplay.xInitialShield[0]);
...
static int32_t IsMyPlayerId(char *pBuffer, int32_t nLen) { return strnicmp(LOCALPLAYER.callsign, pBuffer, nLen) == 0; }
