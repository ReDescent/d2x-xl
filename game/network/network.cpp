...
int32_t NetworkTimeoutPlayer(int32_t nPlayer, int32_t t) {
    ...
    if (PLAYER(nPlayer).TimedOut()) {
        if (t - PLAYER(nPlayer).m_tDisconnect >
            extraGameInfo[0].timeout.nKickPlayer) { // drop player when he disconnected for 3 minutes
...
void NetworkDoFrame(int bFlush) {
    ...
    if ((networkData.nStatus == NETSTAT_PLAYING) &&
        !gameStates.app.bEndLevelSequence) { // Don't send position during escape sequence...
        mineSyncData.Flush();
...
#if DBG
            || ((networkData.xLastSendTime >= I2X(1) / MAX_PPS) &&
...
void NetworkPing(uint8_t flag, uint8_t nPlayer) {
    if (gameStates.multi.nGameType >= IPX_GAME) {
        ...
        networkThread.Send(
            reinterpret_cast<uint8_t *>(mybuf),
            (gameStates.multi.nGameType == UDP_GAME) ? 3 : 2,
...
void NetworkHandlePingReturn(uint8_t nPlayer) {
    ...
    if (pingStats[nPlayer].launchTime > 0) { // negative value suppresses display of returned ping on HUD
        ...
        pingStats[nPlayer].ping =
            SDL_GetTicks() -
            pingStats[nPlayer]
                .launchTime; // X2I (FixMul (xPingReturnTime - pingStats [nPlayer].launchTime, I2X (1000)));
...
void NetworkSendPing(uint8_t nPlayer) { NetworkPing(PID_PING_SEND, nPlayer); }
...
int32_t GetMyNetRanking(void) {
    ...
    int32_t eff = (int32_t)(
        (double)((double)networkData.nNetLifeKills / ((double)networkData.nNetLifeKilled + (double)networkData.nNetLifeKills)) *
        100.0);
    if (rank > 8)
        rank = 8;
    if (eff < 0)
...
void NetworkCheckForOldVersion(uint8_t nPlayer) {
    if ((NETPLAYER((int32_t)nPlayer).versionMajor == 1) && !(NETPLAYER((int32_t)nPlayer).versionMinor & 0x0F))
