                ...
                for (uint8_t nPlayer = 0; nPlayer < N_PLAYERS; nPlayer++)
                    if (m_players & (1 << nPlayer))
                        IpxSendPlayerPacket(
                            nPlayer,
                            m_message,
                            MultiMsgLen(m_message[1]) + 1); // regard the additional PID
            }
        }
    } else {
        m_id = 0;
...
//------------------------------------------------------------------------------
// importantMessages [0] are the messages that have been sent from this client and are waiting for their reception to be
// confirmed importantMessages [1] are the messages that have been received by this client and need to be confirmed, and
// should not be processed again if resent by their originator
