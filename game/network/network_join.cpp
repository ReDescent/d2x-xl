...
void NetworkResetSyncStates(void) { networkData.nJoining = 0; }

//------------------------------------------------------------------------------

void NetworkResetObjSync(int16_t nObject) {
    for (int16_t i = 0; i < networkData.nJoining; i++)
        if ((networkData.syncInfo[i].nState == 1) &&
            ((nObject < 0) || NetworkObjnumIsPast(nObject, networkData.syncInfo + i)))
...
static int32_t FindNetworkPlayer(tPlayerSyncData *player, uint8_t *newAddress) {
    ...
    if (gameStates.multi.nGameType == UDP_GAME) {
        ...
        for (i = 0; i < N_PLAYERS; i++) {
            if (!memcmp(PLAYER(i).netAddress, anyAddress, 6) && !stricmp(PLAYER(i).callsign, player->player.callsign)) {
                memcpy(PLAYER(i).netAddress, newAddress, 6);
...
static int32_t FindPlayerSlot(tPlayerSyncData *player) {
    if (netGameInfo.m_info.gameFlags & NETGAME_FLAG_CLOSED) {
        ...
        if (gameStates.multi.nGameType >= IPX_GAME)
            NetworkDumpPlayer(player->player.network.Network(), player->player.network.Node(), DUMP_CLOSED);
        return -1;
    }
    if ((N_PLAYERS < gameData.multiplayer.nMaxPlayers) && (N_PLAYERS < gameData.multiplayer.nPlayerPositions)) {
    ...
    if (oldestPlayer == -1) {
        ...
        if (gameStates.multi.nGameType >= IPX_GAME)
            NetworkDumpPlayer(player->player.network.Network(), player->player.network.Node(), DUMP_FULL);
        return -1;
    }
    networkData.bPlayerAdded = 1;
    return oldestPlayer;
...
void DeleteSyncData(int16_t nConnection) {
    if (nConnection < --networkData.nJoining)
        memcpy(
            networkData.syncInfo + nConnection,
            networkData.syncInfo + networkData.nJoining,
            sizeof(tNetworkSyncInfo));
...
void NetworkWelcomePlayer(tPlayerSyncData *player) {
    ...
    if (0 > (nPlayer = FindNetworkPlayer(player, newAddress))) {
        ...
        if (gameOpts->multi.bNoRankings)
            ...
            HUDInitMessage("%s'%s' %s", pszRankStrings[NETPLAYER(nPlayer).rank], PLAYER(nPlayer).callsign, TXT_REJOIN);
    }
    ...
    pSyncInfo->player[0] = pSyncInfo->player[1] = *player;
    ...
    if (!networkData.refuse.bWaitForAnswer) {
        ...
        if (IsTeamGame) {
            if (gameOpts->multi.bNoRankings)
                ...
                HUDInitMessage("%s %s wants to join", pszRankStrings[their->player.rank], their->player.callsign);
            HUDInitMessage(
                "Alt-1 assigns to team %s. Alt-2 to team %s",
                netGameInfo.m_info.szTeamName[0],
        ...
        gameData.messages[1].index = gameData.messages[1].currentMsg = &joinMsgIndex;
        ...
        if ((TimerGetApproxSeconds()) > networkData.refuse.xTimeLimit + REFUSE_INTERVAL) {
            ...
            if (!strcmp(their->player.callsign, networkData.refuse.szPlayer)) {
                if (gameStates.multi.nGameType >= IPX_GAME)
                    NetworkDumpPlayer(their->player.network.Network(), their->player.network.Node(), DUMP_DORK);
            }
            return;
        }
    }
...
void CMultiplayerData::Connect(int32_t nPlayer, int8_t nStatus) {
    ...
    if (((nDbgPlayer != -1) || (nDbgStatus != -1)) && ((nDbgPlayer < 0) || (nPlayer == nDbgPlayer)) &&
        ((nDbgStatus < 0) || (nStatus == nDbgStatus)))
        BRP;
