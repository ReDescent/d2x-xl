...
void InitAddressFilter(void) {
    ...
    addressFilter[PID_LITE_INFO] = addressFilter[PID_ADDPLAYER] = addressFilter[PID_ENDLEVEL] =
        addressFilter[PID_ENDLEVEL_SHORT] = addressFilter[PID_GAME_INFO] = addressFilter[PID_MINE_DATA] =
            addressFilter[PID_NAMES_RETURN] = addressFilter[PID_OBJECT_DATA] = addressFilter[PID_PLAYER_DATA] =
                addressFilter[PID_PLAYERSINFO] = addressFilter[PID_EXTRA_GAMEINFO] = addressFilter[PID_DOWNLOAD] =
                    addressFilter[PID_UPLOAD] = addressFilter[PID_XML_GAMEINFO] = addressFilter[PID_RESEND_MESSAGE] =
                        addressFilter[PID_CONFIRM_MESSAGE] = addressFilter[(int32_t)PID_TRACKER_ADD_SERVER] =
...
int32_t IgnoreDataHandler(uint8_t *data, int32_t nLength) { return 1; }

//------------------------------------------------------------------------------

int32_t GameInfoHandler(uint8_t *data, int32_t nLength) { return 1; }
    ...
    if (data && (networkData.xmlGameInfoRequestTime <= 0)) {
        if (to.Expired() && !strcmp((char *)data + 1, "Descent Game Info Request")) {
            ...
            for (int32_t i = 0; i < N_PLAYERS; i++) {
                if (i == N_LOCALPLAYER)
                    ...
                    pingStats[i].ping = -1;
                    pingStats[i].launchTime =
                        -networkData
                             .xmlGameInfoRequestTime; // negative value suppresses display of returned ping on HUD
                    NetworkSendPing(i);
                }
            }
        }
        ...
        for (i = 0; i < N_PLAYERS; i++)
            if (pingStats[i].ping < 0)
                break;
        // send XML game status when all players have returned a ping response or 1 second has passed since the XML game
        // status request has arrived
...
int32_t PingReturnHandler(uint8_t *data, int32_t nLength) {
    // CONNECT ((int32_t) data [1], (gameStates.multi.nGameType == UDP_GAME) ? data [2] : CONNECT_PLAYING);
    NetworkHandlePingReturn(
        data[1]); // data [1] is CPlayerData who told us of reinterpret_cast<tPlayerSyncData*>(data) ping time
    ResetPlayerTimeout((int32_t)data[1], -1);
    return 1;
}
...
int32_t GamePlayersHandler(uint8_t *data, int32_t nLength) {
    if (IAmGameHost() && !NetworkBadSecurity(reinterpret_cast<tPlayerSyncData *>(data)->nSecurity, "PID_GAME_PLAYERS"))
...
void InitPacketHandlers(void) {
    ...
    PHINIT(
        PID_SEND_ALL_GAMEINFO,
        AllGameInfoHandler,
        SEQUENCE_PACKET_SIZE,
        (1 << NETSTAT_PLAYING) | (1 << NETSTAT_STARTING) | (1 << NETSTAT_ENDLEVEL));
    PHINIT(
        PID_PLAYERSINFO,
        PlayersInfoHandler,
        ALLNETPLAYERSINFO_SIZE,
        (1 << NETSTAT_WAITING) | (1 << NETSTAT_PLAYING));
    PHINIT(
        PID_REQUEST,
        RequestHandler,
        SEQUENCE_PACKET_SIZE,
        (1 << NETSTAT_STARTING) | (1 << NETSTAT_PLAYING) | (1 << NETSTAT_WAITING));
    PHINIT(PID_SYNC, SyncHandler, 0, (1 << NETSTAT_PLAYING) | (1 << NETSTAT_WAITING));
    PHINIT(
        PID_PLAYER_DATA,
        PlayerDataHandler,
        0,
        (1 << NETSTAT_PLAYING) | (1 << NETSTAT_WAITING) | (1 << NETSTAT_ENDLEVEL));
    ...
    PHINIT(
        ...
        QuitJoiningHandler,
        SEQUENCE_PACKET_SIZE,
        (1 << NETSTAT_STARTING) | (1 << NETSTAT_PLAYING));
...
int32_t NetworkProcessSinglePacket(uint8_t *data, int32_t nLength) {
    ...
    if (gameStates.multi.nGameType >= IPX_GAME) {
        ...
        data = reinterpret_cast<uint8_t *>(
            &tmpPacket); // reassign reinterpret_cast<tPlayerSyncData*>(data) to point to correctly alinged structure
    }
#endif
