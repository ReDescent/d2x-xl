...
void NetworkProcessMonitorVector(int32_t vector) {
    ...
    for (i = 0; i <= gameData.segData.nLastSegment; i++, pSeg++) {
        for (j = 0, pSide = pSeg->m_sides; j < SEGMENT_SIDE_COUNT; j++, pSide++) {
            if (!pSide->FaceCount())
                continue;
            if (((tm = pSide->m_nOvlTex) != 0) && ((ec = gameData.pigData.tex.pTexMapInfo[tm].nEffectClip) != -1) &&
                ((bm = gameData.effectData.pEffect[ec].destroyed.nTexture) != -1)) {
                if (vector & (1 << count))
                    pSide->m_nOvlTex = bm;
                count++;
                // Assert (count < 32);
...
void NetworkProcessLiteInfo(uint8_t *data) {
    ...
    networkData.bGamesChanged = 1;
    i = FindActiveNetGame(
        reinterpret_cast<tNetGameInfo *>(newInfo)->szGameName,
        reinterpret_cast<tNetGameInfo *>(newInfo)->nSecurity);
    if (i == MAX_ACTIVE_NETGAMES)
        return;
    if (i == networkData.nActiveGames) {
...
void NetworkProcessRequest(tPlayerSyncData *their) {
    ...
    for (int32_t nPlayer = 0; nPlayer < N_PLAYERS; nPlayer++) {
        if (!CmpNetPlayers(
                ...
                NETPLAYER(nPlayer).callsign,
                &their->player.network,
                &NETPLAYER(nPlayer).network)) {
