...
void NetworkProcessSyncPacket(CNetGameInfo *pNetGameInfo, int32_t rsinit) {
    ...
    if ((gameStates.multi.nGameType >= IPX_GAME) &&
        (pNetGameInfo != &netGameInfo)) { // for macintosh -- get the values unpacked to our structure format
...
#define GetShortFrameInfo(old_info, new_info) memcpy(new_info, old_info, sizeof(tFrameInfoShort))
...
void CObjectSynchronizer::Abort(void) { NetworkAbortSync(); }

//------------------------------------------------------------------------------

int32_t NetworkRequestSync(void);

void CObjectSynchronizer::RequestResync(void) { NetworkRequestSync(); }
...
int32_t CObjectSynchronizer::Sync(void) {
    ...
    if (((m_nObjOwner == N_LOCALPLAYER) || (m_nObjOwner == -1)) &&
        (m_nLocalObj != m_nRemoteObj)) { // since the object allocator tries its best to do so, ignore the sync state
                                         // here if that requirement could be met
        m_nState = 0;
    ...
    if (nCompressedSize == sizeof(tBaseObject)) {
        ...
        if ((i = uncompress(
                 uncompressedBuffer,
                 &nUncompressedSize,
                 (uint8_t *)m_data + m_bufI,
                 (uLong)nCompressedSize)) != Z_OK) {
