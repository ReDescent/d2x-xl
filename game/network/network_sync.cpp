...
static inline bool SendObject(int32_t nMode, int32_t nLocalObj, int32_t nPlayer) {
    if (NetworkFilterObject(OBJECT(nLocalObj)))
        return false;
    return nMode ? (gameData.multigame.nObjOwner[nLocalObj] == -1) ||
                       (gameData.multigame.nObjOwner[nLocalObj] ==
                        nPlayer) // send objects owned by the local player or by nobody
                 : (gameData.multigame.nObjOwner[nLocalObj] != -1) &&
                       (gameData.multigame.nObjOwner[nLocalObj] != nPlayer); // send objects owned by other players
}
...
void NetworkSyncObjects(tNetworkSyncInfo *pSyncInfo) {
    ...
    for (nLocalObj = pSyncInfo->objs.nCurrent; nLocalObj <= gameData.objData.nLastObject[0]; nLocalObj++) {
        if (SendObject(pSyncInfo->objs.nMode, nLocalObj, nPlayer)) {
            ...
            if ((compress(
                     compressedBuffer,
                     &nCompressedSize,
                     (uint8_t *)&OBJECT(nLocalObj)->info,
                     sizeof(tBaseObject)) == Z_OK) &&
                (nCompressedSize < sizeof(tBaseObject))) {
    ...
    if (nObjFrames) { // Send any objects we've buffered
        ...
        // if (NetworkObjFrameFilter (pSyncInfo)) { // this statement skips any objects successfully sync'd in case the
        // client has reported missing frames
        if (++pSyncInfo->objs.nFrame > pSyncInfo->objs.nFramesToSkip) {
            objBuf[1] = nObjFrames;
...
void NetworkSyncPlayer(tNetworkSyncInfo *pSyncInfo) {
    ...
    if (gameStates.multi.nGameType >= IPX_GAME)
        networkThread.Send(
            objBuf,
            (gameStates.multi.nGameType == UDP_GAME) ? 9 : 8,
            pSyncInfo->player[1].player.network.Network(),
            pSyncInfo->player[1].player.network.Node());
...
#if SECURITY_CHECK
                if ((pPlayerInfo->m_info.nSecurity == tempNetInfo.m_info.nSecurity) &&
                    (pPlayerInfo->m_info.nSecurity == networkData.nSecurityCheck))
#endif
                {
...
void NetworkSendLifeSign(void) { networkThread.SendLifeSign(true); }
