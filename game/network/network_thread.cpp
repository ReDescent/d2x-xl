...
bool CNetworkPacket::HasId(void) {
    ...
    if (tracker.IsTracker(
            address.m_address.node.portAddress.ip.a,
            address.m_address.node.portAddress.port.p,
            reinterpret_cast<char *>(m_data.buffer)))
        return false;
...
CNetworkPacketQueue::CNetworkPacketQueue() { Create(); }

//------------------------------------------------------------------------------

CNetworkPacketQueue::~CNetworkPacketQueue() { Destroy(); }
...
void CNetworkPacketQueue::Update(void) { m_clients.Cleanup(); }
...
void CNetworkClientList::Destroy(void) { CStack<CNetworkClientInfo>::Destroy(); }
...
void CNetworkThread::Start(void) {
    ...
    if (!m_thread) {
        ...
        m_bUrgent = false;
        m_bActive = true;
        m_thread = SDL_CreateThread(NetworkThreadHandler, "Network", &m_nThreadId);
        while (!Running())
            G3_SLEEP(0);
    }
#endif
...
int32_t CNetworkThread::ConnectionStatus(int32_t nPlayer) {
    ...
    if (PLAYER(nPlayer).m_nLevel && (PLAYER(nPlayer).m_nLevel != missionManager.nCurrentLevel))
        return 4; // the client being tested is in a different level, so immediately disconnect him to make his ship
                  // disappear until he enters the current level
    ...
    if (LOCALPLAYER.connected == CONNECT_PLAYING)
        return 3; // the client being tested is in some level transition mode, so immediately disconnect him to make his
                  // ship disappear until he enters the current level
    ...
    if (to.Expired() /*&& !gameData.reactorData.bDestroyed*/) {
        ...
        for (int32_t nPlayer = 0; nPlayer < N_PLAYERS; nPlayer++) {
            if (nPlayer != N_LOCALPLAYER) {
                ...
                case 2:
                    if ((networkData.nLastPacketTime[nPlayer] == 0) ||
                        (t - networkData.nLastPacketTime[nPlayer] < extraGameInfo[0].timeout.nDisconnectPlayer)) {
                        ResetPlayerTimeout(nPlayer, t);
                        break;
                    }
...
bool CNetworkThread::SendInBackground(void) { return (SEND_IN_BACKGROUND != 0) && m_bActive && m_bSend && Available(); }
