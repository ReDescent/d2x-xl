...
int32_t CTracker::Find(tUdpAddress *addr) {
    ...
    for (i = 0; i < trackerList.nServers; i++)
        if ((a == UDP_ADDR(trackerList.servers + i)) && (p == UDP_PORT(trackerList.servers + i)))
            return i;
    return -1;
}
...
void CTracker::Call(int32_t i, uint8_t *pData, int32_t nDataLen) {
    ...
    networkThread
        .Send(pData, nDataLen, reinterpret_cast<uint8_t *>(&network), NULL, reinterpret_cast<uint8_t *>(&tracker), 1);
}
...
#endif
        if ((addr == UDP_ADDR(trackerList.servers + i)) && (port == UDP_PORT(trackerList.servers + i)))
            return 1;
    }
    return 0;
}
