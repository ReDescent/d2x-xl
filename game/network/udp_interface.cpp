...
static int32_t HaveEmptyAddress(void) { return ipx_MyAddress.IsEmpty(); }
...
int32_t CClientManager::Find(struct sockaddr_in *destAddr) {
    for (uint32_t i = 0; i < m_nClients; i++)
        if ((i < m_nMasks) ? !CmpAddrsMasked(destAddr, &m_clients[i].addr, m_masks + i)
...
int32_t CClientManager::Add(struct sockaddr_in *destAddr) {
    ...
    for (i = 0; i < m_nClients; i++) {
        h = (i < m_nMasks) ? CmpAddrsMasked(destAddr, &m_clients[i].addr, m_masks + i)
                           : CmpAddrs(destAddr, &m_clients[i].addr);
        if (!h)
            break;
    }
...
int32_t CClientManager::BuildInterfaceList(void) {
    ...
    for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if ((ifa->ifa_flags & IF_NOTFLAGS) ||
            !((ifa->ifa_flags & IF_REQFLAGS) && (ifa->ifa_addr->sa_family == AF_INET)))
            continue;
...
int32_t CClientManager::BuildInterfaceList(void) {
    ...
    for (i = j = 0; i < cnt; i++) {
        ...
        if (!_IOCTL(
                sock,
                (ifconf.ifc_req[i].ifr_flags & IFF_BROADCAST) ? SIOCGIFBRDADDR : SIOCGIFDSTADDR,
                ifconf.ifc_req + i)) {
            close(sock);
...
int32_t ReportSafeMode(CClient *pClient) {
    ...
    return sendto(
        ...
        SAFEMODE_ID_LEN,
        0,
        reinterpret_cast<struct sockaddr *>(&pClient->addr),
        sizeof(pClient->addr));
}
...
static int32_t UDPReceivePacket(ipx_socket_t *s, uint8_t *outBuf, int32_t outBufSize, CPacketAddress *rd) {
    ...
    if (!(bTracker
        ...
        networkData.localAddress.SetNode(
            outBuf + dataLen - 6); // this is the local port the sender of this packet sent the packet to
