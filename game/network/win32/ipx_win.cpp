...
static int32_t ipx_win_OpenSocket(ipx_socket_t *sk, int32_t port) {
    ...
    */
    ...
    //  sock = socket(AF_IPX, SOCK_DGRAM, PF_IPX);
    //  sock = socket(AF_IPX, SOCK_DGRAM, 0);
    sock = (int32_t)socket(AF_IPX, SOCK_DGRAM, NSPROTO_IPX); // why NSPROTO_IPX?  I looked in the quake source and thats
                                                             // what they used. :) -MPM  (on w2k 0 and PF_IPX don't work)
    if (sock == -1) {
...
static int32_t ipx_win_SendPacket(ipx_socket_t *mysock, IPXPacket_t *IPXHeader, uint8_t *data, int32_t dataLen) {
    ...
    return sendto(
        ...
        dataLen,
        0,
        reinterpret_cast<struct sockaddr *>(&ipxs),
        sizeof(ipxs));
}
...
static int32_t ipx_win_ReceivePacket(ipx_socket_t *s, uint8_t *buffer, int32_t bufsize, CPacketAddress *rd) {
    ...
    int32_t size =
        recvfrom(s->fd, reinterpret_cast<char *>(buffer), bufsize, 0, reinterpret_cast<struct sockaddr *>(&ipxs), &sz);
    if (size <= 0)
        return size;
