...
uint8_t *IpxGetMyServerAddress() { return reinterpret_cast<uint8_t *>(&ipx_network); }
    ...
    if (gameStates.multi.nGameType > IPX_GAME)
        ...
        for (i = 0; i < nIpxNetworks; i++) {
            if (memcmp(ipxNetworks + i, &ipx_network, 4)) {
                ...
                networkThread
                    .Send(data, dataSize, reinterpret_cast<uint8_t *>(ipxNetworks + i), broadcast, localAddress);
            } else {
