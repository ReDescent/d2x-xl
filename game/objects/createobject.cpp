...
fix CObject::LifeTime(void) { return gameData.timeData.xGame - m_xCreationTime; }
...
void CObject::Initialize(
    ...
    SetContainsType(-1);
    SetLifeLeft(
        (IsEntropyGame && (nType == OBJ_POWERUP) && (nId == POW_HOARD_ORB) &&
         (extraGameInfo[1].entropy.nVirusLifespan > 0))
            ? I2X(extraGameInfo[1].entropy.nVirusLifespan)
            : IMMORTAL_TIME);
    SetAttachedObj(-1);
    SetShield(I2X(20));
...
int32_t CObject::Create(
    ...
    if (nType == OBJ_DEBRIS) {
        if (!gameOpts->render.effects.nDebris &&
            (gameData.objData.nDebris >= gameStates.render.detail.nMaxDebrisObjects))
            RETVAL(-1)
    }
...
int32_t CreateObject(
    ...
    pObj->info.nId = nId;
    pObj->info.vLastPos = pObj->info.position.vPos = vPos;
    pObj->SetOrigin(vPos);
    pObj->SetSize(xSize);
...
int32_t CloneObject(CObject *pObj) {
    ...
    pClone->info.nSegment = pClone->info.nPrevInSeg = pClone->info.nNextInSeg = -1;
#if OBJ_LIST_TYPE == 1
    pClone->InitLinks();
...
int32_t CreateRobot(uint8_t nId, int16_t nSegment, const CFixVector &vPos) {
    ...
    RETVAL(CreateObject(
        ...
        vPos,
        CFixMatrix::IDENTITY,
        gameData.modelData.polyModels[0][pRobotInfo->nModel].Rad(),
        CT_AI,
        MT_PHYSICS,
        RT_POLYOBJ))
}
...
void SetAllowedPowerup(int32_t nPowerup, uint32_t nCount) {
    ...
    if (nCount && powerupFilter[nPowerup]) {
        ...
        if (MultiPowerupIs4Pack(nPowerup))
            ...
            gameData.multiplayer.maxPowerupsAllowed[nPowerup + 1] =
            ...
            if ((nPowerup == POW_VULCAN) || (nPowerup == POW_GAUSS))
                gameData.multiplayer.maxPowerupsAllowed[POW_VULCAN_AMMO] =
                    2 * nCount; // add vulcan ammo for each gatling type gun
        }
    }
    RETURN
}
    ...
    if (nCount && powerupFilter[nPowerup]) {
        ...
        if (MultiPowerupIs4Pack(nPowerup))
            ...
            if (MultiPowerupIs4Pack(nPowerup + 1))
                gameData.multiplayer.maxPowerupsAllowed[nPowerup + 1] =
                    gameData.multiplayer.maxPowerupsAllowed[nPowerup] / 4;
        }
    }
    RETURN
}
...
int32_t CreatePowerup(
    ...
    if (!bIgnoreLimits && TooManyPowerups((int32_t)nId)) {
        PrintLog(
            ...
            nId,
            gameData.multiplayer.powerupsInMine[nId],
            PowerupsOnShips(nId),
    ...
    int16_t nObject = CreateObject(
        ...
        vPos,
        CFixMatrix::IDENTITY,
        gameData.objData.pwrUp.info[nId].size,
        CT_POWERUP,
        MT_PHYSICS,
        RT_POWERUP);
    if (nObject >= 0) {
...
int32_t
CreateWeapon(uint8_t nId, int16_t nCreator, int16_t nSegment, const CFixVector &vPos, fix xSize, uint8_t rType) {
    ENTER(0, 0);
    if (rType == 255) {
...
int32_t CreateFireball(uint8_t nId, int16_t nSegment, const CFixVector &vPos, fix xSize, uint8_t rType) {
    ENTER(0, 0);
    RETVAL(
        CreateObject(OBJ_FIREBALL, nId, -1, nSegment, vPos, CFixMatrix::IDENTITY, xSize, CT_EXPLOSION, MT_NONE, rType))
}
...
int32_t CreateDebris(CObject *pParent, int16_t nSubModel) {
    ...
    RETVAL(CreateObject(
        ...
        0,
        -1,
        pParent->info.nSegment,
        pParent->info.position.vPos,
        pParent->info.position.mOrient,
...
int32_t DropMarkerObject(CFixVector &vPos, int16_t nSegment, CFixMatrix &orient, uint8_t nMarker) {
    ...
    nObject = CreateObject(
        ...
        vPos,
        orient,
        gameData.modelData.polyModels[0][gameData.modelData.nMarkerModel].Rad(),
        CT_NONE,
        MT_NONE,
        RT_POLYOBJ);
