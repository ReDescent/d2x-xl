...
CFixVector CObject::RegisterHit(CFixVector vHit, int16_t nModel) {
    ...
    if (EGI_FLAG(nDamageModel, 0, 0, 0) &&
        (gameStates.app.nSDLTicks[0] > m_damage.tCritical)) { // check and handle critical hits
        ...
        if ((m_damage.bCritical = RandShort() < F2X(fDamage))) {
            if (!EGI_FLAG(nHitboxes, 0, 0, extraGameInfo[0].nHitboxes))
                nModel =
                    RandShort() > 3 * I2X(1) / 8; // 75% chance for a torso hit with sphere based collision handling
#if DBG
            if (nModel < 2)
