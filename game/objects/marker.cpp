...
CMarkerData::CMarkerData() { Init(); }

// ----------------------------------------------------------------------------

void CMarkerData::Init(void) {
    nHighlight = -1;
    viewers[0] = viewers[1] = -1;
    fScale = 2.0f;
    CLEAR(szMessage);
    CLEAR(szOwner);
    CLEAR(viewers);
...
void CMarkerManager::DrawNumber(int32_t nMarker) {
    ...
    if (ogl.SizeVertexBuffer(nPoints[nMarker])) {
        ...
        glPushMatrix();
        glTranslatef(
            X2F(basePoint.ViewPos().v.coord.x),
            X2F(basePoint.ViewPos().v.coord.y),
            X2F(basePoint.ViewPos().v.coord.z));
...
void CMarkerManager::Drop(char nPlayerMarker, int32_t bSpawn) {
    ...
    if (!(bSpawn && markerManager.MoveSpawnPoint(&pPlayer->info.position, pPlayer->info.nSegment))) {
        ...
        if ((nObject >= 0) && (nObject <= gameData.objData.nLastObject[0]) &&
            (OBJECT(nObject)->info.nType == OBJ_MARKER))
            ReleaseObject(nObject);
        if (bSpawn)
            strcpy(m_data.szMessage[nMarker], "SPAWN");
        m_data.objects[nMarker] = DropMarkerObject(
            pPlayer->info.position.vPos,
            (int16_t)pPlayer->info.nSegment,
            pPlayer->info.position.mOrient,
            nMarker);
        if (IsMultiGame)
...
void CMarkerManager::DropForGuidebot(CObject *pObj) {
    ...
    m_data.objects[nMarker] =
        DropMarkerObject(pObj->info.position.vPos, (int16_t)pObj->info.nSegment, pObj->info.position.mOrient, nMarker);
}
    ...
    if ((m_data.nHighlight > -1) && (m_data.objects[m_data.nHighlight] != -1)) {
        ...
        if (bForce || !InfoBox(NULL, NULL, BG_STANDARD, 2, TXT_YES, TXT_NO, TXT_DELETE_MARKER)) {
            ...
            if (i == m_data.nHighlight) {
                ...
                memcpy(
                    m_data.objects + m_data.nHighlight,
                    m_data.objects + m_data.nHighlight + 1,
                    h * sizeof(m_data.objects[0]));
                memcpy(
                    m_data.szMessage[m_data.nHighlight],
