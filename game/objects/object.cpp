...
void CObject::InitTables(void) {
    ...
    m_bIsEquipment[POW_EXTRA_LIFE] = m_bIsEquipment[POW_KEY_BLUE] = m_bIsEquipment[POW_KEY_RED] =
        m_bIsEquipment[POW_KEY_GOLD] = m_bIsEquipment[POW_FULL_MAP] = m_bIsEquipment[POW_BLUEFLAG] =
            m_bIsEquipment[POW_REDFLAG] = m_bIsEquipment[POW_CLOAK] = m_bIsEquipment[POW_INVUL] =
                m_bIsEquipment[POW_HOARD_ORB] = 2;
    m_bIsEquipment[POW_QUADLASER] = m_bIsEquipment[POW_CONVERTER] = m_bIsEquipment[POW_AMMORACK] =
        m_bIsEquipment[POW_AFTERBURNER] = m_bIsEquipment[POW_HEADLIGHT] = m_bIsEquipment[POW_SLOWMOTION] =
...
char szObjectTypeNames[MAX_OBJECT_TYPES][10] = {
    "WALL     ", "FIREBALL ", "ROBOT    ", "HOSTAGE  ", "PLAYER   ", "WEAPON   ", "CAMERA   ",
    "POWERUP  ", "DEBRIS   ", "CNTRLCEN ", "FLARE    ", "CLUTTER  ", "GHOST    ", "LIGHT    ",
    "COOP     ", "MARKER   ", "CAMBOT   ", "M-BALL   ", "SMOKE    ", "EXPLOSION", "EFFECT   "};
#endif
...
void InitMultiPlayerObject(int32_t nStage) {
    if (nStage == 0) {
        ...
        LOCALPLAYER.nInvuls = LOCALPLAYER.nCloaks = 0;
    } else {
...
void CObject::Init(void) {
    info.nType = OBJ_NONE;
    cType.explInfo.attached.nNext = cType.explInfo.attached.nPrev = cType.explInfo.attached.nParent =
        info.nAttachedObj = -1;
    info.nFlags = 0;
    ResetSgmLinks();
#if OBJ_LIST_TYPE == 1
    ...
    m_xCreationTime = m_xTimeLastHit = 0;
    m_xTimeLastEffect = 0;
    m_vStartVel.SetZero();
}
...
int32_t AllocObject(int32_t nRequestedObject, bool bReset) {
    ...
    pObj->info.nAttachedObj = pObj->cType.explInfo.attached.nNext = pObj->cType.explInfo.attached.nPrev =
...
void ReleaseObject(int16_t nObject) {
    ...
    pObj->cType.explInfo.attached.nParent = pObj->cType.explInfo.attached.nPrev = pObj->cType.explInfo.attached.nNext =
        -1;
...
int32_t FreeDebrisFilter(CObject *pObj) { return pObj->info.nType == OBJ_DEBRIS; }

int32_t FreeFireballFilter(CObject *pObj) {
    return (pObj->info.nType == OBJ_FIREBALL) && (pObj->cType.explInfo.nDestroyedObj == -1);
}

int32_t FreeFlareFilter(CObject *pObj) { return (pObj->info.nType == OBJ_WEAPON) && (pObj->info.nId == FLARE_ID); }

int32_t FreeWeaponFilter(CObject *pObj) { return (pObj->info.nType == OBJ_WEAPON); }
...
void ResetObjects(int32_t nObjects) {
    ...
    for (; i < LEVEL_OBJECTS; i++, pObj++) {
        if ((gameData.demoData.nState == ND_STATE_PLAYBACK) &&
            ((pObj->info.nType == OBJ_CAMBOT) || (pObj->info.nType == OBJ_EFFECT)))
            continue;
        gameData.objData.freeList[i] = i;
        pObj->info.nType = OBJ_NONE;
        pObj->info.nSegment = pObj->cType.explInfo.attached.nNext = pObj->cType.explInfo.attached.nPrev =
            pObj->cType.explInfo.attached.nParent = pObj->info.nAttachedObj = -1;
        pObj->rType.polyObjInfo.nSubObjFlags = 0;
        pObj->info.nFlags = 0;
    }
    gameData.objData.nLastObject[0] = gameData.objData.nObjects - 1;
...
int32_t CObject::FindSegment(void) { return FindSegByPos(info.position.vPos, info.nSegment, 1, 0); }
...
void ClearTransientObjects(int32_t bClearAll) {
    ...
    for (CWeaponIterator iter(pObj); pObj; pObj = (pPrevObj ? iter.Step() : iter.Start())) {
        ...
        if ((!(pWeaponInfo && (pWeaponInfo->flags & WIF_PLACABLE)) &&
             (bClearAll || ((pObj->info.nId != PROXMINE_ID) && (pObj->info.nId != SMARTMINE_ID)))) ||
            pObj->info.nType == OBJ_FIREBALL || pObj->info.nType == OBJ_DEBRIS ||
            ((pObj->info.nType != OBJ_NONE) && (pObj->info.nFlags & OF_EXPLODING))) {
            pPrevObj = iter.Back();
            ReleaseObject(pObj->Index());
        }
    }
...
void DetachFromParent(CObject *pChildObj) {
    ...
    pChildObj->cType.explInfo.attached.nNext = pChildObj->cType.explInfo.attached.nPrev =
...
void DetachChildObjects(CObject *pObj) {
    ...
    while (i != -1) {
        ...
        pChild->cType.explInfo.attached.nNext = pChild->cType.explInfo.attached.nPrev = -1;
...
int32_t DelObjChildrenP(CObject *pParent) { return DelObjChildrenN(OBJ_IDX(pParent)); }
...
int32_t DelObjChildP(CObject *pChild) { return DelObjChildN(OBJ_IDX(pChild)); }
...
tObjectRef *GetChildObjP(CObject *pParent, tObjectRef *pChildRef) { return GetChildObjN(OBJ_IDX(pParent), pChildRef); }
...
void CObject::CreateSound(int16_t nSound) { audio.CreateSegmentSound(nSound, info.nSegment, 0, info.position.vPos); }

//------------------------------------------------------------------------------

int32_t CObject::OpenableDoorsInSegment(void) { return SEGMENT(info.nSegment)->HasOpenableDoor(); }
...
int32_t CObject::Index(void) { return OBJ_IDX(this); }

//------------------------------------------------------------------------------

CObject *CObject::Target(void) { return m_target ? m_target : gameData.objData.pConsole; }
...
bool CObject::IsGuidedMissile(int8_t nPlayer) {
    ...
    return (this == gameData.objData.guidedMissile[nPlayer].pObj) &&
           (Signature() == gameData.objData.guidedMissile[nPlayer].nSignature);
}
...
fix CObject::MaxSpeed(void) {
    ...
    return ((info.nType == OBJ_ROBOT) && (pRobotInfo = ROBOTINFO(info.nId)))
               ? pRobotInfo->xMaxSpeed[gameStates.app.nDifficultyLevel]
               : 0;
}
...
bool CObject::IsBoss(void) { return BossId() != 0; }
...
bool CObject::IsPlayerMine(int16_t nId) { return (m_weaponInfo[nId] & OBJ_IS_PLAYER_MINE) != 0; }
...
bool CObject::IsGatlingRound(void) { return IsProjectile() && ((m_weaponInfo[Id()] & OBJ_IS_GATLING_ROUND) != 0); }
...
bool CObject::AttacksRobots(void) {
    return (IsRobot() || IsReactor()) && EGI_FLAG(bRobotsHitRobots, 0, 0, 0) &&
           ((gameStates.app.cheats.bRobotsKillRobots != 0) || Reprogrammed());
}
...
bool CObject::AttacksObject(CObject *pTarget) { return pTarget->IsPlayer() ? AttacksPlayer() : AttacksRobots(); }

//------------------------------------------------------------------------------

bool CObject::IsSplashDamageWeapon(void) { return ((m_weaponInfo[Id()] & OBJ_IS_SPLASHDMG_WEAPON) != 0); }

//------------------------------------------------------------------------------

bool CObject::IsRobotMine(int16_t nId) { return (m_weaponInfo[nId] & OBJ_IS_ROBOT_MINE) != 0; }

//------------------------------------------------------------------------------

bool CObject::IsRobotMine(void) { return IsProjectile() && IsRobotMine(Id()); }

//------------------------------------------------------------------------------

bool CObject::IsMine(int16_t nId) { return IsPlayerMine(nId) || IsRobotMine(nId); }

//------------------------------------------------------------------------------

bool CObject::IsMine(void) { return IsPlayerMine() || IsRobotMine(); }
...
fix CObject::PowerupSize(void) { return (fix)gameData.objData.pwrUp.info[info.nId].size; }
...
float CObject::AppearanceScale(void) {
    return Appearing()
               ? float(gameData.multiplayer.tAppearing[Id()][0]) / float(gameData.multiplayer.tAppearing[Id()][1])
               : 1.0f;
}
...
CObjectIterator::CObjectIterator(CObject *&pObj) : m_pObj(NULL), m_nLink(0) { pObj = Start(); }
...
CObject *CObjectIterator::Head(void) { return gameData.objData.lists.all.head; }

//------------------------------------------------------------------------------

bool CObjectIterator::Done(void) { return m_pObj == NULL; }

//------------------------------------------------------------------------------

CObject *CObjectIterator::Next(void) { return m_pObj ? m_pObj->Links(m_nLink).next : NULL; }
...
CObjectIterator::CObjectIterator() : m_pObj(NULL), m_i(0) {}

CObjectIterator::CObjectIterator(CObject *&pObj) : m_pObj(NULL), m_i(0) { pObj = Start(); }
...
bool CObjectIterator::Done(void) { return m_i >= gameData.objData.nObjects; }

//------------------------------------------------------------------------------

CObject *CObjectIterator::Current(void) { return m_pObj = OBJECT(gameData.objData.freeList[m_i]); }
...
bool CActorIterator::Match(void) {
    ...
    return (nType == OBJ_PLAYER) || (nType == OBJ_GHOST) || (nType == OBJ_ROBOT) || (nType == OBJ_REACTOR) ||
           (nType == OBJ_WEAPON) || (nType == OBJ_MONSTERBALL);
}
