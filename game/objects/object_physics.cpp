...
float CObject::ShieldScale(void) { return shipModifiers[gameData.multiplayer.weaponStates[info.nId].nShip].v.shield; }

//------------------------------------------------------------------------------

float CObject::EnergyScale(void) { return shipModifiers[gameData.multiplayer.weaponStates[info.nId].nShip].v.energy; }
...
void CObject::Wiggle(void) {
    ...
    if (gameData.timeData.xFrame <
        I2X(1)) // Only scale wiggle if getting at least 1 FPS, to avoid causing the opposite problem.
        xWiggle = FixMul(
            xWiggle * 20,
            gameData.timeData.xFrame); // make wiggle fps-independent (based on pre-scaled amount of wiggle at 20 FPS)
    if (SPECTATOR(this))
        OBJPOS(this)->vPos +=
            (OBJPOS(this)->mOrient.m.dir.u * FixMul(xWiggle, gameData.pigData.ship.player->wiggle)) * (I2X(1) / 20);
    else if ((info.nType == OBJ_PLAYER) || ((info.nId == N_LOCALPLAYER) && OBSERVING) || !pParent)
        mType.physInfo.velocity +=
            info.position.mOrient.m.dir.u * FixMul(xWiggle, gameData.pigData.ship.player->wiggle);
    else {
        mType.physInfo.velocity +=
            pParent->info.position.mOrient.m.dir.u * FixMul(xWiggle, gameData.pigData.ship.player->wiggle);
...
void CObject::ApplyFlightControls(void) {
    ...
    if (!pMissileObj)
        mType.physInfo.rotThrust =
            CFixVector::Create(controls[0].pitchTime, controls[0].headingTime, controls[0].bankTime);
    else {
    ...
    if ((LOCALPLAYER.flags & PLAYER_FLAGS_AFTERBURNER) && (RandShort() < OBJECT(N_LOCALPLAYER)->DriveDamage()) &&
        ((Index() != LOCALPLAYER.nObject) || !automap.Active())) {
        if (controls[0].afterburnerState) { // player has key down
            ...
            fix xChargeUp =
                Min(gameData.timeData.xFrame / 8,
                    I2X(1) - gameData.physicsData.xAfterburnerCharge); // recharge over 8 seconds
            if (xChargeUp > 0) {
...
float CObject::CollisionPoint(CFloatVector *vDir, CFloatVector *vHit) { return 0.0f; }
