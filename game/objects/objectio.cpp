...
void CObject::Read(CFile &cf) {
    ...
    default:
        PrintLog(
            0,
            "Object #%d (type=%d, id=%d), has invalid movement type %d",
            OBJ_IDX(this),
            info.nType,
            info.nId,
            info.movementType);
    ...
    default:
        PrintLog(
            0,
            "Object #%d (type=%d, id=%d), has invalid control type %d",
            OBJ_IDX(this),
            info.nType,
            info.nId,
            info.controlType);
    ...
    default:
        ...
        PrintLog(
            0,
            "Object #%d (type=%d, id=%d), has invalid render type %d",
            OBJ_IDX(this),
            info.nType,
            info.nId,
            info.renderType);
...
void CObject::LoadState(CFile &cf) {
    ...
    // PrintLog (0, "object sig=%d, type=%d, renderType=%d, pos = %d, %d\n", info.nSignature, info.nType,
    // info.renderType, fPos, cf.Tell ());
}
...
// int32_t fPos = cf.Tell ();
    ...
    case RT_LIGHTNING:
        cf.WriteInt(
            (rType.lightningInfo.nWayPoint < 0)
                ? -1
                : wayPointManager.WayPoint(rType.lightningInfo.nWayPoint)->WayPointId());
