...
tShipModifier shipModifiers[MAX_SHIP_TYPES] = {{{1.0f, 1.0f, 1.0f}}, {{0.9f, 0.9f, 1.1f}}, {{1.5f, 1.2f, 0.9f}}};
...
int32_t EquippedPlayerGun(CObject *pObj) {
    if (pObj->info.nType == OBJ_PLAYER) {
        ...
        return (nWeapon || (gameData.multiplayer.weaponStates[nPlayer].nLaserLevel <= MAX_LASER_LEVEL))
                   ? nWeapon
                   : SUPER_LASER_INDEX;
    }
    return 0;
        ...
        if (!IsMultiGame || PLAYER(i).IsConnected()) {
            ...
            if (bGatling && bGatlingSound && (pFiringData->bSound == 1)) {
                audio.CreateObjectSound(
                    -1,
                    SOUNDCLASS_PLAYER,
                    (int16_t)PLAYER(i).nObject,
                    0,
                    I2X(1),
                    I2X(256),
                    -1,
                    -1,
                    AddonSoundName(SND_ADDON_GATLING_SPIN),
                    0);
...
void UpdateFiringState(void) {
    ...
    if ((controls[0].firePrimaryState != 0) || (controls[0].firePrimaryDownCount != 0)) {
        ...
        gameData.weaponData.firing[0].nDuration = gameData.weaponData.firing[0].nStart = 0;
    } else if (gameData.weaponData.firing[0].nStop > 0) {
        if (gameStates.app.nSDLTicks[0] - gameData.weaponData.firing[0].nStop >=
            GATLING_DELAY /*WIFireTicks (gameData.weaponData.nPrimary) * 4 / 5*/) {
            gameData.weaponData.firing[0].nStop = 0;
        }
    }
    if ((controls[0].fireSecondaryState != 0) || (controls[0].fireSecondaryDownCount != 0)) {
        ...
        gameData.weaponData.firing[1].nDuration = gameData.weaponData.firing[1].nStart = 0;
    }
}
...
void UpdatePlayerWeaponInfo(void) {
    ...
    if (gameStates.app.bPlayerIsDead)
        gameData.weaponData.firing[0].nStart = gameData.weaponData.firing[0].nDuration =
            gameData.weaponData.firing[0].nStop = gameData.weaponData.firing[1].nStart =
                gameData.weaponData.firing[1].nDuration = gameData.weaponData.firing[1].nStop = 0;
    else
        UpdateFiringState();
...
int32_t CountPlayerObjects(int32_t nPlayer, int32_t nType, int32_t nId) {
    ...
    FORALL_OBJS(pObj)
    if ((pObj->info.nType == nType) && (pObj->info.nId == nId) && (pObj->cType.laserInfo.parent.nType == OBJ_PLAYER) &&
        ...
        if (nSegment >= 0) {
            ...
            if (PlayerInSegment(nSegment)) {
                ...
                pObj->info.position.vPos =
                    SEGMENT(nSegment)->Center() + *VmRandomVector(&v) * (r + fix(r * RandFloat(2.0f)));
            }
        }
    }
}
...
bool CPlayerData::IsLocalPlayer(void) { return Index() == N_LOCALPLAYER; }
...
void CPlayerData::SetObject(int16_t n) { nObject = n; }

//-------------------------------------------------------------------------

CObject *CPlayerData::Object(void) { return (nObject < 0) ? NULL : OBJECT(nObject); }
...
bool CPlayerData::WaitingForWeaponInfo(void) {
    return !m_tWeaponInfo || ((gameStates.app.nSDLTicks[0] - m_tWeaponInfo > 15000) &&
                              (gameStates.app.nSDLTicks[0] - m_tWeaponInfo < 180000) && !m_bExploded && IsConnected());
}
...
int32_t CShipEnergy::RechargeDelayCount(void) { return sizeofa(m_nRechargeDelays); }
...
void CShipEnergy::Recharge(void) {
    ...
    fix delta =
        Min(FixMul(
                gameData.timeData.xFrame,
                gameData.producerData.xFuelGiveAmount / nRechargeSpeeds[extraGameInfo[IsMultiGame].nRechargeSpeed]),
            m_max / 2 - *m_current);
    if (delta > 0)
        Update(delta);
    SetRechargeDelay(0);
