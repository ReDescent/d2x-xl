...
void UpdatePowerupClip(tAnimationInfo *pAnimInfo, tAnimationState *pClip, int32_t nObject) {
    if (pAnimInfo) {
        ...
        if (0 > (h = (gameStates.app.nSDLTicks[0] - pClip->xTotalTime))) {
            ...
            (h = h / 80) && (nFrames = gameData.pigData.tex.addonBitmaps[-pClip->nClipIndex - 1].FrameCount())) { //???
...
void CObject::DoPowerupFrame(void) {
    ...
    if (info.renderType != RT_POLYOBJ) {
        ...
        tAnimationInfo *pAnimInfo = ((pClip->nClipIndex < 0) || (pClip->nClipIndex >= MAX_ANIMATIONS_D2))
                                        ? NULL
                                        : gameData.effectData.animations[0] + pClip->nClipIndex;
...
// return;
    ...
    if (pObj->info.nType == OBJ_MONSTERBALL) {
        ...
        (pObj->rType.animationInfo.nClipIndex >= -MAX_ADDON_BITMAP_FILES) &&
        (pObj->rType.animationInfo.nClipIndex < MAX_ANIMATIONS_D2)) {
        if ((pObj->info.nId < MAX_POWERUP_TYPES_D2) ||
            ((pObj->info.nType == OBJ_EXPLOSION) && (pObj->info.nId < MAX_ANIMATIONS_D2))) {
            ...
            DrawObjectBitmap(
                pObj,
                pObj->rType.animationInfo.nClipIndex,
                pObj->rType.animationInfo.nClipIndex,
                pObj->rType.animationInfo.nCurFrame,
                NULL,
                1);
        }
    }
...
int32_t PickupHeadlight(CObject *pObj, int32_t nPlayer) {
    ...
    sprintf(
        szTemp,
        TXT_GOT_HEADLIGHT,
        (EGI_FLAG(headlight.bAvailable, 0, 0, 1) && gameOpts->gameplay.bHeadlightOnWhenPickedUp) ? TXT_ON : TXT_OFF);
...
void UsePowerup(int32_t id) {
    ...
    if (gameData.objData.pwrUp.info[id].hitSound > -1) {
        if (!bApply && (gameOpts->gameplay.bInventory && (!IsMultiGame || IsCoopGame)) &&
            ((id == POW_CLOAK) || (id == POW_INVUL)))
            id = POW_SHIELD_BOOST;
...
static inline int32_t PowerupCount(int32_t nId) {
    if ((nId == POW_CONCUSSION_4) || (nId == POW_HOMINGMSL_4) || (nId == POW_FLASHMSL_4) || (nId == POW_GUIDEDMSL_4) ||
        (nId == POW_MERCURYMSL_4) || (nId == POW_PROXMINE) || (nId == POW_SMARTMINE))
        return 4;
    return 1;
}
...
int32_t DoPowerup(CObject *pObj, int32_t nPlayer) {
    ...
    if (bLocalPlayer && (gameStates.app.bPlayerIsDead || (gameData.objData.pConsole->info.nType == OBJ_GHOST) ||
                         (pPlayer->Shield() < 0)))
        return 0;
    ...
    if (nType == POWERUP_IS_GUN) {
        ...
        if (bPickedUp < 0) { // only true if hit a gatling gun
            bPickedUp =
                -bPickedUp - 1; // yields 0 if gun still has some ammo (-> keep gun), 1 otherwise (-> remove gun)
        ...
        bPickedUp =
            ((pPickupKey)(pickupHandler[nId]))(pObj, PLAYER_FLAGS_BLUE_KEY << nKey, GAMETEXT(12 + nKey), nPlayer);
...
void SpawnLeftoverPowerups(int16_t nObject) {
    SpawnPowerup(
        gameData.multiplayer.leftoverPowerups[nObject].pSpitter,
        gameData.multiplayer.leftoverPowerups[nObject].nType,
        gameData.multiplayer.leftoverPowerups[nObject].nCount);
    memset(&gameData.multiplayer.leftoverPowerups[nObject], 0, sizeof(gameData.multiplayer.leftoverPowerups[0]));
}
...
void InitPowerupTables(void) {
    ...
    powerupToDevice[POW_CONCUSSION_1] = powerupToDevice[POW_CONCUSSION_4] = CONCUSSION_INDEX;
    powerupToDevice[POW_PROXMINE] = PROXMINE_INDEX;
    powerupToDevice[POW_HOMINGMSL_1] = powerupToDevice[POW_HOMINGMSL_4] = HOMING_INDEX;
    powerupToDevice[POW_SMARTMSL] = SMART_INDEX;
    powerupToDevice[POW_MEGAMSL] = MEGA_INDEX;
    powerupToDevice[POW_FLASHMSL_1] = powerupToDevice[POW_FLASHMSL_4] = FLASHMSL_INDEX;
    powerupToDevice[POW_GUIDEDMSL_1] = powerupToDevice[POW_GUIDEDMSL_4] = GUIDED_INDEX;
    powerupToDevice[POW_SMARTMINE] = SMARTMINE_INDEX;
    powerupToDevice[POW_MERCURYMSL_1] = powerupToDevice[POW_MERCURYMSL_4] = MERCURY_INDEX;
    ...
    powerupToDevice[POW_BLUEFLAG] = powerupToDevice[POW_REDFLAG] = PLAYER_FLAGS_FLAG;

    powerupToDevice[POW_VULCAN_AMMO] = POW_VULCAN_AMMO;

    memset(powerupToWeaponCount, 0, sizeof(powerupToWeaponCount));

    powerupToWeaponCount[POW_LASER] = powerupToWeaponCount[POW_VULCAN] = powerupToWeaponCount[POW_SPREADFIRE] =
        powerupToWeaponCount[POW_PLASMA] = powerupToWeaponCount[POW_FUSION] = powerupToWeaponCount[POW_GAUSS] =
            powerupToWeaponCount[POW_HELIX] = powerupToWeaponCount[POW_PHOENIX] = powerupToWeaponCount[POW_OMEGA] =
                powerupToWeaponCount[POW_SUPERLASER] = 1;

    powerupToWeaponCount[POW_CLOAK] = powerupToWeaponCount[POW_TURBO] = powerupToWeaponCount[POW_INVUL] =
        powerupToWeaponCount[POW_FULL_MAP] = powerupToWeaponCount[POW_CONVERTER] = powerupToWeaponCount[POW_AMMORACK] =
            powerupToWeaponCount[POW_AFTERBURNER] = powerupToWeaponCount[POW_HEADLIGHT] = 1;

    powerupToWeaponCount[POW_CONCUSSION_1] = powerupToWeaponCount[POW_SMARTMSL] = powerupToWeaponCount[POW_MEGAMSL] =
        powerupToWeaponCount[POW_FLASHMSL_1] = powerupToWeaponCount[POW_GUIDEDMSL_1] =
            powerupToWeaponCount[POW_MERCURYMSL_1] = powerupToWeaponCount[POW_EARTHSHAKER] =
                powerupToWeaponCount[POW_HOMINGMSL_1] = 1;

    powerupToWeaponCount[POW_CONCUSSION_4] = powerupToWeaponCount[POW_PROXMINE] = powerupToWeaponCount[POW_FLASHMSL_4] =
        powerupToWeaponCount[POW_GUIDEDMSL_4] = powerupToWeaponCount[POW_SMARTMINE] =
            powerupToWeaponCount[POW_MERCURYMSL_4] = powerupToWeaponCount[POW_HOMINGMSL_4] = 4;

    powerupToWeaponCount[POW_VULCAN_AMMO] = 1;

    memset(powerupClass, 0, sizeof(powerupClass));
    powerupClass[POW_LASER] = powerupClass[POW_VULCAN] = powerupClass[POW_SPREADFIRE] = powerupClass[POW_PLASMA] =
        powerupClass[POW_FUSION] = powerupClass[POW_GAUSS] = powerupClass[POW_HELIX] = powerupClass[POW_PHOENIX] =
            powerupClass[POW_OMEGA] = powerupClass[POW_SUPERLASER] = 1;

    powerupClass[POW_CONCUSSION_1] = powerupClass[POW_CONCUSSION_4] = powerupClass[POW_PROXMINE] =
        powerupClass[POW_SMARTMSL] = powerupClass[POW_MEGAMSL] = powerupClass[POW_FLASHMSL_1] =
            powerupClass[POW_FLASHMSL_4] = powerupClass[POW_GUIDEDMSL_1] = powerupClass[POW_GUIDEDMSL_4] =
                powerupClass[POW_SMARTMINE] = powerupClass[POW_MERCURYMSL_1] = powerupClass[POW_MERCURYMSL_4] =
                    powerupClass[POW_EARTHSHAKER] = powerupClass[POW_HOMINGMSL_1] = powerupClass[POW_HOMINGMSL_4] = 2;

    powerupClass[POW_QUADLASER] = powerupClass[POW_CLOAK] =
        // powerupClass [POW_TURBO] =
        powerupClass[POW_INVUL] = powerupClass[POW_FULL_MAP] = powerupClass[POW_CONVERTER] =
            powerupClass[POW_AMMORACK] = powerupClass[POW_AFTERBURNER] = powerupClass[POW_HEADLIGHT] = 3;

    powerupClass[POW_VULCAN_AMMO] = 4;

    powerupClass[POW_BLUEFLAG] = powerupClass[POW_REDFLAG] = 5;

    powerupClass[POW_KEY_BLUE] = powerupClass[POW_KEY_GOLD] = powerupClass[POW_KEY_RED] = 6;
    ...
    pickupHandler[POW_BLUEFLAG] = pickupHandler[POW_REDFLAG] = reinterpret_cast<void *>(PickupFlag);

    pickupHandler[POW_KEY_BLUE] = pickupHandler[POW_KEY_RED] = pickupHandler[POW_KEY_GOLD] =
        reinterpret_cast<void *>(PickupKey);
    ...
    pickupHandler[POW_SPREADFIRE] = pickupHandler[POW_PLASMA] = pickupHandler[POW_FUSION] = pickupHandler[POW_HELIX] =
        pickupHandler[POW_PHOENIX] = pickupHandler[POW_OMEGA] = reinterpret_cast<void *>(PickupGun);
    pickupHandler[POW_VULCAN] = pickupHandler[POW_GAUSS] = reinterpret_cast<void *>(PickupGatlingGun);

    pickupHandler[POW_PROXMINE] = pickupHandler[POW_SMARTMINE] = pickupHandler[POW_CONCUSSION_1] =
        pickupHandler[POW_CONCUSSION_4] = pickupHandler[POW_HOMINGMSL_1] = pickupHandler[POW_HOMINGMSL_4] =
            pickupHandler[POW_SMARTMSL] = pickupHandler[POW_MEGAMSL] = pickupHandler[POW_FLASHMSL_1] =
                pickupHandler[POW_FLASHMSL_4] = pickupHandler[POW_GUIDEDMSL_1] = pickupHandler[POW_GUIDEDMSL_4] =
                    pickupHandler[POW_MERCURYMSL_1] = pickupHandler[POW_MERCURYMSL_4] = pickupHandler[POW_EARTHSHAKER] =
                        reinterpret_cast<void *>(PickupSecondary);

    powerupType[POW_TURBO] = powerupType[POW_MEGAWOW] = powerupType[POW_MONSTERBALL] = (uint8_t)POWERUP_IS_UNDEFINED;

    powerupType[POW_EXTRA_LIFE] = powerupType[POW_ENERGY] = powerupType[POW_SHIELD_BOOST] = powerupType[POW_CLOAK] =
        powerupType[POW_HOARD_ORB] = powerupType[POW_INVUL] = powerupType[POW_FULL_MAP] = powerupType[POW_CONVERTER] =
            powerupType[POW_AMMORACK] = powerupType[POW_AFTERBURNER] = powerupType[POW_HEADLIGHT] =
                powerupType[POW_SLOWMOTION] = powerupType[POW_BULLETTIME] = powerupType[POW_VULCAN_AMMO] =
                    (uint8_t)POWERUP_IS_EQUIPMENT;

    powerupType[POW_BLUEFLAG] = powerupType[POW_REDFLAG] = (uint8_t)POWERUP_IS_FLAG;

    powerupType[POW_KEY_BLUE] = powerupType[POW_KEY_RED] = powerupType[POW_KEY_GOLD] = (uint8_t)POWERUP_IS_KEY;

    powerupType[POW_LASER] = powerupType[POW_SPREADFIRE] = powerupType[POW_PLASMA] = powerupType[POW_FUSION] =
        powerupType[POW_SUPERLASER] = powerupType[POW_HELIX] = powerupType[POW_PHOENIX] = powerupType[POW_OMEGA] =
            powerupType[POW_VULCAN] = powerupType[POW_GAUSS] = powerupType[POW_QUADLASER] = (uint8_t)POWERUP_IS_GUN;

    powerupType[POW_CONCUSSION_1] = powerupType[POW_CONCUSSION_4] = powerupType[POW_PROXMINE] =
        powerupType[POW_HOMINGMSL_1] = powerupType[POW_HOMINGMSL_4] = powerupType[POW_SMARTMSL] =
            powerupType[POW_MEGAMSL] = powerupType[POW_FLASHMSL_1] = powerupType[POW_FLASHMSL_4] =
                powerupType[POW_GUIDEDMSL_1] = powerupType[POW_GUIDEDMSL_4] = powerupType[POW_SMARTMINE] =
                    powerupType[POW_MERCURYMSL_1] = powerupType[POW_MERCURYMSL_4] = powerupType[POW_EARTHSHAKER] =
                        (uint8_t)POWERUP_IS_MISSILE;
}

//-----------------------------------------------------------------------------

#define ENABLE_FILTER(_type, _flag) \
    if (_flag) \
        powerupFilter[_type] = 1;
...
void SetupPowerupFilter(tNetGameInfo *pInfo) {
    ...
    ENABLE_FILTER(
        POW_HEADLIGHT,
        pInfo->DoHeadlight && (!EGI_FLAG(bDarkness, 0, 0, 0) || EGI_FLAG(headlight.bAvailable, 0, 1, 0)));
...
char PowerupClass(int16_t nPowerup) { return powerupClass[nPowerup]; }

//-----------------------------------------------------------------------------

char PowerupToWeaponCount(int16_t nPowerup) { return powerupToWeaponCount[nPowerup]; }

//-----------------------------------------------------------------------------

char PowerupToObject(int16_t nPowerup) { return powerupToObject[nPowerup]; }

//-----------------------------------------------------------------------------

int16_t PowerupToModel(int16_t nPowerup) { return powerupToModel[nPowerup]; }

//-----------------------------------------------------------------------------

int16_t WeaponToModel(int16_t nWeapon) { return weaponToModel[nWeapon]; }
...
int16_t PowerupsOnShips(int32_t nPowerup) {
	...
	if (!pPlayer->connected && (gameStates.app.nSDLTicks [0] - pPlayer->m_tDisconnect > 600))
        ...
        if (nClass == 4)
            nVulcanAmmo += pPlayer->primaryAmmo[VULCAN_INDEX] +
                           gameData.multiplayer.weaponStates[i].nAmmoUsed % VULCAN_CLIP_CAPACITY;
