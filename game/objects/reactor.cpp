...
int32_t CalcBestReactorGun(int32_t nGunCount, CFixVector *vGunPos, CFixVector *vGunDir, CFixVector *vObjPos) {
    ...
    Assert(
        nBestGun !=
        -1); // Contact Mike.  This is impossible.  Or maybe you're getting an unnormalized vector somewhere.
    if (xBestDot < 0)
        return -1;
    return nBestGun;
}
...
void DoReactorDeadFrame(void) {
    if (gameStates.gameplay.nReactorCount[0] < gameStates.gameplay.nReactorCount[1]) {
        ...
        for (int32_t i = 0; i < gameStates.gameplay.nReactorCount[1]; i++, pStates++) {
            if ((pStates->nDeadObj != -1) && (OBJECT(pStates->nDeadObj)->info.nType == OBJ_REACTOR) &&
...
void DoCountdownFrame(void) {
    ...
    if (X2I(oldTime + I2X(7) / 8) != gameData.reactorData.countdown.nSecsLeft) {
        if ((gameData.reactorData.countdown.nSecsLeft >= 0) && (gameData.reactorData.countdown.nSecsLeft < 10))
            audio.PlaySound(
                (int16_t)(SOUND_COUNTDOWN_0_SECS + gameData.reactorData.countdown.nSecsLeft),
                SOUNDCLASS_GENERIC,
                I2X(3));
    ...
    if (gameData.reactorData.countdown.nTimer > 0) {
        ...
        if ((size != oldSize) && (gameData.reactorData.countdown.nSecsLeft <
                                  gameData.reactorData.countdown.nTotalTime - 5)) // Every 2 seconds!
...
void InitCountdown(CTrigger *pTrigger, int32_t bReactorDestroyed, int32_t nTimer) {
    if (pTrigger && (pTrigger->m_info.time > 0))
        ...
        gameData.reactorData.countdown.nTotalTime =
            ...
            gameStates.app.nBaseCtrlCenExplTime * (DIFFICULTY_LEVEL_COUNT - gameStates.app.nDifficultyLevel - 1) / 2;
    else
        gameData.reactorData.countdown.nTotalTime =
            nAlanPavlishReactorTimes[gameStates.app.bD1Mission][gameStates.app.nDifficultyLevel];
    gameData.reactorData.countdown.nTimer =
        (nTimer < 0) ? I2X(gameData.reactorData.countdown.nTotalTime) : (nTimer ? nTimer : I2X(1));
    ...
    if (bFinalCountdown ||
        ...
        if (bFinalCountdown) {
            ...
            for (i = 0; i < gameData.reactorData.triggers.m_nLinks; i++)
                SEGMENT(gameData.reactorData.triggers.m_segments[i])
                    ->ToggleWall(gameData.reactorData.triggers.m_sides[i]);
...
void RemoveReactor(CObject *pObj) {
    ...
    memset(
        gameData.reactorData.states + gameStates.gameplay.nReactorCount[0],
        0,
...
void DoReactorFrame(CObject *pObj) {
    ...
    if ((pReactorStates->nNextFireTime < 0) &&
        ...
        nBestGun = CalcBestReactorGun(
            gameData.reactorData.props[pObj->info.nId].nGuns,
            pReactorStates->vGunPos,
            pReactorStates->vGunDir,
            (LOCALPLAYER.flags & PLAYER_FLAGS_CLOAKED) ? &gameData.aiData.target.vBelievedPos
        ...
        if (nBestGun != -1) {
            ...
            CreateNewWeaponSimple(
                &vecToGoal,
                &pReactorStates->vGunPos[nBestGun],
                pObj->Index(),
...
void InitReactorForLevel(int32_t bRestore) {
    ...
    extraGameInfo[0].nBossCount[0] = extraGameInfo[0].nBossCount[1] = 0;
    gameStates.gameplay.nReactorCount[0] = gameStates.gameplay.nReactorCount[1] = 0;
    ...
    FORALL_ACTOR_OBJS(pObj) {
        if (pObj->info.nType == OBJ_REACTOR) {
            if (gameStates.gameplay.nReactorCount[0] && !(gameStates.app.bD2XLevel && gameStates.gameplay.bMultiBosses))
                PrintLog(
                    0,
                    "Warning: Two or more control centers including %i and %i\n",
...
void SpecialReactorStuff(void) {
    ...
    if (gameData.reactorData.bDestroyed) {
        gameData.reactorData.countdown.nTimer +=
            I2X(gameStates.app.nBaseCtrlCenExplTime + (DIFFICULTY_LEVEL_COUNT - 1 - gameStates.app.nDifficultyLevel) *
                                                          gameStates.app.nBaseCtrlCenExplTime /
