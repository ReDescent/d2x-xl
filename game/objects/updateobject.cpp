...
void StopPlayerMovement(void) {
    ENTER(0, 0);
    if (OBJECTS.Buffer() && !(gameData.objData.speedBoost.Buffer() &&
...
void CObject::Spin(void) {
    ...
    vRot = CAngleVector::Create(
        (fixang)FixMul(mType.spinRate.v.coord.x, gameData.timeData.xFrame),
...
void CObject::RotateCamera(void) { cameraManager.Rotate(this); }
...
void CObject::HandleSegmentFunction(void) {
    ...
    if ((info.nType == OBJ_PLAYER) && (N_LOCALPLAYER == info.nId)) {
        ...
        if (gameData.appData.GameMode(GM_CAPTURE))
            ...
            if (IsEntropyGame && ((pSeg->m_owner < 0) || (pSeg->m_owner == GetTeam(N_LOCALPLAYER) + 1))) {
                StopConquerWarning();
            }
...
int32_t CObject::UpdateMovement(void) {
    ...
    default:
        Error(
            "Unknown control nType %d in CObject %i, sig/nType/id = %i/%i/%i",
            info.controlType,
            OBJ_IDX(this),
            info.nSignature,
            info.nType,
...
static inline int32_t ShipVolume(int32_t nSpeed) { return I2X(1) / 16 + nSpeed / 512; }
...
if (nSpeed < 0)
    ...
    if (gameData.multiplayer.bMoving < 0)
        audio.CreateObjectSound(
            ...
            -1,
            -1,
            const_cast<char *>(addonSounds[SND_ADDON_JET_ENGINE].szSoundFile),
            1,
            1);
    else
...
#endif
        nConnSide =
            SEGMENT(gameData.physicsData.segments[i + 1])->ConnectedSide(SEGMENT(gameData.physicsData.segments[i]));
        if (nConnSide != -1)
            ...
            PrintLog(
                0,
                "Unconnected segments %d, %d\n",
                gameData.physicsData.segments[i + 1],
                gameData.physicsData.segments[i]);
#endif
...
void CObject::CheckAfterburnerBlobDrop(void) {
    ...
    int32_t nObject = OBJ_IDX(this);
    if (bSmoke || (gameData.objData.xLastAfterburnerTime[nObject] + delay < gameData.timeData.xGame) ||
...
void CObject::UpdateEffects(void) {
    ...
    if ((info.nType == OBJ_ROBOT) && gameOpts->render.lightning.bRobots) {
        if (!ROBOTINFO(info.nId))
            ...
            bNeedEffect =
                ROBOTINFO(info.nId)->energyDrain && (gameStates.app.nSDLTicks[0] - m_xTimeEnergyDrain <= 1000);
...
int32_t CObject::Update(void) {
    ...
    if ((info.xLifeLeft != IMMORTAL_TIME) && (info.xLifeLeft != ONE_FRAME_TIME) && (gameData.physicsData.xTime > 0))
        info.xLifeLeft -= (fix)(
            gameData.physicsData.xTime / gameStates.gameplay.slowmo[0].fSpeed); //...inevitable countdown towards death
    gameStates.render.bDropAfterburnerBlob = 0;
...
int32_t UpdateAllObjects(void) {
    ...
    FORALL_OBJS(pObj) {
        ...
        if (pObj->info.nType == OBJ_REACTOR)
            BRP;
#endif
        if ((pObj->info.nType != OBJ_NONE) && (pObj->info.nType != OBJ_GHOST) &&
            !(pObj->info.nFlags & OF_SHOULD_BE_DEAD) && !pObj->Update()) {
...
void CleanupObjects(void) {
    ...
    for (CObjectIterator iter(pObj); pObj; pObj = (pPrevObj ? iter.Step() : iter.Start())) {
        ...
        if (pObj->info.nType == OBJ_PLAYER) {
            ...
            pPrevObj = iter.Back();
            if ((pObj->info.nType == OBJ_ROBOT) || (pObj->info.nType == OBJ_DEBRIS) || // exploded robot
                (pObj->info.nType == OBJ_REACTOR) || (pObj->info.nType == OBJ_POWERUP) ||
...
void CObject::SetLife(fix xLife) { UpdateLife(m_xTotalLife = xLife); }
