...
int32_t COGL::EnableClientState(GLuint nState, int32_t nTMU) {
    ...
    if (Locked())
        BRP;
    memset(
        &m_data.clientBuffers[m_data.nTMU[0]][nState - GL_VERTEX_ARRAY],
        0,
        sizeof(m_data.clientBuffers[m_data.nTMU[0]][nState - GL_VERTEX_ARRAY]));
#endif
