...
int32_t COGL::DrawBufferWidth(void) { return int32_t(gameData.renderData.screen.Scaled(m_states.nCurWidth)); }

int32_t COGL::DrawBufferHeight(void) { return int32_t(gameData.renderData.screen.Scaled(m_states.nCurHeight)); }
...
void COGL::CreateDrawBuffer(int32_t nType) {
    ...
    DrawBuffer()->Create(
        ...
        DrawBufferHeight(),
        nType,
        (nType != 1) ? 1 : 3 + m_features.bMultipleRenderTargets);
    PrintLog(-1);
}
...
void COGL::ChooseDrawBuffer(void) {
    ...
    if ((gameStates.render.bRenderIndirect < 0) ||
        ((/*gameStates.render.nWindow [0] ||*/ gameStates.render.bBriefing) && !IsSideBySideDevice())) {
...
int32_t COGL::SelectGlowBuffer(void) {
    return SelectDrawBuffer(
               gameStates.render.cameras.bActive ? -cameraManager.CurrentIndex() - 1
                                                 : int32_t(!IsSideBySideDevice() && (m_data.xStereoSeparation > 0)),
               1) > -1;
}

//------------------------------------------------------------------------------

int32_t COGL::SelectBlurBuffer(int32_t nBuffer) { return SelectDrawBuffer(nBuffer + 3) > -1; }

//------------------------------------------------------------------------------

int32_t COGL::SelectFogBuffer(int32_t nBuffer) { return SelectDrawBuffer(nBuffer + 5) > -1; }
...
CFBO *COGL::BlurBuffer(int32_t nBuffer) {
    return (gameStates.render.cameras.bActive && (nBuffer < 0))
               ...
               : m_data.GetDrawBuffer(
                     (nBuffer >= 0) ? nBuffer + 3 : !IsSideBySideDevice() && int32_t(m_data.xStereoSeparation > 0));
}
