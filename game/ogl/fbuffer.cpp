...
int32_t CFBO::CreateColorBuffers(int32_t nBuffers) {
    ...
    m_info.nColorBuffers = m_info.nBufferCount = nBuffers;
...
void CFBO::Destroy(void) {
    ...
    if (m_info.hFBO) {
        ...
        if (m_info.hDepthBuffer) {
            ...
            m_info.hDepthBuffer = m_info.hStencilBuffer = 0;
        }
...
int32_t CFBO::IsBound(void) { return m_info.hColorBuffer[0] && (m_info.hColorBuffer[0] == ogl.BoundTexture()); }
...
void CFBO::Setup(void) {
    ...
    if (ogl.m_features.bRenderToTexture.Available()) {
        ...
        else if (!(glGetRenderbufferParameterivEXT =
        ...
        else if (!(glFramebufferTexture1DEXT =
        ...
        else if (!(glFramebufferTexture3DEXT =
        ...
        else if (!(glGetFramebufferAttachmentParameterivEXT = (PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVEXTPROC)
    ...
    PrintLog(
        -1,
        (ogl.m_features.bRenderToTexture == 2) ? "Rendering to texture is available\n"
