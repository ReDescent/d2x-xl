...
static float PrintTime(tProfilerTags tag, int32_t nLine, int32_t bStyle = 0) {
    ...
    if (bStyle == 0)
        sprintf(
            szValue,
            ": %05.2f %c (%05.2f)",
            t = 100.0f * float(p[0].t[tag]) / float(p[0].t[ptFrame]),
            '%',
            float(p[0].t[tag]) / p[0].nFrameCount);
    else if (bStyle == 1)
...
static void PrintStatistics(void) {
    ...
    if (gameStates.render.bShowProfiler && !gameStates.menus.nInMenu && fontManager.Current() && SMALL_FONT) {
        ...
        if (((gameStates.render.bShowProfiler == 1) && (t1 - t0 >= 10)) ||
            ((gameStates.render.bShowProfiler == 2) && (t1 - t0 >= 1000))) {
        ...
        if (p[0].t[ptRenderFrame]) {
            ...
            GrPrintF(
                ...
                5,
                h * nLine,
                fontManager.Current()->PadString(szPad, "      total", " .", profilerValueOffset));
