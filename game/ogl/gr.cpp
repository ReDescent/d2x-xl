...
int32_t GrSetMode(uint32_t mode) {
    ...
    SetupCanvasses();
    // gameData.renderData.screen.Aspect () = FixDiv(gameData.renderData.screen.Width
    // ()*3,gameData.renderData.screen.Height ()*4);
    gameData.renderData.screen.SetAspect(FixDiv(
        gameData.renderData.screen.Width(),
        (fix)(gameData.renderData.screen.Height() * ((double)w / (double)h))));
...
int32_t FindDisplayMode(int32_t nScrSize) { return FindDisplayMode(SM_W(nScrSize), SM_H(nScrSize)); }

//------------------------------------------------------------------------------

SDL_Rect defaultDisplayModes[] = {{320, 200},   {640, 400},  {640, 480},   {720, 576},   {800, 600},   {1024, 768},
                                  {1152, 864},  {1280, 720}, {1280, 960},  {1280, 1024}, {1600, 1200}, {2048, 1526},
                                  {720, 480},   {1280, 768}, {1280, 800},  {1280, 854},  {1360, 768},  {1400, 1050},
                                  {1440, 900},  {1440, 960}, {1680, 1050}, {1920, 1080}, {1920, 1200}, {2560, 1440},
                                  {2560, 1600}, {3840, 2160}};
...
void CreateDisplayModeInfoTable(void) {
    // TODO: support multiple displays / display selection
    int display_index = 0;
    int numDispModes = SDL_GetNumDisplayModes(0);

    displayModeInfo.Create(numDispModes, "displayModeInfo");
    displayModeInfo.Clear();
    int count_modes = 0;
    for (int mode_index = 0; mode_index < numDispModes; mode_index++) {
        SDL_DisplayMode mode = {SDL_PIXELFORMAT_UNKNOWN, 0, 0, 0, 0};
        if (SDL_GetDisplayMode(display_index, mode_index, &mode) != 0) {
            PrintLog(0, "SDL_GetDisplayMode failed: %s\n", SDL_GetError());
            continue;
        }

        CreateDisplayModeInfo(count_modes, mode.w, mode.h, 1);
        count_modes++;
    }
    displayModeInfo.Resize(count_modes);
    displayModeInfo.SortAscending(0);
}
...
int32_t SetMenuScreenMode(uint32_t sm) {
    ...
    nMenuMode = gameStates.gfx.bOverride ? gameStates.gfx.nStartScrSize
                                         : gameStates.menus.bHires ? (gameData.renderData.screen.Area() >= 640 * 480)
...
int32_t SetGameScreenMode(uint32_t sm) {
    ...
    if (!gameData.renderData.frame.Height() ||
        (gameData.renderData.frame.Height() > gameData.renderData.screen.Height()) ||
        !gameData.renderData.frame.Width() ||
        (gameData.renderData.frame.Width() > gameData.renderData.screen.Width())) {
    ...
    gameStates.render.fonts.bHires =
        gameStates.render.fonts.bHiresAvailable && (gameStates.menus.bHires = (gameStates.video.nDisplayMode > 1));
