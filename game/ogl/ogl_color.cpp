...
void OglPalColor(CPalette *palette, int32_t c, CFloatVector *pColor) {
    ...
    if (c < 0) {
        if (pColor)
            pColor->Red() = pColor->Green() = pColor->Blue() = pColor->Alpha() = 1.0;
        else
...
void SetTMapColor(tUVL *uvlList, int32_t i, CBitmap *pBm, int32_t bResetColor, CFloatVector *pColor) {
    ...
    if (gameStates.app.bEndLevelSequence >= EL_OUTSIDE) {
        if (!pColor)
            ...
            pColor->Red() = pColor->Green() = pColor->Blue() = l;
            pColor->Alpha() = s;
        }
    } else if (pColor) {
        if (tMapColor.index) {
...
inline int32_t sqri(int32_t i) { return i * i; }
...
static inline int32_t IsLightVert(int32_t nVertex, CDynLight *pLight) {
    if ((nVertex >= 0) && pLight->info.pFace) {
        uint16_t *pv =
            gameStates.render.bTriangleMesh ? pLight->info.pFace->triIndex : pLight->info.pFace->m_info.index;
...
int32_t ComputeVertexColor(
    int32_t nSegment,
    int32_t nSide,
    int32_t nVertex,
    CFloatVector3 *pColorSum,
    CVertColorData *colorDataP,
    int32_t nThread) {
    CDynLight *pLight;
    ...
    int32_t i, j, nType, nLights, bSelf, nSelf,
	...
	if (i == colorData.nMatLight)
        ...
        if (bSelf ||
            (gameStates.render.bBuildLightmaps && (pLight->Segment() == nSegment) && (pLight->Side() == nSide))) {
	...
	if ((nDbgSeg >= 0) && (pLight->info.nSegment == nDbgSeg) && ((nDbgSide < 0) || (pLight->info.nSide == nDbgSide)))
            ...
            if ((bDiffuse = pLight->info.bDiffuse[nThread])) {
                if (colorData.vertNorm.IsZero())
                    ...
                    if (nType > 1)
                        ...
                        // for rectangular light sources, using the light source's closest point to the light receiving
                        // point can yield a incident angle too low if the point lies in an adjacent face. In that case
                        // use the angle from the light source's center if it is better
                    ...
                    if ((NdotL < 0.0f) &&
                        (NdotL > -0.01f)) // compensate for faces almost planar with the light emitting face
                        NdotL = 0.0f;
                }
                ...
                NdotL = 1.0f;
                fLightAngle = (1.0f + CFloatVector3::Dot(lightDir, lightRayDir)) *
                              (1.0f - CFloatVector3::Dot(colorData.vertNorm, lightRayDir)) * 0.25f;
#if LIGHTDIST_MATH == 0
        ...
        if (!bDiffuse) {
            ...
            vertColor *= fLightAngle * fLightAngle; // may use quadratic dampening to emphasize attenuation due to point
                                                    // being behind light source
        } else {
...
void InitVertColorData(CVertColorData &colorData) {
    ...
    colorData.bExclusive = !FAST_SHADOWS && (gameStates.render.nShadowPass == 3), colorData.fMatShininess = 0.0f;
...
void GetVertexColor(
    ...
    int32_t nSide,
    int32_t nVertex,
    CFloatVector3 *pvVertNorm,
    CFloatVector3 *pvVertPos,
    CFaceColor *pVertexColor,
    CFaceColor *pBaseColor,
    float fScale,
    int32_t bSetColor,
    int32_t nThread) {
    PROF_START
...
#pragma omp critical(GetVertexColor1)
    ...
    if (gameStates.app.bMultiThreaded) {
        ...
        if (gameStates.app.bEndLevelSequence >= EL_OUTSIDE) {
        colorSum.Red() = colorSum.Green() = colorSum.Blue() = 1.0f;
    } else {
