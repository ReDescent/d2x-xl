...
void OglDrawPixel(int32_t x, int32_t y, CCanvasColor *pColor) {
    if (ogl.SizeVertexBuffer(1)) {
        ...
        ogl.VertexBuffer()[0].v.coord.x =
            float(x /*+ CCanvas::Current ()->Left ()*/) / float(ogl.m_states.viewport[0].m_w);
        ogl.VertexBuffer()[0].v.coord.y =
            1.0f - float(y /*+ CCanvas::Current ()->Top ()*/) / float(ogl.m_states.viewport[0].m_w);
