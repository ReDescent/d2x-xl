...
void G3FlushFaceBuffer(int32_t bForce) {
    ...
    if (faceBuffer.nFaces && (bForce || (faceBuffer.nFaces >= FACE_BUFFER_SIZE))) {
        ...
        try {
            ...
            PrintLog(
                0,
                "error calling glDrawElements (%d, %d) in G3FlushFaceBuffer\n",
                gameStates.render.bTriangleMesh ? "GL_TRIANGLES" : "GL_TRIANGLE_FAN");
        }
        faceBuffer.nFaces = faceBuffer.nElements = 0;
    }
#endif
}
...
static inline int32_t FaceIsColored(CSegFace *pFace) { return gameStates.render.bFullBright || pFace->m_info.nColored; }
...
int32_t RenderFace(CSegFace *pFace, CBitmap *bmBot, CBitmap *bmTop, int32_t bTextured, int32_t bLightmaps) {
...
#endif
    if (bmTop) {
        if ((bmTop = bmTop->Override(BitmapFrame(bmTop, pFace->m_info.nOvlTex, pFace->m_info.nSegment))) &&
            bmTop->Frames()) {
    ...
    if ((bTransparent /*|| (pFace->m_info.nSegColor && gameStates.render.bPerPixelLighting)*/) &&
        (gameStates.render.nType < RENDER_TYPE_SKYBOX) && !bMonitor) {
