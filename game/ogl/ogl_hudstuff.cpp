...
void OglDrawMouseIndicator(void) {
    ...
    if (joyMouse.Load()) {
        ...
        glPushMatrix();
        glTranslatef(
            (float)(mouseData.x) / (float)gameData.renderData.screen.Width(),
            1.0f - (float)(mouseData.y) / (float)gameData.renderData.screen.Height(),
            0);
        glScalef(
            scale / 320.0f,
            scale / 200.0f,
            scale); // the positions are based upon the standard reticle at 320x200 res.
        ...
        OglDrawEllipse(
            ...
            1.5f,
            0,
            1.5f * float(gameData.renderData.screen.Height()) / float(gameData.renderData.screen.Width()),
            0,
            sinCos12);
        glPopMatrix();
    }
    if (deadzone.Load()) {
        ...
        CFloatVector vPosf;
        vPosf.v.coord.x = vPosf.v.coord.y = 0;
        ...
        OglDrawEllipse(
            ...
            r,
            0,
            r * float(gameData.renderData.screen.Height()) / float(gameData.renderData.screen.Width()),
            0,
            sinCos30);
        glPopMatrix();
    }
