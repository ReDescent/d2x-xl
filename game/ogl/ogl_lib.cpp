...
tRenderQuality renderQualities[] = {
    ...
    {GL_LINEAR_MIPMAP_LINEAR,
     GL_LINEAR,
     1,
     1}, // smooth close textures, use smoothed mipmaps for distant ones, anti-aliasing
...
int32_t COGL::StereoDevice(void) {
    ...
    if ((gameOpts->render.stereo.nGlasses == GLASSES_SHUTTER_NVIDIA) && !m_states.nStereo)
        return 0;
    return (gameOpts->render.stereo.nGlasses < DEVICE_STEREO_PHYSICAL) ? gameOpts->render.stereo.nGlasses
                                                                       : -gameOpts->render.stereo.nGlasses;
}
...
void G3Normal(CRenderPoint **pointList, CFixVector *pvNormal) {
...
#endif
    {
        ...
        if ((v[0] < 0) || (v[1] < 0) || (v[2] < 0)) {
            ...
            vNormal = CFixVector::Normal(
                gameData.segData.vertices[vSorted[0]],
...
void COglStates::Initialize(void) {
    ...
    texMinFilter = GL_NEAREST;
    nTexMagFilterState = -1, nTexMinFilterState = -1;
    nTexEnvModeState = -1, viewport[0] = CViewport(), viewport[1] = CViewport(), nCurWidth = nCurHeight = -1;
    bCurFullScreen = -1;
    bpp = 32;
    nRGBAFormat = GL_RGBA;
    nRGBFormat = GL_RGB;
...
void COglData::Initialize(void) {
    ...
    zFar = 5000.0f;
    depthScale.SetZero();
    windowScale.dim.x = windowScale.dim.y = 0;
...
void COGL::SaveViewport(void) { m_states.viewport[1] = m_states.viewport[0]; }
...
#endif
    {
        r_polyc = r_tpolyc = r_tvertexc = r_bitmapc = r_ubitmapc = r_ubitbltc = r_upixelc = 0;
    ...
    if (gameOpts->ogl.bObjLighting || (gameStates.render.bPerPixelLighting == 2)) {
        ...
        if (bSpecular) {
            ...
            glMateriali(
                GL_FRONT_AND_BACK,
                GL_SHININESS,
                (bSpecular < 0) ? 8 : gameOpts->render.bHiresModels[0] ? 127 : 8);
        } else
...
void COGL::SetupTransform(int32_t bForce) {
    if (!m_states.nTransformCalls && (m_states.bUseTransform || bForce)) {
        ...
        glTranslatef(
            -transformation.m_info.posf[1].v.coord.x,
            -transformation.m_info.posf[1].v.coord.y,
            -transformation.m_info.posf[1].v.coord.z);
...
void COGL::SetScreenMode(void) {
    if (/*(gameStates.video.nLastScreenMode == gameStates.video.nScreenMode) &&*/
        ...
        (gameStates.app.bGameRunning ||
         (gameStates.video.nScreenMode == SCREEN_GAME) /*|| (m_states.nDrawBuffer == GL_FRONT)*/))
        return;
    m_data.Initialize();
