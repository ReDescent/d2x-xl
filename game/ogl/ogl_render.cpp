...
int32_t
G3DrawPolyAlpha(int32_t nVertices, CRenderPoint **pointList, CFloatVector *color, char bDepthMask, int16_t nSegment) {
    if (gameStates.render.nShadowBlurPass == 1) {
        G3DrawWhitePoly(nVertices, pointList);
        return 0;
    }
...
int32_t G3DrawTexPolyMulti(
    ...
    int32_t bShaderMerge = 0, bSuperTransp = 0;
    int32_t bLight = 1, bDynLight = gameStates.render.bApplyDynLight && (gameStates.app.bEndLevelSequence < EL_OUTSIDE),
            bDepthSort, bOverlay = 0;
    CFaceColor *pColor;
    ...
    bDepthSort =
        (!bmTop && ((gameStates.render.grAlpha < 1.0f) ||
                    (bmBot->Flags() & (BM_FLAG_TRANSPARENT | BM_FLAG_SEE_THRU | BM_FLAG_TGA)) ==
                        (BM_FLAG_TRANSPARENT | BM_FLAG_TGA)));
...
retry:
        ...
        for (i = 0, pPointP = pointList; i < nVertices; i++, pPointP++) {
            ...
            if (bDynLight)
                GetVertexColor(
                    ...
                    -1,
                    vertIndex[i],
                    pPoint->GetNormal()->XYZ(),
                    vVertPos.XYZ(),
                    vertColors + i,
                    NULL,
                    gameStates.render.nState ? X2F(uvlList[i].l) : 1,
                    -1,
                    0);
            else if (bLight)
    ...
    ogl.BindTexture(0);
    ogl.SetTexturing(false);
    tMapColor.index = lightColor.index = 0;
    if (!bBlend)
        ogl.SetBlending(true);
    return 0;
}
...
int32_t G3DrawTexPolySimple(
    ...
    int32_t bBlend) {
    int32_t i;
    int32_t bLight = 1, bDynLight = gameStates.render.bApplyDynLight && !gameStates.app.bEndLevelSequence;
    CRenderPoint *pPoint, **pPointP;
#if USE_VERTNORMS
    CFloatVector vNormal, vVertPos;
#endif
...
int32_t COGL::BindBuffers(
    ...
    CBitmap *pBm,
    int32_t nTMU) {
    if (!ogl.EnableClientStates(
            m_data.bClientTexCoord = pTexCoord != NULL,
            m_data.bClientColor = ((pColor != NULL) && (nColors == nVertices)),
            0,
            nTMU))
        return 0;
    if (pTexCoord)
...
void COGL::ReleaseBuffers(void) { ogl.DisableClientStates(m_data.bClientTexCoord, m_data.bClientColor, 0); }

//------------------------------------------------------------------------------

int32_t COGL::RenderArrays(
    int32_t nPrimitive,
    CFloatVector *pVertex,
    int32_t nVertices,
    int32_t nDimensions,
...
int32_t
COGL::RenderQuad(CBitmap *pBm, CFloatVector &vPosf, float width, float height, int32_t nDimensions, int32_t nWrap) {
    CFloatVector verts[4];
    verts[0].v.coord.x = verts[3].v.coord.x = vPosf.v.coord.x - width;
    verts[1].v.coord.x = verts[2].v.coord.x = vPosf.v.coord.x + width;
    verts[0].v.coord.y = verts[1].v.coord.y = vPosf.v.coord.y + height;
    verts[2].v.coord.y = verts[3].v.coord.y = vPosf.v.coord.y - height;
    if (nDimensions == 3)
        verts[0].v.coord.z = verts[1].v.coord.z = verts[2].v.coord.z = verts[3].v.coord.z = vPosf.v.coord.z;
    int32_t nColors = 0;
...
bool COglBuffers::SizeTexCoord(int32_t nVerts) {
    ...
    texCoord[1].Destroy();
    return (
        (texCoord[0].Create(nVerts, "COglBuffers::texCoord [0]") != NULL) &&
        (texCoord[1].Create(nVerts, "COglBuffers::texCoord [1]") != NULL));
}
