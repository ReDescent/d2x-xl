...
CShaderManager::CShaderManager() { Init(); }

//------------------------------------------------------------------------------

CShaderManager::~CShaderManager() { Destroy(true); }
...
int32_t CShaderManager::Compile(int32_t nShader, const char *pszFragShader, const char *pszVertShader, bool bFromFile) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        glShaderSource(
            shader.shaders[i],
            1,
            i ? reinterpret_cast<const GLcharARB **>(&pszFragShader)
              : reinterpret_cast<const GLcharARB **>(&pszVertShader),
            NULL);
#if DBG_SHADERS
        if (bFromFile) {
            if (i)
...
void COGL::InitShaders(void) { shaderManager.Setup(); }
