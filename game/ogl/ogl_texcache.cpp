...
int32_t OglCacheTextures(void) {
    ...
    m.AddGauge(
        ...
        "                    ",
        -1,
        gameData.segData.nSegments + gameData.objData.nLastObject[0] + 4);
    nCacheSeg = 0;
    nCacheObj = -3;
    do {
...
int32_t OglCacheLevelTextures(void) {
    ...
    for (bD1 = 0; bD1 <= gameStates.app.bD1Data; bD1++) {
        for (i = 0, pEffectInfo = gameData.effectData.effects[bD1].Buffer(); i < gameData.effectData.nEffects[bD1];
             i++, pEffectInfo++) {
