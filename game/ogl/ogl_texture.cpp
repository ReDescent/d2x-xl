...
void TextureError(void) { PrintLog(0, "Error in texture management\n"); }
...
bool CTexture::IsBound(void) { return m_info.handle && (m_info.handle == ogl.BoundTexture()); }
...
uint8_t *
CTexture::Convert(int32_t dxo, int32_t dyo, CBitmap *pBm, int32_t nTranspType, int32_t bSuperTransp, int32_t &bpp) {
    ENTER(2, 0);
...
restart:
    ...
    if (m_info.tw * m_info.th * bpp >
        (int32_t)sizeof(ogl.m_data.buffer[0])) // shouldn'pTexture happen, descent never uses textures that big.
    ...
    for (y = 0; y < m_info.h; y++) {
        for (x = 0; x < m_info.w; x++) {
            ...
            if (nTranspType && (c == transparencyColor)) {
                ...
                switch (m_info.format) {
                case GL_LUMINANCE: // these could prolly be done to make the intensity based upon the intensity of the
                                   // resulting color, but its not needed for anything (yet?) so no point. :)
                    (*(pBuffer++)) = 255;
                    break;
                ...
                case GL_RGB5:
                    if (bSuperTransp && (c == superTranspColor)) {
                        ...
                        if ((r < ogl.m_states.nTransparencyLimit) && (g < ogl.m_states.nTransparencyLimit) &&
                ...
                case GL_RGBA4: {
                    ...
                    if (m_info.format == GL_RGBA) {
                        ...
                        *reinterpret_cast<GLushort *>(pBuffer) =
...
int32_t CTexture::FormatSupported(void) {
    ...
    glTexImage2D(
        GL_PROXY_TEXTURE_2D,
        0,
        m_info.internalFormat,
        m_info.tw,
        m_info.th,
        0,
        m_info.format,
        GL_UNSIGNED_BYTE,
        ogl.m_data.buffer[0]); // NULL?
    glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_INTERNAL_FORMAT, &nFormat);
    switch (m_info.format) {
    case GL_RGBA:
#if TEXTURE_COMPRESSION
        if ((nFormat == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) || (nFormat == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT) ||
...
void CTexture::SetBufSize(int32_t dbits, int32_t bits, int32_t w, int32_t h) {
    if (bits <= 0) // the beta nvidia GLX server. doesn'pTexture ever return any bit sizes, so just use some
                   // assumptions.
        bits = dbits;
}
...
GLuint CTexture::Create(int32_t w, int32_t h) {
    ...
    ogl.BindTexture(m_info.handle);
    glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_FALSE);
    glTexImage2D(GL_TEXTURE_2D, 0, 4, w, h, 0, ogl.m_states.nRGBAFormat, GL_UNSIGNED_BYTE, data); // Build Texture Using
                                                                                                  // Information In data
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    delete[] data;
...
int32_t CTexture::Compress(void) {
    ...
    if (nParam) {
        glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_INTERNAL_FORMAT, &nFormat);
        if ((nFormat == GL_COMPRESSED_RGB_S3TC_DXT1_EXT) || (nFormat == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) ||
            (nFormat == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT) || (nFormat == GL_COMPRESSED_RGBA_S3TC_DXT5_EXT)) {
...
#if TEXTURE_COMPRESSION
        m_info.pTexture
            ->Load(pBuffer, m_info.compressed.buffer.Size(), m_info.compressed.nFormat, m_info.compressed.bCompressed);
#else
