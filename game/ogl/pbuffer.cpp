...
void CPBO::Init(void) { memset(&m_info, 0, sizeof(m_info)); }
...
int32_t CPBO::Create(int32_t nWidth, int32_t nHeight) {
    ...
    static int32_t pbAttribs[] =
        {WGL_TEXTURE_FORMAT_ARB, WGL_TEXTURE_RGBA_ARB, WGL_TEXTURE_TARGET_ARB, WGL_TEXTURE_2D_ARB, 0};
    ...
    int32_t pbAttribs[] = {GLX_PBUFFER_WIDTH, 0, GLX_PBUFFER_HEIGHT, 0, GLX_LARGEST_PBUFFER, 0, None};
    ...
    if (!(m_info.hRC = glXCreateContext(hGlDC, vi, hGlRC, GL_TRUE))) { // Share display lists and textures with the
                                                                       // regular window
        glXDestroyPbuffer(m_info.hDC, m_info.hBuf);
        XFree(pfd);
        XFree(vi);
...
void CPBO::Setup(void) {
    ...
    if (ogl.m_features.bRenderToTexture /*.Apply ()*/) {
        ...
        ogl.m_features.bRenderToTexture = wglCreatePbufferARB && wglGetPbufferDCARB && wglReleasePbufferDCARB &&
                                          wglDestroyPbufferARB && wglQueryPbufferARB && wglChoosePixelFormatARB &&
                                          wglMakeContextCurrentARB && wglBindTexImageARB && wglReleaseTexImageARB;
#else
    ...
    PrintLog(
        (ogl.m_features.bRenderToTexture == 1) ? "Rendering to pixel buffers is available\n"
