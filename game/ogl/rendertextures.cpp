...
GLuint COGL::CreateDepthTexture(int32_t nTMU, int32_t bFBO, int32_t nType, int32_t nWidth, int32_t nHeight) {
    ...
    if (nType == 1) // depth + stencil
        glTexImage2D(
            ...
            (nWidth > 0) ? nWidth : DrawBufferWidth(),
            (nHeight > 0) ? nHeight : DrawBufferHeight(),
            0,
		...
		if (ogl.StereoDevice () == -GLASSES_SHUTTER_NVIDIA)
            ...
            ogl.SetReadBuffer(
                ...
                (gameStates.render.bRenderIndirect > 0) ||
                    gameStates.render.cameras
                        .bActive); // side effect: If the current render target is an FBO, it will be activated!
            SaveViewport();
            ...
            glCopyTexSubImage2D(
                ...
                gameData.renderData.screen.Height());
            RestoreViewport();
