...
int32_t SdlGlVideoModeOK(int32_t w, int32_t h) { return 1; }
...
int32_t SdlGlInitWindow(int32_t w_init, int32_t h_init, int32_t bForce) {
    ...
    if (ogl.m_states.bInitialized) {
        ...
        if ((w != ogl.m_states.nCurWidth) || (h != ogl.m_states.nCurHeight) ||
            (ogl.m_states.bCurFullScreen != ogl.m_states.bFullScreen)) {
            textureManager.Destroy(); // if we are or were fullscreen, changing vid mode will invalidate current
                                      // textures
            bRebuild = 1;
        }
    }
...
void SdlGlClose(void) { SdlGlDestroyWindow(); }
