...
void COGL::FlushShadowMaps(int32_t nEffects) {
    ...
    float windowScale[2] = {
        1.0f / float(gameData.renderData.screen.Width()),
        1.0f / float(gameData.renderData.screen.Height())};
...
const char *shadowMapFS =
    ...
    if (gameOpts->render.bUseShaders && m_features.bShaders.Available()) {
        ...
        gameStates.render.textures.bHaveShadowMapShader =
            (0 <= shaderManager.Build(shadowShaderProg, shadowMapFS, shadowMapVS));
