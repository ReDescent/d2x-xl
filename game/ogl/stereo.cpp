...
class CDefaultDistortionConfig : public OVR::Util::Render::DistortionConfig {
    public:
    CDefaultDistortionConfig() : OVR::Util::Render::DistortionConfig(1.0f, 0.22f, 0.24f, 0.0f) {
...
static bool RiftWarpFrame(const OVR::Util::Render::DistortionConfig *pDistortion, int32_t nEye) {
    ...
    shaderManager.Set(
        "LensCenter",
        x + (w + (nEye ? -distortion.XCenterOffset : distortion.XCenterOffset) * 0.5f) * 0.5f,
        y + h * 0.5f);
...
const char *enhance3DFS[2][2][3] = {
    ...
    {// no color adjustment
     {// amber/blue
      ...
      "}",
      // red/cyan
      // de-ghosting by reducing red or cyan brightness if they exceed a certain threshold compared to the pixels total
      // brightness
      "uniform sampler2D leftFrame, rightFrame;\r\n"
      "uniform vec2 strength;\r\n"
      "void main() {\r\n"
...
static const char *riftWarpVS = "void main()\n"
                                "{\n"
                                "gl_Position = ftransform ();\n"
                                "gl_TexCoord [0]=gl_MultiTexCoord0;"
                                "}\n";
...
void COGL::InitEnhanced3DShader(void) {
    if (gameOpts->render.bUseShaders && m_features.bShaders.Available()) {
        ...
        gameStates.render.textures.bHaveEnhanced3DShader =
            (0 <= shaderManager.Build(duboisShaderProg, duboisFS, enhance3DVS));
        ...
        for (int32_t h = 0; h < 2; h++) {
            for (int32_t i = 0; i < 2; i++) {
                for (int32_t j = 0; j < 3; j++) {
                    gameStates.render.textures.bHaveEnhanced3DShader =
                        (0 <= shaderManager.Build(enhance3DShaderProg[h][i][j], enhance3DFS[h][i][j], enhance3DVS));
                    if (!gameStates.render.textures.bHaveEnhanced3DShader) {
                        DeleteEnhanced3DShader();
                        gameOpts->render.stereo.nGlasses = GLASSES_NONE;
                        return;
    ...
    if (m_data.xStereoSeparation > 0) {
        ...
        if ((nDevice > 0) && (nDevice <= 3) && ((h < 4) || (nDevice == 2))) {
            GLhandleARB shaderProg = GLhandleARB(
                shaderManager.Deploy((h == 4) ? duboisShaderProg : enhance3DShaderProg[h > 0][i][--nDevice]));
            if (shaderProg) {
                ...
                if (h < 4) {
                    if (h)
                        glUniform2fv(
                            glGetUniformLocation(shaderProg, "strength"),
                            1,
                            reinterpret_cast<GLfloat *>(&nDeghostThresholds[gameOpts->render.stereo.bDeghost]));
                    if (i)
