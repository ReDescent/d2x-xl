...
//##void CollideFireballAndWall (CObject* fireball, fix xHitSpeed, int16_t nHitSeg, int16_t nHitWall, CFixVector&
// vHitPt) {
//##	return;
//##}
...
void CObject::CollideRobotAndWall(fix xHitSpeed, int16_t nHitSeg, int16_t nHitSide, CFixVector &vHitPt) {
    ...
    if ((info.nId != ROBOT_BRAIN) && (cType.aiInfo.behavior != AIB_RUN_FROM) &&
        !(pRobotInfo && pRobotInfo->companion) && (cType.aiInfo.behavior != AIB_SNIPE))
        RETURN;
...
//##void CollideHostageAndWall (CObject* hostage, fix xHitSpeed, int16_t nHitSeg, int16_t nHitSide,   CFixVector&
// vHitPt) {
//##	return;
//##}
...
void CObject::ApplyForceDamage(fix vForce, CObject *pOtherObj) {
...
#else
            if (!(gameStates.app.bGameSuspended & SUSP_ROBOTS) && result &&
                (pOtherObj->cType.laserInfo.parent.nSignature == gameData.objData.pConsole->info.nSignature))
#endif
    ...
    case OBJ_PLAYER:
        //	If colliding with a claw nType pRobot, do xDamage proportional to gameData.timeData.xFrame because you can
        // Collide with those 	bots every frame since they don't move.
    ...
    if (!(mType.physInfo.flags & PF_PERSISTENT)) {
        if (info.nType == OBJ_PLAYER) {
            if (pOtherObj->info.nType == OBJ_MONSTERBALL) {
                ...
                mq =
                    float(pOtherObj->mType.physInfo.mass) / (float(mType.physInfo.mass) * float(nMonsterballPyroForce));
                vForce *= mq;
                ApplyForce(vForce);
            } else {
                vForce *= 0.25f;
            ...
            if (info.nType == OBJ_MONSTERBALL) {
                ...
                if (pOtherObj->info.nType == OBJ_PLAYER) {
                    ...
                    mq = float(pOtherObj->mType.physInfo.mass) / float(mType.physInfo.mass) *
                         float(nMonsterballPyroForce) / 10.0f;
                } else {
...
void CObject::Bump(CObject *pOtherObj, CFixVector vForce, CFixVector vRotForce, int32_t bDamage) {
    ...
    if (info.nType == OBJ_PLAYER) {
        ...
        gameData.hoardData.nLastHitter =
            (pOtherObj->info.nType == OBJ_PLAYER) ? OBJ_IDX(pOtherObj) : pOtherObj->cType.laserInfo.parent.nObject;
...
inline void CBumpForces::Compute(CFixVector &vDist, CFixVector *vNormal, CObject *pObj) {
    ...
    if (m_vVel.IsZero())
        ...
        fix dot =
            CFixVector::Dot(vVelNorm, vDistNorm); // angle between objects movement vector and vector to other object
        if (dot < 0) // moving parallel to or away from other object
            ...
            m_vForce =
                ...
                FixMul(
                    dot,
                    mag); // scale objects movement vector with the angle to calculate the impulse on the other object
...
int32_t
BumpTwoObjects(CObject *pThis, CObject *pOther, int32_t bDamage, CFixVector &vHitPt, CFixVector *vNormal = NULL) {
    ENTER(1, 0);
    CObject *t;
...
if (CFixVector::Dot (f0.m_vVel, f1.m_vVel) <= 0)
    ...
    if (!CollisionModel() &&
        ...
        ((pOther->info.nType == OBJ_PLAYER) || (pOther->info.nType == OBJ_ROBOT) ||
         (pOther->info.nType == OBJ_REACTOR))) {
        fix dist = vDist.Mag();
    ...
    if ((EGI_FLAG(bUseHitAngles, 0, 0, 0) || (pOther->info.nType == OBJ_MONSTERBALL)) ||
        pThis->IsStatic()) { //&& !pThis->IsStatic ()) {
...
void CObject::ScrapeOnWall(int16_t nHitSeg, int16_t nHitSide, CFixVector &vHitPt) {
    ...
    if ((info.nType == OBJ_PLAYER) && (info.nId == N_LOCALPLAYER)) {
        ...
        if ((gameData.timeData.xGame >
             xLastVolatileScrapeSoundTime + (nType ? I2X(1) / (Rand(4) + 1) : I2X(1) + I2X(2) / (Rand(8) + 1))) ||
            ...
            if (!nType) {
                if (gameOpts->sound.bScrape) {
                    ...
                    audio.CreateObjectSound(
                        ...
                        LOCALOBJECT->Index(),
                        0,
                        /*gameOpts->sound.xCustomSoundVolume*/ I2X(1) / 2,
                        I2X(256),
                        -1,
                        -1,
...
//	Copied from laser.c!
#define DESIRED_OMEGA_DIST \
    (I2X(5)) //	This is the desired distance between blobs.  For distances > MIN_OMEGA_BLOBS*DESIRED_OMEGA_DIST, but not
             // very large, this will apply.
...
int32_t CObject::CollideWeaponAndWall(fix xHitSpeed, int16_t nHitSeg, int16_t nHitSide, CFixVector &vHitPt) {
    ...
    if (pSide && ((gameData.pigData.tex.pTexMapInfo[pSide->m_nBaseTex].flags & TMI_VOLATILE) ||
        ...
        //	New by MK: If powerful splash damager, explode with splash damage, not due to lava, fixes megas being wimpy
        // in lava.
        if (xDamageRadius >= VOLATILE_WALL_DAMAGE_RADIUS / 2)
            ExplodeSplashDamageWeapon(vHitPt);
        else
            CreateSplashDamageExplosion(
                this,
                nHitSeg,
                vHitPt,
                vHitPt,
                WI_ImpactSize(info.nId) + VOLATILE_WALL_IMPACT_SIZE,
                tAnimationInfo,
                nStrength / 4 + VOLATILE_WALL_EXPL_STRENGTH,
                xDamageRadius + VOLATILE_WALL_DAMAGE_RADIUS,
                nStrength / 2 + VOLATILE_WALL_DAMAGE_FORCE,
                cType.laserInfo.parent.nObject);
        Die(); // make flares die in lava
    } else if (
        pSide && ((gameData.pigData.tex.pTexMapInfo[pSide->m_nBaseTex].flags & TMI_WATER) ||
        ...
        if (pWeaponInfo->matter) {
            ...
            if (xDamageRadius) {
                audio.CreateObjectSound(
                    IsSplashDamageWeapon() ? SOUND_BADASS_EXPLOSION_WEAPON : SOUND_STANDARD_EXPLOSION,
                ...
                CreateSplashDamageExplosion(
                    ...
                    vHitPt,
                    vHitPt,
                    WI_ImpactSize(info.nId) / 2,
                    pWeaponInfo->nRobotHitAnimation,
                    nStrength / 4,
                    xDamageRadius,
                    nStrength / 2,
                    cType.laserInfo.parent.nObject);
                ...
                CreateExplosion(
                    info.nSegment,
                    info.position.vPos,
                    WI_ImpactSize(info.nId),
                    pWeaponInfo->nWallHitAnimation);
        } else {
        ...
        if (!bBounce) {
            ...
            if ((cType.laserInfo.parent.nType != OBJ_PLAYER) || (((wallType == WHP_NOT_SPECIAL)) && !bBlewUp))
    ...
    if ((cType.laserInfo.parent.nType == OBJ_PLAYER) || bEscort) {
        if (!(info.nFlags & OF_SILENT) && (cType.laserInfo.parent.nObject == LOCALPLAYER.nObject))
        ...
        if ((info.nId == FLARE_ID) &&
            (!pSide || (gameData.pigData.tex.pTexMapInfo[pSide->m_nBaseTex].flags & TMI_FORCE_FIELD))) {
            Die();
        }
        if (!(info.nFlags & OF_SILENT)) {
            ...
            case WHP_NOT_SPECIAL:
                ...
                // audio.CreateSegmentSound (pWeaponInfo->nWallHitSound, info.nSegment, 0, &info.position.vPos, 0, I2X
                // (1));
                break;
...
//##void CollidePowerupAndWall (CObject* powerup, fix xHitSpeed, int16_t nHitSeg, int16_t nHitWall,   CFixVector&
// vHitPt) {
//##	return;
//##}
...
int32_t CObject::CollideDebrisAndWall(fix xHitSpeed, int16_t nHitSeg, int16_t nHitWall, CFixVector &vHitPt) {
    ...
    if (gameOpts->render.nDebrisLife) {
        CFixVector vDir = mType.physInfo.velocity, vNormal = SEGMENT(nHitSeg)->m_sides[nHitWall].m_normals[0];
...
int32_t CObject::CollideRobotAndRobot(CObject *other, CFixVector &vHitPt, CFixVector *vNormal) {
    //		robot1-OBJECTS, X2I (robot1->info.position.vPos.x), X2I (robot1->info.position.vPos.y), X2I
    //(robot1->info.position.vPos.z), 		robot2-OBJECTS, X2I (robot2->info.position.vPos.x), X2I
    //(robot2->info.position.vPos.y), X2I (robot2->info.position.vPos.z), 		X2I (vHitPt->x), X2I (vHitPt->y), X2I
    //(vHitPt->z));
    ...
    if (!IsStatic()) {
        ...
        if (nCollisionSeg != -1)
            CreateExplosion(
                nCollisionSeg,
                vHitPt,
                gameData.weaponData.info[0][0].xImpactSize,
                gameData.weaponData.info[0][0].nWallHitAnimation);
    ...
    if (extraGameInfo[0].nBossCount[0] && !gameData.reactorData.bDestroyed) {
        ...
        if (pReactor && !(pReactor->info.nFlags & (OF_EXPLODING | OF_DESTROYED))) {
            audio.CreateSegmentSound(
                ...
                pReactor->info.nSegment,
                0,
                pReactor->info.position.vPos);
            pReactor->Explode(0);
        }
        RETVAL(1)
    }
...
int32_t CObject::CollidePlayerAndMarker(CObject *pMarker, CFixVector &vHitPt, CFixVector *vNormal) {
    ...
    if (info.nId == N_LOCALPLAYER) {
        ...
        if (IsMultiGame && !IsCoopGame)
            bDrawn = HUDInitMessage(
                TXT_MARKER_PLRMSG,
                PLAYER(pMarker->info.nId / 2).callsign,
                markerManager.Message(pMarker->info.nId));
        else {
            if (*markerManager.Message(pMarker->info.nId))
                bDrawn =
                    HUDInitMessage(TXT_MARKER_IDMSG, pMarker->info.nId + 1, markerManager.Message(pMarker->info.nId));
            else
...
int32_t CObject::ApplyDamageToRobot(fix xDamage, int32_t nKillerObj) {
    ...
    if (bIsBoss) {
        if ((missionManager.nCurrentMission == missionManager.nBuiltInMission[0]) &&
            (missionManager.nCurrentLevel == missionManager.nLastLevel) && (info.xShield < 0) &&
            (extraGameInfo[0].nBossCount[0] == 1)) {
            if (IsMultiGame) {
...
int32_t DoBossWeaponCollision(CObject *pRobot, CObject *pWeapon, CFixVector &vHitPt) {
    ...
    if (bossProps[gameStates.app.bD1Mission][d2BossIndex].bInvulSpot) {
        ...
        CFixVector::Normalize(tvec1); //	Note, if BOSS_INVULNERABLE_DOT is close to I2X (1) (in magnitude), then
                                      // should probably use non-quick version.
        ...
        if (dot > gameData.physicsData.xBossInvulDot) {
            ...
            if ((pWeapon->info.nType == OBJ_WEAPON) && !WI_Matter(pWeapon->info.nId)) {
                int16_t nClone = CreateObject(
                    pWeapon->info.nType,
                    pWeapon->info.nId,
                    -1,
                    pWeapon->info.nSegment,
                    pWeapon->info.position.vPos,
                    pWeapon->info.position.mOrient,
                    pWeapon->info.xSize,
                    pWeapon->info.controlType,
                    pWeapon->info.movementType,
                    pWeapon->info.renderType);
                if (nClone != -1) {
        ...
        (bKinetic && bossProps[gameStates.app.bD1Mission][d2BossIndex].bInvulKinetic) ||
...
int32_t CObject::CollideWeaponAndRobot(CObject *pRobot, CFixVector &vHitPt, CFixVector *vNormal) {
    ...
    if (!pRobotInfo && (pRobot->Type() != OBJ_CAMBOT)) {
        PrintLog(
            0,
            "invalid robot reference in CollideWeaponAndRobot (type = %d, id = %d)\n",
            pRobot->Type(),
            pRobot->Id());
        RETVAL(1)
    }
    ...
    bool bAttackRobots = pParent ? pParent->AttacksRobots() ||
                                       (EGI_FLAG(bRobotsHitRobots, 0, 0, 0) && gameStates.app.cheats.bRobotsKillRobots)
                                 : false;
    ...
    if (((cType.laserInfo.parent.nType == OBJ_PLAYER) || bAttackRobots) && !(pRobot->info.nFlags & OF_EXPLODING)) {
        ...
        if (pRobotInfo && (pRobotInfo->nExp1VClip > -1))
            pExplObj =
                CreateExplosion(info.nSegment, vHitPt, (3 * pRobot->info.xSize) / 8, (uint8_t)pRobotInfo->nExp1VClip);
        else {
            CWeaponInfo *pWeaponInfo = WEAPONINFO(info.nId);
            if (pWeaponInfo && (pWeaponInfo->nRobotHitAnimation > -1))
                pExplObj = CreateExplosion(
                    info.nSegment,
                    vHitPt,
                    WI_ImpactSize(info.nId),
                    (uint8_t)pWeaponInfo->nRobotHitAnimation);
        }
        if (pExplObj)
        ...
        if (!(info.nFlags & OF_HARMLESS)) {
            ...
            if (info.nId == GAUSS_ID) {
                if (pRobotInfo && pRobotInfo->bossFlag)
                    xDamage = (xDamage * (2 * DIFFICULTY_LEVEL_COUNT - gameStates.app.nDifficultyLevel)) /
                              (2 * DIFFICULTY_LEVEL_COUNT);
            ...
            if (!pRobot->ApplyDamageToRobot(xDamage, cType.laserInfo.parent.nObject))
                ...
                pRobotInfo && !(gameStates.app.bGameSuspended & SUSP_ROBOTS) &&
                (cType.laserInfo.parent.nSignature == gameData.objData.pConsole->info.nSignature)) {
#endif
                cockpit->AddPointsToScore(pRobotInfo->scoreValue);
        ...
        if (pRobot && (!pRobotInfo || !(pRobotInfo->companion || pRobotInfo->bossFlag)) && (info.nId == GAUSS_ID)) {
            ...
            if (aip->SKIP_AI_COUNT * gameData.timeData.xFrame < I2X(1)) {
                aip->SKIP_AI_COUNT++;
                pRobot->mType.physInfo.rotThrust.v.coord.x =
                    FixMul(SRandShort(), gameData.timeData.xFrame * aip->SKIP_AI_COUNT);
                pRobot->mType.physInfo.rotThrust.v.coord.y =
                    FixMul(SRandShort(), gameData.timeData.xFrame * aip->SKIP_AI_COUNT);
                pRobot->mType.physInfo.rotThrust.v.coord.z =
                    FixMul(SRandShort(), gameData.timeData.xFrame * aip->SKIP_AI_COUNT);
...
void CObject::ApplyDamageToPlayer(CObject *pAttackerObj, fix xDamage) {
    ...
    if (!pAttackerObj)
        ...
        pAttacker =
            (pAttackerObj->info.nType == OBJ_PLAYER) ? gameData.multiplayer.players + pAttackerObj->info.nId : NULL;
        if (pAttacker)
...
int32_t CObject::CollideWeaponAndPlayer(CObject *pPlayerObj, CFixVector &vHitPt, CFixVector *vNormal) {
    ...
    if ((info.nId == SMARTMINE_ID) && (OBJ_IDX(pPlayerObj) == cType.laserInfo.parent.nObject) &&
    ...
    CObject *pParent;
    if (!WI_DamageRadius(info.nId) && !(info.nFlags & OF_HARMLESS) &&
        (pParent = OBJECT(cType.laserInfo.parent.nObject)))
...
int32_t CObject::CollideRobotAndObjProducer(void) {
    ...
    if (pRobotInfo->nExp1VClip > -1)
        CreateExplosion(
            (int16_t)info.nSegment,
            info.position.vPos,
            3 * info.xSize / 8,
            (uint8_t)pRobotInfo->nExp1VClip);
    CFixVector vExitDir;
    vExitDir.SetZero();
    for (int16_t nSide = 0; nSide < SEGMENT_SIDE_COUNT; nSide++)
...
int32_t CObject::CollideWeaponAndWeapon(CObject *other, CFixVector &vHitPt, CFixVector *vNormal) {
    ...
    if ((id1 == PROXMINE_ID || id2 == PROXMINE_ID) && IsMultiGame && !COMPETITION && EGI_FLAG(bSmokeGrenades, 0, 0, 0))
        RETVAL(1)
    if (((id1 == OMEGA_ID) && !OkToDoOmegaDamage(this)) || ((id2 == OMEGA_ID) && !OkToDoOmegaDamage(other)))
        RETVAL(1)
