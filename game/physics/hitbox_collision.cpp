...
static int32_t FindLinePlaneIntersection(
...
static int32_t FindQuadQuadIntersectionSub(
    CFixVector &intersection,
    CFixVector *vQuad,
    CFixVector *vPlane,
    CFixVector *vNormal,
...
int32_t FindLineTriangleIntersection(
    CFixVector &intersection,
    uint16_t *triangleVerts,
    CFixVector *triangleNormal,
    CFixVector *p0,
    CFixVector *p1) {
    CFixVector vHit;
...
int32_t FindTriangleQuadIntersection(
    CFixVector &intersection,
    uint16_t *triangleVerts,
    CFixVector *triangleNormal,
    CFixVector *vQuad,
    ...
    intersection.SetZero();
    for (i = 0; i < 4; i++)
        if (FindLineTriangleIntersection(vHit, triangleVerts, triangleNormal, vQuad + i, vQuad + ((i + 1) % 4)) <
            0x7fffffff) {
#if DBG
...
CSegMasks CheckFaceHitboxCollision(
    ...
    for (int32_t i = 0; i < 2; i++) {
        for (int32_t j = iModel; j <= nModels; j++) {
            int32_t h = FindTriangleHitboxIntersection(
                intersection,
                normal,
                pSide->m_vertices + 3 * i,
                pSide->m_normals + i,
                &hb[j].box,
                p0,
                dMin);
...
int32_t UseHitbox(CObject *pObj) {
    return !gameStates.app.bNostalgia && (pObj->info.renderType == RT_POLYOBJ) &&
           (pObj->rType.polyObjInfo.nModel >= 0) &&
