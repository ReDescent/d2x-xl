...
void DoPhysicsAlignObject(CObject *pObj) {
    ...
    if (gameOpts->gameplay.nAutoLeveling == 1) // old way: used floor's Normal as upvec
        desiredUpVec =
            pSeg->m_sides[3].FaceCount() ? pSeg->m_sides[3].m_normals[2] : pSeg->m_sides[nBestSide].m_normals[2];
    else if (gameOpts->gameplay.nAutoLeveling == 2) { // new CPlayerData leveling code: use Normal of CSide closest to
                                                      // our up vec
...
void CPhysSimData::Setup(void) {
    ...
    bUseHitbox = bStopped = bBounced = bIgnoreObjFlag = 0;
    nTries = 0;
    ...
    bUseHitbox = 0;
#endif
    bScaleSpeed = !(gameStates.app.bNostalgia || bInitialize) &&
                  (pObj->IsMissile() && (pObj->Id() != EARTHSHAKER_MEGA_ID) && (pObj->Id() != ROBOT_SHAKER_MEGA_ID));
...
void CPhysSimData::GetPhysSegs(void) {
    ENTER(0, 0);
    if (bGetPhysSegs) {
        if (gameData.physicsData.nSegments &&
            (gameData.physicsData.segments[gameData.physicsData.nSegments - 1] == hitResult.segList[0]))
        ...
        if (i > 0) {
            ...
            if (i > 0)
                memcpy(
                    gameData.physicsData.segments + gameData.physicsData.nSegments,
                    hitResult.segList,
                    i * sizeof(gameData.physicsData.segments[0]));
...
void CObject::SetupHitQuery(CHitQuery &hitQuery, int32_t nFlags, CFixVector *vNewPos) {
    ...
    hitQuery.radP0 = hitQuery.radP1 = info.xSize;
...
int32_t CObject::HandleWallCollision(CPhysSimData &simData) {
    ...
    if (gameStates.render.bHaveSkyBox && (info.nType == OBJ_WEAPON) && (simData.hitResult.nSegment >= 0)) {
        if (SEGMENT(simData.hitResult.nSegment)->m_function ==
            SEGMENT_FUNC_SKYBOX) { // allow missiles and shots to leave the level and enter a skybox
        ...
        } else if (SEGMENT(simData.hitResult.nSideSegment)
                       ->CheckForTranspPixel(
                           simData.hitResult.vPoint,
                           simData.hitResult.nSide,
                           simData.hitResult.nFace)) {
...
int32_t CObject::ProcessWallCollision(CPhysSimData &simData) {
    ...
    fix xWallPart =
        (gameData.collisionData.hitResult.nNormals
             ? CFixVector::Dot(simData.vMoved, simData.hitResult.vNormal) / gameData.collisionData.hitResult.nNormals
             : 0);
    ...
    // if (xWallPart < 0)
    {
        ...
        if ((simData.xMovedTime > 0) && ((xHitSpeed = -FixDiv(xWallPart, simData.xMovedTime)) > 0))
            CollideObjectAndWall(
                xHitSpeed,
                simData.hitResult.nSideSegment,
                simData.hitResult.nSide,
                simData.hitResult.vPoint);
    ...
    int32_t bForceFieldBounce =
        (gameData.pigData.tex
             .pTexMapInfo[SEGMENT(simData.hitResult.nSideSegment)->m_sides[simData.hitResult.nSide].m_nBaseTex]
             .flags &
         TMI_FORCE_FIELD) != 0;
...
void CObject::FixPosition(CPhysSimData &simData) {
    ...
    if (((simData.hitResult.nType == HIT_WALL) || (simData.hitResult.nType == HIT_BAD_P0)) &&
        !(simData.bSpeedBoost || simData.bStopped || simData.bBounced)) { // Set velocity from actual movement
    ...
    if (((info.nType == OBJ_PLAYER) || (info.nType == OBJ_ROBOT)) && (info.nSegment != simData.nStartSeg) &&
        ...
        if (nSide != -1) {
            if (!(SEGMENT(simData.nStartSeg)->IsPassable(nSide, (info.nType == OBJ_PLAYER) ? this : NULL) &
                  WID_PASSABLE_FLAG)) {
                // bump object back
                CSide *pSide = SEGMENT(simData.nStartSeg)->m_sides + nSide;
                if (simData.nStartSeg == -1)
                    Error("simData.nStartSeg == -1 in physics");
                fix dist = simData.vStartPos.DistToPlane(
                    pSide->m_normals[0],
                    gameData.segData.vertices[pSide->m_nMinVertex[0]]);
    ...
    if (((Index() != LOCALPLAYER.nObject) || !automap.Active()) && (info.nSegment >= 0) &&
        SEGMENT(info.nSegment)->Masks(info.position.vPos, 0).m_center) {
        if (FindSegment() == -1) {
            int32_t n;
...
int32_t CObject::UpdateSimTime(CPhysSimData &simData) {
    ...
    if ((simData.hitResult.nType == HIT_WALL) &&
        (CFixVector::Dot(vMoveNormal, simData.vOffset) < 0)) { // moved backwards
	...
	UnstickFromWall (simData, Velocity () /*simData.velocity*/);
        ...
        simData.xAttemptedDist = simData.vOffset.Mag();
        simData.xSimTime =
            FixMulDiv(simData.xSimTime, simData.xAttemptedDist - simData.xMovedDist, simData.xAttemptedDist);
        simData.xMovedTime = simData.xOldSimTime - simData.xSimTime;
        if ((simData.xSimTime < 0) || (simData.xSimTime > simData.xOldSimTime)) {
            simData.xSimTime = simData.xOldSimTime;
            simData.xMovedTime = 0;
...
void CObject::DoPhysicsSim(void) {
    ...
    CPhysSimData simData(OBJ_IDX(this)),
        simData2(
            OBJ_IDX(this)); // must be called after initializing gameData.physicsData.xTime! Will call simData.Setup ()!
    ...
    if (DoPhysicsSimRot() && ((info.nType == OBJ_PLAYER) || (info.nType == OBJ_ROBOT)) && CollisionModel()) {
        ...
        SetupHitQuery(
            simData.hitQuery,
            FQ_CHECK_OBJS | FQ_IGNORE_POWERUPS | ((info.nType == OBJ_WEAPON) ? FQ_TRANSPOINT : 0));
...
redoPhysSim:
    ...
    for (;;) { // Move the object
        ...
        do {
            ...
            SetupHitQuery(
                simData.hitQuery,
                FQ_CHECK_OBJS | ((info.nType == OBJ_WEAPON) ? FQ_TRANSPOINT : 0) |
                    (simData.bGetPhysSegs ? FQ_GET_SEGLIST : 0),
                &simData.vNewPos);
            simData.hitResult.nType = FindHitpoint(simData.hitQuery, simData.hitResult, -1);
...
void CObject::TurnTowardsVector(CFixVector vGoal, fix rate) {
    ...
    if (abs(delta_p) < I2X(1) / 16)
        delta_p *= 4;
    if (abs(delta_h) < I2X(1) / 16)
        delta_h *= 4;
    if (!IsMultiGame) {
        int32_t i = (this != gameData.objData.pConsole) ? 0 : 1;
        if (gameStates.gameplay.slowmo[i].fSpeed != 1) {
...
void CObject::DoPhysicsSimOld(void) {
    ...
    if (DoPhysicsSimRot() && ((info.nType == OBJ_PLAYER) || (info.nType == OBJ_ROBOT)) && CollisionModel()) {
        ...
        simData.hitQuery.p0 = simData.hitQuery.p1 = &info.position.vPos;
        simData.hitQuery.nSegment = info.nSegment;
        simData.hitQuery.radP0 = simData.hitQuery.radP1 = info.xSize;
    ...
    do { // Move the object
        ...
        if (bNewPhysCode & 4) {
            ...
            float fScale =
                !(gameStates.app.bNostalgia || simData.bInitialize) &&
                        (IsMissile() && (info.nId != EARTHSHAKER_MEGA_ID) && (info.nId != ROBOT_SHAKER_MEGA_ID))
    ...
    retryMove:
        ...
        if (bNewPhysCode & 128) {
            ...
            if (simData.bGetPhysSegs) {
                if (gameData.physicsData.nSegments &&
                    (gameData.physicsData.segments[gameData.physicsData.nSegments - 1] == simData.hitResult.segList[0]))
                    gameData.physicsData.nSegments--;
                int32_t i = MAX_FVI_SEGS - gameData.physicsData.nSegments - 1;
                if (i > 0) {
        ...
        if (simData.hitResult.nType == HIT_WALL) {
            if (bNewPhysCode & 512)
                ...
                xWallPart = gameData.collisionData.hitResult.nNormals
                                ? CFixVector::Dot(simData.vMoved, simData.hitResult.vNormal) /
                                      gameData.collisionData.hitResult.nNormals
                                : 0;
                if (xWallPart && (simData.xMovedTime > 0) &&
                ...
                if (!(info.nFlags & OF_SHOULD_BE_DEAD) && (info.nType != OBJ_DEBRIS)) {
                    ...
                    /// Assert (gameStates.app.cheats.bBouncingWeapons || ((mType.physInfo.flags & (PF_STICK |
                    /// PF_BOUNCES)) != (PF_STICK | PF_BOUNCES)));	//can't be bounce and stick
                    bForceFieldBounce =
                        (gameData.pigData.tex
                             .pTexMapInfo
                                 [SEGMENT(simData.hitResult.nSideSegment)->m_sides[simData.hitResult.nSide].m_nBaseTex]
                             .flags &
                         TMI_FORCE_FIELD);
                    if (!bForceFieldBounce && (mType.physInfo.flags & PF_STICK)) { // stop moving
                        AddStuckObject(this, simData.hitResult.nSideSegment, simData.hitResult.nSide);
                        ...
                        if (bForceFieldBounce || (mType.physInfo.flags & PF_BOUNCES)) { // bounce off CWall
                            ...
                            if ((mType.physInfo.flags & (PF_BOUNCES | PF_BOUNCES_TWICE)) ==
                                (PF_BOUNCES | PF_BOUNCES_TWICE)) {
    ...
    if (bNewPhysCode & 2048)
        ...
        // avoid that the ship gets driven into the obstacle (most likely a wall, as that doesn't give in ;)
        if (((simData.hitResult.nType == HIT_WALL) || (simData.hitResult.nType == HIT_BAD_P0)) &&
            (simData.speedBoost.bBoosted < 1) && !simData.bStopped && !simData.bBounced)) {	//Set velocity from actual movement
