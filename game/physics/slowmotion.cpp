...
int32_t ToggleSlowMotion(void) {
    ...
    int32_t bSlowMotionOk = gameStates.app.cheats.bSpeed ||
                            ((LOCALPLAYER.Energy() > I2X(10)) && (LOCALPLAYER.flags & PLAYER_FLAGS_SLOWMOTION));
    int32_t bBulletTimeOk =
        bSlowMotionOk &&
        (gameStates.app.cheats.bSpeed || (LOCALPLAYER.flags & (PLAYER_FLAGS_SLOWMOTION | PLAYER_FLAGS_BULLETTIME)));
...
void SpeedupSound(void) {
    ...
    if (HaveSoundThread()) {
        ...
        if (tSlowDown)
            songManager.SetPos(
                tSlowDown - songManager.Start() +
                2 * (SDL_GetTicks() - tSlowDown) / gameOpts->gameplay.nSlowMotionSpeedup);
        else
...
void DoSlowMotionFrame(void) {
    ...
    for (i = 0; i < 2; i++) {
        if (gameStates.gameplay.slowmo[i].nState &&
            (gameStates.app.nSDLTicks[0] - gameStates.gameplay.slowmo[i].tUpdate > 25)) {
