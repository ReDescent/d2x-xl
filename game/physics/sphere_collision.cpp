...
uint32_t PointToFaceRelationf(CFloatVector &vRef, CFloatVector *vertList, int32_t nVerts, CFloatVector &vNormal) {
    ...
    int32_t nBiggest =
        (v.v.coord.x > v.v.coord.y) ? (v.v.coord.x > v.v.coord.z) ? 0 : 2 : (v.v.coord.y > v.v.coord.z) ? 1 : 2;
...
int32_t CheckLineToFaceRegular(
    CFixVector &vIntersection,
    CFixVector *p0,
    CFixVector *p1,
    fix rad,
...
fix CheckVectorObjectCollision(
    ...
    if (rad < 0)
        ...
        if (!pThisObj->IsStatic() && (pRobotInfo = ROBOTINFO(pThisObj)) && pRobotInfo->attackType)
            size = 3 * size / 4;
        // if obj is CPlayerData, and bumping into other CPlayerData or a weapon of another coop CPlayerData, reduce
        // radius
        if ((pThisObj->info.nType == OBJ_PLAYER) &&
            ((pOtherObj->info.nType == OBJ_PLAYER) || (IsCoopGame && (pOtherObj->info.nType == OBJ_WEAPON) &&
                                                       (pOtherObj->cType.laserInfo.parent.nType == OBJ_PLAYER))))
            size /= 2;
    }
    ...
    if ((CollisionModel(nCollisionModel) || pThisObj->IsStatic() || pOtherObj->IsStatic()) &&
        ...
        if (bThisPoly && bOtherPoly) {
            ...
            if (bThisPoly) {
                ...
                // intersects with the hitbox, check whether the radius line of *pThisObj intersects any of the
                // hitboxes.
                if (0x7fffffff == (dist = CheckVectorHitboxCollision(
                                       vHit,
                                       vNormal,
                                       p0,
                                       p1,
...
int32_t ComputeObjectHitpoint(CHitData &hitData, CHitQuery &hitQuery, int32_t nCollisionModel, int32_t nThread) {
    ...
    CObject *pThisObj = (hitQuery.nObject < 0) ? NULL : OBJECT(hitQuery.nObject), *pOtherObj;
    ...
    for (int32_t nSide = 0; nSide < 6; nSide++) {
        ...
        if (pWall && (pWall->IsPassable(NULL) & (WID_TRANSPARENT_WALL | WID_SOLID_WALL)))
            continue;
        for (i = 0; i < gameData.collisionData.nSegsVisited[nThread] &&
                    (nSegment != gameData.collisionData.segsVisited[nThread][i]);
             i++)
            ;
    ...
    restart:
        ...
        for (int16_t nObject = nFirstObj; nObject != -1; nObject = pOtherObj->info.nNextInSeg, nSegObjs--) {
            ...
            if ((nOtherType == OBJ_POWERUP) &&
                ((hitQuery.flags & FQ_IGNORE_POWERUPS) || (gameStates.app.bGameSuspended & SUSP_POWERUPS)))
                continue;
            ...
            // radius
                ...
                if (nOtherType == OBJ_ROBOT) {
                    ...
                    if (IsCoopGame && (nOtherType == OBJ_WEAPON) &&
                        (pOtherObj->cType.laserInfo.parent.nType == OBJ_PLAYER))
            ...
            if (nObject == nDbgObj)
                BRP;
#endif
            fix d = CheckVectorObjectCollision(
                curHit,
                hitQuery.p0,
                hitQuery.p1,
                nFudgedRad,
...
static inline int32_t PassThrough(CHitQuery &hitQuery, int16_t nSide, int16_t nFace, CFixVector &vHitPoint) {
    ...
    if (pChildSeg->HasBlockedProp() ||
        ((gameData.objData.speedBoost[hitQuery.nObject].bBoosted > 0) &&
         ((pSeg->m_function != SEGMENT_FUNC_SPEEDBOOST) || (pChildSeg->m_function == SEGMENT_FUNC_SPEEDBOOST))))
        return 1;
...
// first, see if vector hit any objects in this CSegment
    ...
    CSegment *pSeg = SEGMENT(hitQuery.nSegment);
    if ((hitQuery.nObject > -1) &&
        (gameData.objData.collisionResult[OBJECT(hitQuery.nObject)->info.nType][OBJ_WALL] == RESULT_NOTHING))
            ...
            for (iFace = 0; iFace < 2; iFace++, bit <<= 1) {
                ...
                if (PassThrough(hitQuery, nSide, iFace, curHit.vPoint)) {
...
#endif
                    for (i = 0; i < gameData.collisionData.nSegsVisited[nThread] &&
                                (subHitQuery.nSegment != gameData.collisionData.segsVisited[nThread][i]);
                         i++)
                        ;
                    if (i == gameData.collisionData.nSegsVisited[nThread]) { // haven't visited here yet
                        ...
                        gameData.collisionData.segsVisited[nThread][gameData.collisionData.nSegsVisited[nThread]++] =
                            subHitQuery.nSegment;
                        subHit.nType = ComputeHitpoint(
                            subHit,
                            subHitQuery,
                            subSegList,
                            &nSubSegments,
                        ...
                        if (subHit.nType != HIT_NONE) {
                            ...
                            if (d < dMin) {
                                ...
                                bestHit = subHit;
                                *nSegments +=
                                    CopySegList(segList, *nSegments, subSegList, nSubSegments, hitQuery.flags);
                            } else {
...
hitPointDone:
    ...
    if (bestHit.nType == HIT_NONE) { // didn't hit anything, return end pRef
        ...
        if (hitData.nSegment == -1)
            hitData.nSegment = (gameData.collisionData.hitResult.nAltSegment != -1)
...
int32_t FindHitpoint(CHitQuery &hitQuery, CHitResult &hitResult, int32_t nCollisionModel, int32_t nThread) {
    ...
    if ((curHit.nType == HIT_WALL) && (curHit.nSegment == -1))
        if ((gameData.collisionData.hitResult.nAltSegment != -1) &&
            !SEGMENT(gameData.collisionData.hitResult.nAltSegment)->Masks(curHit.vPoint, 0).m_center)
    ...
    if ((curHit.nSegment != -1) && (hitQuery.flags & FQ_GET_SEGLIST))
        if ((curHit.nSegment != hitResult.segList[hitResult.nSegments - 1]) && (hitResult.nSegments < MAX_FVI_SEGS - 1))
            ...
            for (iFace = 0; iFace < 2; iFace++, bit <<= 1) {
                if (faceMask & bit) { // on the back of this iFace
                    ...
                    if (nFaceHitType) { // through this CWall/door
                        ...
                        for (i = 0; (i < gameData.collisionData.nSegsVisited[0]) &&
                                    (nChild != gameData.collisionData.segsVisited[0][i]);
                             i++)
                            ;
        ...
        for (nSide = 0; nSide < SEGMENT_SIDE_COUNT; nSide++, pSide++) {
            ...
            for (nFace = 0; nFace < nFaceCount; nFace++, vertices += 3) {
                ...
                if ((nChildSeg == nDestSeg) &&
                    !((pWall = pSide->Wall()) &&
                      !(pWall->IsVolatile() || (pWall->IsPassable(NULL, false) & WID_TRANSPARENT_FLAG))))
            ...
            if (0 > nChildSeg) // solid wall
                continue;
            if ((pWall = pSide->Wall()) &&
                !(pWall->IsVolatile() || (pWall->IsPassable(NULL, false) & WID_TRANSPARENT_FLAG))) // impassable
                continue;
#if DBG_FACEGRID
...
int32_t UseSphere(CObject *pObj) {
    ...
    return gameStates.app.bNostalgia || (nType == OBJ_MONSTERBALL) || (nType == OBJ_HOSTAGE) ||
