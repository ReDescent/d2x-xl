...
int32_t CanSeeObject(int32_t nObject, int32_t bCheckObjs, int32_t nThread) {
    ENTER(0, nThread);
    CHitQuery hitQuery(
        bCheckObjs ? FQ_CHECK_OBJS | FQ_TRANSWALL | FQ_VISIBILITY : FQ_TRANSWALL | FQ_VISIBILITY,
...
int32_t ObjectToObjectVisibility(CObject *pViewer, CObject *pTarget, int32_t transType, float fov, int32_t nThread) {
    ...
    do {
        ...
        hitQuery.radP0 = hitQuery.radP1 = 0x10;
        ...
        fate = gameData.segData.SegVis(hitQuery.nSegment, pTarget->Segment())
                   ? FindHitpoint(hitQuery, hitResult, 0, nThread)
                   : HIT_WALL;
...
int32_t TargetInLineOfFire(void) {
    ...
    CFixVector vEndPos = gameData.objData.pViewer->info.position.vPos +
                         gameData.objData.pViewer->info.position.mOrient.m.dir.f * I2X(2000);
    CHitQuery hitQuery(
        FQ_CHECK_OBJS | FQ_VISIBLE_OBJS | FQ_IGNORE_POWERUPS | FQ_TRANSWALL | FQ_VISIBILITY,
        &gameData.objData.pViewer->info.position.vPos,
        &vEndPos,
        gameData.objData.pViewer->info.nSegment,
