...
int32_t CAutomap::Active(void) { return m_bActive || OBSERVING; }

//------------------------------------------------------------------------------

int32_t CAutomap::Texturing(void) { return OBSERVING ? 1 : gameOpts->render.automap.bTextured; }
            ...
            if ((i != N_LOCALPLAYER) && AM_SHOW_PLAYER(i)) {
                if (PLAYEROBJECT(i)->info.nType == OBJ_PLAYER) {
                    ...
                    CCanvas::Current()->SetColorRGBi(
                        RGBA_PAL2(playerColors[color].r, playerColors[color].g, playerColors[color].b));
                    if (bTextured)
                        ogl.SetBlending(true);
    ...
    FORALL_OBJS(pObj) {
        ...
        case OBJ_ROBOT:
            if (AM_SHOW_ROBOTS && ((gameStates.render.bAllVisited && bTextured) || m_visited[pObj->info.nSegment])) {
                ...
                if (pObj->IsGuideBot()) {
                    if (d < 0)
                        CCanvas::Current()->SetColorRGB(
                            0,
                            123 - (int32_t)FRound((123 - 78) * fScale),
                            151 - (int32_t)FRound((151 - 112) * fScale),
                            255);
                    else
                        CCanvas::Current()->SetColorRGB(
                            0,
                            78 + (int32_t)FRound((123 - 78) * fScale),
                            122 + (int32_t)FRound((151 - 112) * fScale),
                            255);
                } else {
                    if (d < 0)
                        CCanvas::Current()->SetColorRGB(
                            123 - (int32_t)FRound((123 - 78) * fScale),
                            0,
                            135 - (int32_t)FRound((135 - 96) * fScale),
                            255);
                    else
                        CCanvas::Current()->SetColorRGB(
                            78 + (int32_t)FRound((123 - 78) * fScale),
                            0,
                            96 + (int32_t)FRound((135 - 96) * fScale),
                            255);
                }
                spherePoint.TransformAndEncode(pObj->info.position.vPos);
...
void CAutomap::DrawLevelId(void) {
    ...
    for (int32_t i = 0; (i < 3) && *szInfo[i]; i++) {
        ...
        GrPrintF(
            ...
            CCanvas::Current()->Height() - offs - (i + 1) * h - i * 2,
            szInfo[i]);
    }
    fontManager.SetCurrent(SMALL_FONT);
    fontManager.SetColorRGBi(GREEN_RGBA, 1, 0, 0);
...
void CAutomap::Render(fix xStereoSeparation) {
    ...
    if (m_bRadar == 2) {
        ...
        SetupTransformation(
            ...
            viewer.vPos,
            viewer.mOrient,
            m_bRadar ? (m_data.nZoom * 3) / 2 : m_data.nZoom,
            1);
    }
    UpdateSlidingFaces();
...
void CAutomap::RenderInfo(void) {
    PROF_START
    int32_t bAutomapFrame = !m_bRadar && !OBSERVING && (gameStates.render.cockpit.nType != CM_FULL_SCREEN) &&
                            (gameStates.render.cockpit.nType != CM_LETTERBOX);
    if (bAutomapFrame) {
...
const char *pszSystemNames[] =
    {"Zeta Aquilae", "Quartzon System", "Brimspark System", "Limefrost Spiral", "Baloris Prime", "Omega System"};
    ...
    while (head < tail) {
        ...
        for (nSide = SEGMENT_SIDE_COUNT; nSide; nSide--, pChild++) {
            ...
            if (nChild >= gameData.segData.nSegments) {
                Error("Invalid segment in SetSegmentDepths()\nsegment=%d, side=%d, child=%d", nSegment, nSide, nChild);
                return 1;
            }
#endif
#if DBG
...
int32_t CAutomap::Setup(int32_t bPauseGame, fix &xEntryTime) {
    if (m_bActive < 0) {
        ...
        if (m_bRadar ||
            (IsMultiGame && (gameStates.app.nFunctionMode == FMODE_GAME) && !gameStates.app.bEndLevelSequence))
            bPauseGame = 0;
        if (bPauseGame)
            PauseGame();
    ...
    if (OBSERVING) {
        ...
        gameOpts->render.automap.bCoronas = gameOpts->render.automap.bLightning = gameOpts->render.automap.bParticles =
            !COMPETITION;
        gameOpts->render.automap.bSparks = !COMPETITION && (gameOptions[0].render.nQuality > 0);
    }

    // m_visited [LOCALOBJECT->nSegment] = 1;
    m_nSegmentLimit = m_nMaxSegsAway = SetSegmentDepths(LOCALOBJECT->info.nSegment, m_visible.Buffer());
    AdjustSegmentLimit();
    m_bActive++;
...
int32_t CAutomap::Update(void) {
    ...
    if (OBSERVING && (LOCALPLAYER.ObservedPlayer() < 0)) {
        ...
        if (controls[0].verticalThrustTime || controls[0].sidewaysThrustTime) {
            ...
            m_data.viewTarget +=
                m_data.viewer.mOrient.m.dir.u * fix(controls[0].verticalThrustTime * SLIDE_SPEED * fZoomScale);
            m_data.viewTarget +=
                m_data.viewer.mOrient.m.dir.r * fix(controls[0].sidewaysThrustTime * SLIDE_SPEED * fZoomScale);
        }
        m_data.viewer.mOrient = pPlayer->info.position.mOrient * m;
...
int32_t CAutomap::ReadControls(int32_t nLeaveMode, int32_t bDone, int32_t &bPauseGame) {
    ...
    while ((c = KeyInKey())) {
        ...
        case KEYDBGGED + KEY_F: {
            ...
            BuildEdgeList();
            m_nSegmentLimit = m_nMaxSegsAway = SetSegmentDepths(LOCALOBJECT->info.nSegment, automap.m_visible.Buffer());
            AdjustSegmentLimit();
        } break;
#endif
        ...
        case KEY_F3:
            if (!gameStates.app.bNostalgia) {
                gameOpts->render.automap.bCoronas = gameOpts->render.automap.bLightning =
                    gameOpts->render.automap.bParticles =
                        !(gameOpts->render.automap.bCoronas || gameOpts->render.automap.bLightning ||
                          gameOpts->render.automap.bParticles);
                gameOpts->render.automap.bSparks =
                    gameOpts->render.automap.bCoronas && gameOpts->render.automap.bLightning &&
                    gameOpts->render.automap.bParticles && (gameOptions[0].render.nQuality > 0);
            }
            break;
        ...
        case KEY_ALTED + KEY_F:
            gameStates.render.bShowFrameRate =
                (gameStates.render.bShowFrameRate + 1) % (6 + (gameStates.render.bPerPixelLighting == 2));
            break;
...
void CAutomap::DoFrame(int32_t nKeyCode, int32_t bRadar) {
    ...
    fix xEntryTime;
    int32_t bPauseGame =
        (gameOpts->menus.nStyle == 0); // Set to 1 if everything is paused during automap...No pause during net.
    fix t1 = 0, t2 = 0;
...
void CAutomap::SetEdgeColor(int32_t nColor, int32_t bFade, float fScale) {
    if ((bFade != m_bFade) || (nColor != m_nColor) || (fScale != m_fScale)) {
        ...
        if (m_bDrawBuffers) {
            ...
            m_color.Set(
                float(canvColor.Red()),
                float(canvColor.Green()),
                float(canvColor.Blue()),
                float(canvColor.Alpha()));
            m_color /= 255.0f;
        }
    }
}
...
void CAutomap::DrawEdges(void) {
    ...
    for (i = 0; i < nbright; i++) {
        ...
        SetEdgeColor(
            int32_t(pEdge->color),
            (pEdge->flags & EF_NO_FADE) != 0,
            X2F(I2X(1) - FixDiv(xDist, m_data.nMaxDist)) * 31);
        DrawLine(pEdge->verts[0], pEdge->verts[1]);
    }
    if (m_bDrawBuffers && m_nVerts) {
        ogl.FlushBuffers(GL_LINES, m_nVerts, 3, 0, 1);
...
void CAutomap::AddSegmentEdges(CSegment *pSeg) {
    ...
    for (nSide = 0; nSide < SEGMENT_SIDE_COUNT; nSide++, pSide++) {
        ...
        if (color != WHITE_RGBA) {
            ...
            if ((LOCALPLAYER.flags & PLAYER_FLAGS_FULLMAP) && !(gameStates.render.bAllVisited || m_visited[nSegment]))
        ...
        for (e1 = 0; e1 < e->nFaces; e1++) {
            for (e2 = 1; e2 < e->nFaces; e2++) {
                if ((e1 != e2) && (e->nSegment[e1] != e->nSegment[e2])) {
                    if (CFixVector::Dot(
                            SEGMENT(e->nSegment[e1])->m_sides[e->sides[e1]].m_normals[0],
