...
int32_t CCamera::HaveTexture(void) { return Texture() && ((int32_t)Texture()->Handle() > 0); }
...
void CCamera::Setup(
    ...
    if (pObj) {
        ...
        m_data.curAngle = m_data.curDelta = 0;
        m_data.t0 = 0;
...
int32_t CCamera::Create(
    ...
    if (bShadowMap) {
        int32_t nSize =
            Pow2ize(int32_t(sqrt(double(gameData.renderData.screen.Width() * gameData.renderData.screen.Height()))));
...
#endif
    {
        ...
        if (bHaveBuffer) {
            ...
            if (!bFitToWall) {
                ...
                if (pVertex)
                    aFace = CFloatVector::Dist(
                                *reinterpret_cast<CFloatVector *>(pVertex),
                                *reinterpret_cast<CFloatVector *>(pVertex + 1)) /
                            CFloatVector::Dist(
                                *reinterpret_cast<CFloatVector *>(pVertex + 1),
                                *reinterpret_cast<CFloatVector *>(pVertex + i2));
                else
                    aFace = dvFace / duFace;
        ...
        if (m_data.bMirror)
            xFlip = !xFlip;
        if (pUVL) {
            pUVL[0].v = pUVL[3].v = F2X(yFlip ? dvImage : dv / nScale);
            pUVL[1].v = pUVL[2].v = F2X(yFlip ? dv / nScale : dvImage);
            pUVL[0].u = pUVL[1].u = F2X(xFlip ? duImage : du / nScale);
            pUVL[2].u = pUVL[3].u = F2X(xFlip ? du / nScale : duImage);
            for (i = 0; i < 4; i++)
                pUVL[i].l = I2X(1);
            if (rotRight) {
                for (i = 1; i < 5; i++) {
            ...
            texCoord[0].v.v = texCoord[3].v.v = yFlip ? dvImage : dv;
            texCoord[1].v.v = texCoord[2].v.v = yFlip ? dv : dvImage;
            texCoord[0].v.u = texCoord[1].v.u = xFlip ? duImage : du / nScale;
            texCoord[2].v.u = texCoord[3].v.u = xFlip ? du / nScale : duImage;
            if (rotLeft) {
...
CCameraManager::~CCameraManager() { Destroy(); }
...
int32_t CCameraManager::Create(void) {
    ...
    if (gameData.trigData.m_nTriggers[0]) {
        for (i = 0, pWall = WALLS.Buffer(); (i < gameData.wallData.nWalls) && (m_cameras.ToS() < MAX_CAMERAS);
             i++, pWall++) {
            t = pWall->nTrigger;
            ...
            if (pTrigger && (pTrigger->m_info.nType == TT_CAMERA)) {
                for (j = 0; j < pTrigger->m_nLinks; j++)
                    if (m_cameras.Grow() && m_cameras.Top()->Create(
                                                m_cameras.ToS() - 1,
                                                (int16_t)pWall->nSegment,
                                                (int16_t)pWall->nSide,
                                                pTrigger->m_segments[j],
                                                pTrigger->m_sides[j],
                                                NULL,
                                                0,
                                                0))
                ...
                if (pTrigger->m_info.nType == TT_CAMERA) {
                    for (k = 0; k < pTrigger->m_nLinks; k++)
                        if (m_cameras.Grow() && m_cameras.Top()->Create(
                                                    ...
                                                    -1,
                                                    -1,
                                                    pTrigger->m_segments[k],
                                                    pTrigger->m_sides[k],
                                                    pObj,
                                                    0,
                                                    0))
                            SetFaceCamera(
                                pTrigger->m_segments[k] * 6 + pTrigger->m_sides[k],
                                (char)m_cameras.ToS() - 1);
                }
            }
        }
    }
