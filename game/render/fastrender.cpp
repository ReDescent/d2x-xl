...
void LoadFaceBitmaps(CSegment *pSeg, CSegFace *pFace) {
    ...
    if (gameOpts->ogl.bGlTexMerge) {
        ...
        if (pFace->m_info.nOvlTex != 0) {
            pFace->bmBot =
                TexMergeGetCachedBitmap(pFace->m_info.nBaseTex, pFace->m_info.nOvlTex, pFace->m_info.nOvlOrient);
            if (pFace->bmBot)
...
bool RenderGeometryFace(CSegment *pSeg, CSegFace *pFace) {
    ...
    faceRenderFunc(
        ...
        pFace->bmBot,
        pFace->bmTop,
        pFace->m_info.bTextured,
        (gameStates.render.bPerPixelLighting != 0) && !gameStates.render.bFullBright &&
            !(gameOpts->render.debug.bWireFrame & 1));
    RETVAL(true)
}
...
int32_t BeginRenderFaces(int32_t nType) {
    ...
    // int32_t	bVBO = 0;
    int32_t bLightmaps =
        (nType == RENDER_TYPE_GEOMETRY) && !gameStates.render.bFullBright && lightmapManager.HaveLightmaps();
    ...
    gameStates.render.history.bmBot = gameStates.render.history.bmTop = gameStates.render.history.bmMask = NULL;
    ...
    if ((gameStates.render.bPerPixelLighting == 2) &&
        ((nType == RENDER_TYPE_GEOMETRY) || (nType == RENDER_TYPE_SKYBOX))) {
        ...
        for (j = flx.roots[flx.usedKeys[i]]; j >= 0; j = pIndex->nNextItem) {
            ...
            if (nPass == 1) {
                ...
                glBeginQuery(
                    ...
                    gameData.renderData.lights.coronaQueries[pFace->m_info.nCorona - 1]);
                if (!glGetError())
...
int32_t SetupCoronaFaces(void) {
    ...
    for (i = 0; i < gameData.renderData.mine.visibility[0].nSegments; i++) {
        if (0 > (nSegment = gameData.renderData.mine.visibility[0].segments[i]))
            continue;
        if ((SEGMENT(nSegment)->m_function == SEGMENT_FUNC_SKYBOX) || (SEGMENT(nSegment)->HasOutdoorsProp()))
            continue;
        pSegFace = SEGFACES + nSegment;
#if DBG
        if (nSegment == nDbgSeg)
...
#endif
        for (j = pSegFace->nFaces, pFace = pSegFace->pFace; j; j--, pFace++)
            if (pFace->m_info.bVisible && (pFace->m_info.widFlags & WID_VISIBLE_FLAG) && pFace->m_info.bIsLight &&
                (pFace->m_info.nCamera < 0) && glareRenderer.FaceHasCorona(nSegment, pFace->m_info.nSide, NULL, NULL))
...
const char *fogVolumeFS = "uniform sampler2D depthTex;\r\n"
...
const char *fogVolumeVS = "void main (void){\r\n"
