...
void CGPGPULighting::Init(void) { m_nShaderProg = -1; }
...
int32_t CGPGPULighting::Render(void) {
    ...
    static float quadCoord[4][2] = {
        {0, 0},
        {0, GPGPU_LIGHT_BUF_WIDTH},
        {GPGPU_LIGHT_BUF_WIDTH, GPGPU_LIGHT_BUF_WIDTH},
...
for (i = 0; i < VL_SHADER_BUFFERS; i++) {
    ...
    for (i = 0, pColor = m_vld.colors; i < m_vld.nVertices; i++) {
        ...
        if (gameOpts->render.color.nSaturation == 2) {
            ...
            if (gameOpts->render.color
                    .nSaturation) { // if a color component is > 1, cap color components using highest component value
...
int32_t CGPGPULighting::Compute(int32_t nVertex, int32_t nState, CFaceColor *pColor) {
	...
	glUniform3fv (glGetUniformLocation (shaderProg, "matSpecular"), 1, reinterpret_cast<GLfloat*> (&matSpecular));
        ...
        CDynLight *pLight;
        int32_t bSkipHeadlight =
            ogl.m_states.bHeadlight && (lightManager.Headlights().nLights > 0) && !gameStates.render.nState;
        CFloatVector vPos = gameData.segData.fVertices[nVertex], vNormal = *RENDERPOINTS[nVertex].GetNormal();
...
const char *vertLightVS = "void main(void){"
