...
void RenderPowerupCorona(CObject *pObj, float red, float green, float blue, float alpha) {
    ENTER(1, 0);
    if ((IsEnergyPowerup(pObj->info.nId) ? gameOpts->render.coronas.bPowerups : gameOpts->render.coronas.bWeapons) &&
        glare.Load()) {
        ...
        if ((pObj->info.nId >= POW_KEY_BLUE) && (pObj->info.nId <= POW_KEY_GOLD)) {
            ...
            float b = (pObj->info.nId >= POW_SHIELD_BOOST) ? Max(red, Max(green, blue)) * 0.5f
                                                           : float(sqrt((red + green + blue) / 3.0f));
...
void RenderLaserCorona(CObject *pObj, CFloatVector *pColor, float alpha, float fScale) {
    ...
    if (gameOpts->render.coronas.bShots && glare.Load()) {
        ...
        ogl.RenderSprite(
            glare.Bitmap(),
            pObj->info.position.vPos + pObj->info.position.mOrient.m.dir.f * (F2X(fLength - 0.5f)),
            I2X(1),
            I2X(1),
            alpha,
            LIGHTTRAIL_BLENDMODE,
            1);
        glare.Bitmap()->SetColor(NULL);
    }
    RETURN
...
int32_t RenderWeaponCorona(
    ...
    if ((pObj->info.nType == OBJ_WEAPON) && (pObj->info.renderType == RT_POLYOBJ))
        ...
        return transparencyRenderer.AddSprite(
            corona.Bitmap(),
            vPos,
            &color,
            FixMulDiv(xSize, corona.Bitmap()->Width(), corona.Bitmap()->Height()),
            xSize,
            0,
            LIGHTTRAIL_BLENDMODE,
            3);
...
void RenderLightTrail(CObject *pObj) {
    ...
    if (pObj->HasLightTrail() && gameStates.app.bHaveExtraGameInfo[IsMultiGame] && EGI_FLAG(bLightTrails, 0, 0, 0)) {
        if (gameOpts->render.particles.bPlasmaTrails)
            ...
            EGI_FLAG(bLightTrails, 1, 1, 0) && (pObj->info.nType == OBJ_WEAPON) && pObj->HasLightTrail() &&
            !pObj->Velocity().IsZero() && glare.Load()) {
            ...
            vNorm *= r;
            vTrailVerts[2] = vTrailVerts[5] = vCenter + vNorm;
            vTrailVerts[3] = vTrailVerts[4] = vCenter - vNorm;
            // vNorm /= 4;
