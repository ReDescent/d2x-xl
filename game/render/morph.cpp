...
int32_t MorphInitPoints(CPolyModel *pModel, CFixVector *vBoxSize, int32_t nSubModel, tMorphInfo *pMorphInfo) {
    ...
    while (nVerts--) {
        ...
        if (vBoxSize) {
            fix t;
            k = 0x7fffffff;
            if (v.v.coord.x && X2I((*vBoxSize).v.coord.x) < abs(v.v.coord.x) / 2 &&
                (t = FixDiv((*vBoxSize).v.coord.x, abs(v.v.coord.x))) < k)
                k = t;
            if (v.v.coord.y && X2I((*vBoxSize).v.coord.y) < abs(v.v.coord.y) / 2 &&
                (t = FixDiv((*vBoxSize).v.coord.y, abs(v.v.coord.y))) < k)
                k = t;
            if (v.v.coord.z && X2I((*vBoxSize).v.coord.z) < abs(v.v.coord.z) / 2 &&
                (t = FixDiv((*vBoxSize).v.coord.z, abs(v.v.coord.z))) < k)
                k = t;
            if (k == 0x7fffffff)
                k = 0;
        } else
...
void CObject::MorphStart(void) {
    ...
    for (i = 0; i < MAX_MORPH_OBJECTS; i++, pMorphInfo++)
        if (pMorphInfo->pObj == NULL || pMorphInfo->pObj->info.nType == OBJ_NONE ||
...
void MorphDrawModel(
    ...
    for (i = m; i < n; i++) {
        ...
        if (m == nSubModel) {
            ...
            for (j = 0; j < h; j++) {
                gameData.modelData.textureIndex[j] =
                    gameData.pigData.tex.objBmIndex[gameData.pigData.tex.pObjBmIndex[pModel->FirstTexture() + j]];
                gameData.modelData.textures[j] =
                    gameData.pigData.tex.bitmaps[0] +
                    /*gameData.pigData.tex.objBmIndex [gameData.pigData.tex.pObjBmIndex [pModel->FirstTexture ()+j]]*/
                    gameData.modelData.textureIndex[j].index;
            }
            ...
            G3DrawMorphingModel(
                ...
                NULL,
                light,
                pMorphInfo->vecs + pMorphInfo->submodelStartPoints[nSubModel],
                nModel);
        } else {
            CFixMatrix orient;
