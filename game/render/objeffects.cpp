...
void CObject::CreateAppearanceEffect(void) {
    ...
    bool bWarp = (nPlayer >= 0) &&
                 ((gameOpts->render.effects.bWarpAppearance == 2) ||
                  ((gameOpts->render.effects.bWarpAppearance == 1) && !gameData.multiplayer.bTeleport[nPlayer]));
    ...
    if (bWarp && (nPlayer == N_LOCALPLAYER) && !AppearanceStage()) {
        ...
        CObject *pEffectObj =
            bWarp ? CreateExplBlast(true) : CreateExplosion(info.nSegment, vPos, info.xSize, ANIM_PLAYER_APPEARANCE);
        if (bWarp || pEffectObj) {
            if (pEffectObj) {
                ...
                if (gameData.effectData.animations[0][ANIM_PLAYER_APPEARANCE].nSound > -1)
                    audio.CreateObjectSound(
                        gameData.effectData.animations[0][ANIM_PLAYER_APPEARANCE].nSound,
                        SOUNDCLASS_PLAYER,
                        OBJ_IDX(pEffectObj));
            }
            if (!bWarp)
                postProcessManager.Add(new CPostEffectShockwave(
                    SDL_GetTicks(),
                    pEffectObj ? pEffectObj->LifeLeft() : I2X(1),
                    info.xSize,
                    1,
                    OBJPOS(this)->vPos));
            else {
