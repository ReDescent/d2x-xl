...
void ConvertWeaponToPowerup(CObject *pObj) {
    if (!InitAddonPowerup(pObj)) {
        ...
        pObj->rType.animationInfo.xFrameTime =
            gameData.effectData.vClipP[pObj->rType.animationInfo.nClipIndex].xFrameTime;
        pObj->rType.animationInfo.nCurFrame = 0;
...
static int32_t DrawPowerupSphere(CObject *pObj, CBitmap *pBm, CFloatVector &color) {
    ...
    gameData.renderData.shield->SetupSurface(
        &powerupPulse,
        ((pObj->Id() == POW_SHIELD_BOOST) && !SHOW_LIGHTNING(3)) ? shield[1].Bitmap() : shield[2].Bitmap());
#if ICON_IN_FRONT
    if (pBm)
        transparencyRenderer
            .AddSprite(pBm, pObj->Position(), &color, 2 * pObj->info.xSize / 3, 2 * pObj->info.xSize / 3, 0, 0, 0.0f);
#endif
    // alpha < 0 will cause the sphere to have an outline, additive < 0 will cause the sphere to be blurred
    int32_t h =
        (pObj->Id() == POW_SHIELD_BOOST)
...
void DrawObjectBitmap(CObject *pObj, int32_t bmi0, int32_t bmi, int32_t iFrame, CFloatVector *pColor, float fAlpha) {
    ...
    bool b3DShield =
        ((pObj->Type() == OBJ_POWERUP) &&
         ((pObj->Id() == POW_SHIELD_BOOST) || (pObj->Id() == POW_INVUL) || (pObj->Id() == POW_CLOAK) ||
          (pObj->Id() == POW_EXTRA_LIFE) || (pObj->Id() == POW_HOARD_ORB)) &&
    ...
    if ((nType == OBJ_POWERUP) &&
        ((bEnergy && gameOpts->render.coronas.bPowerups) || (!bEnergy && gameOpts->render.coronas.bWeapons)))
        RenderPowerupCorona(
            pObj,
            color.Red(),
            color.Green(),
    ...
    if (b3DShield && DrawPowerupSphere(pObj, PowerupIcon(pObj), color)) {
        // if ((nType == OBJ_POWERUP) && ((bEnergy && gameOpts->render.coronas.bPowerups) || (!bEnergy &&
        // gameOpts->render.coronas.bWeapons))) 	RenderPowerupCorona (pObj, color.Red (), color.Green (), color.Blue
        // (),
        // coronaIntensities [gameOpts->render.coronas.nObjIntensity]);
    } else {
        // if ((nType == OBJ_POWERUP) && ((bEnergy && gameOpts->render.coronas.bPowerups) || (!bEnergy &&
        ...
        if (fAlpha < 1) {
            ...
            if (pBm->Width() > pBm->Height())
                transparencyRenderer.AddSprite(
                    pBm,
                    vPos,
                    &color,
                    xSize,
                    FixMulDiv(xSize, pBm->Height(), pBm->Width()),
                    iFrame,
                    bAdditive,
                    (nType == OBJ_FIREBALL) ? 10.0f : 0.0f);
            else
                transparencyRenderer.AddSprite(
                    pBm,
                    vPos,
                    &color,
                    FixMulDiv(xSize, pBm->Width(), pBm->Height()),
                    xSize,
                    iFrame,
                    bAdditive,
                    (nType == OBJ_FIREBALL) ? 10.0f : 0.0f);
...
int32_t GetCloakInfo(CObject *pObj, fix xCloakStartTime, fix xCloakEndTime, tCloakInfo *ciP) {
    ...
    ci.xDeltaTime =
        (xCloakStartTime == 0x7fffffff) ? gameData.timeData.xGame : gameData.timeData.xGame - xCloakStartTime;
    ...
    if (ci.xDeltaTime < ci.xFadeinDuration / 2) {
        ...
        ci.nFadeValue =
            X2I(FixDiv(ci.xDeltaTime - ci.xFadeinDuration / 2, ci.xFadeinDuration / 2) * CLOAKED_FADE_LEVEL);
    else if (
        (xCloakStartTime == 0x7fffffff) ||
        (gameData.timeData.xGame < xCloakEndTime - ci.xFadeoutDuration)) { // fully cloaked
        ...
        ci.nFadeValue =
            X2I(FixDiv(ci.xTotalTime - ci.xFadeoutDuration / 2 - ci.xDeltaTime, ci.xFadeoutDuration / 2) *
                CLOAKED_FADE_LEVEL);
    else {
        ci.xLightScale = (fix)(
            (float)(ci.xFadeoutDuration / 2 - (ci.xTotalTime - ci.xDeltaTime) / (float)(ci.xFadeoutDuration / 2)));
        ci.bFading = 1;
    }
    if (ciP)
        *ciP = ci;
...
static inline int32_t ObjectIsCloaked(CObject *pObj) { return gameStates.render.bBuildModels ? 0 : pObj->Cloaked(); }
...
int32_t DrawPolygonObject(CObject *pObj, int32_t bForce) {
    ...
    if (gameStates.render.bBuildModels)
        ...
        if ((gameStates.render.nType != RENDER_TYPE_TRANSPARENCY) && (bCloaked || bEnergyWeapon) &&
    ...
    if (pObj->rType.polyObjInfo.nTexOverride != -1) {
        ...
        if (bCloaked) {
            if (pObj->info.nType == OBJ_PLAYER)
                bOk = DrawCloakedObject(
                    pObj,
                    xLight,
                    xEngineGlow,
                    PLAYER(id).cloakTime,
                    PLAYER(id).cloakTime + CLOAK_TIME_MAX);
                ...
                if (!ROBOTINFO(id)->bossFlag)
                    bOk = DrawCloakedObject(
                        ...
                        xLight,
                        xEngineGlow,
                        gameData.timeData.xGame - I2X(10),
                        gameData.timeData.xGame + I2X(10));
                else if (0 <= (i = gameData.bossData.Find(pObj->Index())))
                    bOk = DrawCloakedObject(
                        pObj,
                        xLight,
                        xEngineGlow,
                        gameData.bossData[i].m_nCloakStartTime,
                        gameData.bossData[i].m_nCloakEndTime);
            }
        } else {
            tBitmapIndex *bmiAltTex = (pObj->rType.polyObjInfo.nAltTextures > 0)
    ...
    if (gameStates.render.nShadowPass != 2) {
        ...
        if (pObj->info.nType != OBJ_WEAPON)
            ...
            if (pObj->IsMine()) {
                ...
                (pObj->info.nId != OMEGA_ID) ||
                !(SHOW_LIGHTNING(1) && gameOpts->render.lightning.bOmega && !gameStates.render.bOmegaModded)) {
                DrawWeaponVClip(pObj);
...
int32_t RenderObject(CObject *pObj, int32_t nWindow, int32_t bForce) {
    ...
    if (nObject != LOCALPLAYER.nObject) {
        ...
        if ((bSpectate = ((gameStates.render.bFreeCam > 0) && !nWindow)))
            ...
            (nWindow || (!gameStates.render.bChaseCam &&
                         (gameStates.app.bEndLevelSequence <
                          EL_LOOKBACK)))) { // don't render ship model if neither external view nor main view
#else
        else if (
            (gameStates.render.nShadowPass != 2) && !gameStates.app.bPlayerIsDead &&
    ...
    case RT_POLYOBJ:
        if (nType == OBJ_EFFECT) {
            pObj->info.renderType = (pObj->info.nId == PARTICLE_ID)
                                        ? RT_PARTICLES
                                        : (pObj->info.nId == LIGHTNING_ID) ? RT_LIGHTNING : RT_SOUND;
            RETVAL(0)
        }
        if (nType == OBJ_PLAYER) {
