...
int32_t OOF_FacingViewer(CFloatVector *modelVertP, CFloatVector *pn) { return OOF_FacingPoint(modelVertP, pn, &vPos); }
...
CFloatVector *OOF_CalcFacePerp(CSubModel *pso, CFace *pFace) {
    ...
    CFloatVector::Normalize(CFloatVector::Perp(
        pFace->m_vRotNormal,
        modelVertP[pFaceVertex[0].m_nIndex],
        modelVertP[pFaceVertex[1].m_nIndex],
        modelVertP[pFaceVertex[2].m_nIndex]));
...
void OOF_SetCullAndStencil(int32_t bCullFront) { SetCullAndStencil(bCullFront); }
                    ...
                    if (bDynLighting) {
                        if (pVertexColor[h].index != gameStates.render.nFrameFlipFlop + 1)
                            GetVertexColor(
                                ...
                                -1,
                                -1,
                                reinterpret_cast<CFloatVector3 *>(modelNormalP + h),
                                reinterpret_cast<CFloatVector3 *>(phv),
                                pVertexColor + h,
                                NULL,
                                1,
                                0,
...
int32_t CModel::RenderShadow(CObject *pObj, float *fLight) {
    int16_t i, *nearestLightP =
                   lightManager.NearestSegLights() + gameData.objData.pConsole->info.nSegment * MAX_NEAREST_LIGHTS;

    gameData.renderData.shadows.nLight = 0;
    for (i = 0; (gameData.renderData.shadows.nLight < gameOpts->render.shadows.nLights) && (*nearestLightP >= 0);
         i++, nearestLightP++) {
...
int32_t CModel::Render(CObject *pObj, float *fLight, int32_t bCloaked) {
    ...
    if (m_fAlpha < 0.01f)
        m_fAlpha = 0.01f;
    return (!gameStates.render.nShadowMap && (gameStates.render.nShadowPass == 2)) ? RenderShadow(pObj, fLight)
                                                                                   : Draw(pObj, fLight);
}
