...
CPolyModel *GetPolyModel(CObject *pObj, CFixVector *pos, int32_t nModel, int32_t flags, int32_t *bCustomModel) {
    ...
    if (!pObj)
        pModel = ((gameStates.app.bAltModels && bIsDefModel && bHaveAltModel) ? gameData.modelData.polyModels[2]
                                                                              : gameData.modelData.polyModels[0]) +
                 nModel;
    else if (!pModel) {
        if (!(bIsDefModel && bHaveAltModel)) {
            ...
            RETVAL(NULL)
        if ((gameStates.render.nShadowPass == 2) && (pObj->info.nType == OBJ_REACTOR) &&
            !(nModel & 1)) // use the working reactor model for rendering destroyed reactors' shadows
            pModel--;
    }
    ...
    if (!(SHOW_DYN_LIGHT || SHOW_SHADOWS) && pModel->SimplerModel() && !flags && pos) {
        ...
        while (pModel->SimplerModel() &&
               (depth > cnt++ * gameData.modelData.nSimpleModelThresholdScale * pModel->Rad()))
...
int32_t DrawPolyModel(
    ...
    if (!flags) { // draw entire object
        if (!gameStates.app.bNostalgia && G3RenderModel(
                                              pObj,
                                              bCustomModel ? -nModel : nModel,
                                              -1,
                                              pModel,
        ...
        for (int32_t i = 0; flags > 0; flags >>= 1, i++) {
            if ((flags & 1) && (i < pModel->ModelCount())) {
                ...
                if (!G3RenderModel(
                    ...
                    G3DrawPolyModel(
                        pObj,
                        pModel->Data() + pModel->SubModels().ptrs[i],
                        gameData.modelData.textures,
                        animAngles,
                        NULL,
                        light,
