...
static CFloatVector radarColors[3][3] = {
    {{{{0.0f, 0.6f, 0.0f, 0.5f}}}, {{{0.0f, 0.8f, 0.0f, 0.5f}}}, {{{0.0f, 1.0f, 0.0f, 0.5f}}}},
...
float CRadar::yOffs[2][CM_LETTERBOX + 1] = {
    {17.0f, -20.5f, -18.0f, -20.5f, -19.0f},
    {16.0f, 19.5f, 17.0f, 19.5f, 18.0f}};
...
CRadar::CRadar() : m_lineWidth(1.0f) {
...
void CRadar::Render(void) {
    ...
    m_offset.v.coord.y += sizeOffsets[gameOpts->render.cockpit.nRadarPos][gameOpts->render.cockpit.nRadarSize] *
                          (m_offset.v.coord.y / fabs(m_offset.v.coord.y));
    if (ogl.IsOculusRift())
        m_offset.v.coord.y *= 1.5f;
    if (EGI_FLAG(nWeaponIcons, 1, 1, 0) && (gameOpts->render.cockpit.bHUD || cockpit->ShowAlways())) {
        if ((extraGameInfo[0].nWeaponIcons < 3) ||
            ((extraGameInfo[0].nWeaponIcons & 1) == !gameOpts->render.cockpit.nRadarPos))
    ...
    m_lineWidth = float(CCanvas::Current()->Width()) / 640.0f * radarSizes[gameOpts->render.cockpit.nRadarSize] /
                  radarSizes[sizeofa(radarSizes) - 1];
