...
// 0: use average of the normals of all faces a vertex belongs to for lighting and only compute light once per frame and
// vertex (faster and smoother) 1: compute light for each face using the face's normal, computing lights multiple times
// for each vertex per frame (more correct)
...
int32_t
SetupFace(int16_t nSegment, int16_t nSide, CSegment *pSeg, CSegFace *pFace, CFaceColor *faceColors, float &fAlpha) {
    ENTER(1, 0);
    ...
    bCloaked = 0;
    bTransparent = 0;
    fAlpha = bWall ? WallAlpha(
                         nSegment,
                         nSide,
                         pFace->m_info.nWall,
                         pFace->m_info.widFlags,
                         pFace->m_info.nCamera >= 0,
                         pFace->m_info.bAdditive,
                         &faceColors[1],
                         nColor,
                         bTextured,
                         bCloaked,
...
void ComputeDynamicFaceLight(int32_t nStart, int32_t nEnd, int32_t nThread) {
    ...
    // memset (&gameData.renderData.lights.dynamic.shader.index, 0, sizeof
    // (gameData.renderData.lights.dynamic.shader.index));
    ogl.SetTransform(1);
    ...
    for (i = nStart; i < nEnd; i++) {
        ...
        if (bNeedLight)
            nLights = lightManager.SetNearestToSegment(
                nSegment,
                -1,
                0,
                0,
                nThread); // only get light emitting objects here (variable geometry lights are caught in
                    ...
                    if (nLights + lightManager.VariableVertLights(nVertex) == 0) {
                        *pVertexColor = gameData.renderData.color.ambient[nVertex];
                        ...
                        GetVertexColor(
                            ...
                            FVERTICES[nVertex].XYZ(),
                            NULL,
                            &gameData.renderData.color.ambient[nVertex],
                            1,
                            0,
                            nThread);
                ...
                *pColor = *pVertexColor;
                if (nColor) {
                    if (gameStates.render.bPerPixelLighting == 2)
                        *pColor =
                            faceColor[nColor]; // multiply the material color in for not lightmap driven lighting models
                    else
                        *pColor *=
                            faceColor[nColor]; // multiply the material color in for not lightmap driven lighting models
                }
...
#pragma omp critical(FixTriangleFan)
    ...
    { // rearrange vertex order for TRIANGLE_FAN rendering
        ...
        memcpy(
            FACES.vertices + pFace->m_info.nIndex,
            FACES.vertices + pFace->m_info.nIndex + 1,
            3 * sizeof(CFloatVector3));
        ...
        memcpy(
            ...
            FACES.texCoord + pFace->m_info.nIndex + 1,
            3 * sizeof(tTexCoord2f));
        FACES.texCoord[pFace->m_info.nIndex + 3] = tc;
        tc = FACES.lMapTexCoord[pFace->m_info.nIndex];
        memcpy(
            FACES.lMapTexCoord + pFace->m_info.nIndex,
            FACES.lMapTexCoord + pFace->m_info.nIndex + 1,
            3 * sizeof(tTexCoord2f));
        FACES.lMapTexCoord[pFace->m_info.nIndex + 3] = tc;
        if (pFace->m_info.nOvlTex) {
            tc = FACES.ovlTexCoord[pFace->m_info.nIndex];
            memcpy(
                FACES.ovlTexCoord + pFace->m_info.nIndex,
                FACES.ovlTexCoord + pFace->m_info.nIndex + 1,
                3 * sizeof(tTexCoord2f));
                            ...
                            *pColor = *brightColors[0]; // gameData.renderData.color.ambient [nVertex];
...
#endif
                            {
                                ...
                                if ((nSegment == nDbgSeg) && ((nDbgSide < 0) || (nSide == nDbgSide)))
                                    BRP;
#endif
                                if (nLights + lightManager.VariableVertLights(nVertex) ==
                                    0) { // no dynamic lights => only ambient light contribution
                                    *pVertexColor = gameData.renderData.color.ambient[nVertex];
                                    pVertexColor->index = gameStates.render.nFrameFlipFlop + 1;
                                } else {
#if LIGHTING_QUALITY == 1
...
void ComputeStaticFaceLight(int32_t nStart, int32_t nEnd, int32_t nThread) {
    ...
    for (i = nStart; i < nEnd; i++) {
        ...
        pSegFace = SEGFACES + nSegment;
        if (!(/*gameStates.app.bMultiThreaded ||*/ SegmentIsVisible(pSeg))) {
            gameData.renderData.mine.visibility[0].segments[i] =
                -gameData.renderData.mine.visibility[0].segments[i] - 1;
            continue;
        }
        for (j = pSegFace->nFaces, pFace = pSegFace->pFace; j; j--, pFace++) {
            nSide = pFace->m_info.nSide;
            ...
            pUVL = pSide->m_uvls;
            for (h = 0, uvi = 0 /*(pSeg->m_sides [nSide].m_nType == SIDE_IS_TRI_13)*/; h < pSide->CornerCount();
                 h++, pColor++, uvi++) {
                if (gameStates.render.bFullBright)
                    *pColor = nColor ? faceColor[nColor] : *brightColors[gameStates.render.bFullBright - 1];
                else {
