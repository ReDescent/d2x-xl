...
int32_t bOutLineMode = 0, bShowOnlyCurSide = 0;
...
int32_t FaceIsVisible(int16_t nSegment, int16_t nSide) {
    ...
    if (pSide->m_nType == SIDE_IS_QUAD) {
        v = gameData.renderData.mine.viewer.vPos -
            pSeg->SideCenter(nSide); // gameData.segData.vertices + pSeg->m_vertices [sideVertIndex [nSide][0]]);
...
int32_t ToggleOutlineMode(void) { return bOutLineMode = !bOutLineMode; }

//------------------------------------------------------------------------------

int32_t ToggleShowOnlyCurSide(void) { return bShowOnlyCurSide = !bShowOnlyCurSide; }
...
if (nColor > 0)
...
#endif
    {
        ...
        if (pConnSeg && (pSeg->m_function != pConnSeg->m_function)) {
            if ((nFogType != nOtherFogType) && (pSeg->m_function != SEGMENT_FUNC_TEAM_BLUE) &&
                (pSeg->m_function != SEGMENT_FUNC_TEAM_RED) && (pConnSeg->m_function != SEGMENT_FUNC_TEAM_BLUE) &&
        ...
        if (gameData.renderData.lights.bGotDynColor[nVertex]) {
            ...
            if (gameOpts->render.color.bMix) {
                if (gameOpts->render.color.nLevel) {
                    ...
                    pColor->Red() = pColor->Green() = pColor->Blue() = fl + dl;
                }
...
int32_t SetFaceLight(tFaceProps *pProps) {
    ...
    for (int32_t i = 0; i < pProps->nVertices; i++) {
        pProps->uvls[i].l =
            SetVertexLight(pProps->segNum, pProps->sideNum, pProps->vp[i], vertColors + i, pProps->uvls[i].l);
...
int32_t IsTransparentTexture(int16_t nTexture) {
    return !gameStates.app.bD1Mission && ((nTexture == 378) || (nTexture == 353) || (nTexture == 420) ||
                                          (nTexture == 432) || ((nTexture >= 399) && (nTexture <= 409)));
}
...
float WallAlpha(
    ...
    if (SHOW_DYN_LIGHT) {
        ...
        bTransparent = 0;
        bCloaked = (pWall->state == WALL_DOOR_CLOAKING) || (pWall->state == WALL_DOOR_DECLOAKING) ||
                   ((widFlags & WID_CLOAKED_FLAG) != 0);
    }
    if (bCloaked || bTransparent || (widFlags & WID_TRANSPCOLOR_FLAG)) {
        ...
        if (bCloaked || bTransparent) {
            *pColor = cloakColor;
            ...
            if (bTransparent)
                pColor->Red() = pColor->Green() = pColor->Blue() = fAlpha;
            return fAlpha;
        }
        ...
        if (WALL(nWall)->hps)
            ...
            fAlpha =
                COMPETITION ? 0.5f : (float)(FADE_LEVELS - extraGameInfo[1].grWallTransparency) / (float)FADE_LEVELS;
        else
    ...
    if (gameOpts->render.effects.bAutoTransparency &&
        IsTransparentTexture(SEGMENT(nSegment)->m_sides[nSide].m_nBaseTex))
...
int32_t SetupMonitorFace(int16_t nSegment, int16_t nSide, int16_t nCamera, CSegFace *pFace) {
    ...
    bHaveMonitorBg = pCamera->Valid() && /*!pCamera->bShadowMap &&*/
                     (pCamera->Texture() || bCamBufAvail) && (!bIsTeleCam || EGI_FLAG(bTeleporterCams, 0, 1, 0));
    if (bHaveMonitorBg) {
...
void AdjustVertexColor(CBitmap *pBm, CFaceColor *pColor, fix xLight) {
    ...
    if (!pColor->index || !gameStates.render.bAmbientColor || (gameStates.app.bEndLevelSequence >= EL_OUTSIDE)) {
        pColor->Red() = pColor->Green() = pColor->Blue() = l * s;
    } else if (s != 1.0f)
        *pColor *= s;
    pColor->Alpha() = 1.0f;
}
    ...
    for (int32_t i = 0, j = 1; nRadius; nRadius--) {
        for (int32_t h = i, i = j; h < i; h++) {
            ...
            if ((bVisible[nSegment] == nVisible) &&
                (!nMaxDist ||
                 (CFixVector::Dist(SEGMENT(nStartSeg)->Center(), SEGMENT(nSegment)->Center()) <= nMaxDist)))
                return 1;
            CSegment *pSeg = SEGMENT(nSegment);
            for (int32_t nChild = 0; nChild < SEGMENT_SIDE_COUNT; nChild++) {
                int32_t nChildSeg = pSeg->m_children[nChild];
                if ((nChildSeg >= 0) && (visitedP[nChildSeg] != nVisited) &&
                    (pSeg->IsPassable(nChild, NULL) & WID_TRANSPARENT_FLAG)) {
...
int32_t ToggleRearView(void) { return SetRearView(!gameStates.render.bRearView); }
...
void ResetRearView(void) {
    ...
    if ((gameStates.render.cockpit.nType < 0) || (gameStates.render.cockpit.nType > 4) ||
        (gameStates.render.cockpit.nType == CM_REAR_VIEW)) {
...
#endif
    return SetChaseCam((OBSERVING) ? 1 : !gameStates.render.bChaseCam);
}
...
void ToggleRadar(void) { gameOpts->render.cockpit.nRadarRange = (gameOpts->render.cockpit.nRadarRange + 1) % 5; }
        ...
        if (!gameStates.zoom.nState) {
            if (!bAllow || ZoomKeyPressed()) {
                if (!bAllow) {
                    ...
                    gameStates.zoom.nDestFactor = (fix)FRound(
                        gameStates.zoom.nFactor *
                        pow(float(gameStates.zoom.nMaxFactor) / float(gameStates.zoom.nMinFactor), 0.25f));
                ...
                gameStates.zoom.nChannel = audio.StartSound(
                    -1,
                    SOUNDCLASS_GENERIC,
                    I2X(1),
                    0xFFFF / 2,
                    ...
                    -1,
                    I2X(1),
                    AddonSoundName(SND_ADDON_ZOOM1));
            }
        }
        ...
        if (bAllow && ZoomKeyPressed()) {
            if ((gameStates.zoom.nState <= 0) && (gameStates.zoom.nFactor < gameStates.zoom.nMaxFactor)) {
                ...
                gameStates.zoom.nChannel = audio.StartSound(
                    ...
                    -1,
                    I2X(1),
                    AddonSoundName(SND_ADDON_ZOOM2));
