...
void RenderPlayerShield(CObject *pObj) {
    ...
    if (!SHOW_OBJ_FX)
        return;
    if (SHOW_SHADOWS && (FAST_SHADOWS ? (gameStates.render.nShadowPass != 1) : (gameStates.render.nShadowPass != 3)))
        return;
    ...
    if (bAppearing || EGI_FLAG(nShieldEffect, 0, 1, gameOpts->render.effects.bShields)) {
        ...
        if (gameData.multiplayer.bWasHit[i]) {
            alpha =
                ((!bAppearing && gameOpts->render.effects.bOnlyShieldHits)
                     ? (float)cos(sqrt((double)dt / float(SHIELD_EFFECT_TIME)) * PI / 2)
                     : 1);
            scale *= alpha;
        } else if (PLAYER(i).flags & PLAYER_FLAGS_INVULNERABLE)
            alpha = 1;
        ...
        RenderHitbox(
            pObj,
            shieldColors[nColor].Red() * scale,
            shieldColors[nColor].Green() * scale,
            shieldColors[nColor].Blue() * scale,
            alpha);
#else
        DrawShieldSphere(
            pObj,
...
void RenderRobotShield(CObject *pObj) {
    ...
    if (dt < SHIELD_EFFECT_TIME) {
        scale *= gameOpts->render.effects.bOnlyShieldHits
                     ? float(cos(sqrt(float(dt) / float(SHIELD_EFFECT_TIME)) * PI / 2))
                     : 1;
        DrawShieldSphere(
            pObj,
            shieldColors[2].Red() * scale,
            shieldColors[2].Green() * scale,
            shieldColors[2].Blue() * scale,
            0.5f * scale,
            1);
