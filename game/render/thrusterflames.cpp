...
void CThrusterFlames::Create(void) {
    if (!m_bHaveFlame[m_bPlayer]) {
        ...
        int32_t i, j, m, n;
        double phi, sinPhi;
        float z = 0, fScale = 2.0f / 3.0f, fStep[2] = {1.0f / 4.0f, 1.0f / 3.0f};
...
int32_t CThrusterFlames::CalcPos(CObject *pObj, tThrusterInfo *tiP, int32_t bAfterburnerBlob) {
    ...
    if (gameOpts->render.bHiresModels[0] && (pObj->info.nType == OBJ_PLAYER) && !GetASEModel(pObj->ModelId())) {
        ...
        ti.fLength[0] = ti.fLength[1] = ti.fScale;
        ti.fSize[0] = ti.fSize[1] = (ti.fLength[0] + 1) / 2;
        ti.nType[0] = ti.nType[1] = 1;
        m_nThrusters = 2;
        ...
        *ti.vPos = pObj->info.position.vPos + pObj->info.position.mOrient.m.dir.f * (-nObjRad);
        ...
        (pObj->info.nType == OBJ_PLAYER) || ((pObj->info.nType == OBJ_ROBOT) && !pObj->cType.aiInfo.CLOAKED) ||
        bMissile) {
        ...
        if (!m_nThrusters) {
            ...
            for (i = 0; i < m_nThrusters; i++) {
                ...
                float h = ((ti.nType[i] & (REAR_THRUSTER | FRONTAL_THRUSTER)) == (REAR_THRUSTER | FRONTAL_THRUSTER))
                              ? ti.fScale
                              : 1.5f;
                if (m_nStyle == 1)
                    h *= 4.0f;
...
void CThrusterFlames::Render2D(CFixVector &vPos, CFixVector &vDir, float fSize, float fLength, CFloatVector *pColor) {
    ...
    transparencyRenderer.AddLightTrail(
        thruster.Bitmap(),
        vCap,
        tcCap[m_bPlayer],
        (dotTrail < dotCap) ? vTrail : NULL,
        tcTrail[m_bPlayer],
        pColor);
}
    ...
    if (m_nStyle == 1) { // 2D
        ...
        if (gameStates.render.nType != RENDER_TYPE_TRANSPARENCY) {
            ...
            transformation.Begin(
                m_ti.vPos[nThruster],
                (m_ti.pp && !m_bSpectate) ? m_ti.pp->mOrient : m_ti.mOrient,
                __FILE__,
                __LINE__);
