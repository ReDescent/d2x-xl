...
void CTranspPoly::RenderFace(void) {
    ...
    int32_t bDecal, bLightmaps = transparencyRenderer.Data().bLightmaps && !gameStates.render.bFullBright,
                    bTextured = (pFace->m_info.nSegColor == 0) && (bmBot != NULL),
                    bColored = (nColors == nVertices) && (bTextured || (gameStates.render.bPerPixelLighting != 2)) &&
    ...
    if (bTextured) {
        ...
        if (bmTop && !(bmTop->Flags() & (BM_FLAG_SUPER_TRANSPARENT | BM_FLAG_TRANSPARENT | BM_FLAG_SEE_THRU))) {
            ...
            bDecal = (bmTop != NULL);
            bmMask = (bDecal && ((bmTop->Flags() & BM_FLAG_SUPER_TRANSPARENT) != 0) &&
                      gameStates.render.textures.bHaveMaskShader)
                         ? bmTop->Mask()
                         : NULL;
        }
    } else {
        ...
        if (!pFace->m_info.bColored) {
            ...
            if (gameStates.render.bHeadlights) {
                ...
                lightManager.Headlights().SetupShader(
                    bTextured ? bmMask ? 3 : bDecal ? 2 : 1 : 0,
                    1,
                    bTextured ? NULL : &pFace->m_info.color);
                if (bAdditive != 2) {
                    bAdditive = 2;
...
void CTransparencyRenderer::InitBuffer(int32_t zMin, int32_t zMax, int32_t nWindow) {
...
#endif
        m_data.zScale =
            (double)(ITEM_DEPTHBUFFER_SIZE - 1) / (double)(m_data.zMax * sqrt(double(gameStates.app.nThreads)));
    ...
    if (ogl.StereoSeparation() <= 0)
        m_data.bRenderGlow = m_data.bSoftBlend = 0;
    m_data.bReady = 1;
    RETURN
}
...
CTransparencyRenderer::Add(CTranspItem *item, CFixVector vPos, int32_t nOffset, bool bClamp, int32_t bTransformed) {
    ...
    // find the first particle to insert the new one *before* and place in pj; pi will be it's predecessor (NULL if to
...
int32_t CTransparencyRenderer::SplitPoly(tTranspPoly *item, int32_t nDepth) {
    ...
    if ((nDepth > 1) || !nMaxLen || (nMaxLen < 10) ||
        ((nMaxLen <= 30) && ((split[0].nVertices == 3) || (nMaxLen <= nMinLen / 2 * 3)))) {
    ...
    if (split[0].nVertices == 3) {
        ...
        split[0].vertices[i0] = split[1].vertices[i1] = vSplit;
        split[0].sideLength[i0] = split[1].sideLength[i0] = nMaxLen / 2;
        if (split[0].pBm) {
            split[0].texCoord[i0].dir.u = split[1].texCoord[i1].dir.u =
                (split[0].texCoord[i1].dir.u + split[0].texCoord[i0].dir.u) / 2;
            split[0].texCoord[i0].dir.dir = split[1].texCoord[i1].dir.dir =
                (split[0].texCoord[i1].dir.dir + split[0].texCoord[i0].dir.dir) / 2;
        }
        ...
        i3 = (i1 + 2) % 4;
        vSplit = CFloatVector::Avg(split[0].vertices[i0], split[0].vertices[i1]);
        split[0].vertices[i1] = split[1].vertices[i0] = vSplit;
        vSplit = CFloatVector::Avg(split[0].vertices[i2], split[0].vertices[i3]);
        split[0].vertices[i2] = split[1].vertices[i3] = vSplit;
        if (split[0].pBm) {
            split[0].texCoord[i1].dir.u = split[1].texCoord[i0].dir.u =
                (split[0].texCoord[i1].dir.u + split[0].texCoord[i0].dir.u) / 2;
            split[0].texCoord[i1].dir.dir = split[1].texCoord[i0].dir.dir =
                (split[0].texCoord[i1].dir.dir + split[0].texCoord[i0].dir.dir) / 2;
            split[0].texCoord[i2].dir.u = split[1].texCoord[i3].dir.u =
                (split[0].texCoord[i3].dir.u + split[0].texCoord[i2].dir.u) / 2;
            split[0].texCoord[i2].dir.dir = split[1].texCoord[i3].dir.dir =
                (split[0].texCoord[i3].dir.dir + split[0].texCoord[i2].dir.dir) / 2;
        }
        ...
        split[0].sideLength[i0] = split[1].sideLength[i0] = split[0].sideLength[i2] = split[1].sideLength[i2] =
            nMaxLen / 2;
    }
...
int32_t CTransparencyRenderer::AddPoly(
    CSegFace *pFace,
    tFaceTriangle *pTriangle,
    CBitmap *pBm,
    CFloatVector *vertices,
    char nVertices,
    tTexCoord2f *texCoord,
    CFloatVector *color,
    CFaceColor *altColor,
...
int32_t CTransparencyRenderer::AddFaceTris(CSegFace *pFace) {
    ...
    for (h = pFace->m_info.nTris; h; h--, pTriangle++) {
        ...
        if (!AddPoly(
                ...
                vertices,
                3,
                FACES.texCoord + pTriangle->nIndex,
...
int32_t CTransparencyRenderer::AddFaceQuads(CSegFace *pFace) {
    ...
    RETVAL(
        AddPoly(
            ...
            vertices,
            4,
            FACES.texCoord + pFace->m_info.nIndex,
...
int32_t CTransparencyRenderer::AddSprite(
    CBitmap *pBm,
    const CFixVector &position,
    CFloatVector *color,
    int32_t nWidth,
    int32_t nHeight,
    char nFrame,
...
int32_t CTransparencyRenderer::AddSpark(
    const CFixVector &position,
    char nType,
    int32_t nSize,
    char nFrame,
...
int32_t CTransparencyRenderer::AddSphere(
...
int32_t CTransparencyRenderer::AddLightning(CLightning *pLightning, int16_t nDepth) {
    ...
    if (!Add(
            &item,
            bSwap ? pLightning->m_vEnd
                  : pLightning->m_vPos /*CFixVector::Avg (pLightning->m_vPos, pLightning->m_vEnd)*/,
            0,
            true,
            0)) // -1))
        RETVAL(0)
...
int32_t CTransparencyRenderer::AddLightTrail(
    CBitmap *pBm,
    CFloatVector *vThruster,
    tTexCoord2f *tcThruster,
    CFloatVector *vFlame,
    tTexCoord2f *tcFlame,
    CFloatVector *pColor) {
    ENTER(0, 0);
...
inline void CTransparencyRenderer::ResetBitmaps(void) {
    ENTER(0, 0);
    m_data.pBm[0] = m_data.pBm[1] = m_data.pBm[2] = NULL;
    m_data.bDecal = 0;
    m_data.bTextured = 0;
    m_data.nFrame = -1;
...
int32_t CTransparencyRenderer::LoadTexture(
    CSegFace *pFace,
    CBitmap *pBm,
    int16_t nTexture,
    int32_t nFrame,
...
void CTransparencyRenderer::FlushBuffers(int32_t nType, CTranspItem *item) {
    ...
    if (USE_PARTICLE_SHADER) {
        ...
        if (nType == tiSpark) {
            ...
            if (!item || sparkArea.Overlap(
                             ((CTranspParticle *)item)->particle->Posf(),
                             ((CTranspParticle *)item)->particle->Rad()))
                FlushSparkBuffer();
        } else {
            FlushSparkBuffer();
            FlushParticleBuffer(nType);
...
#endif
            try {
                // if ((m_data.nCurType != tiSphere) && (m_data.nPrevType == tiSphere)) // spheres somehow mess up the
                // glow renderer; I cannot determine why though 	glowRenderer.End ();
...
void CTransparencyRenderer::RenderBuffer(CTranspItemBuffers &buffer, bool bCleanup) {
    ...
    do {
        ...
        else if (pCurrent->bTransformed) { // remove items that have transformed coordinates when stereo rendering since
                                           // these items will be reentered with different coordinates
...
void CTransparencyRenderer::Render(int32_t nWindow) {
    ...
    if (gameStates.render.cameras.bActive)
        nWindow = nWindow;
    // HUDMessage (0, "transp. render heap size: %d.%03d.%03d", HeapSize () / 1000000, (HeapSize () % 1000000) / 1000,
    // HeapSize () % 1000);
    ...
    m_data.nWrap = 0;
    m_data.nFrame = -1;
    m_data.pBm[0] = m_data.pBm[1] = NULL;
    ...
    if (nBuffers < 2) {
        ...
        for (buffer.pBuffer = &buffer.depthBuffer[buffer.nMaxOffs];
             buffer.nItems[0] && (buffer.pBuffer >= buffer.depthBuffer.Buffer());
             buffer.pBuffer--)
            if (*buffer.pBuffer)
