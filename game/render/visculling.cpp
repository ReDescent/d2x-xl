...
int32_t FindAdjacentSideNorms(CSegment *pSeg, int16_t s0, int16_t s1, tSideNormData *s) {
    ...
    s[0].facePortal =
        gameData.segData.vertices + seg0->m_vertices[sideVertIndex[otherSide0][(s[0].t = side0->m_nType) == 3]];
    s[1].facePortal =
        gameData.segData.vertices + seg1->m_vertices[sideVertIndex[otherSide1][(s[1].t = side1->m_nType) == 3]];
    return 1;
}
...
int32_t QuickSortSegChildren(CSegment *pSeg, int16_t left, int16_t right, int16_t *childList) {
    int16_t h, l = left, r = right, mat = (l + r) / 2, median = childList[mat], bSwap = 0;
    ...
    for (nListPos = 0; nListPos < nSegCount; nListPos++) {
        ...
        for (nObject = SEGMENT(nSegment)->m_objects; nObject != -1; nObject = pObj->info.nNextInSeg) {
            ...
            if ((pObj->info.nType != OBJ_REACTOR) &&
                ((pObj->info.nType != OBJ_ROBOT) || (pObj->info.nId == 65))) { // don't migrate controlcen
...
int32_t CVisibilityData::BuildAutomapSegList(void) {
    ...
    for (int32_t i = nSegments = 0; i < gameData.segData.nSegments; i++)
        if (automap.Visible(i) && (bSkyBox || (SEGMENT(i)->m_function != SEGMENT_FUNC_SKYBOX)) &&
...
void CVisibilityData::BuildSegList(
    ...
    portals[0].left = portals[0].top = 0;
...
#pragma omp parallel for
    ...
    // it. This goes on until the current portal has dimensions of zero (because only solid geometry is visible through
    // it). The end result is a list of all segments that have faces the viewer can see (fully or partially).
    ...
    for (l = 0; l < nRenderDepth; l++) {
        for (nHead = nStart, nTail = nCurrent; nHead < nTail; nHead++) {
            ...
            for (nChild = 0; nChild < nChildren; nChild++) {
                ...
                // see if this segment has already been visited, and if so, does the current portal expand the old
                // portal?
                if (nPos == -1) {
