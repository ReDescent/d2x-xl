...
int32_t CreateWeaponSpeed(CObject *pWeapon, bool bFix) {
    ...
    fix xParentSpeed;
    fix xLaserLength = (pWeaponInfo->renderType == WEAPON_RENDER_POLYMODEL)
                           ? gameData.modelData.polyModels[0][pWeapon->ModelId()].Rad() * 2
                           : 0;
...
void CreateWeaponSound(CObject *pWeapon, int32_t bMakeSound) {
    ...
    if (pParent && (nParent != nViewer)) {
        if (bGatling && (pParent->info.nType == OBJ_PLAYER) && (gameOpts->UseHiresSound() == 2) &&
            gameOpts->sound.bGatling)
            audio.CreateSegmentSound(
                pWeaponInfo->flashSound,
        ...
        if (bGatling && (gameOpts->UseHiresSound() == 2) && gameOpts->sound.bGatling)
            audio.PlaySound(
                -1,
                (nParent == nViewer) ? SOUNDCLASS_PLAYER : SOUNDCLASS_LASER,
                volume,
                DEFAULT_PAN,
                0,
                -1,
    ...
    if (gameOpts->sound.bMissiles && CObject::IsMissile(nWeaponType)) {
        int32_t bBigMsl = (nWeaponType == SMARTMSL_ID) || (nWeaponType == MEGAMSL_ID) ||
                          (nWeaponType == EARTHSHAKER_ID) || (nWeaponType == ROBOT_SMARTMSL_ID) ||
                          (nWeaponType == ROBOT_MEGAMSL_ID) || (nWeaponType == ROBOT_EARTHSHAKER_ID);
        audio.CreateObjectSound(
            ...
            -1,
            -1,
            AddonSoundName(nMslSounds[bBigMsl]),
            1);
        ...
        audio.CreateObjectSound(
            ...
            -1,
            -1,
            AddonSoundName(SND_ADDON_FLARE));
    RETURN
}
...
if (pParent == gameData.objData.pConsole) {
    ...
    if (nWeaponType == OMEGA_ID) {
        ...
        if (((nParent != nViewer) || bSpectator) && (pParent->info.nType != OBJ_WEAPON)) {
            ...
            if ((pWeaponInfo->nFlashAnimation > -1) &&
                ((nWeaponType != OMEGA_ID) || !gameOpts->render.lightning.bOmega || gameStates.render.bOmegaModded))
                CreateMuzzleFlash(
                    pObj->info.nSegment,
                    pObj->info.position.vPos,
                    WI_FlashSize(nWeaponType),
                    pWeaponInfo->nFlashAnimation);
        }
        ...
        nRotDir = !nRotDir;
        pObj->mType.physInfo.rotVel.v.coord.x = pObj->mType.physInfo.rotVel.v.coord.y = 0;
    ...
    if (pParent && (pParent->info.nType == OBJ_PLAYER)) {
        if (nWeaponType == FUSION_ID) {
            ...
            (nWeaponType <= MAX_SUPERLASER_LEVEL) && (PLAYER(pParent->info.nId).flags & PLAYER_FLAGS_QUAD_LASERS))
    ...
    if (((nParent != nViewer) || SPECTATOR(pParent)) && (pParent->info.nType != OBJ_WEAPON)) {
        ...
        if (!SHOW_SMOKE || (gameOpts->render.particles.nQuality < 3) ||
            ((nWeaponType != VULCAN_ID) && (nWeaponType != GAUSS_ID)))
            if (pWeaponInfo->nFlashAnimation > -1)
                CreateMuzzleFlash(
                    pObj->info.nSegment,
                    pObj->info.position.vPos,
...
int32_t CreateNewWeaponSimple(
    ...
    CHitQuery hitQuery(
        ...
        &pParentObj->info.position.vPos,
        vPosition,
        pParentObj->info.nSegment,
        OBJ_IDX(pParentObj),
        0,
        0,
    ...
    if (!hitResult.nSegment) {
        ...
        hitQuery.radP0 = hitQuery.radP1 = 0;
