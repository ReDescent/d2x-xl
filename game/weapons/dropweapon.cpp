...
void DropCurrentWeapon(void) {
    ...
    if (gameData.weaponData.nPrimary == 0) { // special laser drop handling
        ...
        } else if (gameData.weaponData.nPrimary && !IsBuiltinWeapon(gameData.weaponData.nPrimary)) { // if selected
                                                                                                     // weapon was not
                                                                                                     // the laser
...
void DropSecondaryWeapon(int32_t nWeapon, int32_t nAmount, int32_t bSilent) {
    ...
    if ((LOCALPLAYER.secondaryAmmo[nWeapon] == 0) ||
        (IsMultiGame && (nWeapon == 0) &&
         (LOCALPLAYER.secondaryAmmo[nWeapon] <= gameData.multiplayer.weaponStates[N_LOCALPLAYER].nBuiltinMissiles))) {
        if (!bSilent)
    ...
    if (bMine)
        ...
        if ((nAmount % 4 == 0) && (secondaryWeaponToPowerup[1][nWeapon] >
                                   -1)) { // amount is multiple of four and four pack of this powerup available
