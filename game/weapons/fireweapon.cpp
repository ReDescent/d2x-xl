...
void TrackWeaponObject(int16_t nObject, int32_t nOwner) {
    if (gameData.multigame.weapon.nFired[0] < sizeofa(gameData.multigame.weapon.nObjects)) {
        if (gameData.multigame.weapon.nFired[0] < gameData.multigame.weapon.nFired[1])
            SetObjNumMapping(
                nObject,
                gameData.multigame.weapon.nObjects[1][gameData.multigame.weapon.nFired[0]],
                nOwner);
...
CFixVector *TransformGunPoint(
    ...
    int32_t bLaserOffs =
        ((gameStates.render.cockpit.nType == CM_FULL_COCKPIT) && (pObj->Index() == LOCALPLAYER.nObject));
#else
...
int32_t FireWeaponDelayedWithSpread(
    ...
    CHitQuery hitQuery(
        ...
        &pPos->vPos,
        &vLaserPos,
        (bSpectate ? gameStates.app.nPlayerSegment : pObj->info.nSegment),
        pObj->Index(),
        0x10,
        0x10,
...
bool CObject::RemoveWeapon(void) {
    ...
    if (!((gameData.appData.nFrameCount ^ info.nSignature) & 3) && (info.nType == OBJ_WEAPON) &&
        (info.nId != FLARE_ID) && (WI_Speed(info.nId, gameStates.app.nDifficultyLevel) > 0) && (xSpeed < I2X(2))) {
        ReleaseObject(Index());
        RETVAL(true)
    }
    RETVAL(false)
...
void CObject::UpdateHomingWeapon(int32_t nThread) {
    ENTER(0, 0);
    for (fix xFrameTime = gameData.laserData.xUpdateTime; xFrameTime >= HOMING_WEAPON_FRAMETIME;
         xFrameTime -= HOMING_WEAPON_FRAMETIME) {
...
void CObject::UpdateWeapon(void) {
    ...
    if (info.nType == OBJ_WEAPON) {
        ...
        if ((gameOpts->legacy.bHomers ||
            ...
            !(info.nFlags & PF_BOUNCED_ONCE) && !IsGuidedMissile()) {
#if USE_OPENMP //> 1
...
int32_t LocalPlayerFireGun(void) {
    ...
    int32_t bGatling =
        (gameData.weaponData.nPrimary == GAUSS_INDEX) ? 2 : (gameData.weaponData.nPrimary == VULCAN_INDEX) ? 1 : 0;
    fix addval;
...
int32_t CreateHomingWeapon(CObject *pObj, int32_t nGoalObj, uint8_t objType, int32_t bMakeSound) {
    ...
    nObject =
        CreateNewWeapon(&vGoal, &pObj->info.position.vPos, pObj->info.nSegment, pObj->Index(), objType, bMakeSound);
    pObj = OBJECT(nObject);
    if (!pObj)
        RETVAL(-1)
...
void CreateSmartChildren(CObject *pObj, int32_t nSmartChildren) {
    ...
    uint8_t nBlobId, nObjType = pObj->info.nType, nObjId = pObj->info.nId;
...
void DoMissileFiring(int32_t bAutoSelect) {
    ...
    for (i = 0; (i <= h) && (pPlayer->secondaryAmmo[gameData.weaponData.nSecondary] > 0); i++) {
        ...
        if ((gameData.weaponData.nSecondary == GUIDED_INDEX) || (gameData.weaponData.nSecondary == SMART_INDEX))
            ...
            (gameData.weaponData.nSecondary == MEGA_INDEX) || (gameData.weaponData.nSecondary == EARTHSHAKER_INDEX)) {
            CFixVector vForce;
...
void GetPlayerMslLock(void) {
    ...
    gameData.objData.trackGoals[0] = gameData.objData.trackGoals[1] = NULL;
    if ((pObj = GuidedInMainView())) {
        nObject = pObj->FindVisibleHomingTarget(pObj->info.position.vPos, MAX_THREADS);
        gameData.objData.trackGoals[0] = gameData.objData.trackGoals[1] = (nObject < 0) ? NULL : OBJECT(nObject);
        RETURN
    }
    ...
    if (LOCALPLAYER.secondaryAmmo[gameData.weaponData.nSecondary] <= 0)
        RETURN
    if (!gameStates.app.cheats.bHomingWeapons && (nWeapon != HOMINGMSL_ID) && (nWeapon != MEGAMSL_ID) &&
        (nWeapon != GUIDEDMSL_ID))
        RETURN
