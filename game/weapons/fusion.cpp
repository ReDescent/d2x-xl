...
int32_t FusionBump(void) {
    if (gameData.fusionData.xAutoFireTime) {
        if (gameData.weaponData.nPrimary != FUSION_INDEX)
            ...
            gameData.objData.pConsole->RandomBump(
                I2X(1) / 8,
                (gameData.FusionCharge() > I2X(2)) ? gameData.FusionCharge() * 4 : I2X(4));
        }
    }
    return 1;
}
...
void ChargeFusion(void) {
    if ((LOCALPLAYER.Energy() < I2X(2)) && (gameData.fusionData.xAutoFireTime == 0)) {
        ...
        fix h = (gameData.fusionData.xFrameTime <= LOCALPLAYER.Energy()) ? gameData.fusionData.xFrameTime
                                                                         : LOCALPLAYER.Energy();
