    ...
    for (i = 0; i < nOmegaBlobs; i++) {
        ...
        if (nSegment != -1) {
            ...
            pObj->info.xShield = FixMul(
                OMEGA_DAMAGE_SCALE * gameData.timeData.xFrame,
                WI_Strength(pObj->info.nId, gameStates.app.nDifficultyLevel));
...
void OmegaChargeFrame(void) {
    ...
    if ((gameData.weaponData.nPrimary == OMEGA_INDEX) && !gameData.omegaData.xCharge[IsMultiGame] &&
        !LOCALPLAYER.Energy()) {
        omegaLightning.Destroy(LOCALPLAYER.nObject);
    ...
    if (LOCALPLAYER.Energy()) {
        ...
        gameData.omegaData.xCharge[IsMultiGame] +=
            (fix)(gameData.timeData.xFrame / OMEGA_CHARGE_SCALE / gameStates.gameplay.slowmo[0].fSpeed);
        if (gameData.omegaData.xCharge[IsMultiGame] > MAX_OMEGA_CHARGE)
...
void DoOmegaStuff(CObject *pParentObj, CFixVector *vMuzzle, CObject *pWeaponObj) {
    ...
    if (pWeaponInfo) {
        if (pParentObj == gameData.objData.pViewer)
            ...
            audio.CreateSegmentSound(
                ...
                pWeaponObj->info.nSegment,
                0,
                pWeaponObj->info.position.vPos,
                0,
                I2X(1));
    }
    ...
    if (nTargetObj != -1)
        ...
        vPerturb = CFixVector::Random();
        perturbed_fvec = bSpectate ? gameStates.app.playerPos.mOrient.m.dir.f
                                   : pParentObj->info.position.mOrient.m.dir.f + vPerturb * (I2X(1) / 16);
        vTargetPos = *vMuzzle + perturbed_fvec * MAX_OMEGA_DIST;
        CHitQuery hitQuery(
            FQ_IGNORE_POWERUPS | FQ_TRANSPOINT | FQ_CHECK_OBJS,
            vMuzzle,
            &vTargetPos,
            nFiringSeg,
            OBJ_IDX(pParentObj),
