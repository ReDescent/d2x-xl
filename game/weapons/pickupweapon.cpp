...
int32_t PickupPrimary(int32_t nWeaponIndex, int32_t nPlayer) {
    ...
    if (ISLOCALPLAYER(nPlayer)) {
        ...
        if ((gameOpts->gameplay.nAutoSelectWeapon == 2) && (POrderList(nWeaponIndex) < nCutPoint) &&
...
int32_t MaxSecondaryAmmo(int32_t nWeapon) {
    ...
    if (IsMultiGame && !IsCoopGame && gameStates.app.bHaveExtraGameInfo[1] &&
    ...
    if (ISLOCALPLAYER(nPlayer)) {
        ...
        if (gameOpts->gameplay.nAutoSelectWeapon) {
            if (gameOpts->gameplay.nAutoSelectWeapon == 1) {
                ...
                (SOrderList(nWeaponIndex) < nCutPoint) &&
                ...
                if ((nWeaponIndex == PROXMINE_INDEX || nWeaponIndex == SMARTMINE_INDEX) &&
                    !(gameData.weaponData.nSecondary == PROXMINE_INDEX ||
                      gameData.weaponData.nSecondary == SMARTMINE_INDEX)) {
        ...
        if (nAmount > 1) {
            ...
            HUDInitMessage(
                "%d %s%s",
                nPickedUp,
                bSmokeGrens ? TXT_SMOKE_GRENADES : SECONDARY_WEAPON_NAMES(nWeaponIndex),
                TXT_SX);
        } else {
...
int32_t PickupAmmo(int32_t classFlag, int32_t nWeaponIndex, int32_t ammoCount, const char *pszMsg, int32_t nPlayer) {
    ...
    if (ammoCount > nMaxAmmo) {
        if (!nMaxAmmo /* || (nWeaponIndex == VULCAN_INDEX)*/) // only pick up Vulcan ammo if player can take the entire
                                                              // clip
            RETVAL(0)
        ammoCount = nMaxAmmo;
    }
    ...
    if ((nPlayer = N_LOCALPLAYER)) {
        ...
        if ((gameData.weaponData.nPrimary == LASER_INDEX) && pPlayer->HasSuperLaser())
            nSupposedWeapon = SUPER_LASER_INDEX; // allotment for stupid way of doing super laser
        if ((pPlayer->primaryWeaponFlags & (1 << nWeaponIndex)) && (nWeaponIndex > gameData.weaponData.nPrimary) &&
            (nOldAmmo == 0) && (POrderList(nWeaponIndex) < nCutPoint) &&
            (POrderList(nWeaponIndex) < POrderList(nSupposedWeapon)))
            SelectWeapon(nWeaponIndex, 0, 0, 1);
    }
...
int32_t PickupVulcanAmmo(CObject *pObj, int32_t nPlayer) {
    ...
    int32_t nAmmo = gameStates.app.bNostalgia ? VULCAN_CLIP_CAPACITY
                                              : pObj->cType.powerupInfo.nCount; // ammo available in this clip
    int32_t nUsed =
        PickupAmmo(CLASS_PRIMARY, VULCAN_INDEX, nAmmo, NULL, nPlayer); // what the player actually took this time
    ...
    if (nUsed) {
        ...
        if (ISLOCALPLAYER(nPlayer))
            HUDInitMessage(
                "%s %d %s!",
                TXT_ALREADY_HAVE,
                X2I((uint32_t)VULCAN_AMMO_SCALE * (uint32_t)nMaxAmmo),
                TXT_VULCAN_ROUNDS);
        gameData.weaponData.nPrimary = pwSave;
        RETVAL(0)
    }
