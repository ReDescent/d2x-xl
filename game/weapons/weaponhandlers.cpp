...
#define GAUSS_SPREAD (VULCAN_SPREAD / 5)
    ...
    if (0 <= LaserPlayerFireSpread(
                 ...
                 GAUSS_SPREAD,
                 GAUSS_SPREAD,
                 (pObj->info.nId != N_LOCALPLAYER) || (gameData.laserData.xNextFireTime > gameData.timeData.xGame),
                 0,
                 -1))
        nFired++;
    if (nRoundsPerShot > 1) {
...
// on show machines can fire (for themselves 	or other players) often enough for things like the vulcan cannon.
