...
void CreateAwarenessEvent (CObject *pObj, int32_t nType);         // CObject *pObj can create awareness of player, amount based on "nType"
void DoAIFrameAll (void);
void DoD1AIFrameAll (void);
void InitAISystem (void);
void ResetAIStates (CObject *pObj);
int32_t CreatePathPoints (CObject *pObj, int32_t start_seg, int32_t end_seg, tPointSeg *tPointSegs, int16_t *num_points, int32_t max_depth, int32_t randomFlag, int32_t safetyFlag, int32_t avoid_seg);
...
void ComputeVisAndVec (CObject *pObj, CFixVector *pos, tAILocalInfo *pLocalInfo, tRobotInfo *robptr, int32_t *flag, fix xMaxVisibleDist);
...
void AIMoveRelativeToTarget (CObject *pObj, tAILocalInfo *ailp, fix dist_to_player, CFixVector *vec_to_player, fix circleDistance, int32_t evade_only, int32_t player_visibility);
