...
int32_t SortLightsGaugeSize (void)
{
    ...
    if (gameStates.app.bMultiThreaded)
        return 0;
    return
        PROGRESS_STEPS (gameData.segData.nSegments) * 2 +
...
int32_t LoadLightData (int32_t nLevel)
{
    ...
    if (bOk)
        bOk =
            (ldh.nVersion == LIGHT_DATA_VERSION)
    ...
    if (bOk)
        bOk =
            (gameData.segData.bSegVis.Read (cf, gameData.segData.SegVisSize (ldh.nSegments), 0, ldh.bCompressed) == size_t (gameData.segData.SegVisSize (ldh.nSegments))) &&
