...
VmVecInc (vGunPoint, &objP->info.position.vPos);
...
gameData.objs.consoleP->mType.physInfo.rotVel.p.x += (FixMul (d_rand () - 16384, h)) / xScale;
gameData.objs.consoleP->mType.physInfo.rotVel.p.z += (FixMul (d_rand () - 16384, h)) / xScale;
...
if ((oldTime > COUNTDOWN_VOICE_TIME) && (gameData.reactor.countdown.nTimer <= COUNTDOWN_VOICE_TIME))	{
	DigiPlaySample (SOUND_COUNTDOWN_13_SECS, I2X (3));
	}
if (X2I (oldTime + I2X (7) / 8) != gameData.reactor.countdown.nSecsLeft) {
	if ((gameData.reactor.countdown.nSecsLeft >= 0) && (gameData.reactor.countdown.nSecsLeft < 10))
		DigiPlaySample ((short) (SOUND_COUNTDOWN_0_SECS + gameData.reactor.countdown.nSecsLeft), I2X (3));
	if (gameData.reactor.countdown.nSecsLeft == gameData.reactor.countdown.nTotalTime - 1)
		DigiPlaySample (SOUND_COUNTDOWN_29_SECS, I2X (3));
	}					
if (gameData.reactor.countdown.nTimer > 0) {
	...
	if ((size != oldSize) && (gameData.reactor.countdown.nSecsLeft < gameData.reactor.countdown.nTotalTime - 5))	// Every 2 seconds!
		DigiPlaySample (SOUND_CONTROL_CENTER_WARNING_SIREN, I2X (3));
	}
else {
	int flashValue = X2I (-gameData.reactor.countdown.nTimer * (64 / 4));	// 4 seconds to total whiteness
	if (oldTime > 0)
		DigiPlaySample (SOUND_MINE_BLEW_UP, I2X (1));
...
if (!(rStatP->bHit || rStatP->bSeenPlayer)) {
	if (gameStates.app.tick40fps.bTick) {		//	Do ever so often...
		...
		VmVecSub (&vecToPlayer, &gameData.objs.consoleP->info.position.vPos, &objP->info.position.vPos);
...
if ((rStatP->nNextFireTime < 0) && 
	...
	if (LOCALPLAYER.flags & PLAYER_FLAGS_CLOAKED)
		nBestGun = CalcBestReactorGun (gameData.reactor.props [objP->info.nId].nGuns, rStatP->vGunPos, rStatP->vGunDir, &gameData.ai.vBelievedPlayerPos);
	else
		nBestGun = CalcBestReactorGun (gameData.reactor.props [objP->info.nId].nGuns, rStatP->vGunPos, rStatP->vGunDir, &gameData.objs.consoleP->info.position.vPos);
	...
	if (nBestGun != -1) {
		...
		if (LOCALPLAYER.flags & PLAYER_FLAGS_CLOAKED) {
			...
			VmVecSub (&vecToGoal, &gameData.objs.consoleP->info.position.vPos, &rStatP->vGunPos [nBestGun]);
			xDistToPlayer = VmVecNormalize (&vecToGoal);
			}
		...
		while ((d_rand () > nRandProb) && (count < 4)) {
			...
			VmVecScaleInc (&vecToGoal, &vRand, I2X (1)/6);
...
#endif
if (gameData.reactor.bDestroyed) {
	gameData.reactor.countdown.nTimer += I2X (gameStates.app.nBaseCtrlCenExplTime + (NDL-1-gameStates.app.nDifficultyLevel)*gameStates.app.nBaseCtrlCenExplTime/ (NDL-1));
